    ...
    execute: function () {
      ...
      _export("AnimationControllersPoolManager", AnimationControllersPoolManager = (_dec = ccclass('AnimationControllersPoolManager'), _dec2 = property({
        ...
        setPrefabForPropertyList(prefabForPropertyList) {
          ...
          for (var prefabProperty of this._prefabForPropertyList) {
        ...
        pushInstanceToPool(prefabNodeListID, prefabNode) {
          var prefabNodeList = this._prefabObjectPool.has(prefabNodeListID) ? this._prefabObjectPool.get(prefabNodeListID) : this._prefabObjectPool.get(PREFAB_TYPE_DEFAULT);
        ...
        cleanAllPools() {
          for (var targetList of this._prefabObjectPool.values()) {
            for (var node of targetList) {
              node.destroy();
            }
        ...
        expandPool(objListId, count) {
          if (this._prefabObjectPool.has(objListId)) {
            var prefab = this.getTargetPrefab(objListId);
        ...
        getInstantiatedObjFromPool(objListId) {
          var prefabNode = null;
          var aniExtensionComponent = null;

          if (this._prefabObjectPool.has(objListId)) {
            var prefabNodeInfoList = this._prefabObjectPool.get(objListId);

            if (prefabNodeInfoList.length > 0) {
              prefabNode = prefabNodeInfoList.pop(); //console.log('popObjFromPool:', objListId);
            } else {
              var prefab = this._prefabMap.get(objListId);
            ...
            var _prefab = this._prefabMap.get(objListId);

            if (_prefab) {
              prefabNode = instantiate(_prefab);
              aniExtensionComponent = (_crd && AniSysTools === void 0 ? (_reportPossibleCrUseOfAniSysTools({
                error: Error()
              }), AniSysTools) : AniSysTools).findAndGetIAniComponent(prefabNode);
              aniExtensionComponent.onObjInstance();
        ...
        checkPoolStatusAfterPush() {
          ...
          for (var [key, list] of this._prefabObjectPool.entries()) {
...
//# sourceMappingURL=459f88a20e3a8c6e2ddaf0b09fb584c771b621de.js.map