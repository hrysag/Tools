    ...
    execute: function () {
      ...
      _export("WaysWinScoreAnalyzer", WaysWinScoreAnalyzer = class WaysWinScoreAnalyzer {
        ...
        getAllWaysWinDataList(iconData) {
          ...
          for (var i = 0; i < this.waysTable.length; i++) {
            var line = this.waysTable[i];
            var oneLineAllData = this.getOneLineData(i, line, iconData);

            if (oneLineAllData.length > 0) {
              var result = this.outPutCondition(oneLineAllData);
        ...
        getOneLineData(lineID, line, iconData) {
          ...
          for (var i = 1; i < line.length; i++) {
            ...
            var newSymbolID = this.connectCondition(tempSymbolId, symbolID);

            if (newSymbolID > -1) {
              tempSymbolId = newSymbolID;
              var iconIndex = this.iconList.indexOf(tempSymbolId);
              var newLineOdds = this.oddList[iconIndex][i];

              if (newLineOdds > 0) {
                var posList = line.slice(0, i + 1);
                var pos2DData = this.convertWinIndexTo2DArray(posList);
                var oneLineWinData = new WaysWinData(lineID, tempSymbolId, newLineOdds, posList, pos2DData);
...
//# sourceMappingURL=694cdf4c8c8dd933fec3d0ea0e90178c97830a89.js.map