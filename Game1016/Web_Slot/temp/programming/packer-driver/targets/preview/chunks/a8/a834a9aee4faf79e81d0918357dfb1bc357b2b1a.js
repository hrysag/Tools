    ...
    execute: function () {
      ...
      _export("TestLibs", TestLibs = class TestLibs {
        ...
        getRandomDataForStartSpin(reels) {
          var slotBoardData = this.generate2dsData(this.generateFlatAryData(reels));
          return slotBoardData;
        }

        generateFlatAryData(reels) {
          var reelIds = reels == undefined ? [0] : reels;
          var aryRandomData = [];

          for (var i = 0; i < reelIds.length; i++) {
            var reelId = reelIds[i];
            var randomValueData = this.generateRandomSymbolList(reelId);
        ...
        generate2dsData(reelsData) {
          var reelAmount = this._reelAmount;
          var reelSymbolAmount = this._reelSymbolAmount;
          var result = [];

          for (var i = 0; i < reelAmount; i++) {
            var startIndex = i * reelSymbolAmount;
            var endIndex = startIndex + reelSymbolAmount;
        ...
        generateRandomSymbolList(reelIndex) {
          ...
          var testReelIndex = reelIndex !== undefined ? reelIndex : 0; // 預設為0，如果reelIndex未提供則使用0

          var uniqueList = this.getTargetUniqueSymbolList(testReelIndex);
          var pickedSymbols = [];
          var possibleSymbols = [];
          var usedUniqueSymbols = [];
          var iconAmount = 4; //--要產生幾組
          // 產生所有可能的符號組合
...
//# sourceMappingURL=a834a9aee4faf79e81d0918357dfb1bc357b2b1a.js.map