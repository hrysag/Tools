      ...
      _export("ConditionLine", ConditionLine = (_dec = ccclass('ConditionLine'), _dec2 = property(Toggle), _dec3 = property(Node), _dec4 = property(Label), _dec5 = property({
        ...
        setThresholdLabelString() {
          ...
          if (this.useNumberComma) {
            var dotChar = (_crd && KeyboardData === void 0 ? (_reportPossibleCrUseOfKeyboardData({
              error: Error()
            }), KeyboardData) : KeyboardData).KEY_DOT;
            var intergerPart = this.pureString.split(dotChar)[0];
            var decimalPart = this.pureString.split(dotChar)[1];
            var hasDot = this.pureString.includes(dotChar);
            var commaInterger = Number(intergerPart).numberComma();
            result = "" + commaInterger + (hasDot ? dotChar : '') + (decimalPart || '');
          } else {
            result = this.pureString;
          }

          this.thresholdLabel.string = this.addPrefixX ? "\xD7" + result : result;
        }
        ...
        isValidThresholdString() {
          var string = this.thresholdLabel.string;
          var toNumberResult = (_crd && StringExt === void 0 ? (_reportPossibleCrUseOfStringExt({
            error: Error()
          }), StringExt) : StringExt).ToNumber(string);
          var isValid = toNumberResult[0];
          var nonMinusNumber = toNumberResult[1] >= 0;
        ...
        isMeetsStopCondition(context) {
          var threshold = (_crd && StringExt === void 0 ? (_reportPossibleCrUseOfStringExt({
            error: Error()
          }), StringExt) : StringExt).ToNumber(this.pureString)[1];
          var target = context[this.targeAttributeKey];
          var evaluator = ConditionLine.Evaluator[this.conditionType];
        ...
        getConditionLineData() {
          var conditionLineData = new ConditionLineData();
        ...
        onKeyboardBtnClick(customData) {
          ...
          var numberDigit = ((_this$pureString$matc = this.pureString.match(/\d/g)) == null ? void 0 : _this$pureString$matc.length) || 0;
        ...
        isThresholdMaxLength() {
          ...
          var numberDigit = ((_this$pureString$matc2 = this.pureString.match(/\d/g)) == null ? void 0 : _this$pureString$matc2.length) || 0;
...
//# sourceMappingURL=e7ff665d8cb0e99c394acc0f6706568707420616.js.map