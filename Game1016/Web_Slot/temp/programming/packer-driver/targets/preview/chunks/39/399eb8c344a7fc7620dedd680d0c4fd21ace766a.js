    ...
    execute: function () {
      ...
      _export("GameViewManager1016", GameViewManager1016 = (_dec = ccclass('GameViewManager1016'), _dec2 = property({
        ...
        constructor() {
          ...
          //---slot callback(單軸停止)
          this._oneReelRollEndCallBackFromSlot = /*#__PURE__*/_asyncToGenerator(function* (reelID) {
            //--20250915 修改:企劃要求Scatter要直接隨著轉輪落下
            var stopMode = (_crd && GlobalAccessWriter === void 0 ? (_reportPossibleCrUseOfGlobalAccessWriter({
              error: Error()
            ...
            if (!_this._slotMachine.isFastStopClick && stopMode == (_crd && NewFlashModeEnum === void 0 ? (_reportPossibleCrUseOfNewFlashModeEnum({
              error: Error()
            }), NewFlashModeEnum) : NewFlashModeEnum).None) {
              //--鎖定狀態下就不播放SpinStop音效
              if (!_this._slotMachine.getSingleReelIsFirstRoundLock(reelID)) {
            ...
            var checkWildCondition = _this._processedServerData.getReelWildData(reelID);

            if (checkWildCondition != null) {
              //--回傳有資料的狀態就是代表這軸匹配wild(showWildAnimation---appear)
              var promiseTask = _this._slotMachine.playWildAppearAnimation(reelID).catch(e => console.warn('scatter err', reelID, e));

              _this._waitTask.set(reelID, promiseTask); //--存playWildAppearAnimation的任務
            ...
            var checkScatterInReel = _this._processedServerData.getReelScatterData(reelID);

            if (checkScatterInReel) {
              var goReel = reelID; //-_waitScatterTask

              var promiseScatterTask = _this._slotMachine.forceToHandoffSingleScatter(goReel).catch(e => console.warn('scatter err', goReel, e));

              _this._waitScatterTask.set(goReel, promiseScatterTask); //--存scatterAppearAnimation的任務

            } //--20251022新增reel bounce任務存取


            var bouncePromise = _this._slotMachine.getEndBouncePromise(reelID);

            if (bouncePromise) {
              _this._waitReelBounceTask.set(reelID, bouncePromise);
        ...
        init() {
          ...
          var processor = new (_crd && ProcessSlotSymbolAniData1016 === void 0 ? (_reportPossibleCrUseOfProcessSlotSymbolAniData({
            error: Error()
          }), ProcessSlotSymbolAniData1016) : ProcessSlotSymbolAniData1016)();
          var hooks = new (_crd && SymbolAniMediatorHooks1016 === void 0 ? (_reportPossibleCrUseOfSymbolAniMediatorHooks({
            error: Error()
        ...
        registerSystem() {
          ...
          var gameStepDelayTime = new (_crd && BasicGameStepDelayTime === void 0 ? (_reportPossibleCrUseOfBasicGameStepDelayTime({
            error: Error()
          ...
          var globalDataStore = (_crd && BasicGameGlobalData === void 0 ? (_reportPossibleCrUseOfBasicGameGlobalData({
            error: Error()
          ...
          var globalDataWriter = globalDataStore.createWriter(_crd && GLOBAL_DATA_WRITE_KEY === void 0 ? (_reportPossibleCrUseOfGLOBAL_DATA_WRITE_KEY({
            error: Error()
        ...
        setGameTimeScale() {
          ...
          var basicGameStepDelayTime = (_crd && GlobalAccessWriter === void 0 ? (_reportPossibleCrUseOfGlobalAccessWriter({
            error: Error()
          }), GlobalAccessWriter) : GlobalAccessWriter).getGlobalData((_crd && GameGlobalKeys === void 0 ? (_reportPossibleCrUseOfGameGlobalKeys({
            error: Error()
        ...
        setTwoLevelTurboMode(turboMode) {
          ...
          var speedMode = this._flashToSpeedMap[turboMode]; //--動態映射,將加速模式轉成SpeedTimeMode
        ...
        reSetDataForBeforeSpin(isTurboMode) {
          var _this2 = this;

          return _asyncToGenerator(function* () {
            _this2._currentTurboSpeed = (_crd && GenericUIManager === void 0 ? (_reportPossibleCrUseOfGenericUIManager({
              error: Error()
            }), GenericUIManager) : GenericUIManager).instance.isTurboOn; //--每次startSpin都要更新目前的加速狀態

            yield _this2._basicShowAniProcess.cleanAllPlayingAniForNewStart(); //GameUtilsTools.debugLog('BasicShowAniProcess_debug', 'reSetDataForBeforeSpin', { isTurboMode });

            var currentData = _this2._processedServerData.getPrevData();

            if (currentData) {
              var cards = currentData.reelInfo.symbolData2ds;

              if (cards) {
                //--退場
                _this2._slotMachine.sortReelLayerIndex(cards, false);
              } //console.log('check_reSetDataForBeforeSpin_state:', cards);

            }

            _this2._basicShowAniProcess.resetRoundData();

            _this2._handoffManager.releaseAll(); //console.log('check_release_handoff_newRound', this._handoffManager.debugCheckAllOwners());
            //--上保險,在確保全新round開始前(server資料還沒回來時,清除掉全部的動畫資料)


            if (!_this2._isThisRound) {
              _this2._basicShowAniProcess.stopAndRemoveAllAnis();
            }
          })();
        }
        ...
        testFunc() {
          ...
          return _asyncToGenerator(function* () {
            var promises = [_this3._ngUI.openReSpinCountUI(), _this3._ngUI.openFGCountUI()];
        ...
        doStartSpin() {
          ...
          var currentState = (_crd && GlobalAccessWriter === void 0 ? (_reportPossibleCrUseOfGlobalAccessWriter({
            error: Error()
          }), GlobalAccessWriter) : GlobalAccessWriter).getGlobalData((_crd && GameGlobalKeys === void 0 ? (_reportPossibleCrUseOfGameGlobalKeys({
            error: Error()
          ...
          if (currentState === (_crd && GameState === void 0 ? (_reportPossibleCrUseOfGameState({
            ...
            var readyHands = this._processedServerData.getReadyToHandForThisRound();
          ...
          var gameSpeed = (_crd && GlobalAccessWriter === void 0 ? (_reportPossibleCrUseOfGlobalAccessWriter({
            error: Error()
          }), GlobalAccessWriter) : GlobalAccessWriter).getGlobalData((_crd && GameGlobalKeys === void 0 ? (_reportPossibleCrUseOfGameGlobalKeys({
            error: Error()
        ...
        beforeStopSpin() {
          ...
          //--放入聽牌軸資訊
          var currentState = this._processedServerData.getCurrentState();
        ...
        doStopSpin(slotData, other) {
          var _this4 = this;

          return _asyncToGenerator(function* () {
            if (!slotData) return;
            var wildCardData = slotData.reelInfo.wildGroup; //--20260127-要再算分開始運作前就要知道這把有無獲獎--莫名其妙

            _this4._basicShowAniProcess.preRoundOddsForAni = _this4._processedServerData.getRoundBetAndOdds().odds; //--從getCurrentData取該局資料
            //this._currentwildCardData = wildCardData; //--存要塞進去處理的特殊牌資料

            var cloneCards = (_crd && GameUtilsTools === void 0 ? (_reportPossibleCrUseOfGameUtilsTools({
              error: Error()
            }), GameUtilsTools) : GameUtilsTools).deepClone(slotData.reelInfo.symbolData2ds);
            yield _this4._slotMachine.stopRoll(cloneCards, wildCardData); //let currentTime = Date.now();
        ...
        beforeAllReelRollEnd() {
          ...
          return _asyncToGenerator(function* () {
            ...
            if (_this5._slotMachine.isFastStopClick || stopMode != (_crd && NewFlashModeEnum === void 0 ? (_reportPossibleCrUseOfNewFlashModeEnum({
              error: Error()
            }), NewFlashModeEnum) : NewFlashModeEnum).None) {
              (_crd && AudioManager === void 0 ? (_reportPossibleCrUseOfAudioManager({
                error: Error()
            ...
            if (_this5._waitTask.size > 0) {
              //--allSettled=我不管結果如何就算error也繼續往下走,promise.all遇到error會直接reject
              yield Promise.allSettled(_this5._waitTask.values());

              _this5._waitTask.clear();
            } //--等待scatter動畫(appear)徹底完成


            if (_this5._waitScatterTask.size > 0) {
              yield Promise.allSettled(_this5._waitScatterTask.values());

              _this5._waitScatterTask.clear();
            } //--等待reel bounce徹底完成


            if (_this5._waitReelBounceTask.size > 0) {
              yield Promise.allSettled(_this5._waitReelBounceTask.values());

              _this5._waitReelBounceTask.clear();
            } //--一階段和二階段Scatter appear不會等待播完就會resolve


            _this5._basicShowAniProcess.hasScatterAppearInThisRound = false; //--重置scatterAppear狀態

            var nextGameState = _this5._processedServerData.getNextStepGameState();

            _this5._basicShowAniProcess.afterWholeReelStopIdleSpAni(nextGameState);

            _this5._slotMachine.playWildIdleAnimation();

            var needToMove = _this5._processedServerData.checkNeedToMovement();

            var moveData = _this5._processedServerData.getWildMovementData(); //--這邊要先算出有沒有wild需要演出的資料(位移or無位移)


            var movedLock = false;

            if (moveData.wildGroup) {
              movedLock = _this5._slotMachine.checkMovedReel(moveData.wildGroup);
            }

            if (needToMove || movedLock) {
              //-需要啟動wild表演
              var waitTimeForWild = _this5._gameStepDelayTimeList.get(cfg => {
                var _cfg$wild;
              ...
              _this5._roundStepMapCondition.set(ROUND_STEP_CONDITION_KEY.WILD_MOVE, true); //const startWildWaitTime = GameUtilsTools.getTimeStamp();


              yield _this5.addTweenDelay(waitTimeForWild); //const endWildWaitTime = GameUtilsTools.getTimeStamp();
              ...
              _this5._isInterrupting = false;
              (_crd && GlobalAccessWriter === void 0 ? (_reportPossibleCrUseOfGlobalAccessWriter({
                error: Error()
              }), GlobalAccessWriter) : GlobalAccessWriter).setGlobalData((_crd && GameGlobalKeys === void 0 ? (_reportPossibleCrUseOfGameGlobalKeys({
                error: Error()
              }), GameGlobalKeys) : GameGlobalKeys).InterruptProcess, _this5._isInterrupting);
            }

            if (needToMove) {
              //--表演有位移的狀態
              yield _this5.processWildMovement(moveData.wildMovement);
            } else if (movedLock) {
              //--20251031表演沒有位移的狀態<處理沒釘死的wild,因為會有包含條件下,需要剔除整軸狀態的wild>
              var filterData = (_crd && GameUtilsTools === void 0 ? (_reportPossibleCrUseOfGameUtilsTools({
                error: Error()
              ...
              yield _this5.processWildNoMovement(filterData.wildGroup);
            }
          })();
        } //============================== slotMachine控制 =========================
        //============================== 下注與FG面板 =============================
        ...
        processWildMovement(moveData) {
          ...
          return _asyncToGenerator(function* () {
            ...
            var _loop = function* _loop() {
              var reelId = moveData[i];

              var task = /*#__PURE__*/function () {
                var _ref3 = _asyncToGenerator(function* (reelIndex) {
                  var wildData = yield _this6._slotMachine.processDataBeforeWildMovement(reelId);
                  yield _this6._basicShowAniProcess.triggerWildMoveAnimation(wildData);
                  yield _this6._slotMachine.reSetWildDataAfterMove(reelIndex); //--reWriteData 
                });
            ...
            var idsExclude = FLATTEN_REEL_ID.filter(id => !moveData.includes(id));
            yield _this6.withReelDarkEffect(idsExclude, /*#__PURE__*/_asyncToGenerator(function* () {
              yield Promise.all(promises);
            }));
          })();
        } //--統一處理wild位移+開關燈
        ...
        withReelDarkEffect(idsExclude, action) {
          var _this7 = this;

          return _asyncToGenerator(function* () {
            var currentGameState = (_crd && GlobalAccessWriter === void 0 ? (_reportPossibleCrUseOfGlobalAccessWriter({
              error: Error()
            }), GlobalAccessWriter) : GlobalAccessWriter).getGlobalData((_crd && GameGlobalKeys === void 0 ? (_reportPossibleCrUseOfGameGlobalKeys({
              error: Error()
            }), GameGlobalKeys) : GameGlobalKeys).GameState);

            var currentBGs = _this7._showContainerManager.getContainerListByState(currentGameState);

            var targetBgAni = currentBGs.find(it => it.isBkgAni === true); //-- Step 1: go dark

            yield Promise.all([//(<UniSlotMachine1016>this._slotMachine).setReelsLightTween(idsExclude, true),
            _this7._slotMachine.setReelsLightTweenExcludeIds(idsExclude, true, WILD_LIST), _this7._basicShowAniProcess.changeAniCtrlColorBySymbolId(SCATTER_LIST[0], true)]);
            targetBgAni == null || targetBgAni.openDark(); //-- Step 2: wildMove

            yield action(); //-- Step 3: goBack normal

            yield Promise.all([_this7._slotMachine.setAllLightTween(false), _this7._basicShowAniProcess.changeAniCtrlColorBySymbolId(SCATTER_LIST[0], false)]);
            targetBgAni == null || targetBgAni.closeDark();
          })();
        }

        processWildNoMovement(wildGroup) {
          var _this8 = this;
          ...
          return _asyncToGenerator(function* () {
            ...
            var _loop2 = function* _loop2() {
              var reelId = wildGroup[i].reelIndex;

              var task = /*#__PURE__*/function () {
                var _ref5 = _asyncToGenerator(function* (reelIndex) {
                  var wildData = yield _this8._slotMachine.processDataBeforeWildNoMovement(reelId);
                  yield _this8._basicShowAniProcess.triggerWildNoMoveAnimation(wildData);
                  var reel = wildData.movement.reelIndex;
                  var index = wildData.movement.iconIndex;
                  yield _this8._slotMachine.reSetWildDataAfterWithoutMove(reel, index);
                });
            ...
            var idsExclude = FLATTEN_REEL_ID.filter(id => !includes.includes(id));
            yield _this8.withReelDarkEffect(idsExclude, /*#__PURE__*/_asyncToGenerator(function* () {
              yield Promise.all(promises);
            }));
          })();
        }

        processReSpineCount(reSpinData) {
          var _this9 = this;
          ...
          return _asyncToGenerator(function* () {
            ...
            var destinationWpos = _this9._ngUI.getReSpinCountWPos();

            var handoffWildData = yield _this9._slotMachine.forceToHandoffWild(reSpinData.reels);
            yield _this9._ngUI.openReSpinCountUI(); //--開播前的等待時間

            var turboMode = (_crd && GlobalAccessWriter === void 0 ? (_reportPossibleCrUseOfGlobalAccessWriter({
              error: Error()
            }), GlobalAccessWriter) : GlobalAccessWriter).getGlobalData((_crd && GameGlobalKeys === void 0 ? (_reportPossibleCrUseOfGameGlobalKeys({
              error: Error()
            }), GameGlobalKeys) : GameGlobalKeys).TurboMode);
            ...
            if (turboMode != (_crd && NewFlashModeEnum === void 0 ? (_reportPossibleCrUseOfNewFlashModeEnum({
              ...
              var beforePlayShootAniDelay = _this9._gameStepDelayTimeList.get(cfg => {
                var _cfg$wild2;
              ...
              if (beforePlayShootAniDelay > 0) {
                yield _this9.addTweenDelay(beforePlayShootAniDelay);
              } //--播放特殊wild動畫+開噴粒子


              yield _this9._basicShowAniProcess.showGetReSpinEffect({
            ...
            yield _this9._ngUI.triggerReSpinCountUp(reSpinData.total);
          })();
        }

        processScatterFGCount(fgData, usePreviousData) {
          var _this10 = this;

          return _asyncToGenerator(function* () {
            //--會進來一定就是有
            var multiScatterReel = _this10._processedServerData.getScatterByMultiReel(fgData.reels, usePreviousData);

            var destinationWpos;

            var currentGameState = _this10._processedServerData.getCurrentState();

            var currentGUI = currentGameState == (_crd && GameState === void 0 ? (_reportPossibleCrUseOfGameState({
              error: Error()
            }), GameState) : GameState).FREE_GAME ? _this10._fgUI : _this10._ngUI; //--開播前的等待時間

            var beforePlayShootAniDelay = _this10._gameStepDelayTimeList.get(cfg => {
              var _cfg$wild3;
            ...
            if (beforePlayShootAniDelay > 0) {
              yield _this10.addTweenDelay(beforePlayShootAniDelay);
            }
            ...
            if (multiScatterReel.length > 0) {
              ...
              if (turboMode == (_crd && NewFlashModeEnum === void 0 ? (_reportPossibleCrUseOfNewFlashModeEnum({
                ...
                yield currentGUI.openFGCountUI();
                yield currentGUI.triggerFGCountUp(fgData.total);
              } else {
                yield currentGUI.openFGCountUI();
                var handoffScatterData = yield _this10._slotMachine.forceToHandoffScatter(multiScatterReel);
                destinationWpos = currentGUI.getFGCountWPos();
                yield _this10._basicShowAniProcess.showGetScatterFGEffect({
                  endPos: destinationWpos,
              ...
              */
              ...
              var handoffWildData = yield _this10._slotMachine.forceToHandoffWild(fgData.reels);
              ...
              if (turboMode == (_crd && NewFlashModeEnum === void 0 ? (_reportPossibleCrUseOfNewFlashModeEnum({
                ...
                destinationWpos = currentGUI.getFGCountWPos();
                yield currentGUI.openFGCountUI();
                yield _this10._basicShowAniProcess.showGetReSpinEffect({
                  endPos: destinationWpos,
                  info: handoffWildData,
                  index: fgData.reels
                });
        ...
        newRoundDataToStopSpin() {
          var _superprop_getNewRoundDataToStopSpin = () => super.newRoundDataToStopSpin,
              _this11 = this;

          return _asyncToGenerator(function* () {
            _this11.beforeProcessNewRoundData(); //--第一把資料先算


            yield _superprop_getNewRoundDataToStopSpin().call(_this11);
          })();
        }

        beforeProcessNewRoundData() {
          //return;
          var currentSate = this._processedServerData.getCurrentState();

          var nextState = this._processedServerData.getNextStepGameState();

          var prevState = this._processedServerData.getPrevState();
          /**
        ...
        processCalculatingRSandFG() {
          ...
          return _asyncToGenerator(function* () {
            ...
            */
            ...
            if (_this12._currentFGAndRSRecord1016.firstNgToRsData) {
              reSpinCount = _this12._currentFGAndRSRecord1016.firstNgToRsData.reSpinCount;
              fgCount = _this12._currentFGAndRSRecord1016.firstNgToRsData.fgCount;
              usePreviousData = _this12._currentFGAndRSRecord1016.firstNgToRsData.usePreviousData;
              _this12._currentFGAndRSRecord1016.firstNgToRsData = null;
            } else {
              reSpinCount = _this12._currentFGAndRSRecord1016.reSpinCount;
              fgCount = _this12._currentFGAndRSRecord1016.fgCount;
            ...
            if (reSpinCount && reSpinCount.total > 0) {
              promises.push(_this12.processReSpineCount(reSpinCount));
            }

            if (fgCount.total > 0) {
              //-在沒有reSpin觸發FG不需要噴了-直接進位讓資料往下就好
              var currentSate = _this12._processedServerData.getCurrentState();

              if (currentSate != (_crd && GameState === void 0 ? (_reportPossibleCrUseOfGameState({
                error: Error()
              }), GameState) : GameState).NORMAL) {
                promises.push(_this12.processScatterFGCount(fgCount, usePreviousData));
              }
            }

            if (promises.length > 0) {
              _this12._isInterrupting = false;
              (_crd && GlobalAccessWriter === void 0 ? (_reportPossibleCrUseOfGlobalAccessWriter({
                error: Error()
              }), GlobalAccessWriter) : GlobalAccessWriter).setGlobalData((_crd && GameGlobalKeys === void 0 ? (_reportPossibleCrUseOfGameGlobalKeys({
                error: Error()
              }), GameGlobalKeys) : GameGlobalKeys).InterruptProcess, _this12._isInterrupting);

              var delay = _this12._gameStepDelayTimeList.get(cfg => {
                var _cfg$wild4;

                return (_cfg$wild4 = cfg.wild) == null ? void 0 : _cfg$wild4.others.beforeCountWait;
              });

              yield _this12.addTweenDelay(delay);
            }
        ...
        doShowResultAfterStopRoll() {
          var _this13 = this;

          return _asyncToGenerator(function* () {
            //console.log('====================doShowResultAfterStopRoll================');
            var currentRoundData = _this13._processedServerData.getCurrentData();

            var winScoreData = _this13.createWinScoreData();

            var currentGameState = _this13._processedServerData.getCurrentState();

            var nextGameState = _this13._processedServerData.getNextStepGameState();

            var isFinalRound = _this13._processedServerData.getIsLastStep();

            var nexNew = currentGameState != nextGameState ? true : false; //--寫入當前與下一把的狀態(是否進入輪播使用)

            var gameStateCondition = {
              currentRoundState: currentGameState,
              nextRoundState: nextGameState,
              isDifferentStateNext: nexNew,
              isFinal: isFinalRound
            };
            _this13._basicShowAniProcess.gameStateCondition = gameStateCondition;
            var useSequence = false; //let playWinFlag: boolean;

            var roundWinData;
            var beforeShowWinDelay = 0;
            ...
            if (currentGameState === (_crd && GameState === void 0 ? (_reportPossibleCrUseOfGameState({
              ...
              roundWinData = yield _this13._basicShowAniProcess.beforeProcessWinScoreData(winScoreData, currentRoundData.winLine);

              _this13._roundStepMapCondition.set(ROUND_STEP_CONDITION_KEY.WIN, roundWinData.hasWin);

              _this13._roundStepMapCondition.set(ROUND_STEP_CONDITION_KEY.JP, roundWinData.bigWin);
              ...
              if (currentGameState === (_crd && GameState === void 0 ? (_reportPossibleCrUseOfGameState({
                ...
                if (roundWinData.hasWin) {
                  beforeShowWinDelay = _this13._gameStepDelayTimeList.get(cfg => {
                    var _cfg$result;

                    return (_cfg$result = cfg.result) == null ? void 0 : _cfg$result.beforeShowWin;
                  });
                  yield _this13.addTweenDelay(beforeShowWinDelay);
                }

                _this13._isInterrupting = false;
                (_crd && GlobalAccessWriter === void 0 ? (_reportPossibleCrUseOfGlobalAccessWriter({
                  error: Error()
                }), GlobalAccessWriter) : GlobalAccessWriter).setGlobalData((_crd && GameGlobalKeys === void 0 ? (_reportPossibleCrUseOfGameGlobalKeys({
                  error: Error()
                }), GameGlobalKeys) : GameGlobalKeys).InterruptProcess, _this13._isInterrupting);
                useSequence = yield _this13._basicShowAniProcess.runShowProcess(roundWinData.hasWin);
                /*
                ...
                */
                ...
                var fgCount = _this13._currentFGAndRSRecord1016.firstNgToRsData ? _this13._currentFGAndRSRecord1016.firstNgToRsData.fgCount : _this13._currentFGAndRSRecord1016.fgCount;

                if (fgCount.total > 0) {
                  yield _this13._basicShowAniProcess.showScatterAndWildWinAniBeforeFG(fgCount.reels);
                }
                /**
                ...
                yield _this13.processCalculatingRSandFG();
              } else {
                /**
                 * 1.FG數字異動相關處理
                 * 2.中線播放
              ...
              useSequence = yield _this13._basicShowAniProcess.runShowProcess(roundWinData.hasWin); //await this.processCalculatingRSandFG();
            } //await this.runTest();
            //--AUTO模式之下不需要跑輪播了
            ...
            if (useSequence && currentGameState === (_crd && GameState === void 0 ? (_reportPossibleCrUseOfGameState({
              ...
              var isAutoMode = (_crd && GenericUIManager === void 0 ? (_reportPossibleCrUseOfGenericUIManager({
                error: Error()
              }), GenericUIManager) : GenericUIManager).instance.isAutoMode;

              if (!isAutoMode) {
                _this13._basicShowAniProcess.playMultipleSequence();
              }
            }
          })();
        }
        ...
        prepareForNextFGandReSpin() {
          ...
          var state = (_crd && GlobalAccessWriter === void 0 ? (_reportPossibleCrUseOfGlobalAccessWriter({
            error: Error()
          }), GlobalAccessWriter) : GlobalAccessWriter).getGlobalData((_crd && GameGlobalKeys === void 0 ? (_reportPossibleCrUseOfGameGlobalKeys({
            error: Error()
          }), GameGlobalKeys) : GameGlobalKeys).GameState);
        ...
        checkConditionForRoundStep(isFinalRound) {
          if (isFinalRound === void 0) {
            isFinalRound = false;
          }
          ...
          var previousState = this._processedServerData.getPrevState();
          ...
          var isWin = this._roundStepMapCondition.get(ROUND_STEP_CONDITION_KEY.WIN);

          var isWildMove = this._roundStepMapCondition.get(ROUND_STEP_CONDITION_KEY.WILD_MOVE);

          var isBigWin = this._roundStepMapCondition.get(ROUND_STEP_CONDITION_KEY.JP);
        ...
        processRollToStopTime(gameState) {
          ...
          var earlyToStopStep = this._gameStepDelayTimeList.get(cfg => {
            var _cfg$stop;
        ...
        beforeProcessReSpinRound(value) {
          ...
          return _asyncToGenerator(function* () {
            if (_this14._processedServerData.isFirstReSpin()) {
              var _this14$_currentFGAnd;
              ...
              var reSpinTotalTimes = ((_this14$_currentFGAnd = _this14._currentFGAndRSRecord1016) == null || (_this14$_currentFGAnd = _this14$_currentFGAnd.firstNgToRsData) == null ? void 0 : _this14$_currentFGAnd.reSpinCount.total) || 0;

              _this14._reSpinBoard.setReSpinTimes(reSpinTotalTimes);

              var callBackFlag = false;

              var asyncEvtCallback = /*#__PURE__*/function () {
                var _ref6 = _asyncToGenerator(function* () {
                  ...
                  _this14._gameModeManager.changeAllGameState((_crd && GameState === void 0 ? (_reportPossibleCrUseOfGameState({
                    error: Error()
                  }), GameState) : GameState).RE_SPINE);
                });

                return function asyncEvtCallback() {
                  return _ref6.apply(this, arguments);
                };
              }();

              var beforeOpenBoardDelay = _this14._gameStepDelayTimeList.get(cfg => {
                var _cfg$respin;
              ...
              if (beforeOpenBoardDelay > 0) {
                yield _this14.addTweenDelay(beforeOpenBoardDelay);
              }

              yield _this14._reSpinBoard.openWithEvtAndFinishPromise(asyncEvtCallback);
              _this14._isInterrupting = false;
              (_crd && GlobalAccessWriter === void 0 ? (_reportPossibleCrUseOfGlobalAccessWriter({
                error: Error()
              }), GlobalAccessWriter) : GlobalAccessWriter).setGlobalData((_crd && GameGlobalKeys === void 0 ? (_reportPossibleCrUseOfGameGlobalKeys({
                error: Error()
              }), GameGlobalKeys) : GameGlobalKeys).InterruptProcess, _this14._isInterrupting);
              yield _this14.processCalculatingRSandFG();
            } //---表演觸發respin次數(上方的UI)
        ...
        beforeProcessFGRound(value) {
          ...
          return _asyncToGenerator(function* () {
            if (!_this15._processedServerData.isFirstFreeGame()) return;
            (_crd && GlobalAccessWriter === void 0 ? (_reportPossibleCrUseOfGlobalAccessWriter({
              error: Error()
            }), GlobalAccessWriter) : GlobalAccessWriter).setGlobalData((_crd && GameGlobalKeys === void 0 ? (_reportPossibleCrUseOfGameGlobalKeys({
              error: Error()
            ...
            var prevState = _this15._processedServerData.getPrevState();

            var dt = _this15._gameStepDelayTimeList.get(cfg => {
              var _cfg$fg;
            ...
            if (prevState === (_crd && GameState === void 0 ? (_reportPossibleCrUseOfGameState({
              ...
              if (_this15._currentFGAndRSRecord1016.firstNgToFgData != null) {
                var fgCount = _this15._currentFGAndRSRecord1016.firstNgToFgData.fgCount;
                _this15._currentFGAndRSRecord1016.firstNgToFgData = null;

                if (fgCount.total > 0) {
                  yield _this15._async.waitSecondsRaw(dt); //--等一等

                  yield Promise.all([_this15._basicShowAniProcess.showScatterAndWildWinAniBeforeFG(fgCount.reels), _this15._slotMachine.playWildAniToFg(fgCount.reels)]); //await this._basicShowAniProcess.showScatterWinAni();

                  yield _this15.addTweenDelay(0.2); //--scatter connect動畫的等待時間
                }
              }
            }
            ...
            _this15._gameBGSoundCtrl1016.fadeOutNGorRSWithBGM(); //--開啟面板....


            var fgTotalTimes = _this15._processedServerData.getCurrentFGOpenCount(); //-_fgUI


            _this15._fgUI.setTotalFgCount(fgTotalTimes);

            var boardOutCallBack = () => {
              _this15._ngUI.closeReSpinCountUI();

              _this15._gameModeManager.changeAllGameState((_crd && GameState === void 0 ? (_reportPossibleCrUseOfGameState({
                error: Error()
              }), GameState) : GameState).FREE_GAME);
            };

            _this15._slotMachine.reSetLockReels(); //--LOCK 解鎖
            //this._processedServerData.clearCountForFg();--20260130取消,更改流程再開始前就會檢查清除


            _this15._fgUIBoard.setBoardMode((_crd && GameState === void 0 ? (_reportPossibleCrUseOfGameState({
              error: Error()
            }), GameState) : GameState).FREE_GAME);

            _this15._fgUIBoard.setFGPlayTimes(fgTotalTimes);

            yield _this15._fgUIBoard.openFGUIBoard(fgTotalTimes, boardOutCallBack);

            var beforeFirstFgRoundWait = _this15._gameStepDelayTimeList.get(cfg => {
              var _cfg$other;

              return (_cfg$other = cfg.other) == null ? void 0 : _cfg$other.beforeFirstFgRoundWait;
            });

            yield _this15._async.waitSecondsRaw(beforeFirstFgRoundWait); //--第一把FG開始前的等待時間-20260209

            _this15._isInterrupting = false;
            (_crd && GlobalAccessWriter === void 0 ? (_reportPossibleCrUseOfGlobalAccessWriter({
              error: Error()
            }), GlobalAccessWriter) : GlobalAccessWriter).setGlobalData((_crd && GameGlobalKeys === void 0 ? (_reportPossibleCrUseOfGameGlobalKeys({
              error: Error()
            }), GameGlobalKeys) : GameGlobalKeys).InterruptProcess, _this15._isInterrupting);
            console.log();
          })();
        ...
        beforeProcessNormalRound() {
          ...
          return _asyncToGenerator(function* () {
            ...
            //---例如:擷取資料之類的關閉面板之類的
            var previousState = _this16._processedServerData.getPrevState();

            if (previousState === (_crd && GameState === void 0 ? (_reportPossibleCrUseOfGameState({
              error: Error()
            }), GameState) : GameState).RE_SPINE) {
              //--關閉reSpin面板
              yield _this16._ngUI.closeReSpinCountUI();
            }

            _this16._ngUI.closeAllUI(); //--準備把isLock按鈕重新設定
            ...
            if (previousState === (_crd && GameState === void 0 ? (_reportPossibleCrUseOfGameState({
              ...
              var totalRoundScoreInfo = _this16._processedServerData.getFGRoundTotalScoreAndBetFixed();
              ...
              _this16._fgUIBoard.setBoardMode((_crd && GameState === void 0 ? (_reportPossibleCrUseOfGameState({
                error: Error()
              }), GameState) : GameState).NORMAL);

              _this16._basicShowAniProcess.cleanAllPlayingBeforeNewStart(); //--切掉輪播


              var endJpRoundDelay = _this16._gameStepDelayTimeList.get(cfg => {
                var _cfg$fg2;

                return (_cfg$fg2 = cfg.fg) == null ? void 0 : _cfg$fg2.beforeOpenWait;
              });

              var p = _this16._async.waitSecondsTracked(endJpRoundDelay, 'beforeOpenFGBoardDelay', null, true);

              yield p.promise;
              yield _this16._basicShowAniProcess.showBigWinAfterFG(totalRoundScoreInfo.odds, totalRoundScoreInfo.betValue); //--1006新增功能(要結束FG後才秀大獎)

              var endOpenDelay = _this16._gameStepDelayTimeList.get(cfg => {
                var _cfg$Jackpot;

                return (_cfg$Jackpot = cfg.Jackpot) == null ? void 0 : _cfg$Jackpot.beforeOpenWait;
              });

              yield _this16.addTweenDelay(endOpenDelay); //--關閉FG面板

              yield Promise.all([_this16._fgUIBoard.openFGUIBoard(totalRoundScoreInfo.score), //--20251125新增需求
              _this16._gameBGSoundCtrl1016.fadeOutFGWithBGM()]); //--將等待大獎畫面秀出時機點放到開啟面板結束後-20260109
        ...
        runTest(value) {
          var _superprop_getRunTest = () => super.runTest,
              _this17 = this;

          return _asyncToGenerator(function* () {
            //--做stop的時候,看要不要在processAfterAllReelRollEnd接續往下測試
            //return;
            _superprop_getRunTest().call(_this17); //(<UniSlotMachine1016>this._slotMachine).testFunction();
            //--開啟面板....


            _this17._fgUI.setTotalFgCount(10);

            var boardOutCallBack = () => {
              _this17._ngUI.closeReSpinCountUI();

              _this17._gameModeManager.changeAllGameState((_crd && GameState === void 0 ? (_reportPossibleCrUseOfGameState({
                error: Error()
              }), GameState) : GameState).FREE_GAME);
            };

            _this17._slotMachine.reSetLockReels(); //--LOCK 解鎖


            _this17._fgUIBoard.setBoardMode((_crd && GameState === void 0 ? (_reportPossibleCrUseOfGameState({
              error: Error()
            }), GameState) : GameState).FREE_GAME);

            _this17._fgUIBoard.setFGPlayTimes(10);

            yield _this17._fgUIBoard.openFGUIBoard(10, boardOutCallBack);
          })();
        } //============================== testMode 狀態 =========================
...
//# sourceMappingURL=399eb8c344a7fc7620dedd680d0c4fd21ace766a.js.map