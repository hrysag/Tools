    ...
    execute: function () {
      ...
      _export("UniReel1016", UniReel1016 = (_dec = ccclass('UniReel1016'), _dec2 = property(_crd && ReelBounceConfig1016 === void 0 ? (_reportPossibleCrUseOfReelBounceConfig({
        ...
        constructor() {
          ...
          this.onStopRoll = /*#__PURE__*/_asyncToGenerator(function* () {
            /*
            ...
            _this.setIconDataAfterRollEnd();

            _this.writeContinueWildData(); //await this.regiestMultipleReelData();


            _this.regiestMultipleReelData();

            _this._endBouncePromise = _asyncToGenerator(function* () {
              yield _this.onEndBounceMotion();
              return _this.reelID;
            })();
            /*
        ...
        reset() {
          ...
          this._isWholeBoardReadyHand = false;
          this._wholeReelDark = false;
          var moveIntervalDefault = (_crd && GlobalAccessReader === void 0 ? (_reportPossibleCrUseOfGlobalAccessReader({
            error: Error()
        ...
        setInitIconData(data) {
          for (var index = 0; index < this.iconList.length; index++) {
            ...
            var sym = new (_crd && SymbolNumber === void 0 ? (_reportPossibleCrUseOfSymbolNumber({
              error: Error()
        ...
        setIconLight(isDark, iconIndex) {
          if (iconIndex) {
            ...
            for (var _index = 0; _index < this._iconList.length; _index++) {
              this._iconList[_index].setIconLight(isDark);
            }
          }
        }
        /**
        ...
        setAllLightExcludeSymbolIds(isDark, excludeSymbolIds) {
          ...
          for (var index = 0; index < this._iconList.length; index++) {
            ...
            var icon = this._iconList[index];
            var id = (_icon$symbol$symbolID = (_icon$symbol = icon.symbol) == null ? void 0 : _icon$symbol.symbolID) != null ? _icon$symbol$symbolID : -1;
        ...
        setIconLightTween(isDark, iconIndex) {
          var _this2 = this;
          ...
          return _asyncToGenerator(function* () {
            ...
            if (iconIndex !== undefined) {
              for (var i = 0; i < iconIndex.length; i++) {
                var index = iconIndex[i];
                promises.push(_this2._iconList[index].setTweenBrightness(isDark));
              }
            } else {
              for (var _index2 = 0; _index2 < _this2._iconList.length; _index2++) {
                promises.push(_this2._iconList[_index2].setTweenBrightness(isDark));
              }
            }
        ...
        setIconLightTweenExcludeSymbolIds(isDark, excludeSymbolIds) {
          var _this3 = this;

          return _asyncToGenerator(function* () {
            var promises = [];

            for (var index = 0; index < _this3._iconList.length; index++) {
              var _icon$symbol$symbolID2, _icon$symbol2;

              var icon = _this3._iconList[index];
              var id = (_icon$symbol$symbolID2 = (_icon$symbol2 = icon.symbol) == null ? void 0 : _icon$symbol2.symbolID) != null ? _icon$symbol$symbolID2 : -1;
        ...
        setIconData(movement) {
          ...
          var targetNode = this.iconList[moveOutIndex]; // 1. 清除舊資料（wild / scatter / spine）
          ...
          var beforeCut = this.data.count; //--test

          var reelID = this.reelID; //--test
        ...
        attachScatterSymbol(targetNode, reelData) {
          var _this4 = this;

          return _asyncToGenerator(function* () {
            //const aniNode = await this.getPrefabNodeFromProxy(reelData);
            var aniNode = _this4.getPrefabNodeFromProxy(reelData);
            ...
            try {
              var spineTarget = aniNode[(_crd && DYN_NODE_PROPERTIES === void 0 ? (_reportPossibleCrUseOfDYN_NODE_PROPERTIES({
                error: Error()
              }), DYN_NODE_PROPERTIES) : DYN_NODE_PROPERTIES).ANIMATION_CTRL];
              var {
                leftover
              } = targetNode.addSymbolAniNode(aniNode);
              if (leftover) _this4.clearAndRecycleWildNodes(leftover);
              aniNode.active = true;
              spineTarget.init();
        ...
        attachNormalSymbol(targetNode, reelData) {
          //const aniNode = await this.getPrefabNodeFromProxy(reelData);
          var aniNode = this.getPrefabNodeFromProxy(reelData);
        ...
        removeAndRecycleImmediately(icon) {
          try {
            var wildNode = icon.checkAndRemoveWildNode();

            if (wildNode) {
              this.clearAndRecycleSpineNode(wildNode); //--清空spineNode的資料 
            }

            var aniNode = icon.checkAniSymbolAndRemove();
        ...
        checkIndexZeroSpriteFrame() {
          if (!this.iconList[0].checkSpriteFrameExist() && !this.iconList[0].checkAniSymbolIsExist()) {
            var randomData = this.createRandomNumber();
            var sym = new (_crd && SymbolNumber === void 0 ? (_reportPossibleCrUseOfSymbolNumber({
              error: Error()
            }), SymbolNumber) : SymbolNumber)();
        ...
        fastStopRoll() {
          ...
          var dataKeepCount = this._iconAmount + 1 + this._endCardCount; //GameUtilsTools.debugLog(DEBUG_TITLE, 'calculateRandomDataLength', { fastStopRoll: 'fastStopRoll' }, 'log');
        ...
        setData(symbolData, randomDataLength) {
          ...
          var randomData = []; //根據randomDataLength生成隨機資料-要演多久決定在此資料的長度

          for (var index = 0; index < randomDataLength; index++) {
            var symbol = this.createRandomNumber();
          ...
          var testStartLen = startCards.length; //--test code

          var testEndLen = endCards.length; //--test code

          var downConsecutiveExtraCardsData = endCards; //--頭開始往下連續
          ...
          if (this._takeExtraCardsInRound != null) {
            ...
            if (endCards.length < this._takeExtraCardsInRound.end.len) {
              ...
              var takeCards = this.getTakeCardsRandomList(limit);
          ...
          var testOutPrevious = (_crd && GameUtilsTools === void 0 ? (_reportPossibleCrUseOfGameUtilsTools({
            error: Error()
          ...
          var resultData = [lastRandomSymbol, ...symbolData, ...downConsecutiveExtraCardsData, ...randomData]; //---這邊要新增補牌的資料-iconList也要補牌
        ...
        clearAndRecycleSpineNode(spineNode) {
          ...
          var symbolId = spineNode[(_crd && DYN_NODE_PROPERTIES === void 0 ? (_reportPossibleCrUseOfDYN_NODE_PROPERTIES({
            error: Error()
          ...
          if (symbolId == WILD_LIST[0]) {
             ...
             */
            var realSpine = spineNode.children[0];
            realSpine.setPosition(v3(0, 0, 0));
          } // 重置 spineNode 上的動畫控制介面資料


          var aniComp = spineNode[(_crd && DYN_NODE_PROPERTIES === void 0 ? (_reportPossibleCrUseOfDYN_NODE_PROPERTIES({
            error: Error()
          }), DYN_NODE_PROPERTIES) : DYN_NODE_PROPERTIES).ANIMATION_CTRL];
        ...
        getWildMovementData(yoyo) {
          ...
          var wildData = this._singleWildGroupResultData[index]; //--wild的起始位置(這邊startIndex要+1,因為算的時候沒有上下兩個預備位)

          var wildContinuoData = this._iconList[wildData.startIndex + 1].wildContinue;
          var strLastMapKey = wildContinuoData[wildContinuoData.length - 1];
          var parts = strLastMapKey.split(':');
          var indexTarget = Number(parts[1]); //--取出(這邊startIndex要+1,因為算的時候沒有上下兩個預備位)
          //const targetIcon = this._iconList[wildData.startIndex + 1];

          var targetIcon = this._iconList[indexTarget]; //--一開始加入的容器

          var result = this.resolveFinalDesIconAndOffset(wildData, targetIcon, yoyo);
          var finalDesIcon = result.finalDesIcon;
          var offsetY = result.offsetY;
          finalDesIcon.isFinalDesIcon = 'isFinalDesIcon'; //--debug

          var finalPos = finalDesIcon.node.position.clone();
          ...
          var finalWPos = finalDesIcon.node.parent.getComponent(UITransform).convertToWorldSpaceAR(finalPos); //--reel node
        ...
        resolveFinalDesIconAndOffset(wildData, targetIcon, yoyo) {
          var resultOffsetY = this.iconSize.y / 2;
          var resultFinalDesIcon; //--檢查現在的anchorPoint狀態(確認目前是掛在哪一種狀態的node(頭相連/腳相連))
        ...
        getNewWildMovementData(yoyo) {
          if (yoyo === void 0) {
            yoyo = false;
          }

          var wildMovementData = this.getWildMovementData(yoyo);
          wildMovementData.isYoyo = yoyo;
          var gameIcon = this._iconList[wildMovementData.startIconIndex];
          /*
          ...
          var startPos = gameIcon.getWildNodePosition();

          var startWPos = this._fakeWildLayerContainer.getComponent(UITransform).convertToWorldSpaceAR(startPos); //--78企劃
        ...
        getHandoffWildMoveAniNode(iconIndex) {
          var gameIcon = this._iconList[iconIndex];
          var continueData = gameIcon.wildContinue; //const wildNode = gameIcon.getWildNode();

          var wildNodeWPos = gameIcon.getWildNodePosition(); //--20260105

          var maxIndex = -1; // 預設沒找到

          for (var data of continueData) {
            var parts = data.split(':');

            var _iconIndex = Number(parts[1]); // 取出 iconIndex


            if (_iconIndex >= 1 && _iconIndex <= 4) {
              if (_iconIndex > maxIndex) {
        ...
        getWildNodePosAfterMoved(wildTargetPos, currentContainer) {
          ...
          var localPos = wildTargetPos; //let uiTransform: UITransform = currentContainer.getComponent(UITransform);
          //--20251116 78企劃

          var uiTransform = this._fakeWildLayerContainer.getComponent(UITransform);

          var wPos = uiTransform.convertToWorldSpaceAR(localPos);
          return wPos;
        } //---播放wild出現動畫
        ...
        playWildAppearAnimation() {
          ...
          return _asyncToGenerator(function* () {
            ...
            for (var i = 0; i < _this5._iconList.length; i++) {
              var icon = _this5._iconList[i];
            ...
            if (targetIcon) {
              yield targetIcon.playWildAppearAnimation(); //--中途有可能被取消掉了
              //const reel = this.reelID;//--debug

              if (_this5._isReadyHand && targetIcon.readyHandFlag) {
        ...
        playWildAniToFg() {
          ...
          return _asyncToGenerator(function* () {
            ...
            for (var i = 0; i < _this6._iconList.length; i++) {
              var icon = _this6._iconList[i];
            ...
            if (targetIcon) {
              yield targetIcon.playWildToFgAnimation();
            }
          })();
        ...
        checkWildIsExist() {
          var wildTarget = this._iconList[4];
        ...
        stopAllExpectAni() {
          var reel = this.reelID; //--debug

          for (var i = 0; i < this._iconAmount; i++) {
            var icon = this._iconList[i];
            icon.setReadyHandState(false);

            if (icon.wildData.isWild && icon.wildContinue.length > 0) {
              var continueData = icon.wildContinue;
              var lastMapKey = continueData[continueData.length - 1];
              var parts = lastMapKey.split(':');
              var indexTarget = Number(parts[1]);
              var targetIcon = this._iconList[indexTarget]; //--一開始加入的容器

              var wildAni = targetIcon.getWildAniCtrl();

              if (wildAni != null) {
                var playInfo = wildAni.currentTarget;
        ...
        getWPosWithTargetIcon(icon, targetNode) {
          var targetIcon = icon;
          var tn = targetNode ? targetNode : targetIcon.node;
          var pos = tn.position.clone();
          var uiTransformComponent = targetIcon.getComponent(UITransform);
          var wPos = uiTransformComponent.getComponent(UITransform).convertToWorldSpaceAR(pos);
          return wPos;
        } //--[[預備start],[1],[2],[3],[4],[預備end]]--實際上this._iconList內容的分布


        getIconNodeWorldPosition(iconIndex, targetNode) {
          var targetIcon = this._iconList[iconIndex];
          var tn = targetNode ? targetNode : targetIcon.node;
          var pos = tn.position.clone();
          var uiTransformComponent = targetIcon.getComponent(UITransform);
          var wPos = uiTransformComponent.getComponent(UITransform).convertToWorldSpaceAR(pos);
        ...
        getParticleWorldPosition() {
          var targetIcon = this._iconList[2];
          var tn = targetIcon.node;
          var pos = tn.position.clone().add(v3(0, -this.iconSize.y / 2, 0));
          var uiTransformComponent = targetIcon.getComponent(UITransform);
          var wPos = uiTransformComponent.getComponent(UITransform).convertToWorldSpaceAR(pos);
          /*
        ...
        getScatterWorldPosition() {
          ...
          for (var targetIcon of this._iconList) {
        ...
        getWildAniNode() {
          ...
          return _asyncToGenerator(function* () {
            for (var icon of _this7.iconList) {
              var {
                symbolID
              } = icon.symbol;
              ...
              if (symbolID == WILD_LIST[0] && icon.wildData.isWild) {
                ...
                var data = _this7.getExistingAniDataNode(registerData);

                if (data.aniNode) {
                  _this7._aniCrossServiceProxyFactory.unRegister(registerData);

                  yield _this7._aniCrossServiceProxyFactory.multiRegistryByID([registerData], (_crd && SymbolOwnerAgentID === void 0 ? (_reportPossibleCrUseOfSymbolOwnerAgentID({
                    error: Error()
                  ...
                  var uiTransform = _this7.node.getComponent(UITransform); //--index=1是整條wild被釘住的end位置


                  var targetIconPos = _this7._iconList[1].node.position.clone();

                  var wPos = uiTransform.convertToWorldSpaceAR(targetIconPos);
        ...
        getForceToHandoffWild() {
          for (var icon of this.iconList) {
            ...
            if (symbolID == 9 && icon.wildData.isWild && icon.wildContinue.length > 0) {
              ...
              var lastMapKey = continueData[continueData.length - 1];
              var parts = lastMapKey.split(':');
              var iconIndex = Number(parts[1]); //--key='reel:iocn:symbol';
        ...
        getScatterAniNode() {
          ...
          return _asyncToGenerator(function* () {
            for (var icon of _this8.iconList) {
              if (icon.isScatter) {
                var registerData = {
                  reelIndex: icon.symbol.reelIndex,
                  iconIndex: icon.symbol.iconIndex,
                ...
                var data = _this8.getExistingAniDataNode(registerData);

                if (data.aniNode) {
                  _this8._aniCrossServiceProxyFactory.unRegister(registerData);

                  yield _this8._aniCrossServiceProxyFactory.multiRegistryByID([registerData], (_crd && SymbolOwnerAgentID === void 0 ? (_reportPossibleCrUseOfSymbolOwnerAgentID({
                    error: Error()
                  }), SymbolOwnerAgentID) : SymbolOwnerAgentID).ShowAniController);
                }
        ...
        getScatterMovementData() {
          var scatterMovementData;

          for (var i = 0; i < this._iconList.length; i++) {
            var targetIcon = this._iconList[i];

            if (targetIcon.isScatter) {
              //targetIcon.symbol.symbolID = 10; // 設定為scatter  
              var finalPos = targetIcon.node.position.clone();
              var localFinalPos = targetIcon.node.getComponent(UITransform).convertToWorldSpaceAR(finalPos);
              scatterMovementData = {
        ...
        findRealWild() {
          ...
          for (var i = 0; i < this._iconList.length; i++) {
            var iconTarget = this._iconList[i];
        ...
        processWildDataAfterMove(playAniData) {
          var wildTargetNode = null; //--直接要算world座標寫回資料
          //--因為每一格都會寫資料進去,所以得分的時候他會去跟handoff要資料(但不是每個都是wild實體,所以這邊要找實體出來)

          var resultWildData = this.findRealWild();

          if (resultWildData != null) {
            wildTargetNode = resultWildData.node;
            var iconTarget = this._iconList[resultWildData.iconIndex];
            var wildWPos = iconTarget.getWildNodePosition(); //--20260105

            var wPos = this.getWildNodePosAfterMoved(wildWPos, iconTarget.node); //let wPos: Vec3 = wildWPos;
        ...
        getExistingAniDataNode(data) {
          var targetIcon = this._iconList[data.iconIndex];
          var symbolId = targetIcon.symbol.symbolID;
          var targetNode = null;
          var assignData = data;
          ...
          if (symbolId == WILD_LIST[0]) //--這邊要做  wild的特殊處理
            {
              ...
              var changeData = this.processWildDataAfterMove(data);
        ...
        setAniNodeBackToReel(data, node_tag) {
          var _this9 = this;
          ...
          return _asyncToGenerator(function* () {
            if (data.symbolId == WILD_LIST[0]) {
              if (!node_tag) {
                ...
                var continueData = _this9._iconList[data.iconIndex].wildContinue;
                var lastMapKey = continueData[continueData.length - 1];
                var parts = lastMapKey.split(':');
                var indexTarget = Number(parts[1]);
                var {
                  leftover
                } = yield _this9.iconList[indexTarget].addWildNode(node_tag);

                if (leftover) {
                  _this9.clearAndRecycleWildNodes(leftover);
                }
              }
            } else {
              var {
                leftover: _leftover
              } = _this9.iconList[data.iconIndex].addSymbolAniNode(node_tag);

              if (_leftover) {
                _this9.clearAndRecycleWildNodes(_leftover);
              } //this.addBackAniNodeToGameIcon(data.iconIndex, node);


              var aniPlayer = node_tag[(_crd && DYN_NODE_PROPERTIES === void 0 ? (_reportPossibleCrUseOfDYN_NODE_PROPERTIES({
                error: Error()
              }), DYN_NODE_PROPERTIES) : DYN_NODE_PROPERTIES).ANIMATION_CTRL];
        ...
        getAddExtraCardsForStart() {
          ...
          var startCards = []; //--從腳開始往上長

          var endCards = []; //--從頭開始往下長

          var maxLength = 4; //--wild的最大長度(1*4) 

          var targetValue = WILD_LIST[0]; //--wild的值

          for (var wildData of this.singleWildGroupResultData) {
            var len = maxLength - wildData.matchIndices.length;
        ...
        processSortIcons(icons) {
          ...
          var wLeader = [];
          var wIcons = [];
          var nIcons = []; //===debugList====

          var debugSc = [];
          var debugWLeader = [];
          var debugWIcons = [];
          var debugNIcons = []; //===debugList====
        ...
        changeSibling(icons) {
          ...
          var parent = icons[0].node.parent; //const parent = this.node.parent!;

          var topIndex = parent.children.length - 1; //console.log('check_reelChildren==>', this.node.children.length, parent.children.length);
          // 1. Normal → 依 _iconList 原始順序，最底層

          for (var i = 0; i < normalIcons.length; i++) {
            normalIcons[i].siblingIndex = i; //normalIcons[i].currentSiblingIndex = i;
          } // 2. Wild 普通 → 疊在 Normal 之上


          for (var icon of wildIcons) {
            icon.siblingIndex = topIndex--; //icon.currentSiblingIndex = icon.siblingIndex;
          } // 3. Wild Leader → 疊在 Wild 普通之上


          for (var _icon of wildLeaderIcons) {
            _icon.siblingIndex = topIndex--; //icon.currentSiblingIndex = icon.siblingIndex;
          } // 4. Scatter → 永遠最高


          for (var _icon2 of scatterIcons) {
            _icon2.siblingIndex = topIndex--;
        ...
        checkSortIconList() {
          ...
          for (var i = 0; i < this._iconList.length; i++) {
            var icon = this._iconList[i];
            var siblingData = icon.getSiblingIndex(); //console.log(`=====checkSortIconList======='+this.reelID+'\n'
        ...
        setIconDataAfterRollEnd() {
          for (var i = 0; i < this._iconList.length; i++) {
            ...
            if (i == 0 && iconTarget.isScatter) {
              ...
              var newSymbolID = this.getSpRandomOnce(ALL_SYMBOL_LIST_NG);
            ...
            if (i == 4 && iconTarget.wildData.isWild && iconTarget.checkWildIsExist()) {
              ...
              var wildNode = iconTarget.getWildNode();
              var targetNode = wildNode.children[0];
              var pos = targetNode.position.clone(); //const resultOffsetY = -10;
            ...
            if (i > this._iconAmount) {
              ...
              if (i == this._iconList.length - 1) {
                 ...
                 * ex:[9,9,0,1,9,9,9,9]
                 */
                var _wildNode = iconTarget.checkAndRemoveWildNode();

                if (_wildNode) {
                  this.clearAndRecycleSpineNode(_wildNode);
                }
              }
            }
          }
        ...
        setLockReel() {
          var r1 = this._iconList[1].symbol.symbolID == 9 ? true : false;
          var r2 = this._iconList[2].symbol.symbolID == 9 ? true : false;
          var r3 = this._iconList[3].symbol.symbolID == 9 ? true : false;
          var r4 = this._iconList[4].symbol.symbolID == 9 ? true : false;
          var onlyOneWholeWild = this._singleWildGroupResultData.length == 1 && //--連續長度與icon數量匹配=整軸wild
          ...
          for (var icon of this._iconList) {
        ...
        reSetWildDataAfterWithoutMove(reelIndex, iconIndex) {
          ...
          return _asyncToGenerator(function* () {
            ...
            _this10._aniCrossServiceProxyFactory.unRegister(playData);

            yield _this10._aniCrossServiceProxyFactory.multiRegisty([playData]); //--自己為註冊者

            _this10._aniCrossServiceProxyFactory.debugCheckAllOwners();
          })();
        }
        /**
         * 20251005-NEW
        ...
        reSetWildDataAfterMove(reelId) {
          var _this11 = this;

          return _asyncToGenerator(function* () {
            //--如果牌是[9,*,*,*]之後又位移[9,9,9,9],再重新整理資料後,第0格的9要處理
            yield _this11.reWriteWildDataAfterMovement(); //--回收註冊,並將沒有參與位移的wild移除

            _this11.setWildDataAfterMoveAndAdd(reelId);

            _this11._aniCrossServiceProxyFactory.debugCheckAllOwners();
          })();
        }
        /**
        ...
        setWildDataAfterMoveAndAdd(reelId) {
          var iconTarget = this._iconList[reelId];
        ...
        reWriteWildDataAfterMovement() {
          ...
          return _asyncToGenerator(function* () {
            ...
            for (var i = 0; i < _this12._iconList.length; i++) {
              var iconTarget = _this12._iconList[i]; //--如果牌是[9,*,*,*]之後又位移[9,9,9,9],再重新整理資料後,第0格的9要處理

              if (i > 0) {
                //--預備位的不要處理
                _this12.removeAndRecycleImmediately(iconTarget);

                iconTarget.resetData();
              }

              if (i > 0 && i <= 4) {
                var targetSymbol = iconTarget.symbol;
                removeWildMap.push({
                ...
                var key = _this12.reelID + ":" + i + ":" + 9; //--擠到node裡面

                continueWild.push(key);
                registerDataList.push({
                  reelIndex: _this12.reelID,
                  iconIndex: targetSymbol.iconIndex,
                  symbolId: 9,
                  tokenId: '',
                  aniId: ''
            ...
            for (var j = 0; j < _this12._iconAmount; j++) {
              _this12._iconList[j + 1].wildContinue = continueWild;
              _this12._iconList[j + 1].symbol.symbolID = 9;
              _this12._iconList[j + 1].wildData.isWild = true;
              _this12._iconList[j + 1].wildData.wildIndex = j + 1;
            }

            _this12._wildCount = 0; //--wild的計數器歸零

            _this12._upConsecutiveExtraCardsData = [];
            _this12._hasWildMoved = true;
            _this12._isLock = true;

            _this12._aniCrossServiceProxyFactory.multiUnRegister(removeWildMap);

            yield _this12._aniCrossServiceProxyFactory.multiRegisty(registerDataList); //--自己為註冊者

            _this12._aniCrossServiceProxyFactory.debugCheckAllOwners();
          })();
        }

        writeContinueWildData() {
          for (var i = 0; i < this._singleWildGroupResultData.length; i++) {
            var wildDataTarget = this._singleWildGroupResultData[i];
            var targetData = wildDataTarget.matchIndices;
            var startIndex = wildDataTarget.startIndex + 1;
            var indexInfo = [];
            var continueWild = []; //--直接搜到底*4--從頭開始往下長(拿頭的)--往下連續-補牌在前端

            if (wildDataTarget.direction == (_crd && Direction === void 0 ? (_reportPossibleCrUseOfDirection({
              error: Error()
            }), Direction) : Direction).DOWNWARD) {
              for (var _i = 0; _i < 4; _i++) {
                var iconDataIndex = startIndex + _i;
                var key = this.reelID + ":" + iconDataIndex + ":" + 9; //--擠到node裡面
              ...
              for (var j = 0; j < targetData.length; j++) {
                var _iconDataIndex = targetData[j] + 1;

                var _key = this.reelID + ":" + _iconDataIndex + ":" + 9; //--擠到node裡面


                continueWild.push(_key);
                indexInfo.push(_iconDataIndex);
              }
            }

            for (var _iconDataIndex2 of indexInfo) {
              var target = this._iconList[_iconDataIndex2];
        ...
        regiestMultipleReelData() {
          ...
          for (var i = 0; i < this._iconAmount; i++) {
            var iconTarget = this._iconList[i + 1]; //--因為iconList[0]是預備牌()
        ...
        onEndBounceMotion() {
          ...
          return _asyncToGenerator(function* () {
            if (_this13.bounceConfig.endBounce) {
              _this13.resetMovements(); //GameUtilsTools.debugLog(DEBUG_TITLE, 'onEndBounceMotion', { reel: this.reelID });


              yield _this13.bouncingAsync(_this13.bounceConfig.bounceDis, 'down');
            }
          })();
        }
        /**
        ...
        bouncingAsync(dis, upDown) {
          var _this14 = this;

          return _asyncToGenerator(function* () {
            return new Promise((resolve, reject) => {
              var downEasing = _this14.bounceConfig.downBounceEasing;
              var downDuration = _this14.bounceConfig.downBounceDuration;
              var upEasing = _this14.bounceConfig.upBounceEasing;
              var upDuration = _this14.bounceConfig.upBounceDuration;
              var downRealCurve = _this14.bounceConfig.useRealCurve ? _this14.bounceConfig.downBounceRealCurve : null;
              var upRealCurve = _this14.bounceConfig.useRealCurve ? _this14.bounceConfig.upBounceRealCurve : null;

              for (var i = 0; i < _this14._iconList.length; ++i) {
                if (upDown == 'up') {
                  //--down往上提
                  _this14._iconList[i].moveBy(_this14.moveDir.multiplyScalar(dis).negative(), downDuration, downEasing, downRealCurve); //--up往下沉


                  _this14._iconList[i].moveBy(_this14.moveDir.multiplyScalar(dis), upDuration, upEasing, upRealCurve);
                } else {
                  //--up往下沉
                  _this14._iconList[i].moveBy(_this14.moveDir.multiplyScalar(dis), upDuration, upEasing, upRealCurve); //--down往上提


                  _this14._iconList[i].moveBy(_this14.moveDir.multiplyScalar(dis).negative(), downDuration, downEasing, downRealCurve);
                }
              }

              _this14._iconList[0].addCallback(() => resolve());
            });
          })();
        }
        /**
        ...
        generateRandomSymbolList(amount) {
          ...
          var uniqueList = this.getTargetUniqueSymbolList(); // 混合亂數池 (一般符號 + 特殊符號)

          var randomSingleList = [...sourceList, ...uniqueList]; // 初始化 randomMap (使用 sourceList 的符號)
          ...
          var patterns = randomMap[patternSize];
          var pattern = patterns[Math.floor(Math.random() * patterns.length)]; // 決定補牌位置 (頭或尾)

          var fillPosition = Math.random() < 0.5 ? 'head' : 'tail'; // 計算需要補多少張牌 (中間部分 index 1-4 共 4 格)

          var singlesNeeded = 4 - patternSize; // 從 randomSingleList 中隨機抽取不重複的值

          var availableSingles = [...randomSingleList];
          var fillerValues = [];
          ...
          var reel = [randomSingleList[Math.floor(Math.random() * randomSingleList.length)], // index 0
        ...
        getSpRandomOnce(randomSource) {
          var sourceList = randomSource;
          var randomIndex = randomRangeInt(0, sourceList.length - 1);
        ...
        getTargetAllSymbolList() {
          ...
          var globalGameState = (_crd && GlobalAccessReader === void 0 ? (_reportPossibleCrUseOfGlobalAccessReader({
            error: Error()
          }), GlobalAccessReader) : GlobalAccessReader).getGlobalData((_crd && GameGlobalKeys === void 0 ? (_reportPossibleCrUseOfGameGlobalKeys({
            error: Error()
        ...
        testHideIcon(iconIndex) {
          var iconTarget = this._iconList[iconIndex];
          iconTarget.testHideIcon();
        }

        testAddSymbol(iconIndex) {
          var node = (_crd && AnimationControllersPoolManager === void 0 ? (_reportPossibleCrUseOfAnimationControllersPoolManager({
            error: Error()
...
//# sourceMappingURL=0a92846a5e2506e626a272ebd369bd0b86e25316.js.map