    ...
    execute: function () {
      ...
      _export("NG_UI_Display", NG_UI_Display = (_dec = ccclass('NG_UI_Display'), _dec2 = property({
        ...
        findAndGetPlayData(aniState) {
          for (var aniData of this._animationPlayInfoList.clipsInfo) {
        ...
        playSpinePromise(aniState, duration) {
          ...
          if (duration) {
            ...
            var targetDuration = (_this$_doorSpine$find = this._doorSpine.findAnimation(aniState.targetName)) == null ? void 0 : _this$_doorSpine$find.duration;
        ...
        openReSpinCountUI() {
          ...
          return _asyncToGenerator(function* () {
            //--開門:0.5sec
            if (_this._isRsOpen) return Promise.resolve();
            var timeout = 0.5;

            var data = _this.findAndGetPlayData(ANIMATION_NGUI_TYPE.RS_OPEN_DOOR);
            ...
            if (data) {
              ...
              _this._isRsOpen = true;

              var signal = _this._async.createAbortScope(SIGNAL_KEY);

              _this._doorSpine.timeScale = 1; //--20260209-FIX-開門動畫有被改過,所以每次開門前先還原

              var outCancel = value => {
                //--取消使用-直接開門--(因為美術沒有一個是整個打開的狀態,這邊就加速處理到最後一格)
                _this._doorSpine.timeScale = 100; //--直接加快播到最後
              ...
              var p = _this._async.withTimeout(_this.playSpinePromise(data, dt), timeout, //--race time
              {
              ...
              if (flag) {
                ...
                _this._async.abortAll(SIGNAL_KEY);
              }

              var result = yield p.promise;
            }
          })();
        }
        ...
        closeReSpinCountUI() {
          ...
          return _asyncToGenerator(function* () {
            if (!_this2._isRsOpen) return Promise.resolve();
            var timeout = 0.5;

            var data = _this2.findAndGetPlayData(ANIMATION_NGUI_TYPE.RS_CLOSE_DOOR);

            if (data) {
              _this2._isRsOpen = false;
              ...
              var outCancel = value => {//--取消使用-直接開門--(因為美術沒有一個是整個打開的狀態,這邊就加速處理到最後一格)
              ...
              var p = _this2._async.withTimeout(_this2.playSpinePromise(data, dt), timeout, //--race time
              {
              ...
              if (flag) {
                ...
                _this2._async.abortAll(SIGNAL_KEY);
              }

              var result = yield p.promise; //const result = await this.playSpineWithTimeout(data, 0.5);
        ...
        openFGCountUI() {
          ...
          return _asyncToGenerator(function* () {
            if (_this3._isFGOpen) return Promise.resolve();
            var timeout = 0.5;

            var data = _this3.findAndGetPlayData(ANIMATION_NGUI_TYPE.FG_OPEN_DOOR);

            if (data) {
              _this3._isFGOpen = true; //const result = await this.playSpineWithTimeout(data, 0.5);
              ...
              var signal = _this3._async.createAbortScope(SIGNAL_KEY);

              _this3._doorSpine.timeScale = 1;

              var outCancel = value => {
                //--取消使用-直接開門--(因為美術沒有一個是整個打開的狀態,這邊就加速處理到最後一格)
                _this3._doorSpine.timeScale = 100; //--直接加快播到最後
                //this._doorSpine.clearTrack(data.trackIndex);
                //const defaultRSDoor = this.findAndGetPlayData(ANIMATION_NGUI_TYPE.RS_DEFAULT_DOOR);
                //this._doorSpine.setAnimation(defaultRSDoor.trackIndex, defaultRSDoor.targetName, false);
              };
              ...
              var p = _this3._async.withTimeout(_this3.playSpinePromise(data, dt), timeout, //--race time
              {
              ...
              if (flag) {
                _this3._async.abortAll(SIGNAL_KEY);
              }

              var result = yield p.promise; //GameUtilsTools.debugLog(LOG_TITLE, 'openFGCountUI', { result });
            }
          })();
        } //--美術沒做這個動畫
        ...
        closeFGCountUI() {
          ...
          return _asyncToGenerator(function* () {
            if (!_this4._isFGOpen) return Promise.resolve();
            var timeout = 0.5;

            var data = _this4.findAndGetPlayData(ANIMATION_NGUI_TYPE.FG_CLOSE_DEFAULT_DOOR);
            ...
            if (data) {
              ...
              _this4._isFGOpen = false;

              var signal = _this4._async.createAbortScope(SIGNAL_KEY);

              _this4._doorSpine.timeScale = 1;

              var outCancel = value => {
                //--取消使用-直接開門--(因為美術沒有一個是整個打開的狀態,這邊就加速處理到最後一格)
                console.log('closeFGCountUI outCancel', value);
                _this4._doorSpine.timeScale = 100; //--直接加快播到最後
                //this._doorSpine.clearTrack(data.trackIndex);
                //const defaultRSDoor = this.findAndGetPlayData(ANIMATION_NGUI_TYPE.RS_DEFAULT_DOOR);
                //this._doorSpine.setAnimation(defaultRSDoor.trackIndex, defaultRSDoor.targetName, false);
              };

              var p = _this4._async.withTimeout(_this4.playSpinePromise(data, dt), timeout, //--race time
              {
              ...
              if (flag) {
                _this4._async.abortAll(SIGNAL_KEY);
              }
        ...
        closeAllUI() {
          ...
          this.setReSpinCount();
          this.setFGCount();
          var defaultRSDoor = this.findAndGetPlayData(ANIMATION_NGUI_TYPE.RS_DEFAULT_DOOR);

          this._doorSpine.setAnimation(defaultRSDoor.trackIndex, defaultRSDoor.targetName, defaultRSDoor.loop);

          var defaultFGDoor = this.findAndGetPlayData(ANIMATION_NGUI_TYPE.FG_CLOSE_DEFAULT_DOOR);
        ...
        triggerFGCountUp(value) {
          var _this5 = this;

          return _asyncToGenerator(function* () {
            var timeout = 0.5;

            _this5.setFGCount(value);

            var data = _this5.findAndGetPlayData(ANIMATION_NGUI_TYPE.FG_COUNT);
            ...
            if (data) {
              ...
              var signal = _this5._async.createAbortScope(SIGNAL_KEY);
              ...
              var p = _this5._async.withTimeout(_this5.playSpinePromise(data, dt), timeout, //--race time
              {
              ...
              if (flag) {
                _this5._async.abortAll(SIGNAL_KEY);
              }

              var result = yield p.promise; //GameUtilsTools.debugLog(LOG_TITLE, 'triggerFGCountUp', { result });
        ...
        triggerReSpinCountUp(value) {
          ...
          return _asyncToGenerator(function* () {
            ...
            _this6.setReSpinCount(value);

            var data = _this6.findAndGetPlayData(ANIMATION_NGUI_TYPE.RS_COUNT);
            ...
            if (data) {
              ...
              var signal = _this6._async.createAbortScope(SIGNAL_KEY);
              ...
              var p = _this6._async.withTimeout(_this6.playSpinePromise(data, dt), timeout, //--race time
              {
              ...
              if (flag) {
                _this6._async.abortAll(SIGNAL_KEY);
              }

              var result = yield p.promise; //GameUtilsTools.debugLog(LOG_TITLE, 'triggerReSpinCountUp', { result });
            }
          })();
        }
        ...
        getFGCountWPos() {
          var parentUiTransform = this._fgCountNode.parent.getComponent(UITransform);

          if (parentUiTransform) {
            var lPos = this._fgCountNode.position.clone();

            var wPos = parentUiTransform.convertToWorldSpaceAR(lPos);
            return wPos;
          }
        ...
        getReSpinCountWPos() {
          var parentUiTransform = this._reSpinCountNode.parent.getComponent(UITransform);

          if (parentUiTransform) {
            var lPos = this._reSpinCountNode.position.clone();
...
//# sourceMappingURL=2e45ce9f2478625d25fae15a8e76dfcfd97031c8.js.map