  ...
  return {
    ...
    execute: function () {
      ...
      _export("SymbolAniHandoffManager", SymbolAniHandoffManager = class SymbolAniHandoffManager {
        ...
        register(info, owner) {
          this.ensureOwnerRegistered(owner);
          var key = this.makeKey(info);

          var prev = this._registry.get(key);
          ...
          if (!prev) {
            ...
            var overwritten = prev.ownerId !== owner.ownerId;
        ...
        multiRegistryByID(info, targetOwnerId) {
          var owner = this.findOwnerById(targetOwnerId);
        ...
        getInfoByOwnerAgent(info, owner) {
          ...
          var entry = this._registry.get(key);
        ...
        getOwner(info) {
          var entry = this._registry.get(this.makeKey(info));
        ...
        getAllOwnersInCell(reelIndex, iconIndex) {
          ...
          for (var {
            ...
            if (info.reelIndex === reelIndex && info.iconIndex === iconIndex) {
              var owner = this.getOwnerById(ownerId);
        ...
        handoffByOwnerId(currentOwner, targetOwnerId) {
          var newOwner = this.findOwnerById(targetOwnerId);
          ...
          for (var [key, entry] of this._registry.entries()) {
            if (entry.ownerId !== currentOwner.ownerId) continue;
            var targetNode = currentOwner.beforeRelease(entry.info);
        ...
        handoffSingleByOwnerId(info, targetOwnerId) {
          ...
          return _asyncToGenerator(function* () {
            var key = _this.makeKey(info);

            var entry = _this._registry.get(key);

            if (!entry) {
              _this._log('HANDOFF_SINGLE_MISS_ENTRY', {
                key,
                targetOwnerId
              }, 'warn');
            ...
            if (entry.ownerId === targetOwnerId) {
              _this._log('HANDOFF_SINGLE_NOOP', {
                key,
            ...
            var currentOwner = _this.getOwnerById(entry.ownerId);

            if (!currentOwner) {
              _this._log('HANDOFF_SINGLE_MISS_CURRENT_OWNER', {
                key,
            ...
            var newOwner = _this.findOwnerById(targetOwnerId);
            ...
            if (!newOwner) {
              ...
              var allNewOwner = _this.findOwnerById(targetOwnerId);

              if (!allNewOwner) {
                _this._log('HANDOFF_SINGLE_MISS_TARGET_OWNER', {
                  key,
                  targetOwnerId
                }, 'warn');
                ...
                _this.register(info, allNewOwner);

                newOwner = _this.findOwnerById(targetOwnerId);
              }
            }

            var targetNode = currentOwner.beforeRelease(info);
            yield newOwner.afterAcquire(info, targetNode);

            _this._registry.set(key, {
              info,
              ownerId: newOwner.ownerId
            }); //this.debugCheckAllOwners();


            _this._log('HANDOFF_SINGLE_OK', {
              key,
        ...
        handoff(info, newOwner) {
          var key = this.makeKey(info); //console.log('useHandoff', key, info, newOwner);
          //--for test---
          //this.debugCheckAllOwners();
          ...
          var currentOwner = this.getOwnerById(entry.ownerId);
          ...
          var targetNode = currentOwner.beforeRelease(info); // 位置資料已經在 info 裡
        ...
        multiHandoffBySameOwnerID(infos, targetOwnerId) {
          ...
          return _asyncToGenerator(function* () {
            var multiTargetNode = new Map();

            var newOwner = _this2.findOwnerById(targetOwnerId);

            if (!newOwner) {
              _this2._log('MULTI_HANDOFF_MISS_TARGET_OWNER', {
                targetOwnerId,
                total: infos.length
              }, 'warn');
            ...
            for (var info of infos) {
              var key = _this2.makeKey(info);

              var entry = _this2._registry.get(key);
              ...
              var currentOwner = _this2.getOwnerById(entry.ownerId);
            ...
            yield newOwner.afterMultiAcquire(multiTargetNode);

            _this2._log('MULTI_HANDOFF_BY_ID', {
              targetOwnerId,
              moved,
        ...
        multiHandoffBySameOwner(infos, newOwner) {
          ...
          return _asyncToGenerator(function* () {
            ...
            for (var info of infos) {
              var key = _this3.makeKey(info); //console.log('useHandoff', key, info, newOwner);


              var entry = _this3._registry.get(key);
              ...
              var currentOwner = _this3.getOwnerById(entry.ownerId);
              ...
              _this3.ensureOwnerRegistered(newOwner);
        ...
        unRegister(info) {
          ...
          var had = this._registry.delete(key); //this._registry.delete(this.makeKey(info));
        ...
        multiUnRegister(infos) {
          for (var info of infos) {
            ...
            var had = this._registry.delete(key);
        ...
        debugCheckAllOwners() {
          //return;
          for (var [key, entry] of this._registry.entries()) {
            var owner = this.getOwnerById(entry.ownerId);
            /*
...
//# sourceMappingURL=b3eec8e10d570ba8ac7800572e35ac5e32714944.js.map