  ...
  return {
    ...
    execute: function () {
      ...
      _export("CountTimesFXController", CountTimesFXController = (_dec = ccclass('CountTimesFXController'), _dec(_class = class CountTimesFXController extends Component {
        constructor() {
          ...
          this.cancelAll = () => {
            ...
            while (this.node.children.length > 0) {
              ...
              var testComponent = (_crd && AniSysTools === void 0 ? (_reportPossibleCrUseOfAniSysTools({
                error: Error()
              }), AniSysTools) : AniSysTools).findAndGetIAniComponent(child);

              if (testComponent) {
                var ani = testComponent.ani;
        ...
        playCountTimesFX(info, totalDuration) {
          var _this = this;
          ...
          return _asyncToGenerator(function* () {
            ...
            var uiTransform = _this.node.getComponent(UITransform);

            var count = info.length;
            if (count === 0) return; //---全部同時開始，但每顆飛行時間依照數量壓縮

            var baseFlyTime = 0.3; // 原本每顆粒子基準飛行時間

            var minFlyTime = 0.1; // 不要低於這個值，避免太快不明顯

            var flyTime = Math.max(minFlyTime, totalDuration / count);
            var taskList = [];
            ...
            var _loop = function* _loop() {
              ...
              var localStart = uiTransform.convertToNodeSpaceAR(startPos);
              var localEnd = uiTransform.convertToNodeSpaceAR(endPos);
              var particleNode = yield _this._particlePool.getParticleNode();
              ...
              var p = _this.shootParticle(particleNode, localStart, localEnd, {
                duration: flyTime
              });
              ...
              var task = _asyncToGenerator(function* () {
              ...
              _this._async.registerCancelablePromise(SIGNAL_KEY + ("_" + i), task, cancel, single, SIGNAL_KEY);
        ...
        shootParticle(particleNode, startPos, endPos, opt) {
          ...
          var duration = (_opt$duration = opt == null ? void 0 : opt.duration) != null ? _opt$duration : 0.3; // ← 外部 now 決定長度

          var amplitude = (_opt$amplitude = opt == null ? void 0 : opt.amplitude) != null ? _opt$amplitude : 50;
          var frequency = (_opt$frequency = opt == null ? void 0 : opt.frequency) != null ? _opt$frequency : 0.5;
          ...
          var tw;
          var p = new Promise( /*#__PURE__*/_asyncToGenerator(function* (resolve, reject) {
            resolveFunc = resolve;
            tw = tween(particleNode).to(duration, {}, {
              ...
              try {
                ...
                _this2.node.removeChild(particleNode);
...
//# sourceMappingURL=d9e9c9e71483ab7d85873131513e3e28df3c2f06.js.map