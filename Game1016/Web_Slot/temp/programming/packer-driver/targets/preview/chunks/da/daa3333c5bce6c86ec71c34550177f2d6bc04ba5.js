      ...
      _export("BasicJPShowWinCtrl", BasicJPShowWinCtrl = (_dec = ccclass('BasicJPShowWinCtrl'), _dec2 = property({
        ...
        constructor() {
          ...
          this.blockBtnClickHandler = /*#__PURE__*/_asyncToGenerator(function* () {
            ...
            var currentTime = Date.now() - _this._startTime;

            var canInterruptTime = (_this$_interruptTimeD = _this._interruptTimeData.get(_this._currentJpType)) == null ? void 0 : _this$_interruptTimeD.canInterruptTime; // === 步骤 1: 检查是否可以被中断 ===
            ...
            var boardState = {
              isGoIn: _this._currentJpBoard.isGoIn,
              isInLoop: _this._currentJpBoard.isInLoop,
              isInLoopStage: _this._currentJpBoard.isInLoopStage,
              isClosing: _this._currentJpBoard.isClosing
            ...
            var isSecondClick = boardState.isInLoop && boardState.isInLoopStage;

            if (isSecondClick) {
              // ========== 第二次点击：Loop 等待期间点击，立即中断 ==========
              //GameUtilsTools.debugLog(DEBUG_LOG_TITLE, '[Click] Second click - Force interrupt Loop');
              //  标记第一次点击被中止
              _this._firstClickAborted = true; // 移除监听器（不需要第三次点击了）
            ...
            if (_this._isProcessingClick) {
              //GameUtilsTools.debugLog(DEBUG_LOG_TITLE, '[Click] First click already processing, ignored');
              return;
            }

            _this._isProcessingClick = true;
            _this._firstClickAborted = false; // 🆕 重置中止标记

            try {
              //GameUtilsTools.debugLog(DEBUG_LOG_TITLE, '[Click] First click - Enter Loop Stage');
              // 标记流程被中断
              _this._isInterrupted = true; // 强制结束跑分

              _this._jpDigitsAniNumber.showFinishWinScore(); // 结束音效


              _this.onScoreRunEnd(true); // 恢复背景音乐


              _this.fadeInOrOutBGMusic(0); // 处理板子动画（会进入 Loop 等待阶段）
              // 注意：这里会 await，在等待期间用户可以第二次点击


              yield _this.processAniOnClick(); // 检查是否被第二次点击中止

              if (_this._firstClickAborted) {
              ...
              // 重置处理标记
              _this._isProcessingClick = false;
            }
          });
        ...
        register() {//this._async = AsyncScope.getInstance();
        }
        ...
        showJPWin(odds, totalBet) {
          ...
          return _asyncToGenerator(function* () {
            _this2._startTime = Date.now(); //--記錄開始時間(阻斷的時候會需要用到)

            return new Promise( /*#__PURE__*/_asyncToGenerator(function* (resolve, reject) {
              var _this2$_interruptTime, _this2$_interruptTime2, _this2$_interruptTime3, _this2$_blockSensor;
              ...
              _this2._isInterrupted = false;
              _this2._firstClickAborted = false;
              _this2._currentJpType = _this2.getJpType(odds);

              if (_this2._currentJpType === null) {
                //GameUtilsTools.debugLog(DEBUG_LOG_TITLE, 'showJPWin', { odds, totalBet });
                return;
              }

              _this2._currentJpBoard = _this2._mapJpTypeToBoard.get(_this2._currentJpType);
              _this2._currentJpBoard.jackpotLoopDuration = (_this2$_interruptTime = _this2._interruptTimeData.get(_this2._currentJpType)) == null ? void 0 : _this2$_interruptTime.loopDurationTime;
              _this2._currentJpBoard.fastLoopDuration = (_this2$_interruptTime2 = _this2._interruptTimeData.get(_this2._currentJpType)) == null ? void 0 : _this2$_interruptTime2.fastLoopDuration;
              _this2._jpDigitsAniNumber.duration = (_this2$_interruptTime3 = _this2._interruptTimeData.get(_this2._currentJpType)) == null ? void 0 : _this2$_interruptTime3.runDurationTime;

              _this2.addFrameEventCallBack(); //this._currentJpBoard.node.active = true;
              //--流程-4.8S跑分 2秒停留Loop


              (_this2$_blockSensor = _this2._blockSensor) == null || _this2$_blockSensor.on(Node.EventType.TOUCH_END, _this2.blockBtnClickHandler);
              var totalScore = (odds * totalBet).fixed(); //====step1 進場動畫+播放音效===============================

              _this2.processBoardIn(); //--播放動畫


              _this2.processPlayJPSound(); //--播放音效
              ...
              yield _this2.processRunScoreLabel(totalScore); //--檢查中斷

              if (_this2._isInterrupted) {
                //GameUtilsTools.debugLog(DEBUG_LOG_TITLE, '[showJPWin] Interrupted after runScore');
                return;
              } //====step3 wait loop========================================


              var loopDurationTime = _this2._currentJpBoard.jackpotLoopDuration;
              yield _this2._currentJpBoard.waitLoopDuration(loopDurationTime);
              ...
              _this2.onScoreRunEnd(false);

              yield _this2.processBoardGoLoopAndClose();

              _this2.fadeOutFinish();
        ...
        processRunScoreLabel(value) {
          var _this3 = this;

          return _asyncToGenerator(function* () {
            yield _this3._jpDigitsAniNumber.showJpDigitsAniNumber(value);
          })();
        }
        ...
        processBoardOut() {
          ...
          return _asyncToGenerator(function* () {
            yield _this4._currentJpBoard.forceOutBoard(); //this.fadeOutFinish();
          })();
        } //--override it
        ...
        processBoardGoLoopAndClose() {
          var _this5 = this;

          return _asyncToGenerator(function* () {
            yield _this5._currentJpBoard.goLoopAndClose();
          })();
        }
        ...
        processAniOnClick() {
          ...
          return _asyncToGenerator(function* () {
            yield _this6._currentJpBoard.onClickForceOutJpAni();
          })();
        } //=============================<處理聲音相關程序>================================================================================
        //--override it
...
//# sourceMappingURL=daa3333c5bce6c86ec71c34550177f2d6bc04ba5.js.map