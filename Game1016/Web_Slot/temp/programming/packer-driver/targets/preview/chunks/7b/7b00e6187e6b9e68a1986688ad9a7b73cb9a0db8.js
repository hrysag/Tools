    ...
    execute: function () {
      ...
      _export("UniTimer", UniTimer = class UniTimer {
        ...
        getTaskById(id) {
          ...
          var queueArray = this._params.toArray();

          var found = queueArray.find(task => task.config.id === id);
        ...
        getTasksByTag(tag) {
          ...
          var matchingTasks = queueArray.filter(task => task.config.tag === tag);
        ...
        getTaskSummary() {
          ...
          if (this._currentTask) {
            var config = this._currentTask.config;
            result.push({
              id: config.id,
              tag: config.tag
            });
        ...
        */
        ...
        addTask(durationOrTask, onUpdate, onComplete, onStart) {
          ...
          var execution = new UniRuntimeTask(task);
        ...
        addParallel(tasks) {
          ...
          var maxDuration = Math.max(...tasks.map(task => task.duration)); // 為每個任務創建執行對象來跟蹤狀態

          var executions = tasks.map(task => new UniRuntimeTask(task)); // 創建一個包裝的 UniTask 來管理整個並行組

          var parallelParam = new UniTask();
          ...
          parallelParam.onUpdate = progress => {
            executions.forEach(execution => {
              if (execution.config.duration > 0 && execution.state === UniTaskState.Running) {
                // 計算每個子任務的進度
                var taskProgress = Math.min(1, progress * maxDuration / execution.config.duration);
        ...
        stop() {
          ...
          while (this._params.count > 0) {
            var execution = this._params.dequeue();
        ...
        _onTaskComplete() {
          ...
          var allCompleted = this._params.isEmpty; // 重置當前任務
...
//# sourceMappingURL=7b00e6187e6b9e68a1986688ad9a7b73cb9a0db8.js.map