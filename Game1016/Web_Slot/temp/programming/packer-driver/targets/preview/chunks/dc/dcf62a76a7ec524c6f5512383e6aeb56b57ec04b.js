    ...
    execute: function () {
      ...
      _export("ProcessDataAfterServer", ProcessDataAfterServer = class ProcessDataAfterServer {
        ...
        isFirstOfCurrentState() {
          ...
          var first = this._firstIndexByState.get(step.state);
        ...
        isFirstReSpin() {
          ...
          var first = this._firstIndexByState.get((_crd && GameState === void 0 ? (_reportPossibleCrUseOfGameState({
            error: Error()
        ...
        isLastOfCurrentState() {
          ...
          var last = this._lastIndexByState.get(step.state);
        ...
        buildTimeline(server) {
          ...
          if (Array.isArray(server == null ? void 0 : server.reSpinReelInfo)) {
            for (var i = 0; i < server.reSpinReelInfo.length; i++) {
              var r = server.reSpinReelInfo[i];
              steps.push({
                state: (_crd && GameState === void 0 ? (_reportPossibleCrUseOfGameState({
                  error: Error()
                }), GameState) : GameState).RE_SPINE,
          ...
          if (Array.isArray(server == null ? void 0 : server.freeGameReelInfo)) {
            for (var j = 0; j < server.freeGameReelInfo.length; j++) {
              var f = server.freeGameReelInfo[j];
              steps.push({
                state: (_crd && GameState === void 0 ? (_reportPossibleCrUseOfGameState({
                  error: Error()
                }), GameState) : GameState).FREE_GAME,
        ...
        hasUpcomingState(state) {
          for (var i = this._roundIdx + 1; i < this._timeline.length; i++) {
        ...
        getCurrentStepForClick() {
          var i = this._pendingStopIdx >= 0 ? this._pendingStopIdx : this._roundIdx;
          var step = null;
        ...
        getRoundBetAndOdds() {
          ...
          if (this._server) {
            var currentData = this.getCurrentData();
            var odds = currentData ? currentData.totalOdd : 0;
        ...
        getALLRoundTotalScoreAndBetFixed() {
          ...
          var score = (this._server.allRoundOdds * this._server.betValue).fixed();
        ...
        getReSpinRoundTotalScoreAndBetFixed() {
          ...
          var score = (this._server.totalOddsForReSpin * this._server.betValue).fixed();
        ...
        getFGRoundTotalScoreAndBetFixed() {
          ...
          var score = (this._server.totalOddsForFG * this._server.betValue).fixed();
...
//# sourceMappingURL=dcf62a76a7ec524c6f5512383e6aeb56b57ec04b.js.map