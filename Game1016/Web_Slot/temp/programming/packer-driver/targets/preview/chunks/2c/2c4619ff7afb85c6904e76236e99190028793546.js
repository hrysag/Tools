      ...
      _export("SpineController", SpineController = (_dec = ccclass('SpineController'), _dec2 = property({
        ...
        onLoad() {
          ...
          if (!this._animationPlayInfoList) {
            ...
            for (var data of this._animationPlayInfoList.clipsInfo) {
              if (data.useDefault) {
        ...
        testBtnEvent(ev, value) {
          ...
          return _asyncToGenerator(function* () {
            ...
            _this.playAni((_crd && AnimationStateType === void 0 ? (_reportPossibleCrUseOfAnimationStateType({
              error: Error()
        ...
        saveOriginAniData(spinePlayInfo) {
          ...
          for (var data of this._animationPlayInfoList.clipsInfo) {
            var playData = new (_crd && SpineCtrlPropDef === void 0 ? (_reportPossibleCrUseOfSpineCtrlPropDef({
              error: Error()
        ...
        reSetPlayInfoToOriginData(playId) {
          var originData = this._originAniData.get(playId);

          if (!originData) return;

          var targetData = this._animationPlayInfoList.clipsInfo.find(d => d.targetName === playId);
        ...
        setAniDataInfo(value) {
          ...
          var playData = this._resolver.resolveProp(value.targetName);
        ...
        reversePlay(value) {
          if (this._spine) {
            var playData = this.resolveTargetName(value);
            ...
            var _trackEntry = this._spine.setAnimation(trackIndex, playData.targetName, false);

            _trackEntry.trackTime = _trackEntry.animation.duration;
            this.setCurrentSpineAniData(_trackEntry);
            this.isPlaying = true;
        ...
        gotoAndPlayByTime(value, time) {
          if (this._spine) {
            ...
            var _trackEntry2 = this._spine.setAnimation(trackIndex, playData.targetName, playData.loop);

            this._spine.timeScale = 0; //--停止播放

            this.setCurrentSpineAniData(_trackEntry2);
            var moveToStartTime = time;
            if (moveToStartTime <= 0) moveToStartTime = 0;
            _trackEntry2.animationStart = moveToStartTime;
            _trackEntry2.animationEnd = this._aniEndTime;
        ...
        gotoAndPlayByFrame(value, frame) {
          if (this._spine) {
            ...
            var _trackEntry3 = this._spine.setAnimation(trackIndex, playData.targetName, playData.loop);

            this._spine.timeScale = 0; //--停止播放

            this.setCurrentSpineAniData(_trackEntry3);
            var moveToStartTime = frame * this._secondsPerFrame;
            if (moveToStartTime <= 0) moveToStartTime = 0;
            _trackEntry3.animationStart = moveToStartTime;
            _trackEntry3.animationEnd = this._aniEndTime;
            this._spine.timeScale = timeScale; //---回復播放
          }
        }
        /**
        ...
        changePlayTime(time, value) {
          ...
          if (this._spine) {
            var entry = this._spine.getCurrent(this._currentTarget.trackIndex); //console.log('changePlayTime_entry:', entry);
        ...
        changePlayInfo(value, t, infoTarget) {
          ...
          var aniCtrlInfo = this.getAniPlayDataByPlaySelector(value);
          if (!aniCtrlInfo) return;

          var ani = this._spine.findAnimation(aniCtrlInfo.targetName);

          if (!ani) return;
          var duration = ani.duration;
          var speed = duration / t;
        ...
        gotoPlayLastFrame(value) {
          ...
          if (this._spine) {
            ...
            var entry = this._spine.getCurrent(this._currentTarget.trackIndex); //console.log('gotoPlayLastFrame_entry:', entry, entry.animation.duration, entry.animationEnd);
            ...
            var spFinalTime = spEnd - 0.32; //--0.32是倒數第二格
        ...
        gotoAndStopByTime(value, time) {
          if (this._spine) {
            ...
            var _trackEntry4 = this._spine.setAnimation(trackIndex, playData.targetName, playData.loop);

            this.setCurrentSpineAniData(_trackEntry4);
            _trackEntry4.trackTime = time;
            this._spine.timeScale = 0; //--停止播放
        ...
        gotoAndStopByFrame(value, frame) {
          if (this._spine) {
            ...
            var _trackEntry5 = this._spine.setAnimation(trackIndex, playData.targetName, playData.loop);

            this.setCurrentSpineAniData(_trackEntry5);
            var moveToStartTime = frame * this._secondsPerFrame;
            if (moveToStartTime <= 0) moveToStartTime = 0;
            _trackEntry5.trackTime = moveToStartTime;
            this._spine.timeScale = 0; //--停止播放
        ...
        playToTimeAndStop(value, time) {
          if (this._spine) {
            ...
            var _trackEntry6 = this._spine.setAnimation(trackIndex, playData.targetName, false);

            this._spine.timeScale = 0; //--停止播放

            this.setCurrentSpineAniData(_trackEntry6);
        ...
        playToFrameAndStop(value, frame) {
          if (this._spine) {
            ...
            var _trackEntry7 = this._spine.setAnimation(trackIndex, playData.targetName, false);

            this._spine.timeScale = 0; //--停止播放

            this.setCurrentSpineAniData(_trackEntry7);
            var moveToStartTime = frame * this._secondsPerFrame;
            if (moveToStartTime <= 0) moveToStartTime = 0;
        ...
        goBackToDefault(flag) {
          ...
          var playData = this.resolveTargetName((_crd && AnimationStateType === void 0 ? (_reportPossibleCrUseOfAnimationStateType({
            error: Error()
        ...
        playAniWithCallBack(callBack, backDefault, value) {
          ...
          var spineCompleteHandler = trackEntry => {
          ...
          var trackIndex = tempParams.trackIndex ? tempParams.trackIndex : 0;
          this.isPlaying = true;
        ...
        resolveTargetName(sel) {
          //--查表分開到特殊工具處理
          var target = this._resolver.resolveProp(sel);
        ...
        getAniPlayDataByPlaySelector(value) {
          var target = this._resolver.resolveProp(value);
        ...
        stopWith(opt) {
          ...
          var stopParticles = (_opt$stopParticles = opt.stopParticles) != null ? _opt$stopParticles : true; // 先關狀態與外部效果
        ...
        cleanCurrentTrack(trackIndex) {
          if (!trackIndex) {
            var _trackEntry8 = this._spine.getCurrent(0);

            if (_trackEntry8) {
              this._spine.clearTrack(_trackEntry8.trackIndex);
            }
          } else {
...
//# sourceMappingURL=2c4619ff7afb85c6904e76236e99190028793546.js.map