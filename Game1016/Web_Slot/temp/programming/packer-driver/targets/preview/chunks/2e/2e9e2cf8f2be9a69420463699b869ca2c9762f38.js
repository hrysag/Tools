            ...
            for (var i = 0; i < this.landscape.length; i += 1) {
              ...
              if (this.switchChild) {
                while (portraitNode.children.length !== 0) {
                  var _target = portraitNode.children[0];

                  _target.removeFromParent(); // 強制脫離當前 parent


                  landscapeNode.addChild(_target); //--有針對node做added事件的可以在這邊觸發

                  this._resizeHandler.applyMultiResize(_target, this._resizeStateList, landscapeNode);
                }
              }
          ...
          for (var i = 0; i < this.landscape.length; i += 1) {
            ...
            if (this.switchChild) {
              while (landscapeNode.children.length !== 0) {
                var _target2 = landscapeNode.children[0];

                _target2.removeFromParent(); // 強制脫離當前 parent


                portraitNode.addChild(_target2); //--有針對node做added事件的可以在這邊觸發
                //this.doMultiProcessAfterResize(target);

                this._resizeHandler.applyMultiResize(_target2, this._resizeStateList, portraitNode);
              }
            }
...
//# sourceMappingURL=2e9e2cf8f2be9a69420463699b869ca2c9762f38.js.map