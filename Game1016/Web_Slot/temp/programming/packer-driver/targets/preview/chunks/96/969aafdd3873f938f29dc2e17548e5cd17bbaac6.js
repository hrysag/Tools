  ...
  return {
    ...
    execute: function () {
      ...
      _export("WinScore", WinScore = (_dec = ccclass('WinScore'), _dec2 = property({
        ...
        register() {
          var timeDataList = (_crd && GlobalAccessReader === void 0 ? (_reportPossibleCrUseOfGlobalAccessReader({
            error: Error()
        ...
        showFinalScoreIn(finalScore) {
          ...
          return _asyncToGenerator(function* () {
            if (_this._isForceStopped) return;

            if (_this._finishResolvePromise) {
              _this.safeResolveFinishPromise();

              _this.forceToDefault();
            }

            _this._status = ANIMATION_SCORE_TYPE.In;
            return new Promise( /*#__PURE__*/_asyncToGenerator(function* (resolve) {
              ...
              var aniTime = (_this$_mapAniTimeData = _this._mapAniTimeData.get(ANIMATION_SCORE_TYPE.In)) != null ? _this$_mapAniTimeData : DEFAULT_ANI_TIME; // 預設動畫時間

              _this._winScoreAniController.changeSpeedWithAep({
                aniState: ANIMATION_SCORE_TYPE.In
              }, aniTime);

              yield _this._winScoreAniController.playAniInPromise({
                aniState: ANIMATION_SCORE_TYPE.In
              });
              _this._status = ANIMATION_SCORE_TYPE.Loop;
        ...
        showFinalScoreOut() {
          ...
          return _asyncToGenerator(function* () {
            if (_this2._isForceStopped) return; //--沒用到

            if (_this2._canceling) {
              _this2.cancelAniAndResolve();

              return;
            } //--沒用到


            if (_this2._finishResolvePromise) {
              _this2.safeResolveFinishPromise();

              _this2.forceToDefault();
            }

            _this2._status = ANIMATION_SCORE_TYPE.Out;
            return new Promise( /*#__PURE__*/_asyncToGenerator(function* (resolve) {
              var _this2$_mapAniTimeDat;

              _this2._finishResolvePromise = resolve;
              _this2.node.active = true;
              var aniTime = (_this2$_mapAniTimeDat = _this2._mapAniTimeData.get(ANIMATION_SCORE_TYPE.Out)) != null ? _this2$_mapAniTimeDat : DEFAULT_ANI_TIME; // 預設動畫時間

              _this2._winScoreAniController.changeSpeedWithAep({
                aniState: ANIMATION_SCORE_TYPE.Out
              }, aniTime);

              yield _this2._winScoreAniController.playAniInPromise({
                aniState: ANIMATION_SCORE_TYPE.Out
              });
              _this2._status = ANIMATION_SCORE_TYPE.Default;
        ...
        processAbortCancel() {
          ...
          return _asyncToGenerator(function* () {
            if (_this3._status === ANIMATION_SCORE_TYPE.Default) return; //const gameState = GlobalAccessReader.getGlobalData(GameGlobalKeys.GameState);

            if (_this3._status === ANIMATION_SCORE_TYPE.In) {
              _this3._finishResolvePromise = null;

              _this3._winScoreAniController.gotoPlayLastFrame();

              var wait = (_crd && GameUtilsTools === void 0 ? (_reportPossibleCrUseOfGameUtilsTools({
                error: Error()
              }), GameUtilsTools) : GameUtilsTools).DeferByTweenPromiseWithCancel(0.2);
              yield wait.promise;
              yield _this3.showFinalScoreOut();
            } else if (_this3._status === ANIMATION_SCORE_TYPE.Loop) {
              _this3._finishResolvePromise = null;
              yield _this3.showFinalScoreOut();
            } else if (_this3._status === ANIMATION_SCORE_TYPE.Out) {
              //--已經在退場中--
              _this3._status = ANIMATION_SCORE_TYPE.Default;

              _this3.cancelAniAndResolve();
...
//# sourceMappingURL=969aafdd3873f938f29dc2e17548e5cd17bbaac6.js.map