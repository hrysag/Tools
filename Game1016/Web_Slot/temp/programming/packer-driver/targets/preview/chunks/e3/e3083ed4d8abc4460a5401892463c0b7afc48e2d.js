    ...
    execute: function () {
      ...
      _export("AnimationSelectionResolver", AnimationSelectionResolver = class AnimationSelectionResolver {
        ...
        rebuildAnimationCaches() {
          ...
          var states = (_this$stateListProvid = this.stateListProvider()) != null ? _this$stateListProvid : [];

          for (var st of states) {
            var _ref, _st$getStateKey, _this$keySelector, _st$targetName;

            if (!st) continue;
            var key = ((_ref = (_st$getStateKey = st.getStateKey == null ? void 0 : st.getStateKey()) != null ? _st$getStateKey : (_this$keySelector = this.keySelector) == null ? void 0 : _this$keySelector.call(this, st)) != null ? _ref : '').trim(); //-切掉空白字元

            var tgt = ((_st$targetName = st.targetName) != null ? _st$targetName : '').trim();
            if (!key || !tgt) continue;
            this.stateKey2Target.set(key, tgt);
          }

          var props = (_this$propListProvide = this.propListProvider()) != null ? _this$propListProvide : [];

          for (var def of props) {
            var _def$targetName;

            var _tgt = ((_def$targetName = def == null ? void 0 : def.targetName) != null ? _def$targetName : '').trim();

            if (!_tgt) continue;
            this.targetName2Prop.set(_tgt, def);
          }
        }
        /** 當快取尚未建好時自動重建一次 */
        ...
        processMapInfoNameFast(value) {
          ...
          if (typeof value === 'string') {
            ...
            if (this.targetName2Prop.has(_v)) return _v;
            var mapped = this.stateKey2Target.get(_v);
            if (mapped) return mapped;
            this.logger('warn', '[Resolver] not found by targetName or stateKey:', _v);
            return '';
          ...
          if (typeof value === 'number') {
            ...
            if (this.enumToKey) {
              var key = this.enumToKey(value);

              var _mapped = this.stateKey2Target.get(key);

              if (_mapped) return _mapped; // ← 直接用 enum 名稱對到 targetName
            }

            var list = (_this$stateListProvid2 = (_this$stateListProvid3 = this.stateListProvider) == null ? void 0 : _this$stateListProvid3.call(this)) != null ? _this$stateListProvid2 : [];
            var idx = Math.trunc(value);
            ...
            var tgt = ((_list$idx$targetName = (_list$idx = list[idx]) == null ? void 0 : _list$idx.targetName) != null ? _list$idx$targetName : '').trim();
          ...
          if (value.aniState !== undefined) {
            var _key = typeof value.aniState === 'string' ? value.aniState.trim() : this.enumToKey ? this.enumToKey(value.aniState) : String(value.aniState);

            var _mapped2 = this.stateKey2Target.get(_key);

            if (_mapped2) return _mapped2;
        ...
        resolveProp(value) {
          ...
          var targetName = this.processMapInfoNameFast(value);
...
//# sourceMappingURL=e3083ed4d8abc4460a5401892463c0b7afc48e2d.js.map