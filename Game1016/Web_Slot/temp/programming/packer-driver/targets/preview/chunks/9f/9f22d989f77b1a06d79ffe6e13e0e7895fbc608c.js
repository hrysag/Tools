  ...
  return {
    ...
    execute: function () {
      ...
      _export("BasicSlotGameViewManager", BasicSlotGameViewManager = (_dec = ccclass('BasicSlotGameViewManager'), _dec2 = property({
        ...
        setAutoModeTimer() {
          var gameState = this._processedServerData.getCurrentStep();
          ...
          var delayTime = this.processRollToStopTime(currentState); //GameUtilsTools.debugLog('CHECK_TIME', 'ROLL_time', { delayTime }, 'log');
        ...
        newRoundDataToStopSpin() {
          ...
          return _asyncToGenerator(function* () {
            if (_this._autoTimerPromise) {
              yield _this._autoTimerPromise;
            ...
            var step = _this._processedServerData.getCurrentData();
        ...
        //---玩家按下stop按鈕(空白按鍵)---
        stopSpin(slotData, other) {
          var _this2 = this;

          return _asyncToGenerator(function* () {
            if (_this2._isStop) return;
            if (!slotData) return; // slotData 為 null，直接退出（不會設 _isStop）

            _this2._isStop = true;

            _this2.beforeStopSpin();

            yield _this2.doStopSpin(slotData);
            _this2._isRollEnd = true;
            yield _this2.beforeAllReelRollEnd(); //--20250827---
            ...
            _this2.processAfterAllReelRollEnd();
          })();
        } //---玩家按下stop按鈕---
        ...
        processAfterAllReelRollEnd() {
          ...
          return _asyncToGenerator(function* () {
            //--await處理完的slot停止
            var totalWinScore = _this3.calculateTotalWinScore(); //--感覺可以省了


            if (totalWinScore > 0) {
              //--關閉面板按鈕準備接表演 
              _this3._startGetScoreInThisRound = true;
            } else {
              //--通知下一把 
              //---沒有中獎的情況下要做的事情
              if (!_this3._startGetScoreInThisRound) {
                var evtData = {
                  eventType: (_crd && GameViewEvents === void 0 ? (_reportPossibleCrUseOfGameViewEvents({
                    error: Error()
                  }), GameViewEvents) : GameViewEvents).SET_BOTTOM_TEXT,
            ...
            yield _this3.doShowResultAfterStopRoll(); //return;

            _this3.checkNextRound();
          })();
        } //---表演處理一定要實作
        ...
        processNormalRound(freeEnd) {
          ...
          return _asyncToGenerator(function* () {
            _this4._isThisRound = false; //--這一輪結束後要重置為false

            _this4._isInterrupting = false; //---重置中斷狀態

            _this4.resetDataForNewRound(); //--重置資料相關處理


            var delayRoundStep = _this4.checkConditionForRoundStep(true); //const startT = GameUtilsTools.getTimeStamp();


            yield _this4.addTweenDelay(delayRoundStep); //const endT = GameUtilsTools.getTimeStamp();
        ...
        processRound(gameState, slotData) {
          var _this5 = this;

          return _asyncToGenerator(function* () {
            var _this5$_processedServ;

            _this5.prepareForNextFGandReSpin();

            if (!_this5._startGetScoreInThisRound) {
              (_crd && GenericUIManager === void 0 ? (_reportPossibleCrUseOfGenericUIManager({
                error: Error()
            ...
            var delayRoundStep = _this5.checkConditionForRoundStep();

            var signal = _this5._async.createAbortScope(_this5._flowKeys.PROCESS_ROUND);

            var cancel = () => {}; //const startT = GameUtilsTools.getTimeStamp();


            var waitPromise = _this5._async.waitSecondsTracked(delayRoundStep, _this5._flowKeys.PROCESS_ROUND, cancel, true, signal, _this5._flowKeys.PROCESS_ROUND);

            yield waitPromise.promise; //const endT = GameUtilsTools.getTimeStamp();
            ...
            _this5._isInterrupting = false; //---重置中斷狀態
            ...
            _this5._processedServerData.setSpinIndexForTemporary();

            _this5.startSpin(); //--要滾多久的時間


            var delay = _this5.processRollToStopTime(gameState).fixed(); //const startTRoller = GameUtilsTools.getTimeStamp();


            yield _this5.addTweenDelay(delay); //const endTRoller = GameUtilsTools.getTimeStamp();
            ...
            _this5.stopSpin((_this5$_processedServ = _this5._processedServerData.getCurrentStepForClick()) == null ? void 0 : _this5$_processedServ.data);
          })();
        ...
        checkNextRound() {
          ...
          return _asyncToGenerator(function* () {
            //--移動資料index
            if (_this6._processedServerData.setRoundIdx()) {
              //--注意NG->FG的狀況
              _this6.beforeProcessNewRoundData(); //--提前處理相關局之中的特殊計次資料


              var step = _this6._processedServerData.getCurrentStep();
              ...
              switch (step.state) {
                case (_crd && GameState === void 0 ? (_reportPossibleCrUseOfGameState({
                  ...
                  // ... re-spin 邏輯
                  yield _this6.beforeProcessReSpinRound(step);

                  _this6.processRound(step.state, step.data);
                ...
                case (_crd && GameState === void 0 ? (_reportPossibleCrUseOfGameState({
                  ...
                  // ... FG 邏輯
                  yield _this6.beforeProcessFGRound(step);
                ...
                default:
                  ...
                  _this6.finalizeToNormal(); // 視情況做降級處理
        ...
        finalizeToNormal() {
          ...
          return _asyncToGenerator(function* () {
            yield _this7.beforeProcessNormalRound();
        ...
        calculateTotalWinScore() {
          //---計算這一輪的總分---
          var roundScore = this._processedServerData.getThisRoundTotalWinScore();

          return roundScore;
        }

        createWinScoreData() {
          var thisRoundBetInfo = this._processedServerData.getRoundBetAndOdds();
        ...
        addTweenDelay(time) {
          ...
          return _asyncToGenerator(function* () {
            ...
            _this8._delayTweenCancel = delay.cancel;
            yield delay.promise; // 等待延遲完成

            _this8._delayTweenCancel = null; // 清掉
          })();
        } //============================== other ====================================
        //======testMode 狀態 ========
...
//# sourceMappingURL=9f22d989f77b1a06d79ffe6e13e0e7895fbc608c.js.map