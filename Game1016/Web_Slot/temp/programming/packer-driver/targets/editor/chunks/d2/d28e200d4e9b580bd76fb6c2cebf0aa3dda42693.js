    ...
    execute: function () {
      ...
      _export("FGBoardUI1016", FGBoardUI1016 = (_dec = ccclass('FGBoardUI1016'), _dec2 = property({
        ...
        async closeFGUIBoard() {
          ...
          const playKey = this._triggerGameState === (_crd && GameState === void 0 ? (_reportPossibleCrUseOfGameState({
            error: Error()
          ...
          const callbackWrapper = value => {
            this.cancelBoardAni(true); //--20260209-new
          ...
          this._closingPromise = this.playBoardOut(playKey, dt).finally(() => {
            this.node.active = false;
            this._isClosing = false;
            this._closingPromise = null; //this._closeRequested = false; 
          }); //--20260209-new

          tween(this._skipUIOpacity).to(0.3, {
            opacity: 0
          }).call(() => {//this.node.active = true;
          }).tag(98).start();

          this._async.registerCancelablePromise(SIGNAL_KEY, this._closingPromise, callbackWrapper, single, SIGNAL_KEY);

          return this._closingPromise;
        }

        async openFGUIBoard(value = 0, boardOutCallBack) {
          //--spine動畫0.75s
          const dt = (_crd && GlobalAccessReader === void 0 ? (_reportPossibleCrUseOfGlobalAccessReader({
            error: Error()
          }), GlobalAccessReader) : GlobalAccessReader).getGlobalData((_crd && GameGlobalKeys === void 0 ? (_reportPossibleCrUseOfGameGlobalKeys({
          ...
          let playSoundKey; //const dt = GlobalAccessReader.getGlobalData(GameGlobalKeys.DelayTimeList).get(cfg => cfg.fg?.duringBoard);
          ...
          if (playKey == FG_BOARD_ANI_MAP.FG_In) {
            ...
            this._loopTimeForState = (_crd && GlobalAccessReader === void 0 ? (_reportPossibleCrUseOfGlobalAccessReader({
              error: Error()
            }), GlobalAccessReader) : GlobalAccessReader).getGlobalData((_crd && GameGlobalKeys === void 0 ? (_reportPossibleCrUseOfGameGlobalKeys({
              error: Error()
            }), GameGlobalKeys) : GameGlobalKeys).DelayTimeList).get(cfg => {
          ...
          const playPromise = this.playBoardIn(value, {
            aniState: playKey
          }, dt); //--20260209-new

          tween(this._skipUIOpacity).delay(0.4).to(0.5, {
            opacity: 255
          }).call(() => {//this.node.active = true;
          }).tag(99).start();
          ...
          await this._async.registerCancelablePromise(SIGNAL_KEY, playPromise, callbackWrapper, signal, SIGNAL_KEY); //--結束時關閉
          ...
          const playLoopKey = this._triggerGameState === (_crd && GameState === void 0 ? (_reportPossibleCrUseOfGameState({
            error: Error()
        ...
        playVoiceOut() {
          const odds = (_crd && GlobalAccessReader === void 0 ? (_reportPossibleCrUseOfGlobalAccessReader({
            error: Error()
          }), GlobalAccessReader) : GlobalAccessReader).getGlobalData((_crd && GameGlobalKeys === void 0 ? (_reportPossibleCrUseOfGameGlobalKeys({
            error: Error()
        ...
        ensureLoopThenClose(playLoopKey, dt) {
          ...
          this._loopOncePromise = (async () => {
            ...
            this.openBtnActive(); //void loopP.catch((err) => this.logLoopPlayReject(err, playLoopKey));
        ...
        async setLoopTimeStep() {
          ...
          const delay = this._async.waitSecondsCancelable(this._loopTimeForState, signal, SIGNAL_KEY);
        ...
        getCurrentGameMode() {
          ...
          let gameState = (_crd && GlobalAccessReader === void 0 ? (_reportPossibleCrUseOfGlobalAccessReader({
            error: Error()
          }), GlobalAccessReader) : GlobalAccessReader).getGlobalData((_crd && GameGlobalKeys === void 0 ? (_reportPossibleCrUseOfGameGlobalKeys({
            error: Error()
...
//# sourceMappingURL=d28e200d4e9b580bd76fb6c2cebf0aa3dda42693.js.map