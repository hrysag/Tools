  ...
  return {
    ...
    execute: function () {
      ...
      _export("ProcessSlotDataCore", ProcessSlotDataCore = class ProcessSlotDataCore extends (_crd && BasicProcessSlotData === void 0 ? (_reportPossibleCrUseOfBasicProcessSlotData({
        ...
        getCloneData() {
          const cloneData = new (_crd && BasicProcessSlotData === void 0 ? (_reportPossibleCrUseOfBasicProcessSlotData({
            error: Error()
          }), BasicProcessSlotData) : BasicProcessSlotData)();
          cloneData.betValue = this.betValue; //--test ngData--

          const ngData = this.getGameRoundData(this._currentNGCardsInfo, false);
          const reSpineData = this.getGameRoundData(this._currentReSpinCardsInfo, false);
          const fgData = this.getGameRoundData(this._currentFGCardsInfo, true); //--calculate total odds

          const totalOddsForNG = this.getTotalOddsForRound(ngData);
          const totalOddsForReSpin = this.getTotalOddsForRound(reSpineData);
          const totalOddsForFG = this.getTotalOddsForRound(fgData);
          const totalOdds = (totalOddsForNG + totalOddsForReSpin + totalOddsForFG).fixed();
        ...
        getGameRoundData(target, isFreeGame) {
          const gameRoundData = []; //let accumulatedScore = 0;//--累積的分數
          ...
          for (let i = 0; i < target.length; i++) {
            const targetRoundCardsInfo = target[i];
            let checkTarget1ds = targetRoundCardsInfo.symbolData1ds;
            let checkTarget2ds = targetRoundCardsInfo.symbolData2ds;
            ...
            const awardData = this._calculatePayTable016.getWindData(checkTarget1ds); //console.log('Game Round Award Data:', awardData);


            let matchInfo = []; //--都要拿移動後的盤面做算分
            ...
            const gameRoundReelInfo = this.createNewEmptyProcessSlotData();
            ...
            const beginningWholeWildCount = this.getWholeConsecutiveWildCount(targetRoundCardsInfo.symbolData2ds); //--這個是確實新增的數量

            const afterMovedWholeWildCount = targetRoundCardsInfo.wildCount > 0 ? this.getWholeConsecutiveWildCount(targetRoundCardsInfo.afterMovedSymbolData2ds) : 0; //const afterMovedReSpinCount = (afterMovedWholeWildCount >= 2) ? afterMovedWholeWildCount : 0;
            ...
            if (!isFreeGame) {
              ...
              const checkScatterCondition = this.checkScatterCondition(checkTarget2ds, SCATTER_LIST[0]); //-2.確認是否有連續3軸相連的wild+scatter(混合型)

              const checkMixedCondition = this.checkMixedScatterOrWild(checkTarget2ds, SCATTER_LIST[0], WILD_LIST[0]); //-上述條件滿足一個即可開啟FG(這是整個盤面的總數量),
        ...
        getWinLineData(aw, card1ds) {
          const returnAry = [];
          const aryTargetData = aw.dataList;
        ...
        getMachPosInfo(iconData, card1ds) {
          const machPosInfoList = [];
          const targetWinPos = iconData.WinPos;

          for (let i = 0; i < targetWinPos.length; i++) {
        ...
        getSlotReelDirectionWithWild(cards, compareTarget) {
          ...
          for (let reelIndex = 0; reelIndex < cards.length; reelIndex++) {
            ...
            for (let groupIndex = 0; groupIndex < continuousGroups.length; groupIndex++) {
              ...
              let direction = (_crd && Direction === void 0 ? (_reportPossibleCrUseOfDirection({
                error: Error()
              ...
              if (direction !== (_crd && Direction === void 0 ? (_reportPossibleCrUseOfDirection({
                ...
                const startIndex = direction === (_crd && Direction === void 0 ? (_reportPossibleCrUseOfDirection({
                  error: Error()
        ...
        checkScatterCondition(cards, conditionTarget) {
          let count = 0;
          let flag = false; //const compareTarget = SCATTER_LIST[0];
        ...
        checkMixedScatterOrWild(cards, scatter, wild) {
          for (let i = 0; i <= cards.length - 3; i++) {
            ...
            const hasScatter = row => row.includes(scatter);

            const hasScatterOrWild = row => row.includes(scatter) || row.includes(wild);
        ...
        getWholeConsecutiveWildCount(cards) {
          ...
          for (let i = 0; i < cards.length; i++) {
            ...
            const row = cards[i]; //--檢查整輪是否都是wild
        ...
        getScatterCountWithScatterAndWild(cards) {
          //--拿scatter數量
          const scatterCount = this.checkScatterCount(cards); //--拿相連的wild數量

          const wildCount = this.getConsecutiveCountsForGrid(cards, WILD_LIST[0]);
          let totalWildToScatterForConsecutive = 0;

          for (let i = 0; i < wildCount.length; i++) {
            totalWildToScatterForConsecutive += wildCount[i].length;
          }

          const finalScatterCount = scatterCount + totalWildToScatterForConsecutive; //const finalFgCount = this.getFinalFGTimes(finalScatterCount);
          //return finalFgCount;
        ...
        setNewRoundData(buffer, betValue) {
          ...
          const ngCardInfo = [];
          const reSpinCardInfo = [];
          const fgCardInfo = []; //--大於1就是包含了NG+reSpin

          const totalRoundForNGAndReSpin = buffer.getUint8()[1]; //--ng+reSpin

          for (let i = 0; i < totalRoundForNGAndReSpin; i++) {
            if (i == 0) {
        ...
        testCheckScore() {
          ...
          const awardData = this._calculatePayTable016.getWindData(testCards); //const matchInfo = this.getWinLineData(awardData, testCards);
        ...
        parseSingleRoundData(buffer) {
          ...
          const panelParser = new (_crd && IntArray === void 0 ? (_reportPossibleCrUseOfIntArray({
            error: Error()
          }), IntArray) : IntArray)();
        ...
        * @param replace 要換成的值
        ...
        cloneBinaryBuffer(original) {
          ...
          const clone = new (_crd && BinaryBuffer === void 0 ? (_reportPossibleCrUseOfBinaryBuffer({
            error: Error()
...
//# sourceMappingURL=2c9615ac56670751a64808c697d831b632a8ad97.js.map