      ...
      _export("ProcessDataAfterServer1016", ProcessDataAfterServer1016 = class ProcessDataAfterServer1016 extends (_crd && ProcessDataAfterServer === void 0 ? (_reportPossibleCrUseOfProcessDataAfterServer({
        ...
        */
        ...
        getFGRoundFGConditionReel() {
          ...
          let reel2ds = currentData.reelInfo.afterMovedSymbolData2ds.length > 0 ? currentData.reelInfo.afterMovedSymbolData2ds : currentData.reelInfo.symbolData2ds;

          for (let i = 0; i < reel2ds.length; i++) {
        ...
        preCalculateReSpinCount() {
          ...
          const currentData = this.getPrevData(); //--拿前一局的資料
        ...
        checkOpenFGCondition() {
          ...
          const hasScatter = row => row.includes(SCATTER_LIST[0]);

          const hasScatterOrWild = row => row.includes(SCATTER_LIST[0]) || row.includes(WILD_LIST[0]);
        ...
        */
        ...
        getFgCountForRound() {
          ...
          currentState != (_crd && GameState === void 0 ? (_reportPossibleCrUseOfGameState({
            ...
            if (this.checkOpenFGCondition()) {
              const fgData = this.getFgCountInHasOpenFg();
        ...
        getReelWildData(reelId) {
          const reelData = this.getCurrentData();
          let wildInfo; //--這樣寫有問題,wildCount是算整條1*4的wild數量,不一定會大於0
        ...
        getAllReelWildData() {
          const reelData = this.getCurrentData();
          const wildGroup = []; //if (reelData.reelInfo.wildCount > 0) {
        ...
        getReadyToHandForThisRound() {
          //const 
          const readyToHandData = this.getCurrentData();
          ...
          const hasScatterOrWild = row => row.includes(SCATTER_LIST[0]) || row.includes(WILD_LIST[0]); //--正常情況下
...
//# sourceMappingURL=c83f448438d5284c51b2f7a63cbce8e855bdf680.js.map