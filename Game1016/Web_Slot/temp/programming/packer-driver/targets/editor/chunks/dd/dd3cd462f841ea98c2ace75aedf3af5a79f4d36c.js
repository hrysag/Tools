  ...
  return {
    ...
    execute: function () {
      ...
      _export("EliminationWinScoreAnalyzer", EliminationWinScoreAnalyzer = class EliminationWinScoreAnalyzer {
         ...
         */
        constructor(oddList, connectNum, eliminationCondition, directions = [[0, 1], [1, 0], [0, -1], [-1, 0]]) {
          /** WILD圖示 */
          this.wild = void 0;
        ...
        getEliminationWinDataList() {
          ...
          for (let i = 0; i < blocks.length; i++) {
            const symbol = blocks[i].mainSymbol;
            const length = blocks[i].cells.length;
            const index = this.getWinConnectIndex(length);
          ...
          if (this.wild.length > 0) {
            ...
            for (let r = 0; r < this.reelAmount; r++) {
              for (let c = 0; c < this.symbolLength; c++) {
                ...
                const currentSymbol = this.grid[r][c];
                let mainSymbol = currentSymbol;
                const block = new Block(mainSymbol, []);
...
//# sourceMappingURL=dd3cd462f841ea98c2ace75aedf3af5a79f4d36c.js.map