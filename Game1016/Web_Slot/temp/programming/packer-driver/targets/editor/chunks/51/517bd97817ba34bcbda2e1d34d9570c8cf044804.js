  ...
  return {
    ...
    execute: function () {
      ...
      _export("BasicShowAniProcess", BasicShowAniProcess = class BasicShowAniProcess extends AnimationNodesControllerBase {
         ...
         * (這邊可以開始做不同的狀態判斷)
         */
        async cleanAllPlayingAniForNewStart() {
          var _this$cancelAllDelays, _this$safeResolve;

          this._abortPlaySequence = true; // 3) 通用：清 winLines、重置旗標
          ...
          (_this$safeResolve = this.safeResolve) == null || _this$safeResolve.call(this); // 5) 分支：由子類決定「特殊」或「一般」清理
        ...
        async runShowProcess(showWinFlag) {
          //---建立流程
        ...
        async showWinScoreAni(score, showBottomText) {
          //--顯示得分動畫
          if (showBottomText) {
        ...
        sortAnimationLayer() {
          if (this._arySortLayerSymbol.length > 0) {
            ...
            for (const item of this._aryRunningNode) {
              const id = item[(_crd && DYN_NODE_PROPERTIES === void 0 ? (_reportPossibleCrUseOfDYN_NODE_PROPERTIES({
                error: Error()
...
//# sourceMappingURL=517bd97817ba34bcbda2e1d34d9570c8cf044804.js.map