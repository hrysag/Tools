    ...
    execute: function () {
      ...
      _export("CalculatePayTable016", CalculatePayTable016 = class CalculatePayTable016 extends (_crd && WaysWinScoreAnalyzer === void 0 ? (_reportPossibleCrUseOfWaysWinScoreAnalyzer({
        ...
        getWindData(iconData) {
          ...
          let totalOdd = 0;
          const machMap = this.getWaysWinData(iconData, Game016PayConfig.REEL_AMOUNT, Game016PayConfig.SYMBOL_LENGTH); //console.log(`中獎線數量:${machMap.length}`, machMap);
          ...
          for (let i = 0; i < machMap.length; i++) {
            const item = machMap[i]; //--攤平2dPos

            const flatWin2DPos = item.Win2DPos.reduce((acc, row) => acc.concat(row), []); //--WinLineID起始位置是0,企劃資料編號從1開始所以+1
...
//# sourceMappingURL=7f033b429ae03d04d4612a5e65a7b4dd9f7ef912.js.map