  ...
  return {
    ...
    execute: function () {
      ...
      _export("UniIcon1016", UniIcon1016 = (_dec = ccclass('UniIcon1016'), _dec2 = property({
        ...
        getAndRemoveWildNode(handoff = true) {
          if (handoff) {
            if (this._colorState) {
        ...
        offsetIconPos(wildNode) {
          ...
          let targetNode = wildNode.children[0]; //--外面再包一層node用來改變位置用的

          targetNode.setPosition(v3(0, 0, 0));
          const uiTransform = wildNode.getComponent(UITransform);
          const changeV3 = v3(v3(0, uiTransform.contentSize.height / 2 - offsetY, 0));
          targetNode.setPosition(changeV3);
        ...
        addWildNode(wildNode) {
          let leftover = this.findAndRemoveLeftoverNode(wildNode);
          this._wildNode = wildNode;

          this._fakeWildLayerContainer.addChild(wildNode); //--先用假的容器來放


          const iconTargetWpos = this.node.worldPosition.clone(); //const iconWorldPos = v3(iconTargetWpos.x, iconTargetWpos.y + 10, iconTargetWpos.z);
        ...
        getWildNodePosition() {
          ...
          */
          const uiTransform = this._fakeWildLayerContainer.getComponent(UITransform);
          ...
          if (this._wildNode) {
            ...
            const targetNodeWpos = this.node.worldPosition.clone(); //targetWorldPos = v3(targetNodeWpos.x, targetNodeWpos.y + 10, targetNodeWpos.z);
        ...
        addSymbolAniNode(aniSymbol, offsetY = 0, isDark = false) {
          const leftover = this.findAndRemoveLeftoverNode(aniSymbol);
        ...
        aniNodeGoBackToDefault() {
          if (this.checkAniSymbolIsExist()) {
            const iAnimationControl = (_crd && AniSysTools === void 0 ? (_reportPossibleCrUseOfAniSysTools({
              error: Error()
        ...
        getSymbolAniNodeAndRemove(handoff = true) {
          if (!this._aniSymbol) {
            return null;
          } else {
            if (handoff) {
        ...
        async playWildToFgAnimation() {
          if (this._wildNode) {
            ...
            const iAnimationControl = this._wildNode[(_crd && DYN_NODE_PROPERTIES === void 0 ? (_reportPossibleCrUseOfDYN_NODE_PROPERTIES({
              error: Error()
            }), DYN_NODE_PROPERTIES) : DYN_NODE_PROPERTIES).ANIMATION_CTRL];
            ...
            if (speed == (_crd && NewFlashModeEnum === void 0 ? (_reportPossibleCrUseOfNewFlashModeEnum({
              ...
              await iAnimationControl.playAniInPromise({
                aniState: 'Connect_1'
              });
            }
          }
        ...
        changeSpineColor(colorValue) {
          if (this._aniSymbol) {
            const baseComponent = (_crd && AniSysTools === void 0 ? (_reportPossibleCrUseOfAniSysTools({
              error: Error()
            }), AniSysTools) : AniSysTools).findAndGetIAniComponent(this._aniSymbol);
            ...
            const skeletons = this._wildNode[(_crd && DYN_NODE_PROPERTIES === void 0 ? (_reportPossibleCrUseOfDYN_NODE_PROPERTIES({
              error: Error()
...
//# sourceMappingURL=b9a189328a87b8af4137a8bfaa6bf69ca68a020e.js.map