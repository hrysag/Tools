 ...
 * 用來隔離主核心的邏輯
 */
export class TestLibs {

    private _uniqueSymbolList: number[][] = [];
    private _allSymbolList: number[] = [];
    private _reelAmount: number = 5; //--軸數
    private _reelSymbolAmount: number = 4; //--每軸符號數量
    ...
    public getFinalResultData(): number[][] {
        return [
     ...
     * 沒送資料就只產生一軸的資料
     */
    public getRandomDataForStartSpin(reels?: [0, 1, 2, 3, 4]): number[][] {
        const slotBoardData: number[][] = this.generate2dsData(this.generateFlatAryData(reels));
        return slotBoardData;
    }

    public generateFlatAryData(reels?: [0, 1, 2, 3, 4]): number[] {
        const reelIds = reels == undefined ? [0] : reels;
        const aryRandomData: number[] = [];
    ...
    //--切割1ds資料成2ds資料
    public generate2dsData(reelsData: number[]): number[][] {
    ...
    //--亂數池
    public generateRandomSymbolList(reelIndex?: number): number[] {
    ...
    private getTargetUniqueSymbolList(index: number): number[] {
