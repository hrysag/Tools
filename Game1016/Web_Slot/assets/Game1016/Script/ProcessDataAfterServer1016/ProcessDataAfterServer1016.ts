import { IProcessSlotData, ProcessDataAfterServer, IMatchWildGroupResult, BasicProcessSlotData, GameState } from '../ReferencePath';
...
export class ProcessDataAfterServer1016 extends ProcessDataAfterServer {


    private _fgReelSet: Set<number> = new Set();//--吻合FG條件的軸集合(不重複)
    private _reSpinSet: Set<number> = new Set();//--吻合ReSpin+1條件的軸集合(不重複)
    private _fgCountForOpen: number = 0; //-- 符合FG條件後,所獲得的FG次數(開啟FG資格當下次數)
    private _hasFgOpen: boolean = false;//--是否已經滿足開啟FG條件
    private _hasReSpinOpen: boolean = false;//--是否已經滿足開啟FG條件
    private _fgCurrentTotalCount: number = 0;//--目前FG的次數(畫面要得累進數字)
    //=====20250907新增=====
    /**
    ...
    private _fgBuffer: { reels: number[], total: number } | null;
    //--緩存RS的條件資料,等到進入RS才會使用
    private _rsBuffer: { reels: number[], total: number } | null;
     ...
     * @param data 
     */
    public override setServerReceiveData(data: BasicProcessSlotData): void {
     ...
     * @returns 
     */
    public override setRoundIdx(): boolean {
        const proxyIndx = super.setRoundIdx();
        if (proxyIndx && this.getCurrentState() === GameState.FREE_GAME) {
    ...
    */
    ...
    public clearCountForReSpin(): void {
        this._reSpinSet.clear();
        this._hasReSpinOpen = false;
    }

    public clearCountForFg(): void {
        this._fgReelSet.clear();
    }


    private checkWholeConsecutiveWildForReel(reelCards: number[]): boolean {
     ...
     * @returns 
     */
    private getFGRoundFGConditionReel(): number[] {

        const reels: number[] = [];
        const currentData: IProcessSlotData = this.getCurrentData();
        let reel2ds = (currentData.reelInfo.afterMovedSymbolData2ds.length > 0) ?
            currentData.reelInfo.afterMovedSymbolData2ds : currentData.reelInfo.symbolData2ds;

        for (let i: number = 0; i < reel2ds.length; i++) {
    ...
    private checkOpenReSpinCondition(): boolean {
        const conditionReels = this.getRoundReSpinConditionReel();
     ...
     */
    private preCalculateReSpinCount(): number[] {

        const reels: number[] = [];
        const localSet = new Set();
        const currentData: IProcessSlotData = this.getPrevData();//--拿前一局的資料
        let reel2ds = (currentData.reelInfo.afterMovedSymbolData2ds.length > 0) ?
            currentData.reelInfo.afterMovedSymbolData2ds : currentData.reelInfo.symbolData2ds;
    ...
    //--檢查滿足開啟FG的條件
    private checkOpenFGCondition(): boolean {

        const currentData: IProcessSlotData = this.getCurrentData();
        const hasScatter = (row: number[]) => row.includes(SCATTER_LIST[0]);
        const hasScatterOrWild = (row: number[]) => row.includes(SCATTER_LIST[0]) || row.includes(WILD_LIST[0]);
        let reel2ds = (currentData.reelInfo.afterMovedSymbolData2ds.length > 0) ?
            currentData.reelInfo.afterMovedSymbolData2ds : currentData.reelInfo.symbolData2ds;
        const row0 = hasScatter(reel2ds[0]);
        const row1 = hasScatterOrWild(reel2ds[1]);
        const row2 = hasScatterOrWild(reel2ds[2]);
    ...
    * @returns 
    */
    private getFgCountInHasOpenFg(): { continuous: number[], hopeReel: number[] } {

        const reels: number[] = [];
        const hopeReel: number[] = [];
        const currentData: IProcessSlotData = this.getCurrentData();
        let reel2ds = (currentData.reelInfo.afterMovedSymbolData2ds.length > 0) ?
            currentData.reelInfo.afterMovedSymbolData2ds : currentData.reelInfo.symbolData2ds;
        ...
        return { continuous: reels, hopeReel: hopeReel };
    }
     ...
     * @returns 
     */
    private getScInBoardWithContinuous(): { hopeReel: number[] } {
     ...
     * @returns 
     */
    private getRoundReSpinConditionReel(): number[] {
        const reels: number[] = [];
        const currentData: IProcessSlotData = this.getCurrentData();
        let reel2ds = (currentData.reelInfo.afterMovedSymbolData2ds.length > 0) ?
            currentData.reelInfo.afterMovedSymbolData2ds : currentData.reelInfo.symbolData2ds;
     ...
     * @returns 
     */
    public getReSpinCountForRound(): { reels: number[], total: number } {
        if (this._rsBuffer) {
            // 如果已經有 buffer，先回傳 buffer
            return this._rsBuffer;
        }
        const currentState = this.getCurrentState();
        let conditionReels: number[];//--吻合當下條件的軸
        const returnData = { reels: [], total: 0 };
        ...
        } else if (currentState !== GameState.FREE_GAME) {
     ...
     */
    public getCurrentRSOpenCount(): number {
        ...
        */
        ...
        const conditionReels = Array.from(this._reSpinSet);
        if (conditionReels.length == 2) {
            return 2;
        } else if (conditionReels.length == 3) {
            return 3;
     ...
     */
    public getCurrentFGOpenCount(): number {
     ...
     * @returns 
     */
    public getCurrentFGTotalCount(): number {
        return this._fgCurrentTotalCount;
    }

    // === 暫存當前 round 的計算結果（但不消耗）===
    public stashRoundResults(): void {
        // 只計算一次，避免重複蓋掉
        if (!this._fgBuffer) {
    ...
    public consumeRoundResults(): { fg: { reels: number[], total: number }, rs: { reels: number[], total: number } } {
        const result = {
            fg: this._fgBuffer ?? { reels: [], total: 0 },
            rs: this._rsBuffer ?? { reels: [], total: 0 }
        };
        this._fgBuffer = null;
        this._rsBuffer = null;
        return result;
     ...
     */
    public getFGBuffer(): { reels: number[], total: number } | null {
     ...
     */
    public getRSBuffer(): { reels: number[], total: number } | null {
        return this._rsBuffer;
    }
    public checkHasFGBufferData(): boolean {
     ...
     */
    public getFgCountForRound(): { reels: number[], total: number, hope: number[] } {
        /*
        //--20260130調整流程取消
        if (this._fgBuffer) {
            // 如果已經有 buffer，先回傳 buffer
        ...
        const currentState = this.getCurrentState();
        let conditionReels: number[];//--吻合當下條件的軸
        const returnData = { reels: [], total: 0, hope: [] };
        if (/*!this._hasFgOpen &&*/ currentState != GameState.FREE_GAME) {
            if (this.checkOpenFGCondition()) {
                const fgData: { continuous: number[], hopeReel: number[] } = this.getFgCountInHasOpenFg();
                conditionReels = fgData.continuous;//--找符合條件的軸
                const fgCount = this.getFinalFGTimes(conditionReels.length);
                ...
                const hopeReelNoOpen = this.getScInBoardWithContinuous();
                returnData.hope = hopeReelNoOpen.hopeReel;
            }
        } else if (/*this._hasFgOpen && */currentState == GameState.FREE_GAME) {
            //---已經是FG當中
    ...
    public checkNeedToMovement(): boolean {
     ...
     * @returns 
     */
    public getReelScatterData(reelId: number): boolean {

        const reelData = this.getCurrentData();
        const reel2ds = reelData.reelInfo.symbolData2ds;
    ...
    public getScatterByMultiReel(reelIds: number[], usePreviousData: boolean): number[] {

        let scatterCountReel = [];
        const reelData = (usePreviousData) ? this.getPrevData() : this.getCurrentData();
        const reel2ds = reelData.reelInfo.symbolData2ds;
     ...
     */
    public getReelWildData(reelId: number): IMatchWildGroupResult {

        const reelData = this.getCurrentData();
        let wildInfo: IMatchWildGroupResult;
        //--這樣寫有問題,wildCount是算整條1*4的wild數量,不一定會大於0
    ...
    public getAllReelWildData(): IMatchWildGroupResult[] | null {

        const reelData = this.getCurrentData();
        const wildGroup: IMatchWildGroupResult[] = [];
        //if (reelData.reelInfo.wildCount > 0) {
        const wildData = reelData.reelInfo.wildGroup;
        for (const wild of wildData) {
            wildGroup.push(wild);
    ...
    public checkNeedToMoveWild(): boolean {
     ...
     * @returns 
     */
    public getMovementReelIndexData(): number[] {
        ...
        if (reelData.reelInfo.wildCount <= 0 && reelData.reelInfo.wildIndex.length <= 0) {
            ...
            let aryWildIndex = [...reelData.reelInfo.wildIndex];
     ...
     * @returns 
     */
    public getWildMovementData(): { wildMovement: number[], wildGroup: IMatchWildGroupResult[] } {
        const movement = this.getMovementReelIndexData();//--沒有位移就沒有資料
        const group = this.getAllReelWildData();//--這個是只要有wild就會有資料紀錄
        return { wildMovement: movement, wildGroup: group };
    }
     ...
     * 只要算前3軸就可以了
     */
    public getReadyToHandForThisRound(): number[] {
        //const 
        const readyToHandData: IProcessSlotData = this.getCurrentData();
        const currentState = this.getCurrentState();//--檢查目前的狀態
        const reel2ds = readyToHandData.reelInfo.symbolData2ds;
        const hasFgOpenConditions = [];//--可以聽牌的條件軸
        const row0 = reel2ds[0];
        const row1 = reel2ds[1];
        const row2 = reel2ds[2];
        const row3 = reel2ds[3];
        const hasScatter = (row: number[]) => row.includes(SCATTER_LIST[0]);
        const hasScatterOrWild = (row: number[]) => row.includes(SCATTER_LIST[0]) || row.includes(WILD_LIST[0]);
        //--正常情況下
        //if (this._hasFgOpen) return hasFgOpenConditions;//--已經開啟FG就不需要再算聽牌
        if (currentState == GameState.FREE_GAME) return hasFgOpenConditions;//--已經開啟FG就不需要再算聽牌
        /**
        ...
        if (hasScatter(row0)) {
             ...
             * 第一軸沒有但後面居然湊滿連續條件
             */
            if (currentState == GameState.RE_SPINE && hasFgOpenConditions.length === 0) {
                if (hasScatterOrWild(row2)) {
