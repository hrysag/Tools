    ...
    execute: function () {
      ...
      _export("SlotWheel", SlotWheel = (_dec = ccclass('SlotWheel'), _dec2 = property({
        ...
        generateInitialSymbols() {
          ...
          for (let i = this.mainSymbolAmount - 1; i >= 0; i--) {
            symbol = this.spawnSymbolByID();
          }

          symbol = this.arrSymbol[0];
          let top = symbol.node.getPosition().y + 0.5 * symbol.height;
          ...
          let initialBounceDisplacement = this.getBounceDisplacement(this.initialVelocity, this.maxVelocity, this.timeToAchieveMaxVelocity);
          symbol = this.arrSymbol[this.arrSymbol.length - 1];
          let bottom = symbol.node.getPosition().y - 0.5 * symbol.height + initialBounceDisplacement;
        ...
        getBounceDisplacement(bounceVelocity, destVelocity, totalTime, currentVelocity) {
          if (bounceVelocity * destVelocity >= 0) {
            ...
            let acceleration = (destVelocity - bounceVelocity) / totalTime;
            let time = (0 - bounceVelocity) / acceleration;
        ...
        stop(cards, extendedCards, time) {
          ...
          let displacement = this.mainRect.y + 0.5 * newSymbol.height - this.arrMainSymbol[this.arrMainSymbol.length - 1].node.getPosition().y;
        ...
        spawnSymbolByID(id = this.generateSymbolID(), prepend = true, y = null) {
        ...
        addSymbol(symbol, prepend = true, y) {
