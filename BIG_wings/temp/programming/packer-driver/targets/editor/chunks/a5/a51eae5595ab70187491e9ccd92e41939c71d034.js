    ...
    execute: function () {
      ...
      _export("BaseView", BaseView = class BaseView extends Component {
        ...
        configCommandEvent() {
          ...
          const command = (_this$gameManager = this.gameManager) == null ? void 0 : _this$gameManager.command;
          ...
          if (command) {
            ...
            command.event.on((_crd && CommandEventName === void 0 ? (_reportPossibleCrUseOfCommandEventName({
              error: Error()
            }), CommandEventName) : CommandEventName).SPIN, async betInfo => {
        ...
        configToolbarEvent() {
          ...
          const toolbar = (_this$gameManager2 = this.gameManager) == null ? void 0 : _this$gameManager2.toolbar;
        ...
        initGameManager(...args) {// TODO: Game Initialized
        }
        /**
        ...
        createPresenter() {
          let presenter = new (_crd && BasePresenter === void 0 ? (_reportPossibleCrUseOfBasePresenter({
            error: Error()
        ...
        async startPresenter(address) {
          const {
            presenter
          } = this;
          let event = await presenter.connect(address).catch(() => {
            return false;
          });
        ...
        handleExchanePanelEvent() {
          ...
          if (exchangePanel) {
            exchangePanel.event.on((_crd && ExchangePanelEventName === void 0 ? (_reportPossibleCrUseOfExchangePanelEventName({
              error: Error()
            }), ExchangePanelEventName) : ExchangePanelEventName).CREDIT_EXCHANGE, async data => {
              await presenter.creditExchange(data.betBase, data.amount);
            });
            exchangePanel.event.on((_crd && ExchangePanelEventName === void 0 ? (_reportPossibleCrUseOfExchangePanelEventName({
              error: Error()
            }), ExchangePanelEventName) : ExchangePanelEventName).BALANCE_EXCHANGE, async () => {
              await presenter.balanceExchange();
            });
            exchangePanel.event.on((_crd && ExchangePanelEventName === void 0 ? (_reportPossibleCrUseOfExchangePanelEventName({
              error: Error()
            }), ExchangePanelEventName) : ExchangePanelEventName).CHANGE_RATIO, async data => {
              await presenter.fastExchange(data.ratio);
            });
            exchangePanel.event.on((_crd && ExchangePanelEventName === void 0 ? (_reportPossibleCrUseOfExchangePanelEventName({
              error: Error()
            }), ExchangePanelEventName) : ExchangePanelEventName).LEAVE_GAME, async () => this.exit());
          }
        }
