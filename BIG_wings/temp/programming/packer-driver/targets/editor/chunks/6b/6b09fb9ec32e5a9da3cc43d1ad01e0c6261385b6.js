  ...
  return {
    ...
    execute: function () {
      ...
      _export("mahjong_TA", mahjong_TA = (_dec = ccclass('mahjong_TA'), _dec2 = property({
        ...
        onLoad() {
          //按鈕觸發設置
          const thisScriptName = this.name.split('<')[1].split('>')[0]; //spin按鈕
        ...
        stopGameSlotRunNow() {
          ...
          for (let i = 0; i < this.stopSlot.length; i++) {
            ...
            if (!this.stopSlot[i]) this.stopSlotRun(i); //停止slot轉動(排除已停止的slot)
          }
        } //-------------------按鈕事件觸發-------------------/
        //-------------------主要slot流程-----------------------/
        //開始遊戲slot轉動
        ...
        scatterSave(slotLine) {
          const mainSlotRun = this.slotRun[slotLine].children[1]; //每行第一個資料不判斷
                ...
                if (typeName !== 'eye') if (typeName === 'free') {
                  ...
                  this.scheduleOnce(() => {
                    ...
                    for (let data of this.symbolWinLayer.children) {
                      data.getComponent(_crd && symbolWin_TA === void 0 ? (_reportPossibleCrUseOfsymbolWin_TA({
                        error: Error()
                ...
                this.scheduleOnce(() => {
                  ...
                  this.scheduleOnce(() => {
                    ...
                    if (typeRound === typeAward.length) {
                      ...
                      let waitReadyTime = 0; //如果目前剛好有四組碰牌，表演聽(胡)牌
          ...
          if (updataAll) {
            ...
            for (let i = 0; i < this.slotRun.length; i++) {
              ...
              for (let j = 0; j < symbolAmount; j++) {
                ...
                slotSymbol.children[j].getComponent(_crd && symbolSetting_TA === void 0 ? (_reportPossibleCrUseOfsymbolSetting_TA({
                  error: Error()
          ...
          const drop = slotLine => {
            if (slotLine === slotLength) {
              ...
              for (const moveSymbol of slotSymbol) {
                ...
                const yPos = moveSymbol.position.y; //判斷是否執行掉落
        ...
        winSymbolSet(name, symID, symPos) {
          ...
          let symbolMovePos = []; //紀錄symbol移動的位置
          ...
          for (let i = 0; i < symbolMovePos.length; i++) {
            ...
            tween(symbolWinNode).to(0.5, {
              ...
              symbolMovePos[i].parent.getComponent(_crd && symbolSet_TA === void 0 ? (_reportPossibleCrUseOfsymbolSet_TA({
                error: Error()
        ...
        setResult() {
          ...
          for (let i = 0; i < 8; i++) {
            ...
            } else flowerTile.children[i].active = false; //隱藏花牌
          ...
          for (let i = 0; i < 8; i++) {
            ...
            } else winPoints.children[i].active = false; //隱藏牌型台數
