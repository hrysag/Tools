  ...
  return {
    ...
    execute: function () {
      ...
      _export("SymbolSpine", SymbolSpine = (_dec = ccclass('SymbolSpine'), _dec2 = property({
        ...
        onLoad() {
          const wheelCount = this.node.parent.getComponent(_crd && BigWingsRoller === void 0 ? (_reportPossibleCrUseOfBigWingsRoller({
            error: Error()
          }), BigWingsRoller) : BigWingsRoller).arrWheel.length;
          const mainSymCount = this.node.parent.getComponentInChildren(_crd && BigWingsWheel === void 0 ? (_reportPossibleCrUseOfBigWingsWheel({
            error: Error()
          }), BigWingsWheel) : BigWingsWheel).mainSymbolAmount;

          for (let i = 0; i < wheelCount; i++) {
            for (let j = 0; j < mainSymCount; j++) {
              let sym = (_crd && PrefabInstancePoolManager === void 0 ? (_reportPossibleCrUseOfPrefabInstancePoolManager({
                error: Error()
