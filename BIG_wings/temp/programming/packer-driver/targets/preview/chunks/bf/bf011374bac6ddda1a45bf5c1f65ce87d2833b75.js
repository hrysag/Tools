    ...
    execute: function () {
      ...
      _export("tipMove_TA", tipMove_TA = (_dec = ccclass('tipMove_TA'), _dec2 = property(CCInteger), _dec3 = property(CCInteger), _dec4 = property(CCFloat), _dec5 = property(CCFloat), _dec(_class = (_class2 = class tipMove_TA extends Component {
        ...
        runTip(freeGameMode) {
          ...
          var runningTip = tipNode => {
            var startXPos = tipNode.getComponent(UITransform).width / 2 + this.tipStartXPos; //計算該tip的起點X座標
            ...
            if (startXPos < 0) {
              ...
              var endXPos = -(tipNode.getComponent(UITransform).width / 2) + this.tipExitXPos; //計算該tip的終點X座標

              var runTime = (startXPos - endXPos) / 100; //計算移動時間(每秒移動100單位)
          ...
          if (freeGameMode) {
            var tipNode = this.node.children[this.freeGameTipId];
            runningTip(tipNode);
          } else {
            this.runTipId = this.runTipId < this.mainTipNum ? this.runTipId : 0; //判斷執行中的tipId

            var _tipNode = this.node.children[this.runTipId];
            runningTip(_tipNode);
