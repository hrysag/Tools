  ...
  return {
    ...
    execute: function () {
      ...
      _export("CalculationCupboard", CalculationCupboard = (_dec = ccclass('CalculationCupboard'), _dec2 = property({
         ...
         * @returns 
         */
        place(typeName, elementID, symbols) {
          var _this = this;

          return _asyncToGenerator(function* () {
            var symbolNode = []; //紀錄symbol移動的位置

            var len = symbols.length;
            var symbolSetInst; //判斷是否是中途槓牌(不新增牌組)

            if (typeName === WinLineType.KONG && symbols.length === 1) {
              //中途槓牌類型(要移到置牌區)
              var _len = _this.setArea.children.length;

              for (var i = 0; i < _len; i++) {
                symbolSetInst = _this.setArea.children[i];
              ...
              symbolSetInst = (_crd && PrefabInstancePoolManager === void 0 ? (_reportPossibleCrUseOfPrefabInstancePoolManager({
                error: Error()
              }), PrefabInstancePoolManager) : PrefabInstancePoolManager).instance.takeOut(_this.symbolSet);
              ...
              switch (typeName) {
                ...
                case WinLineType.FLOWER:
                  symbolSetInst.parent = _this.flowerArea; //花牌置牌區
                ...
                case WinLineType.PAIR:
                  symbolSetInst.parent = _this.setArea; //眼、碰、槓牌置牌區
            ...
            var _loop = function* _loop() {
              var symbolWinNode = symbols[_i].node;
              var m1 = symbolWinNode.parent.worldMatrix;

              var m2 = symbolNode[_i].parent.worldMatrix.clone().invert();

              var m = m2.multiply(m1);
              Vec3.transformMat4(symbolWinNode.position, symbolWinNode.position, m);
              symbolWinNode.parent = symbolNode[_i].parent;
              tween(symbolWinNode).to(0.5, {
                position: symbolNode[_i].getPosition()
              }, {
                easing: 'quartOut'
              }).start();
              tween(symbolWinNode).to(0.5, {
        ...
        clean() {
          while (this.setArea.children.length > 0) {
            var symbolSetInst = this.setArea.children[0];
          ...
          while (this.flowerArea.children.length > 0) {
            var _symbolSetInst = this.flowerArea.children[0];
            this.flowerArea.removeChild(_symbolSetInst);
            (_crd && PrefabInstancePoolManager === void 0 ? (_reportPossibleCrUseOfPrefabInstancePoolManager({
              error: Error()
            }), PrefabInstancePoolManager) : PrefabInstancePoolManager).instance.pushIn(_symbolSetInst);
          }
        }
        /**
         * 是否聽牌
