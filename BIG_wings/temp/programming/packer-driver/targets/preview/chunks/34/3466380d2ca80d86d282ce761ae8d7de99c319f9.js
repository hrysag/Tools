    ...
    execute: function () {
      ...
      _export("BetSetPanel", BetSetPanel = (_dec = ccclass('BetSetPanel'), _dec(_class = class BetSetPanel extends (_crd && BaseAutoSet === void 0 ? (_reportPossibleCrUseOfBaseAutoSet({
        ...
        init() {
          ...
          var betToggle = this.node.getChildByName("toggle");
          var len = betToggle.children.length; // console.log(betToggle, len)

          var _loop = function _loop(i) {
            var toggleNode = betToggle.children[i];
            var toggle = toggleNode.getComponent(Toggle);
            var labelNode = toggleNode.getChildByName("label");
            var checkmarkLabel = toggleNode.getChildByName("checkmark").getChildByName("label");

            _this.arrToggle.push(toggle);

            var betNumber = "" + _this.betCreditList[i];
            labelNode.getComponent(Label).string = betNumber;
            checkmarkLabel.getComponent(Label).string = betNumber;
            toggle.node.on("click", () => {
              _this.clickToggle(_this.betCreditList[i]);
        ...
        clickToggle(bet) {
          ...
          // }
          var index = this.betCreditList.indexOf(bet);
          var toggle = this.arrToggle[index];
