      ...
      _export("SymbolInfo", SymbolInfo = (_dec = ccclass('SymbolInfo'), _dec2 = property({
        ...
        initOddText() {
          ...
          for (var i = 1; i < 4; i++) {
            var count = new Node();
            count.active = true;
            var cl = count.addComponent(Label);
            ...
            var value = new Node();
            value.active = true;
            var vl = value.addComponent(Label);
        ...
        setBg(wheelIndex) {
          this.bg.active = true;
          var [w, h] = this._bgSize[this._card] || this._bgSize["else"];
        ...
        setNormalContent(wheelIndex) {
          var data = this._rates[this._card].slice().reverse();

          var length = data.filter(e => e > 0).length;
          var diff = length > 3 ? 55 : 60;
