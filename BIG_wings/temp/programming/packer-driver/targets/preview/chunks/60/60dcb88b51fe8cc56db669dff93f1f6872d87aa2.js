      ...
      _export("BaseModel", BaseModel = class BaseModel extends (_crd && Model === void 0 ? (_reportPossibleCrUseOfModel({
        ...
        connect(path) {
          ...
          return _asyncToGenerator(function* () {
            if (!path) {
              path = yield _this.getConnectPath();
            }

            return new Promise((resolve, reject) => {
              _this.connection.socket.once((_crd && ClientRecvAction === void 0 ? (_reportPossibleCrUseOfClientRecvAction({
                error: Error()
              }), ClientRecvAction) : ClientRecvAction).WSClose, event => reject(event.code));

              _this.connection.receiver.once((_crd && ClientRecvAction === void 0 ? (_reportPossibleCrUseOfClientRecvAction({
                error: Error()
              }), ClientRecvAction) : ClientRecvAction).Ready, () => resolve(true));

              _this.connection.connect(_this.url = path, true).catch(() => reject(false));
            });
          })();
        } // 送出事件並回應
