  ...
  return {
    ...
    execute: function () {
      ...
      _export("WinAndMarquee", WinAndMarquee = (_dec = ccclass('WinAndMarquee'), _dec2 = property({
        ...
        updateScore(score, bet, run, inFree) {
          if (run === void 0) {
            run = true;
          }
        ...
        updatePosition() {
          ...
          var totalWidth = 900;
          var restWidth = totalWidth - this.winSprite.getComponent(UITransform).width - 50;
          var availableWidth = 0; // console.error("num Width", this.winScore.getComponent(UITransform).width);
        ...
        runScore(label, duration, value, float) {
          if (float === void 0) {
            float = 2;
          }

          var nowScore = Number(label.string);
          var o = {
            v: nowScore
          };
          var t = tween(o).to(duration, {
            v: value
          }, {
        ...
        /**更新贏得分數欄位 */
        updateWinBlock(score, bet, inFree) {
          var _this = this;

          return _asyncToGenerator(function* () {
            var times = score / bet;
            _this._frameLevel = times >= 10 ? "03" : times >= 5 ? "02" : ""; // let winNode = this.node.getChildByName('winScore');

            var winNode = _this.winScore.node.parent;
            tween(winNode).call(() => {
              _this.winScore.string = score.toFixed(2).toString();
              ...
              scale: new Vec3(1, 1, 1)
            }).tag(_this.tagNum).start();

            if (_this._frameLevel) {
              _this.spine.getComponent(UIOpacity).opacity = 255;
              _this.botSpine.getComponent(UIOpacity).opacity = 255;

              var s = _this.spine.getComponent(sp.Skeleton);

              s.setAnimation(0, _this._frameLevel + "_begin", false);
              s.addAnimation(0, _this._frameLevel + "_loop", true);

              var bs = _this.botSpine.getComponent(sp.Skeleton);

              bs.setAnimation(0, _this._frameLevel + "_begin", false);
              bs.addAnimation(0, _this._frameLevel + "_loop", true);
              if (!inFree) tween(_this.botSpine.getComponent(UIOpacity)).delay(0.8).to(0.2, {
                opacity: 0
              }).start();
              tween(_this.spine.getComponent(UIOpacity)).delay(0.8).to(0.2, {
                opacity: 0
              }).start();
            }
          })();
