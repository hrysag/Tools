    ...
    execute: function () {
      ...
      _export("coinDropAnim", coinDropAnim = (_dec = ccclass('coinDropAnim'), _dec2 = property({
        ...
        onEnable() {
          ...
          for (var i = 0; i < this.anim.children.length; i++) {
            var _loop = function _loop(node) {
              var waitTime = Math.random() * _this.lifeTime.x;

              tween(_this.node).delay(waitTime).call(() => {
                node.active = true;
            ...
            for (var node of this.anim.children[i].children) {
              _loop(node);
            }
          }
        } //執行循環掉落表演
        ...
        loopRun(node) {
          ...
          var randomX = size.width / 2 - Math.random() * size.width; //根據節點的寬度決定掉落範圍
          ...
          var randomTime = this.lifeTime.x + Math.random() * this.lifeTime.y;
