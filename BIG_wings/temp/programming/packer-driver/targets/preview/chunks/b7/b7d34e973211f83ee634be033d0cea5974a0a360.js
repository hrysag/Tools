  ...
  return {
    ...
    execute: function () {
      ...
      _export("common_TA", common_TA = (_dec = ccclass('common_TA'), _dec2 = property({
        ...
        onLoad() {
          //設置按鈕功能
          var thisScriptName = this.node.components[0].name.split('<')[1].split('>')[0]; //公版按鈕
          ...
          for (var i = 0; i < this.popupClose.length; i++) {
            var popupCloseBtnEventHandler = new EventHandler();
          ...
          var settingBtnEventHandler = new EventHandler();
          ...
          for (var _i = 0; _i < this.musicBtn.length; _i++) {
            var musicBtnEventHandler = new EventHandler();
            ...
            musicBtnEventHandler.customEventData = _i.toString();

            this.musicBtn[_i].getComponent(Button).clickEvents.push(musicBtnEventHandler);
          } //【視窗】回大廳取消按鈕


          var homeCancleBtnEventHandler = new EventHandler();
          ...
          var autoConfirmBtnEventHandler = new EventHandler();
        ...
        update() {
          //【視窗】自動視窗出現時，同步判斷進度條
          if (this.popupAuto) {
            var subSlider = this.popupAuto.children[1].getChildByName('subSlider');
            subSlider.getComponent(ProgressBar).progress = subSlider.getComponent(Slider).progress;
            var addSlider = this.popupAuto.children[1].getChildByName('addSlider');
            addSlider.getComponent(ProgressBar).progress = addSlider.getComponent(Slider).progress;
            var winSlider = this.popupAuto.children[1].getChildByName('winSlider');
