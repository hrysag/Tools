      ...
      _export("BigWingsSymbol", BigWingsSymbol = (_dec = ccclass("BigWingsSymbol"), _dec2 = property({
        ...
        win(id) {
          ...
          return new Promise( /*#__PURE__*/_asyncToGenerator(function* (resolve) {
            ...
            _this.sprite.active = false;
            var changed = false;

            if (id && _this._symbolID != id) {
              _this.changeSymbolID(id);

              changed = true;
            }

            var skeletonComponent = _this.symSpine.getComponent(sp.Skeleton);

            if (!skeletonComponent.skeletonData || changed) skeletonComponent.skeletonData = _this.symbolSpineData[_this._symbolID]; // console.error("playWin", this._symbolID);

            skeletonComponent.premultipliedAlpha = false;
            _this.node.active = true;
            var ani = "";

            switch (_this._symbolID) {
              case 12:
                ani = "11_connect";
                break;
            ...
            _this.symSpine.active = true;

            _this.symSpine.getComponent(sp.Skeleton).setAnimation(0, ani, true);

            _this.botSpine.active = false; // 底板

            if (_this.hasBot()) {
              var s = _this.botSpine.getComponent(sp.Skeleton);

              s.skeletonData = _this.botSpineData[_this._symbolID - 2];
              _this.botSpine.active = true;
              s.setAnimation(0, "connect", true);
            } // 連線框


            var frame = _this.frame.getComponent(sp.Skeleton);

            ani = _this._symbolID == 13 ? "connect_1x4" : "connect_1x1";
            _this.frame.active = true;
            frame.setAnimation(0, ani, true);

            _this.frame.setSiblingIndex(_this.isWild() ? 3 : 2); // await UtilsKit.SetSkeletonAnimation(this.frame, 0, ani, false, true);
        ...
        appear() {
          ...
          return new Promise( /*#__PURE__*/_asyncToGenerator(function* (resolve) {
            ...
            _this2.sprite.active = false;
            _this2.symSpine.active = true;

            var skeletonComponent = _this2.symSpine.getComponent(sp.Skeleton);

            if (!skeletonComponent.skeletonData) skeletonComponent.skeletonData = _this2.symbolSpineData[_this2._symbolID];
            var ani = "";

            switch (_this2._symbolID) {
              case 0:
                ani = "12_appear";
                break;
            ...
            yield (_crd && UtilsKit === void 0 ? (_reportPossibleCrUseOfUtilsKit({
              error: Error()
            }), UtilsKit) : UtilsKit).SetSkeletonAnimation(_this2.symSpine, 0, ani, false, true); // this.reset();
        ...
        idle() {
          ...
          return new Promise( /*#__PURE__*/_asyncToGenerator(function* (resolve) {
            ...
            _this3.sprite.active = false;
            _this3.frame.active = false;
            _this3.symSpine.active = true;

            var skeletonComponent = _this3.symSpine.getComponent(sp.Skeleton);

            if (((_skeletonComponent$ge2 = skeletonComponent.getCurrent(0)) == null ? void 0 : _skeletonComponent$ge2.animation.name) == ani) resolve();
            if (!skeletonComponent.skeletonData) skeletonComponent.skeletonData = _this3.symbolSpineData[_this3._symbolID];
        ...
        expand(pos) {
          ...
          return new Promise( /*#__PURE__*/_asyncToGenerator(function* (resolve) {
            _this4.sprite.active = false;
            _this4.symSpine.active = true;

            var skeletonComponent = _this4.symSpine.getComponent(sp.Skeleton);

            if (!skeletonComponent.skeletonData) skeletonComponent.skeletonData = _this4.symbolSpineData[_this4._symbolID];
            skeletonComponent.setAnimation(0, "12_expanding" + (pos + 1), false);
            var siblingIndex = _this4.node.parent.children.length - 1;

            _this4.node.setSiblingIndex(siblingIndex); // console.log(pos);
            ...
            yield (_crd && UtilsKit === void 0 ? (_reportPossibleCrUseOfUtilsKit({
              error: Error()
            }), UtilsKit) : UtilsKit).SetSkeletonAnimation(_this4.symSpine, 0, "12_expanding" + (pos + 1), false, true);
            resolve();
          }));
        }
        ...
        changeToWild(pos) {
          ...
          var siblingIndex = this.node.parent.children.length - 1;
        ...
        lock() {
          ...
          return new Promise( /*#__PURE__*/_asyncToGenerator(function* (resolve) {
            _this5.lockSpine.active = true;
            yield (_crd && UtilsKit === void 0 ? (_reportPossibleCrUseOfUtilsKit({
              error: Error()
            }), UtilsKit) : UtilsKit).SetSkeletonAnimation(_this5.lockSpine, 0, "lock", false, true);
            resolve();
          }));
        }
