System.register(["__unresolved_0", "cc", "ua-parser-js", "__unresolved_1", "__unresolved_2"], function (_export, _context) {
  ...
  function uaSlashObject(ua) {
    var keyValuePairs = ua == null ? void 0 : ua.split(' ').map(pair => pair.split('/'));
    var userAgentObject = {};
    ...
    execute: function () {
      ...
      _export("DeviceClass", DeviceClass = class DeviceClass {
        ...
        get gpuInfo() {
          ...
          var gl = (_document$createEleme = document.createElement('canvas')) == null ? void 0 : _document$createEleme.getContext('webgl');
          var debugInfo = gl == null ? void 0 : gl.getExtension('WEBGL_debug_renderer_info');

          if (debugInfo) {
            var renderer = gl == null ? void 0 : gl.getParameter(debugInfo.UNMASKED_RENDERER_WEBGL);
            return renderer || "";
          }
        ...
        get wvString() {
          if (this.iOS || this.android) {
            var keywordIS = this.webview ? 'is' : 'isnot';
            var keywordDevice = this.iOS ? 'iOS' : 'Android';
        ...
        get ubInfo() {
          ...
          if (this.ubBrowser.UniverseBrowser) {
            var keyword = ['Chrome', 'UB', 'CustomBrowser'];

            for (var key in this.uaSlashObject) {
              if (keyword.includes(key)) {
                var info = key + "/" + this.uaSlashObject[key];
        ...
        get newaio() {
          ...
          if (this.aio) {
            var game_portal = this.uaSlashObject['game_portal'] || "";
            var game_platform = this.uaSlashObject['game_platform'] || "";
