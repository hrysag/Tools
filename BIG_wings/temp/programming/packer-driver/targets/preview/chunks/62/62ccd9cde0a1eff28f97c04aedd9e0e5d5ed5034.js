    ...
    execute: function () {
      ...
      _export("PlayerLoaderInfoImpl", PlayerLoaderInfoImpl = class PlayerLoaderInfoImpl {
        ...
        send() {
          ...
          var GetMachineDetail = () => {
            var loadinfo = this.getTimeStamp('loadInfo');
            var costOfGetMachineDetail = this.getTimeStamp('onGetMachineDetail') - this.getTimeStamp('getMachineDetail');
        ...
        canSend() {
          var checkNotSet = ['loader', 'ws', 'login', 'takeMachine', 'loadInfo', 'done'];

          for (var i = 0; i < checkNotSet.length; i++) {
            var key = checkNotSet[i];
