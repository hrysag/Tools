import { _decorator, Component, Graphics, CCInteger, Vec3, Color, Rect, Mask, CCBoolean, Prefab } from 'cc';
...
export class SlotWheel extends Component {
    ...
    @property({ type: CCInteger, tooltip: "啟動後耗時多久至最大速度(單位:sec)" })
    public timeToAchieveMaxVelocity: number = 1; // 啟動後耗時多久至最大速度(單位:sec)
    ...
    protected _isRunning: boolean = false; // 輪軸是否滾動中
