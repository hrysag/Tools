import { _decorator, Component, Node, Animation, EventHandler, Button, tween, Vec3, Toggle, sp, Label, Tween, UIOpacity, Prefab, UITransform, Sprite } from 'cc';
...
export class mahjong_TA extends Component {
    ...
    winSymbolSet(name: string, symID: number[], symPos: number[][]) {
        ...
        for (let i = 0; i < symbolMovePos.length; i++) {
            ...
            tween(symbolWinNode).to(0.5, { position: moveWorldPos }, { easing: 'quartOut' }).start();
