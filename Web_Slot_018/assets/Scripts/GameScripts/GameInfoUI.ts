...
import { RotationResize } from "../Utils/RotationResize";
import { Orientation } from "../Utils/Config";
import { KeySpriteFramePair } from "../Utils/KeySpriteFramePair";
import { DragNodeEvent } from "./DragNodeEvent";
import { SkeletonExtension } from "./SkeletonExtension";
...
export class LoadingPage {
    ...
    public playAnchorAnimation(orientation: Orientation) {
        for (let i = 0; i < this.spineList.length; i++) {
            this.spineList[i].playLocalizationSpine(orientation === Orientation.Landscape ? this.ANCHOR_L : this.ANCHOR_P, 1, false);
        }
    }
}