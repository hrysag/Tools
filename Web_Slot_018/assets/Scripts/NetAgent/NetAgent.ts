import { askHistoryBodyFormat as AskHistoryBodyFormat, ConfigType, Environment, historyHeightFormat, HistoryHeightMax, ICommonSetting, IGameConfig, IGameSetting, IHistory, IPlatformSetting, IPlayerInfo, LoginReDispatcherConnectFail, PARSER_URL_FAIL, REQUEST_TIMEOUT, REQUEST_TIMEOUT_ERROR_CODE, SpinReDispatcherConnectFail } from "./AgentDefine";
...
import { GameType } from "./NetAgentDefine";
...
export class NetAgent {
    ...
    private onCConnectRecv(_: number, data: Uint8Array) {
        const bt = new ByteReaderHelper(data.buffer);
     ...
     */
    public ParserBaseConfig(url: string): boolean {
        try {
    ...
    public async AskWebConfig(configType: ConfigType): Promise<void> {
        try {
            ...
            if (!this.parserWebConfig(connectSetting)) {
                throw Error(`parser WebConfigUrl ${this._playerInfo.webConfigUrl} Fail`);
            }
    ...
    private async askPlayerHistory(historyHeight: number = 0): Promise<void> {
        if (!this._playerInfo.webConfig.CommonSetting.ESAPIHistoryURL) {
        ...
        for (let i = historyHeight, count = 0;
            i > 0 && count < HistoryHeightMax;
            i--, count++) {
            askBody.push(AskHistoryBodyFormat
        ...
        try {
            ...
            const result = await this.Post(this._playerInfo.webConfig.CommonSetting.ESAPIHistoryURL, body) as string;
            this.parserHistoryHeight(JSON.parse(result) as historyHeightFormat[]);
        }
        catch (error) {
            console.error("askPlayerHistory Fail", error);
        }
    }
    ...
    private parserHistoryHeight(historyHeight: historyHeightFormat[]) {
        ...
        this._history.sort((a, b) => b.Time - a.Time);
    }
     ...
     * @returns @SpinAck
     */
    public async Spin(bet: number, additionalPurchaseType: AdditionalPurchaseType = AdditionalPurchaseType.None): Promise<SpinAck> {
    ...
    public Post<T>(url: string, data: Document | XMLHttpRequestBodyInit | Uint8Array, timeout: number = REQUEST_TIMEOUT): Promise<T> {
        return this.promiseTimeout(new Promise<T>((resolve, reject) => {
            const xhr = new XMLHttpRequest();
            xhr.open("post", url);
    ...
    private parserHistory(history: string[]): IHistory[] {
        return history.map((value) => {
            ...
            if (this.isISO8601TimeFormat(value["T"]) == false) {
                //非 ISO8601 時間格式，轉換為 ISO8601 時間格式
                value["T"] = this.transformISO8601TimeFormat(value["T"]);
            }
    ...
    public insertHistory(spinAck: SpinAck) {
        ...
        if (this._history.length >= HistoryHeightMax) {
            this._history.length = HistoryHeightMax - 1;
        }
