    ...
    execute: function () {
      ...
      _export("GameViewManager", GameViewManager = (_dec = ccclass('GameViewManager'), _dec2 = property({
        ...
        constructor() {
          ...
          */
          this.allReelRollEndHandler = async () => {
            (_crd && AudioManager === void 0 ? (_reportPossibleCrUseOfAudioManager({
              error: Error()
            ...
            if (totalWinScore > 0) {
              this._startGetScoreInThisRound = true; //--強迫關閉右側操作面板按鈕
          ...
          this.oneReelRollEndCallBackFromSlot = reelID => {
            if (this._currentSlotInfo.reelInfo.haveForecast) {
              ...
              if (!this._rpsWildSystem.isWorking) {
                ...
                const wildData = this.getWildIconData(reelID, this._currentSlotInfo.reelInfo.symbolData);
              ...
              const bonusData = this.getFGBonusData(reelID, this._currentSlotInfo.reelInfo.symbolData);

              if (bonusData.length > 0) {
                this._bonusShowAniData = this._bonusShowAniData.concat(bonusData);
                const soundTarget = bonusData[0].camp == 0 ? (_crd && SoundList === void 0 ? (_reportPossibleCrUseOfSoundList({
                  error: Error()
          ...
          this.getLoadLanguageSpine = async (prefabKey, bonusData) => {
        ...
        stopSpin(slotData) {
          ...
          if (reelInfo.haveForecast && (!this._isBuyFG || !this._currentTurboSpeed)) {
            const canTriggerForecast = !this._rpsWildSystem.isWorking && !this._fgBonusManager.isWorking;
          ...
          const rawSymbol2D = this.getIconIDTo2DArray(reelInfo.symbolData); //---這邊要給showAniSystem的2d陣列資料

          const clonedSymbol2D = (_crd && GameUtils === void 0 ? (_reportPossibleCrUseOfGameUtils({
            error: Error()
        ...
        async checkAndLoadBonusData() {
          const allBonusLoadPromises = [];
        ...
        async handleWinCase(winScoreData, allIconAniDataForRound) {
          const targetData = this._currentSlotInfo.winLine;
        ...
        getOutIndex(targetData, groupIndex, iconIndex) {
          const isNormalOrReSpine = this._currentGameState === (_crd && GameState === void 0 ? (_reportPossibleCrUseOfGameState({
            error: Error()
          ...
          const currentCamp = targetData[groupIndex].camp; //--NG/ReSpin模式是兩個盤面,且要連續3個才會連線,所以資料最少長度一定是3
        ...
        async createBonusIconData(bonusData) {
          const aniShowNode = (_crd && FindNode === void 0 ? (_reportPossibleCrUseOfFindNode({
            error: Error()
          }), FindNode) : FindNode).findChildByNameRecursive(this._showAniSystem.node, 'SymbolAniDisplayNode');
          const targetShowAniData = bonusData;
          const prefabKey = targetShowAniData.camp == 0 ? PFB_SYMBOL_ANI + '10' : PFB_SYMBOL_ANI + '11';
          const bonusDataWithLanguage = await this.getLoadLanguageSpine(prefabKey, targetShowAniData);
          let cop = (_crd && AniSysTools === void 0 ? (_reportPossibleCrUseOfAniSysTools({
            error: Error()
          ...
          const localPos = aniShowNode.getComponent(UITransform).convertToNodeSpaceAR(wPos);
        ...
        createWildIconData(wildData) {
          ...
          let localPos = wildDisplayNode.getComponent(UITransform).convertToNodeSpaceAR(wpos);
        ...
        async checkNextRound() {
          if (this._serverBackSlotInfo.reSpinReelInfo.length > 0) {
            ...
            const data = this._serverBackSlotInfo.reSpinReelInfo.shift(); //======這邊要小心,直接換資料了============================================================


            this._currentSlotInfo = data; //-_currentSlotInfo
            ...
            this.processRound((_crd && GameState === void 0 ? (_reportPossibleCrUseOfGameState({
              error: Error()
            }), GameState) : GameState).RE_SPINE, data);
          } else if (this._serverBackSlotInfo.freeGameReelInfo.length > 0) {
            const fgData = this._serverBackSlotInfo.freeGameReelInfo.shift();
            ...
            if (!this._fgBonusManager.isWorking) {
              ...
              this._processSymbolData.currentCamp = symbol2dsAndCamp.camp; //--第一次進FG.準備開啟轉場
              ...
              await this._gateN2FTransition.playAinForStart(this.allTransitionCloseDoorReadyHandler); //---轉場結束相關的資料重設(in)
            ...
            this._currentSlotInfo = fgData; //--每一輪的盤面資料都要倒進去
            ...
            if (this._fgBonusManager.isWorking) {
              ...
              this._gateN2FTransition.transitionState = (_crd && TransitionsState === void 0 ? (_reportPossibleCrUseOfTransitionsState({
                error: Error()
              ...
              let totalWinScore = (this._serverBackSlotInfo.allRoundOdds * this._serverBackSlotInfo.betValue).fixed();

              await this._gateN2FTransition.closeFG(totalWinScore);

              this._showAniSystem.cleanAllPlayingAniForNewRound();

              this._showAniSystem.stopShowVerticalAni(); //--20250610這邊要換掉FG陣營的spine skin圖片
        ...
        async processRound(gameState, data) {
          //--盤面reel重新排列
          ...
          this.startSpin(false); //await GameUtils.Defer(delayTime); // 使用定義的延遲常數
        ...
        changeWildState(data) {
          if (this._rpsWildSystem.isWorking) {
            if (this._rpsWildSystem.wildState >= (_crd && RPSWildState === void 0 ? (_reportPossibleCrUseOfRPSWildState({
              ...
              let connectBox = this._showAniSystem.getWildConnectBoxNode();
            ...
            let leftWildComp = this._rpsWildSystem.wild_left;
            ...
            let rightWildComp = this._rpsWildSystem.wild_right;
        ...
        async processNormalRound(freeEnd = false) {
          //--盤面reel重新排列 
          ...
          this._showContainerController.changeGameMode((_crd && GameState === void 0 ? (_reportPossibleCrUseOfGameState({
            error: Error()
          }), GameState) : GameState).NORMAL, NO_CAMP_DATA); // 使用定義的常數


          if (!this._gateN2FTransition.isRunning && !(_crd && GenericUIManager === void 0 ? (_reportPossibleCrUseOfGenericUIManager({
            error: Error()
          }), GenericUIManager) : GenericUIManager).instance.isAutoMode) {
        ...
        */
        ...
        getFGBonusData(reelID, iconData) {
          ...
          let targetReel = symbolData[reelID]; //--會有不同的遊戲狀態(reSpin/freeGame)的資料,不能直接取結果的盤面

          for (let i = 0; i < targetReel.length; i++) {
        ...
        getIconIDTo2DArrayWithCamp(iconData2DArray) {
          const ary2d = this.getIconIDTo2DArray(iconData2DArray);
...
//# sourceMappingURL=55532f281281be9e4bcbc41b94b58292a113f369.js.map