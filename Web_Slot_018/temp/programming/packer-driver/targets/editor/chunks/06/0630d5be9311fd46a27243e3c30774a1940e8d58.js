    ...
    execute: function () {
      ...
      _export("NetAgent", NetAgent = class NetAgent {
        ...
        async askPlayerHistory(historyHeight = 0) {
          ...
          const askBody = []; //只取 HistoryHeightMax 筆資料

          for (let i = historyHeight, count = 0; i > 0 && count < (_crd && HistoryHeightMax === void 0 ? (_reportPossibleCrUseOfHistoryHeightMax({
            error: Error()
          ...
          try {
            ...
            const result = await this.Post(this._playerInfo.webConfig.CommonSetting.ESAPIHistoryURL, body);
            this.parserHistoryHeight(JSON.parse(result));
          } catch (error) {
