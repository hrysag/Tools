        ...
        calculateOneIconAllMatch(icon, icon2DData) {
          ...
          for (let i = 0; i < this.reelAmount; i++) {
            ...
            if (icon2DData[i].includes(icon) || hasWild) {
              let winPos = []; //console.log('check_winPos', winPos);

              const pos = icon2DData[i].indexesOf(icon).map(x => x + i * this.symbolLength); //console.log('check_pos', pos);

              AllPosList = this.mergeTwoArrays(AllPosList, pos); //console.log('check_AllPosList', AllPosList);

              winPos = this.mergeTwoArrays(winPos, pos); //console.log('check_winPos', winPos);
