      ...
      _export("ReelView018", ReelView018 = (_dec = ccclass('ReelView018'), _dec(_class = class ReelView018 extends (_crd && IconReelView === void 0 ? (_reportPossibleCrUseOfIconReelView({
        ...
        constructor() {
          ...
          this.getAndRemoveSymbolAniNodeWithWorldPos = (reelIndex, iconIndex) => {
            let targetNode = this.getAndRemoveSymbolAniNodeInReel(reelIndex, iconIndex);
            let worldPos = this.getSymbolWorldPosition(reelIndex, iconIndex);
            return {
              target: targetNode,
              worldPos: worldPos
            };
          ...
          this.changeInitSpineAniNode = () => {
            ...
            for (let i = 0; i < this._resultIconList.length; i++) {
              ...
              for (let j = 0; j < this._resultIconList[i].length; j++) {
                let gameIconData = this._resultIconList[i][j].iconData;
                this.getSymbolAniNode(gameIconData.iconID, i, j, campIndex).then(node => {
                  if (node) {
                    // 處理 node
                    this._resultIconList[i][j].addSymbolAniNode(node);
                  }
                });
              }
            }
        ...
        initIcon() {
          for (let reelID = 0; reelID < this._reels.length; reelID++) {
            ...
            let campIndex = this._reels[reelID].camp;
        ...
        initSymbolWorldPosition(aryEmptyTarget) {
          for (let i = 0; i < this._resultIconList.length; i++) {
            ...
            for (let j = 0; j < this._resultIconList[i].length; j++) {
              ...
              */
              let worldPos = this.reGetRealIconWorldPosition(i, j);
        ...
        reGetRealIconWorldPosition(reelIndex, iconIndex) {
          let targetNode = this._resultIconList[reelIndex][iconIndex].node; //--這是產生的prefab

          let pos = targetNode.position;
          let uiTransformComponent = targetNode.parent.getComponent(UITransform);
          let worldPos = uiTransformComponent.convertToWorldSpaceAR(pos);
          return worldPos;
        }
        ...
        changeSpineAniNodeSkinAfterFG() {
          for (let i = 0; i < this._resultIconList.length; i++) {
            for (let j = 0; j < this._resultIconList[i].length; j++) {
              const gameIcon = this._resultIconList[i][j];
              const spineAniNodeName = gameIcon.getSymbolAniNodeName();
              const iconData = gameIcon.getGameIconData(); //-iconData.camp會對應到原本所屬的陣營,iconData.iconID會對應到icon的id(or symbolID)
              //--ps 00,01,02,03這些在資料裡面會是 0 1 2 3

              if (spineAniNodeName != '') {
                const checkSameData = this.isIconIdMatched(iconData.iconID, iconData.camp, spineAniNodeName);
                ...
                if (!checkSameData.sameSpine && checkSameData.prefabKey != '') {
                  ...
                  const prefabAniId = spineAniNode[(_crd && DYN_NODE_PROPERTIES === void 0 ? (_reportPossibleCrUseOfDYN_NODE_PROPERTIES({
                    error: Error()
                  ...
                  const plaData = aniInterfaceComponent.getCustomizeSpineTrackEntry('idle');
        ...
        cleanIdleSymbolAnis() {
          ...
          for (const icon of this._prepareIconList) {
            const spineNode = icon.getSymbolAniNodeAndRemove();
            this.clearAndRecycleSpineNode(spineNode);
          }
        }
        ...
        clearAndRecycleSpineNode(spineNode) {
          if (!spineNode) return;
          const prefabId = spineNode[(_crd && DYN_NODE_PROPERTIES === void 0 ? (_reportPossibleCrUseOfDYN_NODE_PROPERTIES({
            error: Error()
        ...
        changePrepareIconSymbol(reelID) {
          let symbolID = this.getIconSymbolData(reelID); //--原諒我為了拿到這個區域變數才這麼下幹不用super.changePrepareIconSymbol
          ...
          if (this._reelStateList[reelID] === (_crd && ReelRoundState === void 0 ? (_reportPossibleCrUseOfReelRoundState({
            ...
            const currentCamp = prepareIcon.getCurrentCamp();
            const iconReelInfo = prepareIcon.getIconReelInfo();
        ...
        async getSymbolAniNode(symbolId, reelIndex, iconIndex, camp) {
        ...
        async getHighOddSpineAniNode(prefabId, symbolId, reelIndex, iconIndex) {
        ...
        setWildModeForGameIconDarkness() {
          let targetLength = this._resultIconList.length;

          for (let i = 0; i < targetLength; i++) {
            for (let j = 0; j < this._resultIconList[i].length; j++) {
              let gameIcon = this._resultIconList[i][j];
...
//# sourceMappingURL=4a4d05d56a8850f850765196aa617f1626372434.js.map