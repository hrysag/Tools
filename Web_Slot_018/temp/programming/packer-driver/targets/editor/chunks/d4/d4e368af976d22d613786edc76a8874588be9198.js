    ...
    execute: function () {
      ...
      _export("ProcessSymbolData", ProcessSymbolData = class ProcessSymbolData {
        ...
        constructor() {
          ...
          this.getWildIconDataAniBeforeRollEnd = async (symbolId, reelIndex, iconIndex, camp) => {
            const wildPrefabId = 'Icon_0678'; //--動態wild的prefab id

            const wildData = this.getWildIconData(reelIndex, iconIndex, symbolId, camp);
            ...
            let targetNode = (_crd && AnimationControllersPoolManager === void 0 ? (_reportPossibleCrUseOfAnimationControllersPoolManager({
              error: Error()
           ...
           */
          this.getHighOddSpineAniAfterFGEnd = async (prefabId, symbolId, reelIndex, iconIndex) => {
            const {
              symbolIndex,
              scoreState,
              showGroup
            } = this.getSymbolAnimationDataBeforeRollEnd(symbolId);
            ...
            const targetNode = (_crd && AnimationControllersPoolManager === void 0 ? (_reportPossibleCrUseOfAnimationControllersPoolManager({
              error: Error()
          ...
          this.getSymbolIconAniBeforeRollEnd = async (symbolId, reelIndex, iconIndex, camp) => {
            const token = Date.now();
            const {
              symbolIndex,
              scoreState,
            ...
            const reelInfoAndGroup = this.createReelInfoAndGroup(reelIndex, iconIndex, symbolIndex, showGroup);
            const addPlayInfoData = this.createPlayAniData(new Vec3(0, 0, 0), reelInfoAndGroup, showGroup); //this._processGameState === GameState.FREE_GAME ? this._currentCamp : (reelIndex <= 2 ? 0 : 1)

            const targetCamp = this._processGameState === (_crd && GameState === void 0 ? (_reportPossibleCrUseOfGameState({
              error: Error()
            }), GameState) : GameState).FREE_GAME ? this._currentCamp : camp;
            ...
            const tokenID = token + "_" + (_crd && GameUtils === void 0 ? (_reportPossibleCrUseOfGameUtils({
              error: Error()
            }), GameUtils) : GameUtils).getRangeRandom(0, 100);
            const targetNode = (_crd && AnimationControllersPoolManager === void 0 ? (_reportPossibleCrUseOfAnimationControllersPoolManager({
              error: Error()
        ...
        createWildIconData(wildData, wpos) {
          ...
          let wildNode = this._showAniController.addSPNodeInRunning('Icon_0678', slotData, 99);
        ...
        getAnimationPlayInfo(symbolIndex, scoreState, score, camp) {
          const aniLoop = score <= 0; //let ts = 1.2;//-timeScale

          let ts = 1; //--先回復美術原先設定的速度20250505

          let spineSkinName = ''; //----2黑桃/3紅心/4梅花/5方塊--server資料
          // animation name:icon_04_connect(黑桃),icon_05_connect(紅心),icon_06_connect(方塊),icon_07_connect(梅花) 
        ...
        getWildIconData(reelIndex, iconIndex, symbolId, campData = -1) {
          const returnData = {
            reelIndex: reelIndex,
            iconIndex: iconIndex,
            wild: symbolId,
...
//# sourceMappingURL=d4e368af976d22d613786edc76a8874588be9198.js.map