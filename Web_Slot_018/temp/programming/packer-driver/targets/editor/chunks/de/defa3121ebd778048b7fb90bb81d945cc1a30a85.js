System.register(["__unresolved_0", "cc", "__unresolved_1", "__unresolved_2", "cc/env"], function (_export, _context) {
  ...
  var _reporterNs, _cclegacy, __checkObsolete__, __checkObsoleteInNamespace__, Button, EventHandler, js, director, assetManager, Camera, Canvas, view, RenderTexture, resources, instantiate, Prefab, Label, error, gfx, Color, Vec3, UITransform, BinaryBuffer, Debug, EDITOR, PREVIEW, Utility, _crd;
  ...
  return {
    ...
    execute: function () {
      ...
      _cclegacy._RF.push({}, "b5b7eXL1ORE4ZVuZ3L2LYDN", "Utility", undefined);

      __checkObsolete__(['_decorator', 'Button', 'Component', 'EventHandler', 'EventTouch', 'Node', 'js', 'director', 'assetManager', 'Camera', 'Canvas', 'view', 'RenderTexture', 'resources', 'instantiate', 'Prefab', 'Label', 'error', 'gfx', 'Color', 'EventMouse', 'Vec3', 'UITransform']);
      ...
      _export("Utility", Utility = class Utility {
        ...
        static getPayTableURL(gameID, lang) {
          ...
          let payTableURL = `https://gameapi.apex-win.com/ApexWin?gameID=${gameID.toLowerCase()}&lang=${lang}&page=introduction&timestamp=${timestamp}`;
          return payTableURL;
        }

        static getRuleURL(gameID, lang) {
          let timestamp = new Date().getTime();
          let ruleURL = `https://gameapi.apex-win.com/ApexWin?gameID=${gameID.toLowerCase()}&lang=${lang}&page=operation&timestamp=${timestamp}`;
          return ruleURL;
        }

        static getHistoryURL(lang, recordJsonString) {
          let historyURL = `https://dev-gamerecord.apex-win.com/#/game-list?lang=${lang}&history=${recordJsonString}`;

          if (this.isTestEnvironment() === false) {
            historyURL = `https://gamerecord.apex-win.com/#/game-list?lang=${lang}&history=${recordJsonString}`;
          }
        ...
        static checkLabelBold(langKey) {
          ...
          let isSafari = Utility.getBrowserAndDeviceInfo().isSafari; // let isChinese = langKey === "tw" || "cn";
...
//# sourceMappingURL=defa3121ebd778048b7fb90bb81d945cc1a30a85.js.map