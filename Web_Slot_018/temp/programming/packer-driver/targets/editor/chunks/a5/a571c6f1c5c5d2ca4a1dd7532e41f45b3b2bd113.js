  ...
  return {
    ...
    execute: function () {
      ...
      _export("BonusManager", BonusManager = (_dec = ccclass('BonusManager'), _dec2 = property({
        ...
        changeToLandscape() {
          const target = this._portraitNode.children[0] || this._landscapeNode.children[0]; //--直接操作_freeSpineCountTimesPrefabNode不就得了?
        ...
        setWorldPosForRound(pos) {
          ...
          const roundForBonusDataLen = this._roundForBonusData.length;

          const uiTransform = this._bonusParticleContainer.getComponent(UITransform);

          for (let i = 0; i < len; i++) {
            for (let j = 0; j < roundForBonusDataLen; j++) {
        ...
        async playSingleBonusEffect(reelIndex, iconIndex) {
          return new Promise(async (resolve, reject) => {
            let bonusData = this.getBonusData(reelIndex, iconIndex); //--當取得的數量>最大數量時,不再啟動累計燈號,只會做fg局號的累加(最大燈號上限=13)
            ...
            if (this._currentBonusCount < this._maximumCount) {
              ...
              this.getMultiplier(); //await GameUtils.Defer(400);
        ...
        async shootParticle(data, index) {
          return new Promise(async (resolve, reject) => {
            ...
            let ani = (_crd && FindComponent === void 0 ? (_reportPossibleCrUseOfFindComponent({
              error: Error()
            ...
            let particleEmitter = (_crd && FindComponent === void 0 ? (_reportPossibleCrUseOfFindComponent({
              error: Error()
            ...
            let amplitude = 50; // 降低振幅

            let frequency = 0.5; // 降低頻率

            const tempVec3 = new Vec3(); //AudioManager.instance.playSound(SoundList.MoneyCollect, SOUND_TYPE.ONE_SHOT, AudioSourceList.BtnAS);
        ...
        getBonusData(reelIndex, iconIndex) {
          for (let i = 0; i < this._roundForBonusData.length; i++) {
        ...
        getMultiplier(value) {
          const targetCount = value ? value : this._currentBonusCount;
...
//# sourceMappingURL=a571c6f1c5c5d2ca4a1dd7532e41f45b3b2bd113.js.map