    ...
    execute: function () {
      ...
      _export("ShowContainerController", ShowContainerController = (_dec = ccclass('ShowContainerController'), _dec2 = property({
                ...
                if (node.nodeName == 'FG_Bkg_Ali' || node.nodeName == 'FG_Bkg_Thieves') {
                  ...
                  if (node.nodeName == 'FG_Bkg_Thieves') {
                    ...
                    if (!this.checkChildrenContains(node.displayNode, 'FG_Bkg')) {
                      ...
                      const fgComp = shareBGRootContainer.getComponent(_crd && FG_BkgController === void 0 ? (_reportPossibleCrUseOfFG_BkgController({
                        error: Error()
                      ...
                      if (fgComp) {
                        ...
                        const targetComponent = node.displayNode.getComponent(_crd && FG2_BkgController === void 0 ? (_reportPossibleCrUseOfFG2_BkgController({
                          error: Error()
                    ...
                    const fg_Bkg_frontNode = (_crd && FindNode === void 0 ? (_reportPossibleCrUseOfFindNode({
                      error: Error()
                    }), FindNode) : FindNode).findChildByNameRecursive(node.displayNode, 'FG_Bkg_front'); //--只有阿里才有的最前面的2搓盆栽

                    if (fg_Bkg_frontNode) {
                      const frameNode = this.getTargetNodeByNameInGameModeNode(this._fg_Thieves_Show_Sys, 'FG_Thieves'); //console.log('checkParent', frameNode);
        ...
        processFGTransitionNode() {
          const ngNodeContainer = (_crd && FindNode === void 0 ? (_reportPossibleCrUseOfFindNode({
            error: Error()
          ...
          const ngLanguageTitleNode = (_crd && FindNode === void 0 ? (_reportPossibleCrUseOfFindNode({
            error: Error()
        ...
        changeBGContainerLayerDuringTransition(camp) {
          const target = (_crd && FindNode === void 0 ? (_reportPossibleCrUseOfFindNode({
            error: Error()
          }), FindNode) : FindNode).findChildByNameRecursive(this._fakeNgVerticalNode, 'NG_bkg_pic');
          target.active = false;
          const ng_Bkg = (_crd && FindNode === void 0 ? (_reportPossibleCrUseOfFindNode({
            error: Error()
          }), FindNode) : FindNode).findChildByNameRecursive(this._bgContainerNode, 'NG_Bkg');
          const ogContainerNode = (_crd && FindNode === void 0 ? (_reportPossibleCrUseOfFindNode({
            error: Error()
          ...
          const ngLanguageTitleNodeTarget = (_crd && FindNode === void 0 ? (_reportPossibleCrUseOfFindNode({
            error: Error()
          }), FindNode) : FindNode).findChildByNameRecursive(this._fakeNgVerticalNode, 'NG_logo');
          const ogLanguageContainerNode = (_crd && FindNode === void 0 ? (_reportPossibleCrUseOfFindNode({
            error: Error()
          ...
          if (camp == 1) {
            const frameNode = this.getTargetNodeByNameInGameModeNode(this._fg_Thieves_Show_Sys, 'FG_Bkg_Thieves');
            const fg2Comp = (_crd && FindComponent === void 0 ? (_reportPossibleCrUseOfFindComponent({
              error: Error()
        ...
        reSetContainerLayer() {
          const nodes = this._fakeTransitionNode.children;
        ...
        setFGCamp(camp) {
          const target = this._showContainerMap.get((_crd && GameState === void 0 ? (_reportPossibleCrUseOfGameState({
            error: Error()
          }), GameState) : GameState).FREE_GAME);
          ...
          for (const nodeAry of target) {
            for (const displayItem of nodeAry.gameNodeHashInfo) {
              const testComp = (_crd && FindComponent === void 0 ? (_reportPossibleCrUseOfFindComponent({
                error: Error()
          ...
          for (const containerList of this._showContainerMap.values()) {
            for (const nodeAry of containerList) {
              ...
              for (const displayItem of nodeAry.gameNodeHashInfo) {
                ...
                let comp = (_crd && FindComponent === void 0 ? (_reportPossibleCrUseOfFindComponent({
                  error: Error()
        ...
        */
        ...
        changeGameMode(gameState, camp) {
          ...
          const mainTarget = this.getTargetContainer(gameState, camp);
          const processed = new Set(); //this._currentCamp = camp != null ? camp : -1; // 更新當前陣營
          ...
          for (const [mapGameState, containerList] of this._showContainerMap.entries()) {
            for (const group of containerList) {
              ...
              for (const node of group.gameNodeHashInfo) {
                const displayNode = node.displayNode; //-NG_BkgController/NG_FrameController/FG_BkgController/FG2_BkgController/NG_Dec_frame(這個是在前面滾的裝飾性動畫(只有NG有))
...
//# sourceMappingURL=caa5eb35def4f9594c52735a1d684e7aa79b8929.js.map