      ...
      _export("SingleSlot", SingleSlot = (_dec = ccclass('SingleSlot'), _dec2 = property({
        ...
        reset() {
          ...
          if (this._slotSymbolItems) {
            for (var item of this._slotSymbolItems) {
              item.reset();
            }
          }
        }
        ...
        clean() {
          ...
          if (this._slotSymbolItems) {
            while (this._slotSymbolItems.length > 0) {
              var symbol = this._slotSymbolItems.pop();
        ...
        init(camp) {
          ...
          for (var i = 0; i < 2; i++) {
            ...
            var uiTransform = symbolNode.addComponent(UITransform);
            var slotSymbolItem = symbolNode.addComponent(_crd && SlotSymbolItem === void 0 ? (_reportPossibleCrUseOfSlotSymbolItem({
              error: Error()
            ...
            var ogSize = slotSymbolItem.spriteFrame.originalSize;
          ...
          var mask = this.node.parent.getComponent(Mask); //--改用sprit stencil來處理mask

          if (mask) {
            var spr = this.node.parent.getComponent(Sprite); //const uiTransform = this.node.getComponent(UITransform);

            if (spr) {
              var _uiTransform = this.node.parent.getComponent(UITransform);

              _uiTransform.contentSize = maskSize;
            }
            /*
            graphic.clear();
        ...
        createMask(maskSize) {
          var mask = this.node.parent.addComponent(Mask);
        ...
        run(deltaTime) {
          ...
          for (var i = 0; i < this._slotSymbolItems.length; i++) {
            var symbol = this._slotSymbolItems[i];

            if (symbol.isRunning) {
              var symbolY = this._velocity * deltaTime; // 使用deltaTime計算位移
              ...
              if (symbol.node.position.y <= this._maxDistance) {
                ...
                if (this._isStopping) {
                  ...
                  var index = this.getRandomSymbolIndex();
          ...
          if (this._isStopping) {
            var targetSymbol = this._slotSymbolItems.find(item => item.finalRolling);
        ...
        getBlurSpriteFrames(iconId) {
          var symbolIconData = this._targetBlurSpriteFrames.find(item => item.iconId == iconId);

          return symbolIconData.spriteFrame;
        }

        getSymbolIconSpriteFrame(iconId) {
          var symbolIconData = this._targetSymbolSpriteFrames.find(item => item.iconId == iconId);
        ...
        getRandomSymbolIndex(extraNum) {
          ...
          var currentSymbolIndices = this._slotSymbolItems.map(item => item.symbolIndex);
          ...
          for (var i = 0; i < this._targetSymbolSpriteFrames.length; i++) {
        ...
        resetSymbolData(starItem) {
          ...
          for (var symbol of this._slotSymbolItems) {
            if (symbol.targetSymbol) {
              ...
              var index = this.getRandomSymbolIndex(starItem);
              symbol.normalSpriteFrame = this.getSymbolIconSpriteFrame(index);
              symbol.blurSpriteFrame = this.getBlurSpriteFrames(index);
              symbol.setBlur();
              symbol.symbolIndex = index;
        ...
        stopRolling() {
          ...
          return _asyncToGenerator(function* () {
            _this._isStopping = true;
            return new Promise(resolve => {
              _this._stopResolvePromise = resolve; // 儲存 resolve 函式
            });
          })();
        } //---滾動開始並且指定盤面上初始的圖案
        ...
        startRolling(finalIconId, starItem) {
          var previousValue = this._finalIconId;
          this._finalIconId = finalIconId;
          var realStartItem = starItem ? starItem : previousValue; //--沒有填就用上一把的結果當作開始圖案
...
//# sourceMappingURL=ba918b63313f136455a9e8e6374942ee95200572.js.map