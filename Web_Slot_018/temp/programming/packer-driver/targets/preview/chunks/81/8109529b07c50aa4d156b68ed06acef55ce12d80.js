System.register(["cc"], function (_export, _context) {
  ...
  var _cclegacy, __checkObsolete__, __checkObsoleteInNamespace__, AllMatchPayTable018, WinData, PayLineData, _crd;

  _export("AllMatchPayTable018", void 0);
  ...
  return {
    ...
    execute: function () {
      ...
      _cclegacy._RF.push({}, "533deAWbxREYrXjNC7lP6uk", "AllMatchPayTable018", undefined);
      ...
      _export("AllMatchPayTable018", AllMatchPayTable018 = class AllMatchPayTable018 {
        /**
         * @param wild WILD圖示
         * @param payTable 賠率表(要有Wild的賠率) 
         * @param iconList 中獎圖示表 (將有賠率的連線中獎圖示放入，SCATTER請另外算)
         */
        constructor(wild, payTable, iconList) {
          this.wild = void 0;
          ...
          for (var icon of this.iconList) {
            ...
            for (var [key, value] of oneIconMatchMap) {
              for (var item of value) {
                var Win2DPos = this.convertWinIndexTo2DArray(item.Pos);
                var winData = new WinData(key, item.Odd, item.Pos, item.OneMatchPos, Win2DPos);
        ...
        calculateOneIconAllMatch(icon, icon2DData) {
          ...
          var _loop = function _loop(i) {
            ...
            if (icon2DData[i].includes(icon) || hasWild) {
              var winPos = []; //console.log('check_winPos', winPos);

              var pos = icon2DData[i].indexesOf(icon).map(x => x + i * _this.symbolLength); //console.log('check_pos', pos);

              AllPosList = _this.mergeTwoArrays(AllPosList, pos); //console.log('check_AllPosList', AllPosList);

              winPos = _this.mergeTwoArrays(winPos, pos); //console.log('check_winPos', winPos);
              ...
              if (tempOdds > 0) {
                ...
                for (var combinedWireTarget of combinedWire) {
                  var payLineData = new PayLineData(AllPosList, newLineOdds, [combinedWireTarget]);
                  ary.push(payLineData);
                }

                oneIconMatchMap.set(icon, ary); //const payLineData = new PayLineData(AllPosList, tempOdds, combinedWire);
...
//# sourceMappingURL=8109529b07c50aa4d156b68ed06acef55ce12d80.js.map