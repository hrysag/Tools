    ...
    execute: function () {
      ...
      _export("ClassicalSlotAniController", ClassicalSlotAniController = (_dec = ccclass('ClassicalSlotAniController'), _dec2 = property({
        ...
        addAnimationData(IAniData) {
          var token = Date.now();
          var spRuleCheck = this.checkSpRuleForExist(IAniData);
          ...
          var targetNode = super.addAnimationData(IAniData); //--重複的狀況下會回null
          //--重複的狀況下
        ...
        playInSequence() {
          ...
          var component = (_crd && AniSysTools === void 0 ? (_reportPossibleCrUseOfAniSysTools({
            error: Error()
        ...
        playWinInThisRound(winScoreData, lines) {
          ...
          return _asyncToGenerator(function* () {
            ...
            return new Promise( /*#__PURE__*/_asyncToGenerator(function* (resolve, reject) {
              if (lines) _this2.winLinesGroupData = lines; //--依照winLine的長度產生對應的aryGroupIDs

              _this2.changeSpNodeChildrenIndexToTop(); //--交換位置(賠率高的在越上面)


              var groupID = _this2.generateArray(_this2._winLinesGroupData.length);

              var totalScore = (winScoreData.totalOdd * winScoreData.betValue).fixed();

              _this2.showWinScoreAni(totalScore);

              yield _this2.playAniGroupsWithPromise(groupID); //--這種隨流程走的其他裝飾性的動畫要再想辦法解決怎麼串接
              //--將其他的動畫系統開啟播放(非icon表演的動畫)

              _this2.playOtherWinShowAni();

              _this2.playAniGroupInSequence();
              /**
               * 要再調整一下自動玩的得獎流程..有點不順
               */
        ...
        showWinScoreAni(totalScore) {//let baseWinMoney: number = (winScoreData.baseOdds * winScoreData.betValue).fixed();
        ...
        playAniGroupInSequence() {
          ...
          return _asyncToGenerator(function* () {
            ...
            while (_this3._winLinesGroupData.length > 0) {
              _this3.closeAllNode(); //go to next frame
              ...
              _this3.openNodeByIconsWithReelIndexInArray(_this3._winLinesGroupData[playIndex]);

              yield _this3.playAnisByGroupWithPromise(playIndex); //await GameUtils.Defer(100);
              ...
              if (playIndex >= _this3._winLinesGroupData.length) {
                playIndex = 0;
              }
            }
          })();
        ...
        playAnisByGroupWithTimeStepPromise(groupId, timeStep, trackId) {
          ...
          return new Promise( /*#__PURE__*/_asyncToGenerator(function* (resolve, reject) {
            var aniGroups = _this4.getAniNodeListByGroups(groupId);

            var iAnimationControl;

            for (var i = 0; i < aniGroups.length; i++) {
        ...
        playAniGroupsWithPromise(groups) {
          var _this5 = this;

          return _asyncToGenerator(function* () {
            return new Promise( /*#__PURE__*/_asyncToGenerator(function* (resolve, reject) {
              var aniGroups = _this5.getAniNodeListByGroups(groups); //--test--
        ...
        openNodeByIconsWithReelIndexInArray(iconIndexs) {
          for (var node of this._aryRunningNode) {
            for (var j = 0; j < iconIndexs.length; j++) {
...
//# sourceMappingURL=3fe6e5a48cdc1328823863a1e760216830f4e511.js.map