    ...
    execute: function () {
      ...
      _export("GameInfoData", GameInfoData = (_dec = ccclass('GameInfoData'), _dec2 = property({
        ...
        loadImage(langKey, path) {
          ...
          var resourcePath = path.trim();

          if (resourcePath) {
            var _path = "Game/ImageLocalization/" + langString + "/" + resourcePath + "/spriteFrame";

            return new Promise((resolve, reject) => {
              resources.load(_path, SpriteFrame, (err, spriteFrame) => {
                if (err) {
                  // 如果不存在該圖片，則使用英文圖片
                  if (err.message.includes("Bundle resources doesn't contain") && langKey !== (_crd && SlotRelayLang === void 0 ? (_reportPossibleCrUseOfSlotRelayLang({
                    ...
                    this.loadImage((_crd && SlotRelayLang === void 0 ? (_reportPossibleCrUseOfSlotRelayLang({
                      error: Error()
                    }), SlotRelayLang) : SlotRelayLang).en, _path).then(() => {
                      resolve();
                    });
                  } else {
...
//# sourceMappingURL=bf962bf4e682d27b18bf4ec31a643b3e39c8c170.js.map