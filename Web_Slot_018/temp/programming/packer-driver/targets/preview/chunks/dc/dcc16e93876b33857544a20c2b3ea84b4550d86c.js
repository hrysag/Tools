    ...
    execute: function () {
      ...
      _export("AllMachPayNG018", AllMachPayNG018 = class AllMachPayNG018 extends (_crd && MegaWaysWinScoreAnalyzer === void 0 ? (_reportPossibleCrUseOfMegaWaysWinScoreAnalyzer({
        ...
        getWinData(iconList) {
          var totalOdd = 0;
          var dataList = [];
          var matchMap = this.getMegaWaysWinData(iconList, Game018NGPayConfig.REEL_AMOUNT, Game018NGPayConfig.SYMBOL_LENGTH);

          for (var item of matchMap) {
            // 將 Win2DPos 轉換為二維陣列座標
            var convertedWin2DPos = item.OneMatchPos.map(machPos => machPos.map(pos => pos % Game018NGPayConfig.SYMBOL_LENGTH)); //console.log('checkRowData', convertedWin2DPos);
            ...
            var winData = new ClientData(item.WinSymbolID, item.Odd, item.Pos, convertedWin2DPos); //console.log(`第${item.WinSymbolID}個圖示，贏的賠率${item.Odd}，贏的位置${item.Pos}，輪播位置${item.OneMatchPos}，2D位置`);
      ...
      _export("AllMachPayFG018", AllMachPayFG018 = class AllMachPayFG018 extends (_crd && MegaWaysWinScoreAnalyzer === void 0 ? (_reportPossibleCrUseOfMegaWaysWinScoreAnalyzer({
        ...
        getWinData(iconList) {
          var totalOdd = 0;
          var dataList = [];
          var matchMap = this.getMegaWaysWinData(iconList, Game018FGPayConfig.REEL_AMOUNT, Game018FGPayConfig.SYMBOL_LENGTH);

          for (var item of matchMap) {
            // 將 Win2DPos 轉換為二維陣列座標
            var convertedWin2DPos = item.OneMatchPos.map(machPos => machPos.map(pos => pos % Game018FGPayConfig.SYMBOL_LENGTH)); //console.log('checkRowData', convertedWin2DPos);
            //const winData = new ClientData(item.WinSymbolID, item.Odd, item.Pos, item.OneMatchPos);
...
//# sourceMappingURL=dcc16e93876b33857544a20c2b3ea84b4550d86c.js.map