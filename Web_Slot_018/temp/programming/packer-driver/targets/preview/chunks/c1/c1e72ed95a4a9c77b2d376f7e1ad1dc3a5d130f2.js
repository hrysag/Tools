    ...
    execute: function () {
      ...
      _export("RPSWildSystem", RPSWildSystem = (_dec = ccclass('RPSWildSystem'), _dec2 = property({
        ...
        stopSlotRolling() {
          ...
          return _asyncToGenerator(function* () {
            return new Promise( /*#__PURE__*/_asyncToGenerator(function* (resolve, reject) {
              _this._isForceStopped = true;
              var promises = [_this._singleSlot_L.stopRolling(), _this._singleSlot_R.stopRolling()];

              try {
                yield Promise.all(promises); //--如果(rollWild)resolve還活著的話,就釋放掉

                if (_this._resolvePromiseRollEnd) {
                  _this._resolvePromiseRollEnd();

                  _this._resolvePromiseRollEnd = undefined;
                } //--釋放rollWild的resolve


                _this._isRolling = false;
                yield _this.playWildAppearAni();

                _this.changeDefaultTargetSkin(); //--change default ani(for skin)
        ...
        setWildIcon(wildIconComponent, wildData, slotInfo) {
          //this._isWorking = true;
          var targetNode = wildIconComponent.node.parent.children[0].getChildByName('Mask').children[0]; //--這個要想一下怎麼改..這樣很危險

          var singleSlotComponent = targetNode.getComponent(_crd && SingleSlot === void 0 ? (_reportPossibleCrUseOfSingleSlot({
            error: Error()
        ...
        changeDefaultTargetSkin() {
          ...
          var startIndexState = (_crd && RPSWildState === void 0 ? (_reportPossibleCrUseOfRPSWildState({
            error: Error()
          ...
          var key = this._wild_left.getAniNameByWildRoundState(startIndexState, (_crd && RPSWild_AniState === void 0 ? (_reportPossibleCrUseOfRPSWild_AniState({
            error: Error()
          }), RPSWild_AniState) : RPSWild_AniState).CONNECT);
        ...
        playConnectAni(wildState) {
          var _this2 = this;

          return _asyncToGenerator(function* () {
            var state = wildState ? wildState : _this2._wildState;

            var playWildAniData_L = _this2._wild_left.getAniNameByWildRoundState(state, (_crd && RPSWild_AniState === void 0 ? (_reportPossibleCrUseOfRPSWild_AniState({
              error: Error()
            }), RPSWild_AniState) : RPSWild_AniState).CONNECT);

            var playWildAniData_R = _this2._wild_right.getAniNameByWildRoundState(state, (_crd && RPSWild_AniState === void 0 ? (_reportPossibleCrUseOfRPSWild_AniState({
              error: Error()
            }), RPSWild_AniState) : RPSWild_AniState).CONNECT);

            var promises = [_this2._wild_left.playAniInPromise(playWildAniData_L), _this2._wild_right.playAniInPromise(playWildAniData_R)];
            yield Promise.all(promises);
          })();
        }

        playAniInPromiseForFirstRound(wildState) {
          var _this3 = this;

          return _asyncToGenerator(function* () {
            var playWildAniData_L = _this3._wild_left.getAniNameByWildRoundState(wildState, (_crd && RPSWild_AniState === void 0 ? (_reportPossibleCrUseOfRPSWild_AniState({
              error: Error()
            }), RPSWild_AniState) : RPSWild_AniState).CONNECT);

            var playWildAniData_R = _this3._wild_right.getAniNameByWildRoundState(wildState, (_crd && RPSWild_AniState === void 0 ? (_reportPossibleCrUseOfRPSWild_AniState({
              error: Error()
            }), RPSWild_AniState) : RPSWild_AniState).CONNECT);

            var promises = [_this3._wild_left.playAniInPromiseForFirstRound(playWildAniData_L), _this3._wild_right.playAniInPromiseForFirstRound(playWildAniData_R)];
            yield Promise.all(promises);
          })();
        }
        ...
        showAppearWithoutCampDecided() {
          ...
          return _asyncToGenerator(function* () {
            var playWildAniData_L = _this4._wild_left.getAniNameByWildRoundState((_crd && RPSWildState === void 0 ? (_reportPossibleCrUseOfRPSWildState({
              error: Error()
            }), RPSWildState) : RPSWildState).WILD_1, (_crd && RPSWild_AniState === void 0 ? (_reportPossibleCrUseOfRPSWild_AniState({
              error: Error()
            }), RPSWild_AniState) : RPSWild_AniState).APPEAR);

            var playWildAniData_R = _this4._wild_right.getAniNameByWildRoundState((_crd && RPSWildState === void 0 ? (_reportPossibleCrUseOfRPSWildState({
              error: Error()
            }), RPSWildState) : RPSWildState).WILD_1, (_crd && RPSWild_AniState === void 0 ? (_reportPossibleCrUseOfRPSWild_AniState({
              error: Error()
            }), RPSWild_AniState) : RPSWild_AniState).APPEAR);

            var promises = [_this4._wild_left.playAniInPromise(playWildAniData_L), _this4._wild_right.playAniInPromise(playWildAniData_R)];
            yield Promise.all(promises);
          })();
        } //--做完換skin(狀態變更)
        ...
        playWildAppearAni() {
          ...
          return new Promise( /*#__PURE__*/_asyncToGenerator(function* (resolve, reject) {
            _this5._singleSlot_L.node.active = false;
            _this5._singleSlot_R.node.active = false;

            var leftSkin = _this5.getWildRPSData(_this5._wild_left.slotMachineIndexInfo.iconID);

            var rightSkin = _this5.getWildRPSData(_this5._wild_right.slotMachineIndexInfo.iconID);

            _this5._wild_left.changeSkin(leftSkin);

            _this5._wild_right.changeSkin(rightSkin);

            var playWildAniData_L = _this5._wild_left.getAniNameByWildRoundState(_this5._wildState, (_crd && RPSWild_AniState === void 0 ? (_reportPossibleCrUseOfRPSWild_AniState({
              error: Error()
            }), RPSWild_AniState) : RPSWild_AniState).APPEAR);

            var playWildAniData_R = _this5._wild_right.getAniNameByWildRoundState(_this5._wildState, (_crd && RPSWild_AniState === void 0 ? (_reportPossibleCrUseOfRPSWild_AniState({
              error: Error()
            }), RPSWild_AniState) : RPSWild_AniState).APPEAR);

            var promises = [_this5._wild_left.playAniInPromise(playWildAniData_L), _this5._wild_right.playAniInPromise(playWildAniData_R)];

            try {
              yield Promise.all(promises); //-2 wild2_appear wild2_appear
        ...
        forcePlayConnectAniForRPSStart() {
          ...
          //--強制播放連線動畫(拳頭)
          return new Promise( /*#__PURE__*/_asyncToGenerator(function* (resolve, reject) {
            if (_this6._wild_left && _this6._wild_right) {
              _this6._singleSlot_L.node.active = false;
              _this6._singleSlot_R.node.active = false;

              try {
                yield _this6.showAppearWithoutCampDecided(); //--0708只留這個
        ...
        changeWildOutFrame(guessRoundData) {
          ...
          return new Promise( /*#__PURE__*/_asyncToGenerator(function* (resolve, reject) {
            ...
            if (_this7._changeIndex.next != (_crd && RPSWild_AniState === void 0 ? (_reportPossibleCrUseOfRPSWild_AniState({
              error: Error()
            }), RPSWild_AniState) : RPSWild_AniState).NUN) {
              //--第一次放大框的猜拳要播的音效
              if (_this7._changeIndex.next == (_crd && RPSWild_AniState === void 0 ? (_reportPossibleCrUseOfRPSWild_AniState({
                error: Error()
              }), RPSWild_AniState) : RPSWild_AniState).NEXT && _this7._changeIndex.state == (_crd && RPSWildState === void 0 ? (_reportPossibleCrUseOfRPSWildState({
                error: Error()
              }), RPSWildState) : RPSWildState).WILD_0) {
              ...
              promises = [_this7._wild_left.changeWildOutFrame(_this7._changeIndex.state, _this7._changeIndex.next), _this7._wild_right.changeWildOutFrame(_this7._changeIndex.state, _this7._changeIndex.next)];

              try {
                _this7._singleSlot_L.node.active = false;
                _this7._singleSlot_R.node.active = false;
                yield Promise.all(promises);
                _this7._changeIndex = {
                  next: (_crd && RPSWild_AniState === void 0 ? (_reportPossibleCrUseOfRPSWild_AniState({
                    error: Error()
                  }), RPSWild_AniState) : RPSWild_AniState).NUN,
                  state: _this7._wildState
                };
                resolve();
              } catch (e) {
                _this7._changeIndex = {
        ...
        checkRoundAndStartRollWild() {
          var _this8 = this;

          return _asyncToGenerator(function* () {
            return new Promise( /*#__PURE__*/_asyncToGenerator(function* (resolve, reject) {
              if (_this8._wildState == (_crd && RPSWildState === void 0 ? (_reportPossibleCrUseOfRPSWildState({
                error: Error()
              }), RPSWildState) : RPSWildState).WILD_0) {
                //this._wildBattle.setRoundData(this._guess_Round);
                var roundData = {
                  round: _this8._wildState,
                  targetTokenIds: null
                };
                yield _this8.rollWild(roundData);
                resolve();
              } else {
                yield _this8.rollWild();
                resolve();
              }
            }));
          })();
        ...
        rollWild(guessRoundData) {
          ...
          var playWildAniData_Right; //--滾動結束後battle的播放icon資料
          ...
          return new Promise( /*#__PURE__*/_asyncToGenerator(function* (resolve, reject) {
            _this9._resolvePromiseRollEnd = resolve;
            var promises = [_this9._singleSlot_L.runPromiseRolling(slotInfo_L.e, slotInfo_L.s), _this9._singleSlot_R.runPromiseRolling(slotInfo_R.e, slotInfo_R.s)];
            _this9._singleSlot_L.node.active = true;
            _this9._singleSlot_R.node.active = true;
            (_crd && AudioManager === void 0 ? (_reportPossibleCrUseOfAudioManager({
              error: Error()
            }), AudioManager) : AudioManager).instance.playSound((_crd && SoundList === void 0 ? (_reportPossibleCrUseOfSoundList({
              error: Error()
            ...
            try {
              yield Promise.all(promises); //--沒有強制停輪和resolve還活著的情況下才會執行

              if (!_this9._isForceStopped && _this9._resolvePromiseRollEnd) {
                //--接上appear+connect
                //--第一輪的處理要額外做(此時_isCampDecided=false,尚未決定陣營)
                //-showAppearWithoutCampDecided
                if (!_this9._isCampDecided) {
                  yield _this9.forcePlayConnectAniForRPSStart();
                }

                _this9._resolvePromiseRollEnd();

                _this9._resolvePromiseRollEnd = undefined;
              }
            } catch (e) {
              reject(e);
            }
        ...
        guessRPS(battleData) {
          ...
          return _asyncToGenerator(function* () {
            ...
            this._singleSlot_R.node.active = true;
            */
            var resultRound = _this10.getResultForRound();

            var firstCampForLight = _this10.getThisRoundCampForLight(resultRound);

            if (_this10._thisRoundCampForLight == -1) {
              _this10._thisRoundCampForLight = firstCampForLight;
              _this10._resultTitle.thisRoundCampForLight = firstCampForLight; //--把開啟陣營的資料寫進去
            }

            yield _this10._resultTitle.showResultTitle(resultRound); //--每次都來猜

            if (_this10._thisRoundCampForLight != -1) {
              _this10.checkOpenCollectionLightSystem(firstCampForLight);
            } //--滿足累加條件後才開始計算蒐集狀態


            if (_this10._canNextWildRound) {
              //--接勝敗結果+亮燈顯示
              var levelValue = _this10.getThisRoundLevelValue(resultRound);

              _this10.writeEveryRoundForLevel(levelValue);

              yield _this10._collectionLightSystem.setLevel(levelValue);
            }

            if (_this10._resolvePromiseRollEnd) {
              _this10._resolvePromiseRollEnd();

              _this10._resolvePromiseRollEnd = undefined;
            }
          })();
        }
        /**
        ...
        writeEveryRoundForLevel(levelValue) {
          ...
          var changeIndex = (_crd && RPSWild_AniState === void 0 ? (_reportPossibleCrUseOfRPSWild_AniState({
            error: Error()
          }), RPSWild_AniState) : RPSWild_AniState).NUN;
        ...
        getRandomUniqueItem(array, previousItem) {
...
//# sourceMappingURL=c1e72ed95a4a9c77b2d376f7e1ad1dc3a5d130f2.js.map