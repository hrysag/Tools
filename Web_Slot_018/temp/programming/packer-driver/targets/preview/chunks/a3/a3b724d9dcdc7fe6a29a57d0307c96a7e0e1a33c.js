    ...
    execute: function () {
      ...
      _export("BtnController", BtnController = (_dec = ccclass('BtnController'), _dec2 = property({
        ...
        constructor() {
          ...
          this.sensorMouseHandler = e => {
            ...
            if (e.type == Node.EventType.TOUCH_START) {
              ...
              var mousePos = e.getUILocation();
              var wpos = v3(mousePos.x, mousePos.y, 0);

              var localPos = this._mouseSensorNode.getComponent(UITransform).convertToNodeSpaceAR(wpos);

              var localPosV2 = new Vec2(localPos.x, localPos.y);

              var boundingBox = this._mouseSensorNode.getComponent(UITransform).getBoundingBox();

              var isCurrentlyInside = boundingBox.contains(localPosV2);
        ...
        setPlayerBetValue(value) {
          var totalMoney = (value * BUY_FG_MULTIPLIER).fixed();
          this._labelForBtnBetValue.string = totalMoney.numberComma();
          var len = this._labelForBtnBetValue.string.length; //let fontNodeSize = 1;

          var fontSize = 45.4; //default
...
//# sourceMappingURL=a3b724d9dcdc7fe6a29a57d0307c96a7e0e1a33c.js.map