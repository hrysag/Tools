    ...
    execute: function () {
      ...
      _export("ShowAniController", ShowAniController = (_dec = ccclass('ShowAniController'), _dec2 = property({
        ...
        constructor() {
          ...
          */
          this.checkSpRuleForExist = function () {
            ...
            var iconIndex = data.SymbolIconInfoData.iconIndex;
            var iconID = data.SymbolIconInfoData.iconID;
            var reelIndex = data.SymbolIconInfoData.reelIndex;
            var returnData = {
              flag: false,
              tokenId: ''
            };

            for (var _aniNode of _this._aryRunningNode) {
              //--檢查每一軸上的相同位置的icon是否相同
              if (_aniNode[(_crd && DYN_NODE_PROPERTIES === void 0 ? (_reportPossibleCrUseOfDYN_NODE_PROPERTIES({
                error: Error()
              }), DYN_NODE_PROPERTIES) : DYN_NODE_PROPERTIES).SYMBOL_ICON_INFO].reelIndex === reelIndex) {
                if (_aniNode[(_crd && DYN_NODE_PROPERTIES === void 0 ? (_reportPossibleCrUseOfDYN_NODE_PROPERTIES({
                  error: Error()
                }), DYN_NODE_PROPERTIES) : DYN_NODE_PROPERTIES).SYMBOL_ICON_INFO].iconIndex === iconIndex && _aniNode[(_crd && DYN_NODE_PROPERTIES === void 0 ? (_reportPossibleCrUseOfDYN_NODE_PROPERTIES({
                  error: Error()
                }), DYN_NODE_PROPERTIES) : DYN_NODE_PROPERTIES).SYMBOL_ICON_INFO].iconID === iconID) {
                  returnData = {
                    flag: true,
                    tokenId: _aniNode[(_crd && DYN_NODE_PROPERTIES === void 0 ? (_reportPossibleCrUseOfDYN_NODE_PROPERTIES({
                      error: Error()
        ...
        addSPNodeInRunning(prefabKey, aniData, groupId) {
          ...
          var token = Date.now();
          var node = this.getPrefabNode(prefabKey);
        ...
        addSPNodeInRunningForAwait(prefabKey, aniData, groupId) {
          var _this2 = this;
          ...
          return _asyncToGenerator(function* () {
            ...
            var aniShowNode = (_crd && FindNode === void 0 ? (_reportPossibleCrUseOfFindNode({
              error: Error()
            }), FindNode) : FindNode).findChildByNameRecursive(_this2.node, 'SymbolAniDisplayNode');
            var node = yield _this2.createSpineNodeUI(prefabKey, aniShowNode);
            ...
            if (localizationSpine) {
              yield localizationSpine.loadAllSpine(currentLanguageKey);
            }
            ...
            _this2.aryRunningNode.push(node); //--強塞進去
        ...
        playNoWinInThisRound(lines) {
          ...
          return _asyncToGenerator(function* () {
            return new Promise( /*#__PURE__*/_asyncToGenerator(function* (resolve, reject) {
              _this3._resolvePromise = resolve; //const groupID = [0];
              ...
              if (_this3._wildRPSSystem.isWorking || _this3._fgBonusSystem.isWorking) {
                _this3.stopAndRemoveAniWithoutWild();

                if (_this3._wildRPSSystem.wildState != (_crd && RPSWildState === void 0 ? (_reportPossibleCrUseOfRPSWildState({
                  error: Error()
                }), RPSWildState) : RPSWildState).WILD_3) {
                  //this.playOtherWinShowAni();
                  yield _this3.playWildAni();

                  if (_this3._wildRPSSystem.isWorking) {
                    if (_this3._wildRPSSystem.isLastWildRound) {
                      _this3._wildRPSSystem.hideCollectionLights(); //--關閉燈號

                    }
                  }

                  _this3.safeResolve();
                } else if (_this3._fgBonusSystem.isWorking) {
                  yield _this3.playBonusAni();

                  _this3.safeResolve();
                }
              } else {
                //--20250623-急停的話reelView他就不會驅動startShowReadyHand(slotMachineController.startShowReadyHand,就不會啟動相關流程與反黑)
                _this3._closeOrOpenAllGameIconBright(false); //--不反黑(處理有聽牌的情況+不急停)
        ...
        playWinInThisRound(winScoreData, lines) {
          ...
          return _asyncToGenerator(function* () {
            _this4._scoreData = winScoreData;
            return new Promise( /*#__PURE__*/_asyncToGenerator(function* (resolve, reject) {
              _this4._resolvePromise = resolve;
              if (lines) _this4.winLinesGroupData = lines;
              var totalScore = (winScoreData.totalOdd * winScoreData.betValue).fixed();

              if (_this4._fgBonusSystem.isWorking) {
                _this4.playFGProcessAni(totalScore);
              } else {
                ...
                var groupID = _this4.generateArray(_this4._winLinesGroupData.length); //---以下為測試資料
                //this.checkAndSetWildDataInScoreData();//--有wild的情況下會被推進去
                //---以上為測試資料
                //this._scoreData.totalOdd = 230;//--test


                _this4.processWildWithOutSameRange();

                if (_this4._scoreData.totalOdd >= SPECIAL_WIN_THRESHOLD) {
                  yield _this4.playAniGroupsWithPromise(groupID);
                  yield _this4.showBigWinAni(_this4._scoreData, totalScore);
                } else {
                  _this4.playAniGroupsWithPromise(groupID);

                  yield _this4.showWinScoreAni(totalScore);
                }

                _this4.showScoreForBottomText(totalScore); //await GameUtils.Defer(650);
                //await GameUtils.DeferByTweenPromise(650 / 1000);//--原本單位是毫秒現在換算成秒


                var delay = (_crd && GameUtils === void 0 ? (_reportPossibleCrUseOfGameUtils({
                  error: Error()
                }), GameUtils) : GameUtils).DeferByTweenPromiseWithCancel(650 / 1000);
                _this4._delayTweenCancel = delay.cancel;
                yield delay.promise; // 等待延遲完成

                _this4._delayTweenCancel = null; // 清掉

                _this4.stopAndHideConnectBoxAni();

                _this4._closeOrOpenAllGameIconBright(false); //--不反黑


                _this4.changeGroupAniInSameState(groupID, 'idle'); //--將group的動畫狀態改成idle
                //--將其他的動畫系統開啟播放(非icon表演的動畫)


                if (_this4._wildRPSSystem.isWorking) {
                  //--刪掉其它的中線動畫
                  _this4._closeOrOpenAllGameIconBright(true); //--反黑


                  _this4._winScore.stopWinScoreAni();

                  if (_this4._wildRPSSystem.isLastWildRound) {
                    _this4.stopAndPauseAniWithoutWild();

                    yield _this4.playWildAni(); //--進入輪播 +關閉燈號+背景壓暗取消20250731

                    _this4._closeOrOpenAllGameIconBright(false); //--反黑


                    _this4._wildRPSSystem.hideCollectionLights(); //--關閉燈號


                    _this4.postWinCleanupAndPlaySequence({
                      skipPushWild: true,
                    ...
                    _this4.stopAndRemoveAniWithoutWild();

                    yield _this4.playWildAni();
        ...
        sortAnimationLayer() {
          var level1Nodes = []; // iconID = 2, 3, 4, 5 (最下層)

          var level2Nodes = []; // iconID = 0, 1 (在 2-5 之上)

          var level3Nodes = []; // iconID = 6, 7, 8 (_wildID，在 0-1 之上)

          var level4Nodes = []; // iconID = 9 (_bonusID，在最上層)

          for (var target of this._aryRunningNode) {
            var iconID = target[(_crd && DYN_NODE_PROPERTIES === void 0 ? (_reportPossibleCrUseOfDYN_NODE_PROPERTIES({
              error: Error()
          ...
          var currentIndex = 0; // 設定 level 1 (最下層)
        ...
        checkAndSetWildDataInScoreData() {
          var additionalWildData = [];
          var originalWinLines = JSON.parse(JSON.stringify(this._winLinesGroupData));

          for (var i = 0; i < this._ary2dCards.length; i++) {
            for (var j = 0; j < this._ary2dCards[i].length; j++) {
              var iconID = this._ary2dCards[i][j];
              if (!WILD_LIST.includes(iconID)) continue;

              for (var k = 0; k < originalWinLines.length; k++) {
                for (var l = 0; l < originalWinLines[k].length; l++) {
                  var winItem = originalWinLines[k][l]; // 對第2列(i==1) 做補 wild
          ...
          for (var wildItem of additionalWildData) {
            var group = this._winLinesGroupData[wildItem.groupID];
        ...
        isWildOutsideWinLineRange() {
          var wildPositions = this.getWildPositions(); // 僅當 Wild 數量為 1 時才處理，其餘直接 return false

          if (wildPositions.length !== 1) return false;
          var wildPos = wildPositions[0];
          var isWildInFront = wildPos.reelIndex >= 0 && wildPos.reelIndex <= 2;
          var isWildInBack = wildPos.reelIndex >= 3 && wildPos.reelIndex <= 5;
          var originalWinLines = JSON.parse(JSON.stringify(this._winLinesGroupData));

          for (var group of originalWinLines) {
            for (var winItem of group) {
              var _reelIndex = winItem.reelIndex;
              var isWinInFront = _reelIndex >= 0 && _reelIndex <= 2;
              var isWinInBack = _reelIndex >= 3 && _reelIndex <= 5;
        ...
        showAndWaitForVerticalAni(totalScore) {
          ...
          return _asyncToGenerator(function* () {
            if (_this5._currentCampFg == 0) {
              var _this5$_fgAliShowVert;

              yield Promise.all([(_this5$_fgAliShowVert = _this5._fgAliShowVerticalAniNode) == null ? void 0 : _this5$_fgAliShowVert.playWinAni(), _this5.showWinScoreAni(totalScore)]);
            } else if (_this5._currentCampFg == 1) {
              var _this5$_fgThievesShow;

              yield Promise.all([(_this5$_fgThievesShow = _this5._fgThievesShowVerticalAniNode) == null ? void 0 : _this5$_fgThievesShow.playWinAni(), _this5.showWinScoreAni(totalScore)]);
            }
          })();
        }
        ...
        playFGProcessAni(totalScore) {
          var _this6 = this;

          return _asyncToGenerator(function* () {
            var groupID = _this6.generateArray(_this6._winLinesGroupData.length);

            _this6.changeGroupAniInSameState(groupID, 'idle'); //--將group的動畫狀態改成idle(20250730)


            yield _this6.playBonusAni();
            (_crd && AudioManager === void 0 ? (_reportPossibleCrUseOfAudioManager({
              error: Error()
            }), AudioManager) : AudioManager).instance.playSound((_crd && SoundList === void 0 ? (_reportPossibleCrUseOfSoundList({
              error: Error()
            ...
            if (_this6._scoreData.totalOdd >= SPECIAL_WIN_THRESHOLD) {
              yield _this6.playAniGroupsWithPromise(groupID);
              yield _this6.showBigWinAni(_this6._scoreData, totalScore);
            } else {
              _this6.playAniGroupsWithPromise(groupID);

              yield _this6.showAndWaitForVerticalAni(totalScore);
            }

            _this6.showScoreForBottomText(totalScore);
        ...
        showBigWinAni(winScoreData, totalScore) {
          ...
          return new Promise( /*#__PURE__*/_asyncToGenerator(function* (resolve, reject) {
            //--jp
            yield _this7._JpShowController.showJPWin(winScoreData.totalOdd, winScoreData.betValue);
            resolve();
          }));
        }
        /**
        ...
        showWinScoreAni(totalScore) {
          var _this8 = this;

          return _asyncToGenerator(function* () {
            yield _this8._winScore.showFinalScoreInAndOut(totalScore);
          })();
        ...
        playOtherWinShowAni() {
          ...
          return _asyncToGenerator(function* () {
            //--用來播放icon 9的動畫表演
            if (_this9._wildRPSSystem.isWorking) {
              yield _this9.playWildAni();
            } else if (_this9._fgBonusSystem.isWorking) {
              yield _this9.playBonusAni();
            }
          })();
        } //--應付一軸出現多個bonus的情況
        ...
        setPlayGroupForReelIndex(group) {
          var reelGroup = new Map();

          for (var node of group) {
            var _reelIndex2 = node[(_crd && DYN_NODE_PROPERTIES === void 0 ? (_reportPossibleCrUseOfDYN_NODE_PROPERTIES({
              error: Error()
            }), DYN_NODE_PROPERTIES) : DYN_NODE_PROPERTIES).SYMBOL_ICON_INFO].reelIndex;

            if (!reelGroup.has(_reelIndex2)) {
              reelGroup.set(_reelIndex2, []);
            }

            reelGroup.get(_reelIndex2).push(node);
          }
        ...
        playBonusAni() {
          ...
          return _asyncToGenerator(function* () {
            var groups = _this10.getAniNodesByGroupId(98);

            var reelGroupMap = _this10.setPlayGroupForReelIndex(groups);

            var sortedReelIndices = Array.from(reelGroupMap.keys()).sort((a, b) => a - b);
            var allGroupPromises = [];

            for (var index = 0; index < sortedReelIndices.length; index++) {
              var _reelIndex3 = sortedReelIndices[index];
              var nodeArray = reelGroupMap.get(_reelIndex3);

              var _loop = function* _loop() {
                var node = nodeArray[multiIndex];
                var ani = (_crd && AniSysTools === void 0 ? (_reportPossibleCrUseOfAniSysTools({
                  error: Error()
                }), AniSysTools) : AniSysTools).findAndGetIAniComponent(node);
                node.active = true; // 正確延遲時間 (秒 -> 毫秒)

                var delaySec = index * 0.2 + multiIndex * 0.8; //--PS 1.5為動畫spine+1字樣出現時間點
                //const promise = GameUtils.Defer(delaySec * 1000).then(() => this.playSingleBonusAni(node, ani));

                var promise = (_crd && GameUtils === void 0 ? (_reportPossibleCrUseOfGameUtils({
                  error: Error()
                }), GameUtils) : GameUtils).DeferByTweenPromise(delaySec).then(() => _this10.playSingleBonusAni(node, ani));
                allGroupPromises.push(promise);
              };

              for (var multiIndex = 0; multiIndex < nodeArray.length; multiIndex++) {
                yield* _loop();
              }
            }

            yield Promise.all(allGroupPromises); // 等全部完成
          })();
        }
        ...
        playSingleBonusAni(node, ani) {
          ...
          return new Promise(resolve => {
            ...
            var frameCallBack = function frameCallBack() {
              ...
              var symbolData = node[(_crd && DYN_NODE_PROPERTIES === void 0 ? (_reportPossibleCrUseOfDYN_NODE_PROPERTIES({
                error: Error()
              }), DYN_NODE_PROPERTIES) : DYN_NODE_PROPERTIES).SYMBOL_ICON_INFO];

              _this11._fgBonusSystem.playSingleBonusEffect(symbolData.reelIndex, symbolData.iconIndex).then(() => {
                resolveFrame();
              });
            };

            var animationCallBack = /*#__PURE__*/function () {
              ...
              return function animationCallBack() {
                return _ref4.apply(this, arguments);
              };
            }();

            var frameEventPromise = new Promise(res => {
              resolveFrame = res;
              node.setSiblingIndex(node.parent.children.length);
              ani.spineSequencePlayFrameEventCallBack = frameCallBack;
            });
            var animationCompletePromise = new Promise(res => {
              resolveAnim = res;
        ...
        playWildAni() {
          ...
          return _asyncToGenerator(function* () {
            if (!_this12._wildRPSSystem.isCampDecided) {
              _this12._wildRPSSystem.setOpenWildForBegin();

              yield _this12._wildRPSSystem.changeWildOutFrame(); //0 to 1

              yield _this12._wildRPSSystem.checkRoundAndStartRollWild();
            }

            var strBattle = _this12._wildRPSSystem.getWildIconAniType();

            _this12._slotControllerWildDarkness == null || _this12._slotControllerWildDarkness(); //--猜拳的底要比中線時的壓黑更黑

            yield _this12._wildRPSSystem.guessRPS(strBattle); //--更新level和wild數據+猜拳飛行的動畫
            //this.safeResolve();
          })();
        }
        ...
        changeWildFrame() {
          ...
          return _asyncToGenerator(function* () {
            return new Promise( /*#__PURE__*/_asyncToGenerator(function* (resolve, reject) {
              ...
              _this13._wildRPSSystem.checkWildStateToNextRound(); //await this._wildRPSSystem.changeWildOutFrame({ round: this._wildRPSSystem.wildState, targetTokenIds: null });
              //--每局結束後


              yield _this13._wildRPSSystem.changeWildOutFrame(); //--wildState
        ...
        playSequenceAniByGroupWithPromise(groupId, sequenceId) {
          var _this14 = this;
          ...
          return _asyncToGenerator(function* () {
            return new Promise( /*#__PURE__*/_asyncToGenerator(function* (resolve, reject) {
              ...
              for (var node of _this14._aryRunningNode) {
        ...
        postWinCleanupAndPlaySequence(options) {
          var _this15 = this;

          return _asyncToGenerator(function* () {
            if (_this15._hasPlayedPostWinSequence) return;
            _this15._hasPlayedPostWinSequence = true;

            if (!(options != null && options.skipPushWild)) {
              _this15.checkAndSetWildDataInScoreData(); //--有wild的情況下會被推進去

            } //--接上停止wild的動畫狀態


            if (!(options != null && options.skipCloseWildNode)) {
              _this15._wildRPSSystem.closeWildAniNodeWithoutDoubleWild();
            }

            if (!(options != null && options.skipPlayOtherWin)) {
              _this15.playOtherWinShowAni(); //--202507315這段方法在這裡是沒有意義的
            ...
            if (!(_crd && GenericUIManager === void 0 ? (_reportPossibleCrUseOfGenericUIManager({
              ...
              _this15._delayTweenCancel = delay.cancel;
              yield delay.promise; // 等待延遲完成

              _this15._delayTweenCancel = null; // 清掉

              _this15._closeOrOpenAllGameIconBright(true); //--反黑


              _this15.playAniGroupInSequence();
            }
          })();
        }

        playAniGroupInSequence() {
          var _this16 = this;
        ...
        playMultipleSequence() {
          ...
          return _asyncToGenerator(function* () {
            var playIndex = 0;
            _this17._abortPlaySequence = false; //--這個改法很不好..但殺傷力已經是最小的改法了0731

            var hotfix = false;

            if (_this17._wildRPSSystem.isWorking) {
              hotfix = _this17._wildRPSSystem.isLastWildRound;
            } //--這個改法很不好..但殺傷力已經是最小的改法了0731


            while (_this17._winLinesGroupData.length > 0) {
              var lineScore = (_this17._winLinesGroupData[playIndex][0].odd * _this17._scoreData.betValue).fixed(); //--這邊要改成顯示的connectBox用loop


              try {
                if (hotfix) {
                  _this17.removeAndCloseNodeSequenceBackToGameIconWithoutWild();

                  _this17.openNodeByIconsWithReelIndexInArrayWithoutWild(_this17._winLinesGroupData[playIndex]);

                  yield Promise.all([//--排除wild本身不要播放
                  _this17.withTimeout(_this17.playAnisByGroupWithExclusion(playIndex, [99]), 6, playIndex, 'playAniWithOutWild'), _this17.withTimeout(_this17.showWinScoreAni(lineScore), 6, '', 'score')]);
                } else {
                  _this17.removeAndCloseNodeSequenceBackToGameIcon();

                  _this17.openNodeByIconsWithReelIndexInArray(_this17._winLinesGroupData[playIndex]);

                  yield Promise.all([_this17.withTimeout(_this17.playAnisByGroupWithPromise(playIndex), 6, playIndex, 'playAni'), _this17.withTimeout(_this17.showWinScoreAni(lineScore), 6, '', 'score')]);
                }
              } catch (e) {
                //--動畫被中斷掉的情況promise會被reject掉,要回過頭來處理還在播放的動畫,要強制停止
                console.log('<cleanAll_playMultipleSequence_ERROR', e.label, _this17._abortPlaySequence);
                console.warn("[playMultipleSequence] \u8F2A\u64AD\u767C\u751F\u932F\u8AA4\uFF0C\u8DF3\u904E\u6B64\u689D\u64AD\u653E:", e); //playIndex++;
                //continue; 
              }

              if (_this17._abortPlaySequence) break; //--外部中斷時,直接切斷流程

              _this17.stopAndHideConnectBoxAni();

              playIndex++;

              if (playIndex >= _this17._winLinesGroupData.length) {
              ...
              _this17._abortPlaySequence = false;
            }
          })();
        }
        /**
        ...
        withTimeout(promise, seconds, meta, label) {
          if (label === void 0) {
            label = 'timeout';
          }

          return new Promise((resolve, reject) => {
            var finished = false; //--動畫撥放逾時了

            var onTimeout = () => {
              if (finished) return;
              finished = true;
              var error = new Error("[" + label + "] Timeout after " + seconds + "s");
              error.meta = meta;
              error.label = label;
              reject(error);
            };
        ...
        singlePlayWinScoreInCycle(singleScoreCycle) {
          var _this18 = this;

          return _asyncToGenerator(function* () {
            while (_this18._singleScoreCycle) {
              yield _this18._winScore.showFinalScoreIn(singleScoreCycle); //--單獨顯示每條百搭成線的得分
        ...
        compareTargetName(list) {
          var aniDataTarget; //--resetData<AnimationPlayStateList會保留>

          for (var aniData of list.clipsInfo) {
        ...
        setSpineAniNodeLoopByGroup(groupId) {
          ...
          for (var node of nodes) {
            var iAniData = new (_crd && IAniWithAniCtrl === void 0 ? (_reportPossibleCrUseOfIAniWithAniCtrl({
              error: Error()
            ...
            if (node[(_crd && DYN_NODE_PROPERTIES === void 0 ? (_reportPossibleCrUseOfDYN_NODE_PROPERTIES({
              ...
              var cloneAniCtrlPropDef = (_crd && GameUtils === void 0 ? (_reportPossibleCrUseOfGameUtils({
                error: Error()
        ...
        checkIsLowOddsSymbol(reelIndex, iconIndex) {
          for (var node of this._aryRunningNode) {
            var rIndex = node[(_crd && DYN_NODE_PROPERTIES === void 0 ? (_reportPossibleCrUseOfDYN_NODE_PROPERTIES({
              error: Error()
            }), DYN_NODE_PROPERTIES) : DYN_NODE_PROPERTIES).SYMBOL_ICON_INFO].reelIndex;
            var iIndex = node[(_crd && DYN_NODE_PROPERTIES === void 0 ? (_reportPossibleCrUseOfDYN_NODE_PROPERTIES({
              error: Error()
            }), DYN_NODE_PROPERTIES) : DYN_NODE_PROPERTIES).SYMBOL_ICON_INFO].iconIndex;

            if (rIndex == reelIndex && iIndex == iconIndex) {
              var iconId = this._ary2dCards[rIndex][iIndex];
        ...
        checkWild(reelIndex, iconIndex) {
          var iconId = this._ary2dCards[reelIndex][iconIndex];
        ...
        checkWildInWinLinesGroupData() {
          for (var i = 0; i < this._winLinesGroupData.length; i++) {
            for (var j = 0; j < this._winLinesGroupData[i].length; j++) {
        ...
        processWildWithOutSameRange() {
          var isWildOutsideRange = this.isWildOutsideWinLineRange(); //---wild只有一個且,在中線的範位在不同區間

          var wildInRunningNode = this.checkWildInRunningNode(); //--- 這是原本給要輪播的使用
        ...
        openNodeByIconsWithReelIndexInArrayWithoutWild(iconIndexs) {
          ...
          for (var i = 0; i < iconIndexs.length; i++) {
            //--低賠率
            var nodeAni = this.getAniNodeByReelIndexAndIconIndex(iconIndexs[i].reelIndex, iconIndexs[i].iconIndex);
            ...
            if (nodeAni) {
              ...
              if (nodeAni[(_crd && DYN_NODE_PROPERTIES === void 0 ? (_reportPossibleCrUseOfDYN_NODE_PROPERTIES({
                ...
                //--高賠率被拔去gameIcon裡面的
                var nodeAndWPos = this._getAndRemoveSymbolAniNodeWithWorldPos(iconIndexs[i].reelIndex, iconIndexs[i].iconIndex);

                var localNodeContainer = this._aniNodeStageContainerMap[CONTAINER_ANI_SYMBOL];
                ...
                if (nodeAni && localNodeContainer) {
                  ...
                  var spAniNode = (_crd && FindComponent === void 0 ? (_reportPossibleCrUseOfFindComponent({
                    error: Error()
          ...
          for (var i = 0; i < this._aryRunningNode.length; i++) {
            ...
            if (!node[(_crd && DYN_NODE_PROPERTIES === void 0 ? (_reportPossibleCrUseOfDYN_NODE_PROPERTIES({
              ...
              if (HIGH_ODDS_SYMBOL_LIST.includes(node[(_crd && DYN_NODE_PROPERTIES === void 0 ? (_reportPossibleCrUseOfDYN_NODE_PROPERTIES({
                ...
                var _node$parent, _this$_slotController, _this$_setSingleGameI2;
                ...
                (_this$_slotController = this._slotControllerReAddToGameIcon) == null || _this$_slotController.call(this, node[(_crd && DYN_NODE_PROPERTIES === void 0 ? (_reportPossibleCrUseOfDYN_NODE_PROPERTIES({
                  error: Error()
                }), DYN_NODE_PROPERTIES) : DYN_NODE_PROPERTIES).SYMBOL_ICON_INFO].reelIndex, node[(_crd && DYN_NODE_PROPERTIES === void 0 ? (_reportPossibleCrUseOfDYN_NODE_PROPERTIES({
                  error: Error()
            ...
            if (!node[(_crd && DYN_NODE_PROPERTIES === void 0 ? (_reportPossibleCrUseOfDYN_NODE_PROPERTIES({
              ...
              if (HIGH_ODDS_SYMBOL_LIST.includes(node[(_crd && DYN_NODE_PROPERTIES === void 0 ? (_reportPossibleCrUseOfDYN_NODE_PROPERTIES({
                ...
                if (WILD_LIST.includes(node[(_crd && DYN_NODE_PROPERTIES === void 0 ? (_reportPossibleCrUseOfDYN_NODE_PROPERTIES({
                  ...
                  var _node$parent2, _this$_slotController2, _this$_setSingleGameI3;
        ...
        stopAndPauseAniWithoutWild() {
          for (var i = 0; i < this._aryRunningNode.length; i++) {
            var node = this._aryRunningNode[i];
            var aniExtensionComponent = void 0; //--99=wild, 98=bonus
        ...
        stopAndRemoveAniWithoutWild(usePool) {
          if (usePool === void 0) {
            usePool = true;
          }
          ...
          for (var i = 0; i < this._aryRunningNode.length; i++) {
            ...
            if (!node[(_crd && DYN_NODE_PROPERTIES === void 0 ? (_reportPossibleCrUseOfDYN_NODE_PROPERTIES({
              ...
              if (HIGH_ODDS_SYMBOL_LIST.includes(node[(_crd && DYN_NODE_PROPERTIES === void 0 ? (_reportPossibleCrUseOfDYN_NODE_PROPERTIES({
                ...
                var _this$_slotController3, _this$_setSingleGameI4;
        ...
        stopAndRemoveAllAnisWithOutHighOdds() {
          for (var i = this._aryRunningNode.length - 1; i >= 0; i--) {
            ...
            var groupId = node[(_crd && DYN_NODE_PROPERTIES === void 0 ? (_reportPossibleCrUseOfDYN_NODE_PROPERTIES({
              error: Error()
            }), DYN_NODE_PROPERTIES) : DYN_NODE_PROPERTIES).GROUP_ID];
            var symbolInfo = node[(_crd && DYN_NODE_PROPERTIES === void 0 ? (_reportPossibleCrUseOfDYN_NODE_PROPERTIES({
              error: Error()
            }), DYN_NODE_PROPERTIES) : DYN_NODE_PROPERTIES).SYMBOL_ICON_INFO];
            var iconID = symbolInfo.iconID;
            var isBonusGroup = groupId.includes(98);
            var isHighOdds = HIGH_ODDS_SYMBOL_LIST.includes(iconID);
            var isWild = WILD_LIST.includes(iconID); // Wild but not bonus → 執行 wild reset 並回收
            ...
            if (!isBonusGroup && isHighOdds) {
              var _node$parent4, _this$_slotController4;

              (_node$parent4 = node.parent) == null || _node$parent4.removeChild(node);
              aniExtensionComponent == null || aniExtensionComponent.stopAni();
              (_this$_slotController4 = this._slotControllerReAddToGameIcon) == null || _this$_slotController4.call(this, symbolInfo.reelIndex, symbolInfo.iconIndex, node);
        ...
        recycleNode(node, aniComponent, prefabId) {
          ...
          var index = this._aryRunningNode.indexOf(node);
...
//# sourceMappingURL=7cd52df80aa83e01e157cf4bb2bff584f847d2a8.js.map