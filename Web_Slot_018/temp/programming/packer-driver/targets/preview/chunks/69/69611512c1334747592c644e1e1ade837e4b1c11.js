    ...
    execute: function () {
      ...
      _export("FG_BonusBarAniController", FG_BonusBarAniController = (_dec = ccclass('FG_BonusBarAniController'), _dec2 = property({
        ...
        onLoad() {
          ...
          this.init();
          var targetNode = this.node.getChildByName('Root'); //const children=targetNode.children;

          var len = targetNode.children.length;

          for (var i = 0; i < len; i++) {
            var child = targetNode.getChildByName('sp_' + i);
            var spine = child.getComponent(sp.Skeleton);
        ...
        setPositions() {
          this.scheduleOnce(() => {
            ...
            var targetNode = this.node.getChildByName('Root');
            var uiTransform = targetNode.getComponent(UITransform);

            this._mapSpineWorldPos.clear();

            for (var i = 0; i < targetPos.length; i++) {
              var targetSpine = this._mapSpine.get(i);

              if (targetSpine) {
                targetSpine.node.setPosition(targetPos[i]);
                var worldPos = uiTransform.convertToWorldSpaceAR(targetSpine.node.position);
        ...
        playTitleAni(level) {
          ...
          return _asyncToGenerator(function* () {
            var targetSpine = _this._mapSpine.get(level);

            if (targetSpine) {
              yield _this.playTargetAniInPromise(targetSpine, _this._mapAni.get((_crd && FG_BonusAniState === void 0 ? (_reportPossibleCrUseOfFG_BonusAniState({
                error: Error()
              }), FG_BonusAniState) : FG_BonusAniState).OFF_TO_ON));
              targetSpine.setAnimation(0, _this._mapAni.get((_crd && FG_BonusAniState === void 0 ? (_reportPossibleCrUseOfFG_BonusAniState({
                error: Error()
        ...
        playSingleItemAni(itemIndex) {
          var _this2 = this;

          return _asyncToGenerator(function* () {
            var targetSpine = _this2._mapSpine.get(itemIndex); //console.log('playSingleItemAni', itemIndex, targetSpine);


            if (targetSpine) {
              _this2.checkIndexForSound(itemIndex);

              yield _this2.playTargetAniInPromise(targetSpine, _this2._mapAni.get((_crd && FG_BonusAniState === void 0 ? (_reportPossibleCrUseOfFG_BonusAniState({
                error: Error()
              }), FG_BonusAniState) : FG_BonusAniState).OFF_TO_ON));
              targetSpine.setAnimation(0, _this2._mapAni.get((_crd && FG_BonusAniState === void 0 ? (_reportPossibleCrUseOfFG_BonusAniState({
                error: Error()
              }), FG_BonusAniState) : FG_BonusAniState).ON), false);
            }
          })();
        ...
        playRangesAni(startIndex, endIndex) {
          var _this3 = this;

          return _asyncToGenerator(function* () {
            var count = startIndex;
            var maxLen = endIndex - 1;

            while (count < maxLen) {
              var targetSpine = _this3._mapSpine.get(count);

              if (targetSpine) {
                yield _this3.playTargetAniInPromise(targetSpine, _this3._mapAni.get((_crd && FG_BonusAniState === void 0 ? (_reportPossibleCrUseOfFG_BonusAniState({
                  error: Error()
                }), FG_BonusAniState) : FG_BonusAniState).OFF_TO_ON));
                targetSpine.setAnimation(0, _this3._mapAni.get((_crd && FG_BonusAniState === void 0 ? (_reportPossibleCrUseOfFG_BonusAniState({
                  error: Error()
                }), FG_BonusAniState) : FG_BonusAniState).ON), false);
              }
        ...
        resetData() {
          for (var [_key, value] of this._mapSpine) {
            if (value) {
              var trackEntry = value.getCurrent(0);
        ...
        setSkinAndInitSpine(key) {
          var targetMap = this._mapSkin.get(key);

          var skinKey = '';
          var len = this._mapSpine.size;

          for (var i = 0; i < len; i++) {
            var _sp = this._mapSpine.get(i);
            ...
            if (_sp) {
              ...
              _sp.setSkin(key + skinKey);

              _sp.setAnimation(0, this.getAniKeyByState((_crd && FG_BonusAniState === void 0 ? (_reportPossibleCrUseOfFG_BonusAniState({
                error: Error()
...
//# sourceMappingURL=69611512c1334747592c644e1e1ade837e4b1c11.js.map