...
exports.setBundleData = exports.callBackResult = exports.sendResult = exports.methods = exports.unload = exports.load = void 0;
// @ts-ignore
 ...
 */
let getBundleArray = [];
let getScriptUuidArray = [];
let getSpriteFrameArray = [];
let getAudioClipArray = [];
let allUrl = [];
let saveUUIDArray = {};
...
let bundleTypeCount = [];
let bundleData = [];
let checkSelectedAssetsInterval = null;
const load = function () {
...
async function getSelectUUids(ab) {
...
async function analyzeFolder(url, path) {
...
async function analyzeJson(uuid) {
    const assetInfo = await Editor.Message.request('asset-db', 'query-asset-info', uuid);
    const jsonUrl = assetInfo === null || assetInfo === void 0 ? void 0 : assetInfo.library['.json'];
...
async function getAssetPath(uuid) {
...
// 依url向上搜尋使用的bundle
async function returnDirname(url) {
...
// 印出使用的bundleName
async function getUsingBundle(url, bundleName) {
...
exports.setBundleData = setBundleData;
