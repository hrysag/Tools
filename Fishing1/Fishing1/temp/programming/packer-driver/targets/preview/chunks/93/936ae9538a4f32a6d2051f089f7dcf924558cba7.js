    ...
    execute: function () {
      ...
      _export("FishPickUpCollisionBase", FishPickUpCollisionBase = class FishPickUpCollisionBase extends (_crd && CollisionBase === void 0 ? (_reportPossibleCrUseOfCollisionBase({
        ...
        checkCollision(t) {
          ...
          var pointTarget = v2(t.target.x, t.target.y); //--送出來的座標已經是canvas的world position

          log('check_checkCollision_value', t, pointTarget);
          var checkCollisionData;
          var ray = null;
          var f = false;
          ...
          for (var i = 0; i < this._aryCompairs.length; i++) {
            if (!this._aryCompairs[i].fishIsFlash && !this._aryCompairs[i].isDead && !this._aryCompairs[i].prohibit) {
              ...
              for (var k = 0; k < this._aryCompairs[i].collisionArea.length; k++) {
                if (this._aryCompairs[i].fishMeshState == (_crd && fishMeshState === void 0 ? (_reportPossibleCrUseOffishMeshState({
                  ...
                  if (this._aryCompairs[i].fishType != 21) {
                    ...
                    var fish3Dget2DColliderPoints = this._aryCompairs[i].collisionArea[k].worldPoints; //fish3Dget2DColliderPoints,this._aryFishes[j].collisionArea[k]);
                    ...
                    var clickScreenPoint = this._canvasCamera2dComponent.worldToScreen(v3(pointTarget.x, pointTarget.y));
                    ...
                    var shape = this._aryCompairs[i].collisionArea[k].shape;
                    var worldObb = shape.worldObb; //--射線拿來這邊做(做一次就好了)--20231029

                    if (!ray) {
                      var screenPos = this._canvasCamera2dComponent.worldToScreen(v3(pointTarget.x, pointTarget.y));
...
//# sourceMappingURL=936ae9538a4f32a6d2051f089f7dcf924558cba7.js.map