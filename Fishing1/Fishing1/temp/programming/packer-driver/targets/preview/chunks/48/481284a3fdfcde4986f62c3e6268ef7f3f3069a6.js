    ...
    execute: function () {
      ...
      _export("Fish1AniEffectSystemView", Fish1AniEffectSystemView = (_dec = _crd && viewBind === void 0 ? (_reportPossibleCrUseOfviewBind({
          ...
          this.modeleChangeHandler = (sub, value) => {
            ...
            switch (sub) {
              ...
              case '_addbullets':
                ...
                */
                var bulletData = this._viewModel['_addbullets'];
                /**
                 * 20240219
                ...
                var bossStatus = this._viewModel['_bossStatus'];
                log('check_addbullet_with_boss', bossStatus);

                if (bulletData.info.roomStatus == 2 && bossStatus == 'in') {
                  var powerValue = this._mapCannonInfo[bulletData.weaponType].powerup;
                  this.executeAnimation({
                    aniEffectTypeId: (_crd && Fish1AniEffectTypeMap === void 0 ? (_reportPossibleCrUseOfFish1AniEffectTypeMap({
                      error: Error()
        ...
        executeAnimation(executeOption) {
          var r = super.executeAnimation(executeOption);
          ...
          if (!r) {
            switch (executeOption.aniEffectTypeId) {
              ...
              case (_crd && AniEffectTypeMap === void 0 ? (_reportPossibleCrUseOfAniEffectTypeMap({
                ...
                var bg = this.getCommandInstance('BgAniEffect');
                var targetNode = bg.getBg();
              ...
              case (_crd && Fish1AniEffectTypeMap === void 0 ? (_reportPossibleCrUseOfFish1AniEffectTypeMap({
                ...
                if (executeOption.other.isPlayer) {
                  var random = (_crd && GameUtils === void 0 ? (_reportPossibleCrUseOfGameUtils({
                    error: Error()
              ...
              case (_crd && Fish1AniEffectTypeMap === void 0 ? (_reportPossibleCrUseOfFish1AniEffectTypeMap({
                ...
                var midOddsbg = this.getCommandInstance('BgAniEffect');
                var midOddsNode = midOddsbg.getBg();
        ...
        afterSpEffect(afterDataId) {
          var afterEffectData = this._afterAniDataMap[afterDataId]; //log('afterEffectData@@',afterEffectData);

          for (var i = 0; i < afterEffectData.infoData.length; i++) {
            (_crd && SoundsManager === void 0 ? (_reportPossibleCrUseOfSoundsManager({
              error: Error()
            }), SoundsManager) : SoundsManager).getInstance().play('sounds/moneydrop');
        ...
        hitFish(value) {
          ...
          if (value.fish.iskill == false) {//---沒死
            ...
            var isPlayer = false; //--0-3

            if (value.siteIndex == this._viewModel['_playerTableId']) {
              isPlayer = true;
            }

            var spMode = value.fish.bonus;

            var fd = this._gameMediator.getViewUserData((_crd && GameViewMediatorUser === void 0 ? (_reportPossibleCrUseOfGameViewMediatorUser({
              error: Error()
            }), GameViewMediatorUser) : GameViewMediatorUser).FishView, (_crd && GameViewMediatorUserDataKey === void 0 ? (_reportPossibleCrUseOfGameViewMediatorUserDataKey({
              error: Error()
            ...
            if (fd) {
              ...
              if (this._bossType.indexOf(fd.fishType) != -1) {
                //--準備取玩家的暱稱
                var logininName = (_crd && GameUtils === void 0 ? (_reportPossibleCrUseOfGameUtils({
                  error: Error()
                ...
                var chainKillfish = value.fish.killSpFish;
                var chainDatas = [];
                var totalPayforchain = 0; //--把目標魚(閃電/炸彈)塞到chainDatas裡面的第一個
                ...
                for (var i = 0; i < chainKillfish.length; i++) {
                  //--有資料
                  var chainFd = i == 0 ? fd : this._gameMediator.getViewUserData((_crd && GameViewMediatorUser === void 0 ? (_reportPossibleCrUseOfGameViewMediatorUser({
                    error: Error()
                  }), GameViewMediatorUser) : GameViewMediatorUser).FishView, (_crd && GameViewMediatorUserDataKey === void 0 ? (_reportPossibleCrUseOfGameViewMediatorUserDataKey({
                    error: Error()
                ...
                if (chainDatas.length > 1) {
                  ...
                  var commandId = fd.fishType == 22 ? (_crd && AniEffectTypeMap === void 0 ? (_reportPossibleCrUseOfAniEffectTypeMap({
                    error: Error()
        ...
        checkChainDatas(targetFid, chainDatas) {
          ...
          for (var i = 0; i < chainDatas.length; i++) {
        ...
        getplayerLoginName(index) {
          var tables = this._viewModel['_roomTableInfo'].tables;
          return tables[index].userLoginName;
        }

        getFish2Dand3DAnimationPositionToWorld(type, pos) {
          var rpos;
          var fishContainer = find('Canvas/fishNodeContainer/fishNode');
...
//# sourceMappingURL=481284a3fdfcde4986f62c3e6268ef7f3f3069a6.js.map