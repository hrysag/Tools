  ...
  return {
    ...
    execute: function () {
      ...
      _export("RoomBtn", RoomBtn = class RoomBtn extends Component {
        ...
        setRoomName(str) {
          var strIndex = str ? str : this._strRoomTitle;
          ...
          if (strIndex != '' && this._roomType >= 0) {
            ...
            if (spriteFrame) {
              ...
              var spr = this._roomTitleNode.getComponent(Sprite);
      ...
      _export("Fish1LobbyGuiView", Fish1LobbyGuiView = class Fish1LobbyGuiView extends (_crd && GuiBasic === void 0 ? (_reportPossibleCrUseOfGuiBasic({
        ...
        */
        ...
        setLayout() {
          ...
          return _asyncToGenerator(function* () {
            return new Promise(resolve => {
              _this._lobbyNode = instantiate((_crd && LoadingResManager === void 0 ? (_reportPossibleCrUseOfLoadingResManager({
                error: Error()
              }), LoadingResManager) : LoadingResManager).getInstance().getPrefab('prefab/gui/lobby')); //this.addChild(this._lobbyNode);

              var bg = _this._lobbyNode.getChildByName('bg').getComponent(Sprite);

              bg.addComponent(BlockInputEvents);
              log('initLobbyGui', _this._lobbyNode);
              _this._versionLabel = _this._lobbyNode.getChildByName('version').getComponent(Label);
              _this._accoundLabel = _this._lobbyNode.getChildByName('player').getChildByName('label').getComponent(Label); //--這邊長度可能是2或是以上

              var rooms = _this._lobbyNode.getChildByName('room').children; //let roomNames:string[]=['tx_shark','tx_dragon',''];


              for (var i = 0; i < rooms.length; i++) {
                var btn = rooms[i].addComponent(RoomBtn);
                log('check_rooms', rooms[i]); //btn.init(rooms[i]);

                btn.roomType = i;
                btn.strRoomTitle = _this._arylobbyNames[i];

                _this._aryBtn.push(btn);

                btn.node.on((_crd && GUIEvent === void 0 ? (_reportPossibleCrUseOfGUIEvent({
                  error: Error()
                }), GUIEvent) : GUIEvent).SET_PLAYER_ROOM, _this.btnHandler); //超詭異的感應區怪怪的
              ...
              _this.addChild(_this._lobbyNode);

              _this._container.addChild(_this); //---完成的時候做
        ...
        setRoomData(roomRate, uiseId) {
          ...
          var len = 3; //--ps因為server似乎沒有協調好,送進來['1:5', '1:1', '2:1', '4:1', 'NA']這種資料
...
//# sourceMappingURL=e6b53219ca073016603842b08efbbd8629ad6cc8.js.map