      ...
      _export("Fish1BulletView", Fish1BulletView = (_dec = _crd && viewBind === void 0 ? (_reportPossibleCrUseOfviewBind({
          ...
          */
          this.modeleChangeHandler = (sub, value) => {
            ...
            switch (sub) {
              ...
              case '_hitFishs':
                var targetBullet = this.getBulletByID(value[0].bsn);
        ...
        cleanTable() {
          ...
          for (var i = 0; i < this._aryBullets.length; i++) {
            ...
            if (b.state == 1) {
              if (b.isTweening) {
                var tweenComponent = b.bulletShell.getComponent(_crd && TweenMaxCocosPlugin === void 0 ? (_reportPossibleCrUseOfTweenMaxCocosPlugin({
                  error: Error()
              ...
              var effect2DFactory = (_crd && FishBulletEffectCenter === void 0 ? (_reportPossibleCrUseOfFishBulletEffectCenter({
                error: Error()
              }), FishBulletEffectCenter) : FishBulletEffectCenter).getInstance().getEffectProduceFactory(factorOption);
              var effectData = b.bulletEffect[0];

              this._layer2DBulletsContainer.removeChild(b.bulletShell);

              var recycleData = effectData.clean();
        ...
        addBullet(bullets) {
          ...
          */
          //--test--
          var len = bullets.length; //let aryBullets:Bullet[]=[];
          ...
          for (var i = 0; i < len; i++) {
            var aryAction = bullets[i].actionId.split("_"); //BulletImage_2_4_0(actionId的樣子,BulletImage會改成number)

            var id = aryAction[0]; //--子彈動作與外皮

            var effectId = void 0;
            var systemIndex = aryAction[aryAction.length - 1];
            var aryTargetBullet = void 0;
            ...
            var obj = this.getActionInfo(id, effectId, systemIndex);
            var amount = obj.amount;
            var systemId = obj.systemId; //let effectId:number=0; 
            //let sound:string=obj.sound; 

            var system2Dor3D = obj.system2Dor3D; //--這邊先用prefab的id

            var effect2DAssetsID = obj.effect2DAssetsID;
            var strFishNetId = obj.strFishNetId;
            var collisionW = obj.collisionW;
            var collisionH = obj.collisionH;
            var fishNetW = obj.fishNetW;
            var fishNetH = obj.fishNetH;
            var lifeTime = obj.lifeTime;
            var speed = obj.speed;
            var fps = obj.fps;
            var effectFishNetAtlasID = obj.effectFishNetAtlasID;
            log('check_addbullet_obj_bulletsys::' + '\n' + 'obj_', obj, '\n' + 'aryAction::', aryAction, '\n' + 'id::', id, '\n' + 'bullets::', bullets[i], '\n' + '_mapIfaction::', this._mapIfaction); //-(4) ['0', '1', '0', '0']0: "0"1: "1"2: "0"3: "0"length: 4[[Prototype]]: Array(0) 0

            if (obj) {
              var ifAction = void 0; //--舊版的捕魚達人1,鎖定是要使用道具,現在已經移到常規功能20230816
              ...
              var b = this._aryBulletsPool.length > 0 ? this._aryBulletsPool.pop() : new (_crd && Fish1BulletData === void 0 ? (_reportPossibleCrUseOfFish1BulletData({
                error: Error()
          ...
          for (var a in this._mapIfaction) {
            //--這裡只是一次一顆所以才不會爆開..
        ...
        openfishNet(bulletId) {
          var bullet = this.getBulletByID(bulletId);
          var r = bulletId;
          ...
          if (bullet) {
            ...
            r = -1;
            var ifAction = this._mapIfaction[bullet.strSystemId];
            ifAction.changeEffect(bullet); //let mc:Node=bullet.bulletShell;

            bullet.bulletShell.setScale(v3(0, 0, 0));
            var opacity = bullet.bulletShell.getComponent(UIOpacity);
            ...
            var componentTweenMax = bullet.bulletShell.addComponent(_crd && TweenMaxCocosPlugin === void 0 ? (_reportPossibleCrUseOfTweenMaxCocosPlugin({
              error: Error()
            }), TweenMaxCocosPlugin) : TweenMaxCocosPlugin);
            ...
            TweenMax.to(componentTweenMax, .5, {
              ...
              onComplete: value => {
                var b = this.getBulletByID(value.bullet.others); //---20171201 fix
        ...
        beforeaddBullets(bullets) {
          var _this = this;

          return _asyncToGenerator(function* () {
            log('beforeaddBullets__AAAAA', bullets, _this._playerTableIndex);
            /**
             * credit: 495
                info: {
                    actionId: "0_1_0_0"
            ...
            var flag = _this._playerTableIndex == bullets.siteIndex ? true : false; //-_playerTableIndex

            var shootFlag = true; //--可以刪了,沒有意義
            ...
            if (!flag) {
              ...
              if (bullets.info.isFree) {
                ...
                _this._gameMediator.getViewUserData((_crd && GameViewMediatorUser === void 0 ? (_reportPossibleCrUseOfGameViewMediatorUser({
                  error: Error()
                ...
                var gunScore = _this.getGunScore(bullets.info.actionId); //log("changeBulletStyle",gunScore);
            ...
            if (shootFlag) //--這個判斷不知道在聰沙小
              {
                var p = null;
                var addbulletFlag = true; //--寫子彈(server back)
                ...
                if (bullets.lockTarget) {
                  ...
                  if (bullets.lockTarget != -1 && bullets.lockTarget != 0) {
                    //--送出來的座標是世界座標
                    p = yield _this._gameMediator.getViewUserData((_crd && GameViewMediatorUser === void 0 ? (_reportPossibleCrUseOfGameViewMediatorUser({
                      error: Error()
                    ...
                    if (p != null) {
                      ...
                      if (p.previousTarget != 0) {
                        ...
                        _this.cleanPlaerPreviousLockTarget(p.previousTarget, bullets.siteIndex);
                    ...
                    //--因為點擊是拿localpos
                    //---實際
                    var clickNode = find('Canvas/mouseNode');
                    var wposClick = clickNode.getComponent(UITransform).convertToWorldSpaceAR(v3(bullets.info.endX, bullets.info.endY));
                    p = {
                      position: {
                        x: wposClick.x,
                        y: wposClick.y
                ...
                if (addbulletFlag) {
                   ...
                   */
                  //--選轉並且回傳旋轉後的發射座標
                  var cannonPosition = _this._gameMediator.getViewUserData((_crd && GameViewMediatorUser === void 0 ? (_reportPossibleCrUseOfGameViewMediatorUser({
                    error: Error()
                  }), GameViewMediatorUser) : GameViewMediatorUser).GuisSystemView, (_crd && GameViewMediatorUserDataKey === void 0 ? (_reportPossibleCrUseOfGameViewMediatorUserDataKey({
                    error: Error()
                  ...
                  */
                  ...
                  _this.setBulletData(bulletData, data);
                } else {
                  /**
                   *  20240320
                   *  因為根本不會推到bulletsPool裡面所以直接call server回收 
                   */
                  _this._viewModel.sendServer((_crd && ServerSendCode === void 0 ? (_reportPossibleCrUseOfServerSendCode({
                    error: Error()
...
//# sourceMappingURL=68baa12b31c795e469f67a56517a6a350ed9fddf.js.map