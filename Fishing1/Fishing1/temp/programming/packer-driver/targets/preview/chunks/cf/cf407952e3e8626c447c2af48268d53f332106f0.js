    ...
    execute: function () {
      ...
      _export("Cannon", Cannon = class Cannon extends Component {
        ...
        constructor() {
          ...
          var ary0 = {};
          var ary1 = {}; //-{[key:string]:AnimationAndEffect.AniEffectBase};
        ...
        setLayout(value) {
          var len = value.gunBarrel.length; //this._cannonNode=value.cannon;
          //--底座 

          var sprCannon; //--砲管

          var sprGun; //--裝上面兩個的node

          var cannonLv; //log('check_setCannonLayoutInfo',value);
          ...
          var shootCenterNode = new Node('shootCenterNode');
          var content = this.node.getComponent(UITransform);
        ...
        closeOrOpenGunVisible(b) {
          for (var i in this._mapCannonAndGun[this._nowSystem]) {
            /*
            {cannonNode:砲管炮座的容器node,cannon:砲管,gun:砲座}
            */
        ...
        changeGun(outIndex) {
          ...
          var r = -1; //log('check_data',this._mapCannonAndGun[this._nowSystem][outIndex]);
        ...
        shootMotion() {
          var nowIndex = this.checkNowTypeIndex();
          ...
          var childrens = this._mapCannonAndGun[this._nowSystem][nowIndex].gun.components;
          var target;

          for (var i of childrens) {
      ...
      _export("CannonMount", CannonMount = class CannonMount extends Component {
        ...
        setLayout(value) {
          //this._dataDataforBaseCannon=value;
          var len = value.mountData.length;
          var baseSpr;
          var containerNode; //--可以透過uuid去拿
        ...
        closeOrOpenGunVisible(b) {
          ...
          for (var i in this._mapBaseCannon[this._nowSystem]) {
        ...
        playMotion(keyframe, loop, timeScale, reverse) {
          ...
          if (nowIndex != null) {
            ...
            var target = this._mapBaseCannon[this._nowSystem][nowIndex].mount;
            r = target.name;
        ...
        setPlayerMount(value) {
          var target = this._mapBaseCannon[this._nowSystem][this._nowIndex].mount;
          var spFrame = value ? this._textures.player : this._textures.other;
      ...
      _export("WholeCannon", WholeCannon = keepClass(_class4 = class WholeCannon extends Component {
        ...
        constructor() {
          ...
          this.handelClick = e => //public handelClick(e?:EventTouch):void
          {
            ...
            if (changeGunType != -1) {
              var sendObj = changeGunType + '_' + this._outLayerScoreIndex + '_' + this._innerLayerScoreIndex + '_' + this._wholeCannon.nowSystem; //let evt:GUIEvent=new GUIEvent(GUIEvent.CHANG_BULLETS,sendObj);
          ...
          this.fastBtnSensorClickHandler = e => {
            ...
            var len = this._fastBtnMap.length;
            ...
            var ary = [this._btnMini, this._btnPlus]; //--要在加入按鈕在運動中的上鎖與結束的解鎖
            ...
            if (this._flagOpen) {
              ...
              for (var _j = 0; _j < len; _j++) {
                this._fastBtnMap[_j].node.setPosition(this._fastBtnMap[_j].openPosition.x, this._fastBtnMap[_j].openPosition.y);

                this._fastBtnMap[_j].x = this._fastBtnMap[_j].openPosition.x;
                this._fastBtnMap[_j].y = this._fastBtnMap[_j].openPosition.y; //--enabled 阻斷狀態

                this._fastBtnMap[_j].node.getComponent(Button).enabled = false;
                this._fastBtnMap[_j].node.getComponent(Button).interactable = false;
                TweenMax.to(this._fastBtnMap[_j].node.getComponent(_crd && TweenMaxCocosPlugin === void 0 ? (_reportPossibleCrUseOfTweenMaxCocosPlugin({
                  error: Error()
                }), TweenMaxCocosPlugin) : TweenMaxCocosPlugin), 0.1, {
                  x: this._fastBtnMap[_j].closePosition.x,
                  y: this._fastBtnMap[_j].closePosition.y //onCompleteParams:[this._fastBtnMap[j]],
          ...
          this.fastBtnClickHandler = e => {
            var scoreData = this.getScoreIndex(e.currentTarget.getComponent(FastBtn).id);
            ...
            var changeGunType = this._wholeCannon.changeGun(scoreData.outIndex); //this._textDigitsCannon.string=this._cannonScorePool[this._outLayerScoreIndex][this._innerLayerScoreIndex]+'';
          ...
          this.unLockBtnAfterShowOpen = () => {
            var anumationComponent = this.node.getComponent(Animation);
        ...
        setLayout(playerItem) {
          ...
          var sensorComponent = this._fastBtnSensor.addComponent(UITransform);
          ...
          */
          ...
          var wholeCannon = playerItem.getChildByName('cannon').getChildByName('cannonSeat'); //log('WholeCannon',wholeCannon);

          var UITransformfoWholeCannon = wholeCannon.addComponent(UITransform); //--座標轉換需要用的
          ...
          var wholeMount = playerItem.getChildByName('cannon').getChildByName('cannonDish');
          ...
          var cannonLabelNode = playerItem.getChildByName('cannon').getChildByName('cannonScore');
          var labelNode = cannonLabelNode.children[0]; //this._textDigitsCannon=labelNode.getComponent(Label);
          ...
          var playerbarNode = playerItem.getChildByName('playerBar'); //log('check_playerBar',playerbarNode); 
          ...
          var fastBtnTopNode = playerItem.getChildByName('cannonBtn').getChildByName('settingBtn');
          var len = fastBtnTopNode.children.length;
          var fastBtnNode;
          var fastBtnDigitsNode;
          var fastBitDigits;
          ...
          for (var i = 0; i < len; i++) {
            ...
            var btnComponent = fastBtnNode.addComponent(FastBtn);
        ...
        resetFastBtnData() {
          for (var i = 0; i < this._fastBtnMap.length; i++) {
        ...
        addBtnClickEvtHandler(button, handsId, sendObj) {
          ...
          var clickEvtHandler = new EventHandler(); //clickEvtHandler._componentId
        ...
        removeClickEvtHandler(button, handsId) {
          var len = button.clickEvents.length;
        ...
        getScoreIndex(value) {
          ...
          for (var i = 0; i < this._cannonScorePool.length; i++) {
            for (var j = 0; j < this._cannonScorePool[i].length; j++) {
        ...
        setuser(b) {
          ...
          if (b) {
            ...
            for (var _i = 0; _i < this._fastBtnMap.length; _i++) {
              //this.addBtnClickEvtHandler(this._fastBtnMap[i].node.getComponent(Button),'fastBtnClickHandler');
              this._fastBtnMap[_i].node.getComponent(Button).clickEvents.length = 0;
            }
        ...
        openCannon(isPlayer) {
          ...
          if (this._cannonBaseMount.isHidden) {
            ...
            var gunData = this.getScoreIndex(this._defaultGunScore); //---進場的動作
          ...
          */
          ...
          if (!this._firsOpenFlag) {
            ...
            if (anumationComponent) {
              var clips = anumationComponent.clips;
              var aniState; //--檢查所有動畫狀態的物件
        ...
        lockPropBtn(scoreVale) {
          ...
          for (var k = 0; k < this._fastBtnMap.length; k++) {
        ...
        */
        ...
        changeBulletStyle(score) {
          var gunData = this.getScoreIndex(score); //let gunStr:string=this._wholeCannon.changeGun(gunData.outIndex);

          var gunStr = this._wholeCannon.getGunType(gunData.outIndex);
        ...
        rotationCannon(x, y) {
          ...
          */
          ...
          var bulletLocalPoint = this.node.getComponent(UITransform).convertToNodeSpaceAR(v3(x, y));
          var radians = Math.atan2(bulletLocalPoint.y - this._wholeCannon.node.position.y, bulletLocalPoint.x - this._wholeCannon.node.position.x);
          ...
          var offestSinZero = Math.PI / 2 * 180 / Math.PI;
        ...
        getCannonPosition() {
          ...
          var global = this.node.getComponent(UITransform).convertToWorldSpaceAR(new Vec3(this._wholeCannon.node.position.x, this._wholeCannon.node.position.y, 0));

          var g = this._wholeCannon.getShootCenterPositiontoGlobal();

          var local = this.node.getComponent(UITransform).convertToNodeSpaceAR(new Vec3(g.x, g.y, 0));
          var shootCenterGlobal = this.node.getComponent(UITransform).convertToWorldSpaceAR(new Vec3(local.x, local.y, 0));

          var containSizeData = this._wholeCannon.getComponent(UITransform).contentSize;
          /*
        ...
        */
        ...
        getCreditCoinPosition() {
          var coinNode = this.node.getChildByName('playerBar').getChildByName('scoreBox').getChildByName('coin');
          var coinContainSizeData = coinNode.getComponent(UITransform).contentSize; //let gp:Vec3=coinNode.getComponent(UITransform).convertToWorldSpaceAR(v3(coinNode.position.x,coinNode.position.y));

          var gp = this.node.getChildByName('playerBar').getChildByName('scoreBox').getComponent(UITransform).convertToWorldSpaceAR(v3(coinNode.position.x, coinNode.position.y)); //log('check_playerBar',coinNode); 
        ...
        getPlayerTextDigitsInfoData() {
          var creditTextNode = this.node.getChildByName('playerBar').getChildByName('scoreBox').getChildByName('score');
          var coinContainSizeData = creditTextNode.getComponent(UITransform).contentSize;
          var gp = creditTextNode.getComponent(UITransform).convertToWorldSpaceAR(new Vec3(creditTextNode.position.x, creditTextNode.position.y, 0)); //let containerData:PIXI.Container=<PIXI.Container>this._textUserCredit.getChildAt(0);
        ...
        getWholeCannonTopPositions() {
          var gp = this.node.getComponent(UITransform).convertToWorldSpaceAR(new Vec3( //--因為美術的將裡面的原件
        ...
        playMountMotion(keyframe, loop, timeScale, reverse) {
          if (timeScale === void 0) {
            timeScale = 1;
          }
        ...
        getPlayerPositions() {
          ...
          var cannonNode = this.node.getChildByName('cannon');
          var gp = cannonNode.getComponent(UITransform).convertToWorldSpaceAR(new Vec3(cannonNode.position.x, cannonNode.position.y, 0));
          r.x = gp.x;
          r.y = gp.y;
          return r;
        }
      ...
      _export("Fish1CannonGuiView", Fish1CannonGuiView = class Fish1CannonGuiView extends (_crd && GuiBasic === void 0 ? (_reportPossibleCrUseOfGuiBasic({
        ...
        setLayout() {
          ...
          return _asyncToGenerator(function* () {
            ...
            return new Promise(resolve => {
              _this._playerWholeNode = find('Canvas/playerUI');
              _this._playerWholeNode.getComponent(UIOpacity).opacity = 0;

              var playerNodes = _this._playerWholeNode.getChildByName('player');

              var nodesForCannons = playerNodes.children; //log('check_allNode',this._playerWholeNode);
              //let test2=assetManager.assets;//--它會讀整張進來然後拆掉cach住,

              var waittingNodes = find('Canvas/waittingText').children;
              var playerNameNodes = find('Canvas/PlayerNameText').children;
              log('check_waittingNode', waittingNodes);
              var wc; //let nodeCannon:Node;
              //for(const item of nodesForCannons)
              //let cameraComponent=find('Canvas/CameraGUI').getComponent(CameraComponent);

              for (var i = 0; i < nodesForCannons.length; i++) {
                ...
                _this._aryAllCannon.push(wc);

                wc.id = i;
                var sprWaitting = waittingNodes[i].getComponent(Sprite);
                ...
                nodesForCannons[i].on((_crd && GUIEvent === void 0 ? (_reportPossibleCrUseOfGUIEvent({
                  error: Error()
                }), GUIEvent) : GUIEvent).CHANG_BULLETS, _this.changeGunHandler);
              } //---完成的時候做
        ...
        */
        setCannonInfo(bulletInfo, scorePool) {
          ...
          var len = this._aryAllCannon.length;
        ...
        */
        ...
        cleanAllRoom() {
          for (var i = 0; i < this._aryAllCannon.length; i++) {
            if (this._aryAllCannon[i].tableIndex == 1 || this._aryAllCannon[i].tableIndex == 2) {
              var cannonNode = this._playerWholeNode.getChildByName('player');

              var index = i + 1;
              var playerNodes = cannonNode.getChildByName('player' + index);
        ...
        */
        ...
        setPlayerInfoCoordinate(tableId) {
          ...
          for (var i of this._aryAllCannon) {
        ...
        afterCoordinatesChange(table) {
          ...
          if (table == 1 || table == 2) {
            ...
            var playerIDNode = find('Canvas/PlayerNameText');
            var waittingNode = find('Canvas/waittingText');
            var len = cannonNode.children.length;

            for (var i = 0; i < len; i++) {
              var index = i + 1;
              var playerNodes = cannonNode.getChildByName('player' + index);
              playerNodes.setPosition(v3(playerNodes.position.x, playerNodes.position.y - 40));
              var playerId = playerIDNode.getChildByName('id' + index);
              playerId.setPosition(v3(playerId.position.x, playerId.position.y - 40));
              var wait = waittingNode.getChildByName('watting' + index);
        ...
        */
        ...
        getCannonMountPositions() {
          var data = [];
          var len = this._aryAllCannon.length; //-getWholeCannonMountPositions
        ...
        getALLPlayerPositions() {
          ...
          var len = this._aryAllCannon.length;
          var data = []; //-getWholeCannonMountPositions

          for (var i = 0; i < len; i++) {
            var p = this._aryAllCannon[i].getPlayerPositions();
...
//# sourceMappingURL=cf407952e3e8626c447c2af48268d53f332106f0.js.map