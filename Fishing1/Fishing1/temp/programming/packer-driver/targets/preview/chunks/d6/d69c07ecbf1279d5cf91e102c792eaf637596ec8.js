      ...
      _export("PropBtn", PropBtn = class PropBtn extends Component {
        ...
        */
        ...
        updateColdDownTime(elapsedTimeInSeconds) {
          var percentageElapsed = (this._defaultColdDownTime - elapsedTimeInSeconds) / this._defaultColdDownTime * 100;
          var value = percentageElapsed / 100; // 將百分比轉換為 fillRange 的值
        ...
        digitsTween() {
          var tweenComponent = this._effectDigits.node.getComponent(_crd && TweenMaxCocosPlugin === void 0 ? (_reportPossibleCrUseOfTweenMaxCocosPlugin({
            error: Error()
          }), TweenMaxCocosPlugin) : TweenMaxCocosPlugin);
          ...
          if (this._effectDigits) {
            ...
            var _tweenComponent = this._effectDigits.node.getComponent(_crd && TweenMaxCocosPlugin === void 0 ? (_reportPossibleCrUseOfTweenMaxCocosPlugin({
              error: Error()
            }), TweenMaxCocosPlugin) : TweenMaxCocosPlugin);

            if (TweenMax.isTweening(_tweenComponent)) {
              TweenMax.killTweensOf(_tweenComponent);
            ...
            TweenMax.to(_tweenComponent, .3, {
              scale: 1,
              onCompleteParams: [_tweenComponent],
              onComplete: value => {
                TweenMax.to(value, .2, {
        ...
        lightTween() {
          if (this._light) {
            ...
            var tweenComponent = this._light.getComponent(_crd && TweenMaxCocosPlugin === void 0 ? (_reportPossibleCrUseOfTweenMaxCocosPlugin({
              error: Error()
            }), TweenMaxCocosPlugin) : TweenMaxCocosPlugin);
          ...
          if (this._lightCount) {
            ...
            var _tweenComponent2 = this._lightCount.getComponent(_crd && TweenMaxCocosPlugin === void 0 ? (_reportPossibleCrUseOfTweenMaxCocosPlugin({
              error: Error()
            }), TweenMaxCocosPlugin) : TweenMaxCocosPlugin);

            if (TweenMax.isTweening(_tweenComponent2)) {
              TweenMax.killTweensOf(_tweenComponent2);
              this._lightCount.active = false;
            }
            ...
            TweenMax.to(_tweenComponent2, .3, {
              //scale:1,
              sprColorAlpha: 255,
              onCompleteParams: [_tweenComponent2],
              onComplete: value => {
                TweenMax.to(value, .2, {
                  onCompleteParams: [value],
                  onComplete: target => {
        ...
        getPositionData() {
          ...
          var coinContainSizeData = uiTransform.contentSize;
          var gp = this.node.parent.getComponent(UITransform).convertToWorldSpaceAR(new Vec3(this.ogPosition.x, this.ogPosition.y, 0));
          return {
            x: gp.x,
            y: gp.y,
      ...
      _export("Fish1PropGuiView", Fish1PropGuiView = class Fish1PropGuiView extends (_crd && GuiBasic === void 0 ? (_reportPossibleCrUseOfGuiBasic({
        ...
        */
        ...
        setLayout() {
          ...
          return _asyncToGenerator(function* () {
            return new Promise(resolve => {
              var propPrefabNode = instantiate((_crd && LoadingResManager === void 0 ? (_reportPossibleCrUseOfLoadingResManager({
                error: Error()
              }), LoadingResManager) : LoadingResManager).getInstance().getPrefab('prefab/gui/propBtns'));
              log('propPrefabNode', propPrefabNode);

              _this._stageContainer.addChild(propPrefabNode); //let targetNodes=propPrefabNode.children;


              var targetNodes = ['itemCallBtn', 'itemFrozenBtn', 'itemCrazyBtn'];
              ...
              for (var i = 0; i < len; i++) {
                ...
                btnComponent.defaultColdDownTime = _this._defaultcdTimes[i];
                btnNode.on((_crd && GUIEvent === void 0 ? (_reportPossibleCrUseOfGUIEvent({
                  error: Error()
                }), GUIEvent) : GUIEvent).USE_PROP, _this.usepropBtnHandler);
                ...
                _this._ogPositions[i] = btnNode.position.x;
                btnNode.setPosition(v3(ogPosition.x + 200, ogPosition.y));
                _this._props[i] = btnNode;
                btnNode.on('fihishColdDown', _this.coldDownFinish);
              } //---完成的時候做
        ...
        updateColdDownTime(coldDownValue) {
          ...
          var propBtnComponent;

          for (var i in coldDownValue) {
            //log('check_forin',i);
        ...
        reSetDefaultcdTime() {
          ...
          for (var i = 0; i < len; i++) {
            var btnComponent = this._props[i].getComponent(PropBtn);
...
//# sourceMappingURL=d69c07ecbf1279d5cf91e102c792eaf637596ec8.js.map