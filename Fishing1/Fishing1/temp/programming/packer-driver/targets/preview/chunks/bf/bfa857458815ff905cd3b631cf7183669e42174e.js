    ...
    execute: function () {
      ...
      _export("PrefabEffectFactory", PrefabEffectFactory = class PrefabEffectFactory extends (_crd && BaseEffectFactory === void 0 ? (_reportPossibleCrUseOfBaseEffectFactory({
        ...
        createEffect(effectInfo) {
          ...
          var spr = effect.effectObj.getComponent(Sprite); //--reset用的
          ...
          var UITransformComponent = effect.effectObj.getComponent(UITransform);

          if (!UITransformComponent) {
            UITransformComponent = effect.effectObj.addComponent(UITransform);
          }

          var contanSize = UITransformComponent.contentSize; //--reset用的
        ...
        changeEffectSource(option) {
          //--step1檢查prefab裡面的東西
          var targetEffect = option.baseEffect; //--換單張圖-prefab/textures/fishHunterPopup
          ...
          if (option.assetsId && option.spriteAtlas == '') {
            ...
            var spr = targetEffect.effectObj.getComponent(Sprite);
          ...
          if (option.assetsId && option.spriteAtlas != '') {
            //--在檢查能不能換,不能換就要產生一個新的
            var texture2 = (_crd && LoadingResManager === void 0 ? (_reportPossibleCrUseOfLoadingResManager({
              error: Error()
            }), LoadingResManager) : LoadingResManager).getInstance().getSpriteFrameFromSpriteAtlas(option.spriteAtlas, option.assetsId);
            var spr2 = targetEffect.effectObj.getComponent(Sprite);
          ...
          if (option.prefab) {
            var parent = option.baseEffect.effectObj.parent;
            parent.removeChild(option.baseEffect.effectObj);
            option.baseEffect.effectObj = instantiate(option.prefab);
          }

          var UITransformComponent = option.baseEffect.effectObj.getComponent(UITransform);
...
//# sourceMappingURL=bfa857458815ff905cd3b631cf7183669e42174e.js.map