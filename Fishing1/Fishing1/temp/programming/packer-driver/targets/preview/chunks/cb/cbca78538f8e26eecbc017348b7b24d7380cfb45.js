        ...
        */
        ...
        setLayout() {
          ...
          return _asyncToGenerator(function* () {
            return new Promise(resolve => {
              _this._messageGui = instantiate((_crd && LoadingResManager === void 0 ? (_reportPossibleCrUseOfLoadingResManager({
                error: Error()
              }), LoadingResManager) : LoadingResManager).getInstance().getPrefab(_this._strprefab));

              _this.addChild(_this._messageGui);

              var titleSpriteFrame = (_crd && LoadingResManager === void 0 ? (_reportPossibleCrUseOfLoadingResManager({
                error: Error()
              }), LoadingResManager) : LoadingResManager).getInstance().getSpriteFrames(_this._strTitleSpriteFrame)[0];

              var sprTitle = _this._messageGui.children[0].getChildByName('title').getChildByName('Label').addComponent(Sprite);

              sprTitle.spriteFrame = titleSpriteFrame;

              var btn = _this._messageGui.children[0].getChildByName('closeBtn');

              var sprCloseBtn = btn.getChildByName('Label').addComponent(Sprite);
              var closeBtnSpriteFrame = (_crd && LoadingResManager === void 0 ? (_reportPossibleCrUseOfLoadingResManager({
                error: Error()
              }), LoadingResManager) : LoadingResManager).getInstance().getSpriteFrames(_this._strCloseBtnSpriteFrame)[0];
              sprCloseBtn.spriteFrame = closeBtnSpriteFrame;
              btn.on(Button.EventType.CLICK, _this.btnEventHandler);
              var labelNode = new Node('SystemMessageLabelNode');
              ...
              _this._label = labelNode.addComponent(Label);
              _this._label.horizontalAlign = HorizontalTextAlignment.CENTER;
              _this._label.verticalAlign = VerticalTextAlignment.CENTER;
              _this._label.overflow = Overflow.RESIZE_HEIGHT;
              _this._label.fontSize = 32;
              _this._label.lineHeight = 40;
              _this._label.useSystemFont = true;
              _this._label.fontFamily = 'Arial';
              _this._label.color = Color.WHITE; //this._label.isBold=true;
              //---要在處理座位旋轉後的_label旋轉問題
              //this._lableShowContainerNode.addChild(labelNode);

              _this.addChild(labelNode); //this._label=this._messageGui.children[0].getChildByName('message').getComponent(Label);


              _this._label.string = 'mmmmmmmmmmmm';
              ...
              _this.active = false;
              _this._label.node.active = false; //---完成的時候做
        ...
        showAlert(errorType, dictString, autoDisappearTime) {
          ...
          if (autoDisappearTime > 0) {
            var tween = this.getComponent(_crd && TweenMaxCocosPlugin === void 0 ? (_reportPossibleCrUseOfTweenMaxCocosPlugin({
              error: Error()
            }), TweenMaxCocosPlugin) : TweenMaxCocosPlugin);
...
//# sourceMappingURL=cbca78538f8e26eecbc017348b7b24d7380cfb45.js.map