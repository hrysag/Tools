    ...
    execute: function () {
      ...
      _export("AnimationSequencePlayer", AnimationSequencePlayer = class AnimationSequencePlayer extends Component {
        ...
        set sequenceData(value) {
          ...
          for (let i in this._sequenceData) {
            for (let j of this._sequenceData[i].sequence) {
              if (j.sendEvtFrame) {
                let durationInSeconds = j.clip.duration; //-totaltime

                let totalFrame = Math.floor(durationInSeconds * j.clip.frameRate);
                let triggerTime = j.sendEvtFrame / totalFrame;
                j.clip.events = [{
                  frame: triggerTime,
        ...
        checkandPlay() {
          ...
          this._isPlay = true;
          let playId = this._sequenceData[this._status].sequence[index].clip.name;
          /*
...
//# sourceMappingURL=7099e0802f73547ae15bca7f31613e0ea2abfeb3.js.map