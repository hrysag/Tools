    ...
    execute: function () {
      ...
      _export("BulletView", BulletView = class BulletView extends (_crd && BulletViewBase === void 0 ? (_reportPossibleCrUseOfBulletViewBase({
        ...
        removeSingleBullet(id) {
          let b;
          let len = this._aryBullets.length;
        ...
        setMoreBulletIsDeath(ids) {
          for (let i = 0; i < ids.length; i++) {
            let b = this.getBulletByID(ids[i]);
        ...
        cleanMoreFishTarget(bulletIds) {
          ...
          for (let i = 0; i < bulletIds.length; i++) {
            let b = this.getBulletByID(bulletIds[i]);
        ...
        cleanFishTarget(bulletId) {
          let b = this.getBulletByID(bulletId); //log("cleanFishTarget>>>"+bulletId);
        ...
        */
        ...
        changeBullet(id) {
          ...
          let ary = this._strNowAction.split('_');
        ...
        setBulletData(b, startGlobalPositions) {
          let t = new Vec3();
          let mouse = new Vec3();
          let mouse2D; //---emitter2D/3D座標(在bulletContainer裡面的坐標系)

          let emitter2D;
          let mouse3D;
          let emitter3D; //let layer2DBulletsContainer:Node=find('Canvas/bulletNodeContainer/bulletNode');

          let layer2DBulletsContainer = this.node;
          /**
        ...
        resetEndPositionAndFishTargetId(position, bulletId, lockFishId) {
          let b = this.getBulletByID(bulletId); //let testCloneBulletData=GameUtils.deepCloneForObject(b);
        ...
        updateAction(t) {
          let aryIsDeathbullet = []; //--死掉的魚id

          let drillobj = null; //log('updateBulletAction',this._aryBullets);
...
//# sourceMappingURL=45d815ad9c254f72f29e8bf6efbc6b103b582609.js.map