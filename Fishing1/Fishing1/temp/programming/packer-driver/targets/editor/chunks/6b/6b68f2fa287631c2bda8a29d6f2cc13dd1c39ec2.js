      ...
      _export("PathCenter", PathCenter = class PathCenter extends EventTarget {
        ...
        testPathPos() {
          ...
          for (let i in this._hasMapPaths) {
            let len = this._hasMapPaths[i].length;
        ...
        createPath(pathId, pathTokenID, reverse, gloupSN) {
          let pathGroup;
          let pathData = this.getPathData(pathId, reverse);
          log('check_createPath', pathData, pathId, pathTokenID, reverse, gloupSN);

          if (pathData) {
            let p = new (_crd && PathActionBase === void 0 ? (_reportPossibleCrUseOfPathActionBase({
              error: Error()
            ...
            let gloupId = gloupSN ? gloupSN : pathTokenID;
            pathGroup = {
              id: gloupId,
              path: [p],
              bornTime: 0
        ...
        setUnitInFlock(id, ary, t, fishType, transforme) {
          let a = this.getPlayingPathClass(id);
        ...
        reSetNewFishBronTime(value) {
          ...
          for (let i = 0; i < len; i++) {
            for (let j = 0; j < this._aryPlayingPathAction.length; j++) {
              if (this._aryPlayingPathAction[j].id == value[i].pathTokenID) {
                let lenPath = this._aryPlayingPathAction[i].path.length;

                for (let k = 0; k < lenPath; k++) {
        ...
        updataPath(t) {
          let len = this._aryPlayingPathAction.length;
        ...
        removeSingleUnit(pathId, flockunit) {
          let a = this.getPlayingPathClass(pathId);

          if (a) {
            let len = a.removeSingleUnit(flockunit);
            return len;
          } else {
            return -1;
          }
        ...
        deleteGroupPath(pathId) {
          ...
          for (let i = 0; i < len; i++) {
            if (this._aryPlayingPathAction[i].id == pathId) {
              let pathLen = this._aryPlayingPathAction[i].path.length;
        ...
        exitAllPath() {
          for (let i = 0; i < this._aryPlayingPathAction.length; i++) {
            let a = this.getPlayingPathClass(this._aryPlayingPathAction[i].id);
        ...
        setPath(jsonData, newIndex, len) {
          ...
          let jsonDatas = jsonData; //let keys:string=['type1'];
          ...
          for (let i = 0; i < maxLen; i++) {
            ...
            for (let j = 0; j < data.pathData.length; j++) {
              paths = [];

              for (let k = 0; k < data.pathData[j].NodeData.length; k++) {
        ...
        getPlayingPathClass(id) {
          let t = null;
          let len = this._aryPlayingPathAction.length; //-{id:string,path:PathActionBase[],bornTime:number}

          for (var i = 0; i < len; i++) {
            if (this._aryPlayingPathAction[i].id == id) {
              let aryPaths = this._aryPlayingPathAction[i].path;

              for (let j = 0; j < aryPaths.length; j++) {
...
//# sourceMappingURL=6b68f2fa287631c2bda8a29d6f2cc13dd1c39ec2.js.map