      ...
      _export("ResizeTool", ResizeTool = class ResizeTool {
        ...
        constructor() {
          ...
          this.resize = () => {
            ...
            const designW = designResolutionSize.width;
            const designH = designResolutionSize.height;
            ...
            let x = Math.round(0.5 * (containerW - contentW) / devicePixelRatio);
            let y = Math.round(0.5 * (containerH - contentH) / devicePixelRatio);
            let w = Math.round(contentW / devicePixelRatio);
            let h = Math.round(contentH / devicePixelRatio);
            ...
            if (this._cameraComponent) {
              ...
              const newViewportWidth = currentWidth * widthRatio; // 將新的相機視口寬度限制在最小為1的範圍內

              const clampedNewWidth = Math.max(newViewportWidth, 1); // 計算新的 x 座標以保持視口中心對齊

              const newViewportX = currentViewport.x + (currentViewport.width - clampedNewWidth) / 2;
              let rect = new Rect(newViewportX, currentViewport.y, clampedNewWidth, currentViewport.height);
...
//# sourceMappingURL=62114b6fee76b665eea552bebdf39a7277e2973e.js.map