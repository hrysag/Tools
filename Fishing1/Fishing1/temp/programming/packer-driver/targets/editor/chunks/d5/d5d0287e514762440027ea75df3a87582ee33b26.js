    ...
    execute: function () {
      ...
      _export("Main", Main = (_dec = ccclass('Main'), _dec2 = property(Animation), _dec(_class = (_class2 = class Main extends Component {
        ...
        async onLoad() {
          ...
          this.updateViewport();
          const util = window.util; // 本地開發用，production 則是直接取location.search上的 d=xxxx
          ...
          const data = util.general.parseEntryData(url.split('d=')[1]); // i18n.init(data.lang);
        ...
        start() {
          ...
          const ls = s.getComponent(_crd && LocalizedSprite === void 0 ? (_reportPossibleCrUseOfLocalizedSprite({
            error: Error()
          ...
          resources.load('common-path.json', (err, asset) => {
            ...
            fetch(asset.nativeUrl).then(response => response.blob()).then(blob => {
              ...
              fileReader.onload = event => {
                ...
                const decodedStr = (_crd && LZMA === void 0 ? (_reportPossibleCrUseOfLZMA({
                  error: Error()
...
//# sourceMappingURL=d5d0287e514762440027ea75df3a87582ee33b26.js.map