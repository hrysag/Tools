    ...
    execute: function () {
      ...
      _export("Flock", Flock = class Flock {
        ...
        update(updateTime) {
          ...
          let flockUnit; //--強迫每次都以1/60的更新率更新
            ...
            if (this.skipMotion) {
              ...
              for (let i = 0; i < this._units.length; i++) {
                ...
                /*&& flockUnit.time < flockUnit.pathTotalTime - flockUnit.goAwayTime*/
                  ...
                  let p0Index = flockUnit.nowIndex == 0 ? 0 : flockUnit.nowIndex - 1;
                  let p1Index = flockUnit.nowIndex; //log('check_nowIndex',p0Index,p1Index);

                  if (p0Index != p1Index) {
                    let p0 = flockUnit.positions[p0Index].position;
                    let p1 = flockUnit.positions[p1Index].position;
                    ...
                    let testX = p0.x + flockUnit.velocity.x * ((flockUnit.time - flockUnit.positions[p0Index].duringTime) / flockUnit.positions[p0Index].time);
                    let testY = p0.y + flockUnit.velocity.y * ((flockUnit.time - flockUnit.positions[p0Index].duringTime) / flockUnit.positions[p0Index].time); //--PS-要0-1之間的值(-0~-1也不行)--20231115
...
//# sourceMappingURL=63faf71f724737eec22dee781d9170d4971ebae4.js.map