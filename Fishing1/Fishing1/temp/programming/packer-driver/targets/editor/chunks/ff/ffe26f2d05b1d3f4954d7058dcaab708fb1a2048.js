    ...
    execute: function () {
      ...
      _export("AwardDiscAniEffect", AwardDiscAniEffect = class AwardDiscAniEffect {
        constructor(...args) {
          ...
          for (let i = 0; i < this._maxmumTable; i++) {
            let awardNode = instantiate((_crd && LoadingResManager === void 0 ? (_reportPossibleCrUseOfLoadingResManager({
              error: Error()
            ...
            let dg = awardNode.getChildByName('coinBg').getChildByName('label').addComponent(_crd && Digits === void 0 ? (_reportPossibleCrUseOfDigits({
              error: Error()
            }), Digits) : Digits);
        ...
        setDataAfterSetRoom(positions, playerIndex) {
          let pos = positions;
          let tableIndex = playerIndex;
          let awardNode;

          for (let i = 0; i < this._maxmumTable; i++) {
            awardNode = this._aryPlayerDiscItem[i].disc[0];
            let offsetY = 0;
            let heightDistance = awardNode.getComponent(UITransform).contentSize.height / 2 + 90;
            ...
            let lpos = this._containerNode.getComponent(UITransform).convertToNodeSpaceAR(v3(pos[i].x, pos[i].y + offsetY)); //log('check_node_award',awardNode);
        ...
        checkDataIntheQueue(playerIndex) {
          let len = this._aryPlayerDiscItem[playerIndex].queue.length;
          let index = 0; //--用來檢查每個位置上的彩盤形式(1代只有一個, index=0)
...
//# sourceMappingURL=ffe26f2d05b1d3f4954d7058dcaab708fb1a2048.js.map