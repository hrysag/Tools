      ...
      _export("Fish1View", Fish1View = (_dec = _crd && viewBind === void 0 ? (_reportPossibleCrUseOfviewBind({
          ...
          */
          ...
          this.updateFish = t => {
            ...
            if (this._aryFishData.length > 0 && this._canUpdate) {
              ...
              //log('updateFishData@@');
              let second = t / 1000; //this.sortAllFishLayer();
        ...
        createFish(fishInfo) {
          ...
          for (let fish of fishInfo) {
            ...
            let t = Date.now(); //--ms(正式要打開20240401)
            ...
            if (fd.fishType == 999) {//--特殊需要自己建立的魚種(FishCustomAnimation)
              ...
              if (fd.fishMeshState == (_crd && fishMeshState === void 0 ? (_reportPossibleCrUseOffishMeshState({
                ...
                */
                ...
                let aniComponent = mesh.getChildByName('fish').getComponent(Animation); //-有些物件沒有動作(例如海星fisd_type 1 ,fish_02)

                let clips = aniComponent.clips;
                let playerFrame;
                let timeOffsetInSeconds;
                let animationState;
                ...
                if (aniComponent) {
                  ...
                  for (let a = 0; a < clips.length; a++) {
                    let durationInSeconds = clips[a].duration; //-totaltime

                    let frameRate = clips[a].sample; //--frameRate

                    let totalFrame = Math.floor(durationInSeconds * frameRate); //--(int)(Math.random() * (Y-X+1)) + X;
                ...
                let clipsId = this._fishTypeKeyMap[fish.type].meshId.match(/\d+/)[0];

                log('check_clipID-->', 'fish' + clipsId + 'Swim');
                let aniShadowClip = (_crd && LoadingResManager === void 0 ? (_reportPossibleCrUseOfLoadingResManager({
                  error: Error()
                }), LoadingResManager) : LoadingResManager).getInstance().getAnimationClip('fish' + clipsId + 'Swim');
                log('check_aniClip', aniShadowClip);
                let topFishNode = new Node('fish');
                let otherFishNode = new Node('fish');
                ...
                let aniShadow = topFishNode.addComponent(Animation);
                ...
                if (this._playerTableCoordinate > 0) {
                  if (this._coordinateMode == (_crd && GameCoordinateMode === void 0 ? (_reportPossibleCrUseOfGameCoordinateMode({
                    ...
                    if (fd.rotationState == (_crd && FishRotationState === void 0 ? (_reportPossibleCrUseOfFishRotationState({
                      ...
                      if (fd.fishType != 1) {
                        let rv = this._playerTableCoordinate == 1 || this._playerTableCoordinate == 2 ? -1 : 1;
                ...
                */
                ...
                let meshScaleRate = this._fishTypeKeyMap[fish.type].meshScale;
                ...
                if (fd.fishType != 21) {
                  ...
                  let pos3dchange = (_crd && GameUtils === void 0 ? (_reportPossibleCrUseOfGameUtils({
                    error: Error()
                  }), GameUtils) : GameUtils).conver2dposTo3dpos(changeposData);
                  let zindex = this.checkNowThreeDfishZindex(fd.fishType); //-有些模型的動作表較大,這是安全深度
                ...
                if (fd.fishType != 21) {
                  //--播放預設
                  let aniClip = this.getAnimationClip(fd.animation, defaultAniClipId);
        ...
        delayCreateFish(aryFishs) {
          //---測試關閉
          let nowTime = 0; //let dgFlag:number=0;

          for (let i = 0; i < aryFishs.length; i++) {
        ...
        resetFreezeDuringTime() {
          ...
          for (let i = 0; i < this._delayFdata.length; i++) {
            //--實際上的路徑時間
        ...
        checkNowThreeDfishZindex(flshType) {
          let baseZinex = (_crd && GameUtils === void 0 ? (_reportPossibleCrUseOfGameUtils({
            error: Error()
          }), GameUtils) : GameUtils).deepCloneForObject(this._defaultZindex);
          let ogLength = baseZinex.length;
          let ogzindex;
          let index;
          let returnValue = -1;

          for (let i = 0; i < this._aryFishData.length; i++) {
        ...
        async removeAllFish() {
          ...
          return new Promise((resolve, reject) => {
            for (let i = 0; i < this._aryFishData.length; i++) {
              let fd = this._aryFishData[i];

              if (fd.fishMeshState == (_crd && fishMeshState === void 0 ? (_reportPossibleCrUseOffishMeshState({
                error: Error()
              }), fishMeshState) : fishMeshState).fish2D) {
                let ani = fd.animation;
                ani.stop();
                ...
                let tweenmax = fd.fishMesh.getComponent(_crd && TweenMaxCocosPlugin === void 0 ? (_reportPossibleCrUseOfTweenMaxCocosPlugin({
                  error: Error()
                }), TweenMaxCocosPlugin) : TweenMaxCocosPlugin);
                ...
                let timeUpdate = fd.fishMesh.getComponent(_crd && TimeUpdateForMove === void 0 ? (_reportPossibleCrUseOfTimeUpdateForMove({
                  error: Error()
              ...
              if (fd.animationSequencePlayer) {
                ...
                fd.animationSequencePlayer.eventTarget.off((_crd && BaseEvent === void 0 ? (_reportPossibleCrUseOfBaseEvent({
                  error: Error()
                }), BaseEvent) : BaseEvent).PLAY_ANI, this.animationSequencePlayerEvent);
              }
        ...
        async removeFishById(id) {
          ...
          return new Promise((resolve, reject) => {
            let len = this._aryFishData.length;
        ...
        reSetFishBronTime() {
          ...
          for (let i = 0; i < this._aryFishData.length; i++) {
            let realTime = nowTimestamp - this._aryFishData[i].createServerTime;
          ...
          if (fd != null) {
            ...
            if (fd.fishMesh instanceof Animation) {//--spine?
              ...
              if (fd.fishMeshState == (_crd && fishMeshState === void 0 ? (_reportPossibleCrUseOffishMeshState({
                ...
                let spr = fd.fishMesh.getChildByName('fish').getChildByName('fish').getComponent(Sprite); //log('check_fishMeshNode',fd.fishMesh,spr);
                ...
                if (fd.fishType == 21) {//--boss
                  ...
                  //--other 3d fish
                  if (this._viewModel['_roomStatus'].status != 1) {
                    let clip = this.getAnimationClip(fd.animation, 'hurt');
        ...
        hitFishAniComplete(fd) {
          ...
          if (fd.fishMesh instanceof Animation) {//--spine?
            ...
            if (fd.fishMeshState == (_crd && fishMeshState === void 0 ? (_reportPossibleCrUseOffishMeshState({
              ...
              let spr = fd.fishMesh.getChildByName('fish').getChildByName('fish').getComponent(Sprite); //log('finishHitComplete',fd.fishMesh,spr);
              ...
              if (fd.fishType == 21) {//--boss
                ...
                //--other 3d fish
                let defaultClipId = fd.fishType == 14 || fd.fishType == 15 || fd.fishType == 18 || fd.fishType == 19 ? 'swim' : 'swin';

                if (this._viewModel['_roomStatus'].status != 1) {
                  let clip = this.getAnimationClip(fd.animation, defaultClipId);
          ...
          if (fd) {
            ...
            if (fd.fishMeshState == (_crd && fishMeshState === void 0 ? (_reportPossibleCrUseOffishMeshState({
              ...
              if (fd.fishType != 21) {
                ...
                let shape = fd.collisionArea[6].shape;
            ...
            let tweenComponent = this._aimLockNode.getComponent(_crd && TweenMaxCocosPlugin === void 0 ? (_reportPossibleCrUseOfTweenMaxCocosPlugin({
              error: Error()
            }), TweenMaxCocosPlugin) : TweenMaxCocosPlugin);
          ...
          if (this._aryFishData.length > 0) {
            ...
            for (let i = 0; i < this._aryFishData.length; i++) {
              if (!this._aryFishData[i].fishIsFlash && !this._aryFishData[i].isDead) {
                ...
                if (this._aryFishData[i].isHit) {
                  this._aryFishData[i].countHitAni -= t; //log('check_isHitFish',this._aryFishData[i].countHitAni);
                ...
                let theta = Math.atan2(flockUnit.velocity.y, flockUnit.velocity.x);
                ...
                if (this._aryFishData[i].fishMeshState == (_crd && fishMeshState === void 0 ? (_reportPossibleCrUseOffishMeshState({
                  ...
                  this._aryFishData[i].fishMesh.setPosition(new Vec3(flockUnit.position.x, flockUnit.position.y, flockUnit.position.z)); //--一般正常狀態的旋轉(一般魚,非自訂義的魚)


                  if (this._aryFishData[i].fishMesh instanceof Node && this._aryFishData[i].rotationState == (_crd && FishRotationState === void 0 ? (_reportPossibleCrUseOfFishRotationState({
                    error: Error()
                  }), FishRotationState) : FishRotationState).normalRotation) {
                    this._aryFishData[i].fishMesh.angle = math.toDegree(theta);

                    if (this._aryFishData[i].fishShadow) {
                      this._aryFishData[i].fishShadow.setPosition(v3(flockUnit.position.x + 30, flockUnit.position.y - 30, flockUnit.position.z));

                      this._aryFishData[i].fishShadow.angle = this._aryFishData[i].fishMesh.angle;
                    }

                    if (theta < Math.PI / 2 && theta > -Math.PI / 2) {} else {}
                  } else if (this._aryFishData[i].rotationState == (_crd && FishRotationState === void 0 ? (_reportPossibleCrUseOfFishRotationState({
                    error: Error()
                  ...
                  if (this._aryFishData[i].fishAimLock) {
                  ...
                  */
                  ...
                  if (this._aryFishData[i].fishType != 21) {
                    //--一般3d的魚
                    this._aryFishData[i].fishMesh.setPosition(pos3dchange.x, pos3dchange.y, this._aryFishData[i].fishMesh.position.z);

                    if (this._aryFishData[i].fishAimLock) {
                      localPos = (_crd && GameUtils === void 0 ? (_reportPossibleCrUseOfGameUtils({
                        error: Error()
                      }), GameUtils) : GameUtils).conver3dposTo2dpos(this._fishAimContainer, 'Canvas/Camera', 'Main Camera', this._aryFishData[i].fishMesh.position);

                      this._aimLockNode.setPosition(localPos);
                    }
                  } else {
                    let p; //let tweenPlugin:TweenMaxCocosPlugin;

                    let tweenPlugin = this._aryFishData[i].fishMesh.getComponent(_crd && TimeUpdateForMove === void 0 ? (_reportPossibleCrUseOfTimeUpdateForMove({
                      error: Error()
                    }), TimeUpdateForMove) : TimeUpdateForMove);

                    let doNextActionFlag = false;
                    let finaldDstination = 0;
                    ...
                    if (flockUnit.time >= 5 && flockUnit.time <= 19) {
                      ...
                      if (flockUnit.time >= 5) {
                        ...
                        if (this._playerTableCoordinate == 1 || this._playerTableCoordinate == 2) {
                          if (!this._aryFishData[i].spBossInStates.top) {
                            doNextActionFlag = true;
                            this._aryFishData[i].spBossInStates.top = true;

                            this._aryFishData[i].fishMesh.setPosition(v3(0, this._frustumData.topPoint + 1000, -500));

                            finaldDstination = this._frustumData.topPoint * 1.4;
                          }
                        } else {
                          if (!this._aryFishData[i].spBossInStates.bottom) {
                            doNextActionFlag = true;
                            this._aryFishData[i].spBossInStates.bottom = true;

                            this._aryFishData[i].fishMesh.setPosition(v3(0, this._frustumData.bottomPoint - 1000, -500));
                        ...
                        if (doNextActionFlag) {
                          ...
                          this._aryFishData[i].animationSequencePlayer.play((_crd && AnimationStatus === void 0 ? (_reportPossibleCrUseOfAnimationStatus({
                            error: Error()
                          }), AnimationStatus) : AnimationStatus).start);

                          if (!tweenPlugin.isRunning) {
                            let startPos = {
                              x: this._aryFishData[i].fishMesh.position.x,
                              y: this._aryFishData[i].fishMesh.position.y
                            };
                            let endPos = {
                              x: this._aryFishData[i].fishMesh.position.x,
                              y: finaldDstination
                      ...
                      if (flockUnit.time >= 18) {
                        ...
                        if (this._playerTableCoordinate == 1 || this._playerTableCoordinate == 2) {
                          if (!this._aryFishData[i].spBossInStates.to) {
                            doNextActionFlag = true;
                            this._aryFishData[i].spBossInStates.to = true;
                            p = this._aryFishData[i].fishMesh.position;
                            finaldDstination = p.y + 1000; //--下end指令的時候要停止particle--20231019 
                          }
                        } else {
                          if (!this._aryFishData[i].spBossInStates.bo) {
                            doNextActionFlag = true;
                            this._aryFishData[i].spBossInStates.bo = true;
                            p = this._aryFishData[i].fishMesh.position;
                      ...
                      if (flockUnit.time >= 20) {
                        ...
                        if (this._playerTableCoordinate == 1 || this._playerTableCoordinate == 2) {
                          if (!this._aryFishData[i].spBossInStates.right) {
                            doNextActionFlag = true;
                            this._aryFishData[i].spBossInStates.right = true;

                            this._aryFishData[i].fishMesh.setPosition(v3(this._frustumData.rightPoint + 1000, 0, -500));

                            finaldDstination = this._frustumData.rightPoint;
                          }
                        } else {
                          if (!this._aryFishData[i].spBossInStates.left) {
                            doNextActionFlag = true;
                            this._aryFishData[i].spBossInStates.left = true;

                            this._aryFishData[i].fishMesh.setPosition(v3(this._frustumData.leftPoint - 1000, 0, -500));
                      ...
                      if (flockUnit.time >= 33) {
                        ...
                        if (this._playerTableCoordinate == 1 || this._playerTableCoordinate == 2) {
                          if (!this._aryFishData[i].spBossInStates.ro) {
                            doNextActionFlag = true;
                            this._aryFishData[i].spBossInStates.ro = true;
                            p = this._aryFishData[i].fishMesh.position;
                            finaldDstination = p.x + 2000;
                          }
                        } else {
                          if (!this._aryFishData[i].spBossInStates.lo) {
                            doNextActionFlag = true;
                            this._aryFishData[i].spBossInStates.lo = true;
                            p = this._aryFishData[i].fishMesh.position;
                  ...
                  */
                  ...
                  let rotationAxis; //--PS boss龍王自己轉

                  if (this._aryFishData[i].fishType != 21) {
                    //let rotationQuaternion:Quat;
                    if (this._aryFishData[i].rotationState == (_crd && FishRotationState === void 0 ? (_reportPossibleCrUseOfFishRotationState({
                      error: Error()
                    }), FishRotationState) : FishRotationState).normalRotation) {
                      //--旋轉(隨著路徑旋轉)
                      rotationQuaternion = Quat.identity(this._aryFishData[i].rotationQuaternion); //let rotationAxis:Vec3;
                      ...
                      this._aryFishData[i].fishMesh.setRotation(rotationQuaternion);
                    } else if (this._aryFishData[i].rotationState == (_crd && FishRotationState === void 0 ? (_reportPossibleCrUseOfFishRotationState({
                      error: Error()
                    }), FishRotationState) : FishRotationState).noRotation) {
                      //--不旋轉(自身旋轉,不隨著路徑旋轉)
                       ...
                       */
                      //--這是弧度
                      this._aryFishData[i].other.rotation += 0.05; //--outX

                      let quatX = Quat.identity(this._aryFishData[i].other.AxisXQuat);
                      let rotationAxisX = Fish1View.Axis_Negative.x.normalize();
                      Quat.fromAxisAngle(quatX, rotationAxisX, this._aryFishData[i].other.rotation); //--outZ

                      let quatZ = Quat.identity(this._aryFishData[i].other.AxisZQuat);
                      let rotationAxisZ = Fish1View.Axis_Negative.z.normalize();
                      Quat.fromAxisAngle(quatZ, rotationAxisZ, this._aryFishData[i].other.rotation); //--out final

                      rotationQuaternion = Quat.identity(this._aryFishData[i].rotationQuaternion); //--先轉X軸再轉Z軸
                    ...
                    rotationQuaternion = Quat.identity(this._aryFishData[i].rotationQuaternion);
                    rotationAxis = Fish1View.Axis.z.normalize();
                    Quat.fromAxisAngle(rotationQuaternion, rotationAxis, theta);

                    this._aryFishData[i].fishMesh.setRotation(rotationQuaternion); //--鎖定準心 for boss
                    ...
                    if (this._aryFishData[i].fishAimLock) {
                      ...
                      let shape = this._aryFishData[i].collisionArea[6].shape;
                      let worldobb = shape.worldObb;
                      localPos = (_crd && GameUtils === void 0 ? (_reportPossibleCrUseOfGameUtils({
                        error: Error()
                      }), GameUtils) : GameUtils).conver3dposTo2dpos(this._fishAimContainer, 'Canvas/Camera', 'Main Camera', worldobb.center);
                ...
                if (this._aryFishData[i].fishType != 21) {
                  for (let c = 0; c < this._aryFishData[i].collisionArea.length; c++) {
                    ...
                    this._aryFishData[i].collisionArea[c].impl.update();
                  }
                }
              }
...
//# sourceMappingURL=ba3e0f88832765f455c75f12a06d0d2938e8dcdd.js.map