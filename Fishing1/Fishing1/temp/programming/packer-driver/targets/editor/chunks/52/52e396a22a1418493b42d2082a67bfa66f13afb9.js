System.register(["__unresolved_0", "cc", "__unresolved_1", "__unresolved_2"], function (_export, _context) {
  ...
  var _reporterNs, _cclegacy, __checkObsolete__, __checkObsoleteInNamespace__, AbstractBaseBullet, BulletActionType, math, v3, UITransform, Collider2D, log, Fish1BulletPrefabAction, _crd;
  ...
  _export("Fish1BulletPrefabAction", void 0);
  ...
  return {
    ...
    execute: function () {
      ...
      _cclegacy._RF.push({}, "1edc9Rd+9BFCqeB89r1Wrxd", "Fish1BulletPrefabAction", undefined);
      /**
       * Created by EricHuang on 2023/08/25.
       */


      __checkObsolete__(['Node', 'math', 'Layers', 'v3', 'VerticalTextAlignment', 'HorizontalTextAlignment', 'Overflow', 'Label', 'UITransform', 'Size', 'BoxCollider', 'BoxCollider2D', 'Collider2D', 'Vec3', 'Collider']);

      __checkObsolete__(['log', 'BuiltinShape2D']);

      _export("Fish1BulletPrefabAction", Fish1BulletPrefabAction = class Fish1BulletPrefabAction extends (_crd && AbstractBaseBullet === void 0 ? (_reportPossibleCrUseOfAbstractBaseBullet({
        error: Error()
        ...
        initBulletState(bullet) {
          ...
          let b = bullet[0];
          let theta = Math.atan2(b.ePosition.y - b.position.y, b.ePosition.x - b.position.x); //-30 * Math.PI/180
          //-以度數表示的角度，把數字乘以π/180便轉換成弧度；以弧度表示的角度，乘以180/π便轉換成度數。

          b.vx = b.speed * Math.cos(theta);
          b.vy = b.speed * Math.sin(theta);
          let spr; //--這邊是子彈的prefab

          if (b.amount > 1) {//--霰彈槍用的--實作要補
          } else {
            spr = b.bulletEffect[0].effectObj; //spr.interactive=false;--這個要再看一下底層,有否掛上監聽是否會參與搜尋事件的流程

            b.bulletShell = spr; //---for test

            /*
            b.show=new Node();
             b.show.layer=Layers.Enum.UI_2D;
             let labelComp=b.show.addComponent(Label);
             labelComp.horizontalAlign=HorizontalTextAlignment.CENTER;
            ...
            labelComp.verticalAlign=VerticalTextAlignment.CENTER;
             ...
             labelComp.useSystemFont=true;
             labelComp.fontFamily='Arial';
             labelComp.string=b.id+'';
             let ui=b.show.addComponent(UITransform);
             let ogContain=b.bulletShell.getComponent(UITransform).contentSize;
            
            ui.contentSize=new Size(ogContain.width,ogContain.height);
            
            ui.anchorX=ui.anchorY=0.5;
             b.show.setPosition(v3(0,0,0));
            */
            //--for test
          ...
          */
          ...
          //this.container.addChild(b.show);//--20240227
        ...
        updateBullet(t, b) {
          //return;
          //log('update_test',this._testCount);
          ...
          let right = 1920; // var right:number=CoreRenderMode.RenderCore.app.renderer.width;
          //let left:number=0;

          let left = 0; //let top:number=0;

          let top = 1080; //top=b.gameBoundaryfoBullet.h;---要改這個20230816
          //let bottom:number=1080;

          let bottom = 0;
          b.ePosition.x = b.position.x;
          b.ePosition.y = b.position.y; //let vxvalue:number=(b.isDrill)?600:1000;
          ...
          */
          ...
          b.position.x += b.vx * t / vxvalue;
          b.position.y += b.vy * t / vxvalue; //log('update_checkBulletPosition',b.position.x,b.position.y);

          let contentSize = b.bulletShell.getComponent(UITransform).contentSize; //--原本是依照左上角(0,0)坐標系去設計的判斷
          //--cocos是依照右下角(0,0)坐標系  
          ...
          if (b.position.y >= top + contentSize.height / 2) {
            //--超出上面邊界
            b.vy *= -1;
            b.position.y = top + contentSize.height / 2;
            changeflag = true; //}else if(b.position.y>=bottom-b.bulletShell.height/2){
          } else if (b.position.y <= bottom - contentSize.height / 2) {
            //--超出下面邊界 
            b.vy *= -1;
            b.position.y = bottom - contentSize.height / 2;
            changeflag = true;
          }
          ...
          if (b.position.x <= left + contentSize.width / 2) {
            //--超出左邊邊界
            b.vx *= -1;
            b.position.x = left + contentSize.width / 2;
            changeflag = true;
          } else if (b.position.x >= right - contentSize.width / 2) {
            //--超出右邊邊界
            b.vx *= -1;
            b.position.x = right - contentSize.width / 2;
            changeflag = true;
          } //--測試關閉 20240227--(正式要打開) 


          b.bulletShell.setPosition(v3(b.position.x, b.position.y)); //b.show.setPosition(v3(b.position.x,b.position.y));
          //--測試用的 20240227(正式關閉)
          //b.isDead=false;//--test
          //b.bulletShell.setPosition(v3(767.366699596394,1096.7060816812743));//--test
          //b.bulletShell.setPosition(v3((1920/2)+35,(1080/2)+35));//--test
          //b.show.setPosition(v3((1920/2)+35,(1080/2)+35));//--test

          if (!changeflag) {
            let theta = Math.atan2(b.position.y - b.ePosition.y, b.position.x - b.ePosition.x);
            b.bulletShell.angle = math.toDegree(theta); //b.show.angle=math.toDegree(theta);
        ...
        updateColliders(b, t) {
          for (let i = 0; i < b.collisions.length; i++) {
...
//# sourceMappingURL=52e396a22a1418493b42d2082a67bfa66f13afb9.js.map