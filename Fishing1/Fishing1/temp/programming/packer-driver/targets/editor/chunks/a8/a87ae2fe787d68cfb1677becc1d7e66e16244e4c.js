    ...
    execute: function () {
      ...
      _export("FishGameAutoAndLockData", FishGameAutoAndLockData = class FishGameAutoAndLockData extends EventTarget {
        ...
        removeLockBulletData(value) {
          let len = value.length; //---每次碰撞成立只會有一個子彈進來阿
        ...
        addFishAimTarget(value, table) {
          ...
          let returnLockFish = this._aryLockFishBullets[this._userTableIndex].lockFish;
        ...
        setLockFishBullet(fishData, bulletSn, index, isPlayer) {
          ...
          //--這時候子彈還沒新增實體化
          let p = null;
          let previousLockTarget = 0; //log("setLockFishBullet___fishSn_____"+fishSn);
          ...
          if (this._aryLockFishBullets[index].lockFish == fishData.id) {
            ...
            if (this._spUpdateFishTypeforBullet.indexOf(fishData.fishType) != -1) {
              ...
              let worldPos = this.getCanvasWorldPosition(fishData.fishMeshState, fishData.fishMesh.position);

              let localPos = this._mouseNode.getComponent(UITransform).convertToNodeSpaceAR(worldPos); //--這邊要換成滑鼠點擊的座標
        ...
        updateLockBullets() {
          //return;
          let len = this._aryLockFishBullets.length; //let testClone=GameUtils.deepCloneForObject(this._aryLockFishBullets);
          ...
          for (let i = 0; i < len; i++) {
            ...
            if (this._aryLockFishBullets[i].lockFish != 0) {
              ...
              if (f == null || f.isDead) {
                let deathTarget = this._aryLockFishBullets[i].lockFish;
                ...
                let rp; //let fishGlobal:PIXI.Point;
                //let stageLocalPoint:PIXI.Point;

                let destoryLockBullet = [];

                for (var j = 0; j < bulletLen; j++) {
                  let bullet = this._view.getDataFromgameMediator((_crd && GameViewMediatorUser === void 0 ? (_reportPossibleCrUseOfGameViewMediatorUser({
                    error: Error()
                  ...
                  if (this._spUpdateFishTypeforBullet.indexOf(f.fishType) != -1) {
                    ...
                    let worldPos = this.getCanvasWorldPosition(f.fishMeshState, f.fishMesh.position);

                    let localPos = this._mouseNode.getComponent(UITransform).convertToNodeSpaceAR(worldPos); //--這邊要小心,要確認mouseClick node是否為0,0 在左下角,1920,1080 在右下角
...
//# sourceMappingURL=a87ae2fe787d68cfb1677becc1d7e66e16244e4c.js.map