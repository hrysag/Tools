      ...
      */
      _export("CocoDigits", CocoDigits = class CocoDigits extends Component {
        ...
        */
        ...
        displayWithStr(digits, direction = "left") {
          this.floatNumIndex = Number.MAX_VALUE;
          /*
          ...
          for (var i = 0; i < length; i++) {
            ...
            let testSizecheck = spNode.getComponent(UITransform).contentSize;
            let testSP = spNode.getComponent(Sprite).spriteFrame.originalSize;
        ...
        layout(digitsStr) {
          ...
          for (var i = 0; i < length; i++) {
            sp = this.arrDigit[i];
            let test = this.container.getComponent(UITransform).contentSize;
            ...
            if (i != 0) {
              ...
              if (i > this.floatNumIndex) {
                ...
                let firstHiget = this.arrDigit[0].getComponent(UITransform).contentSize.height;
                let spHeight = sp.getComponent(UITransform).contentSize.height;
          ...
          let containerSize = this.container.getComponent(UITransform).contentSize;
          let containerAnchor = this.container.getComponent(UITransform).anchorPoint;
          let parentAnchor = this.node.getComponent(UITransform).anchorPoint;

          if (this.direction == "center") {
            // 计算子节点相对于父节点的位置
            let posX = containerSize.width * containerAnchor.x - parentSize.width * parentAnchor.x;
            let posY = containerSize.height * containerAnchor.y - parentSize.height * parentAnchor.y; // 设置子节点的位置
            ...
            this.container.setPosition(posX, posY); // 将子节点添加到父节点

            this.node.addChild(this.container);
          } else {
            //--靠左 bug
            //distance=0;
            let posX = -(parentSize.width * parentAnchor.x) + containerSize.width * containerAnchor.x;
            let posY = containerSize.height * containerAnchor.y - parentSize.height * parentAnchor.y; // 设置子节点的位置
