  ...
  return {
    ...
    execute: function () {
      ...
      _export("FishCollisionBase", FishCollisionBase = class FishCollisionBase extends (_crd && CollisionBase === void 0 ? (_reportPossibleCrUseOfCollisionBase({
        ...
        checkCollision(t) {
          let target = t.target;
          let bulletColliders = target.collisions;
          let fishColliderPoint;
          let bulletPoints;
          let scBulletPoints; //--只要取一次就好了(螢幕坐標系)
          ...
          if (target.isHitFlag) {
            ...
            for (let i of bulletColliders) {
              ...
              for (let j = 0; j < this._aryCompairs.length; j++) {
                //--檢查要不要碰--
                if (!this._aryCompairs[j].fishIsFlash && !this._aryCompairs[j].isDead && !this._aryCompairs[j].prohibit) {
                  ...
                  //){
                  for (let k = 0; k < this._aryCompairs[j].collisionArea.length; k++) {
                    if (this._aryCompairs[j].fishMeshState == (_crd && fishMeshState === void 0 ? (_reportPossibleCrUseOffishMeshState({
                      ...
                      if (this._aryCompairs[j].fishType != 21) {
                        let fish3Dget2DColliderPoints = this._aryCompairs[j].collisionArea[k].worldPoints; //fish3Dget2DColliderPoints,this._aryFishes[j].collisionArea[k]);
                        ...
                        let shape = this._aryCompairs[j].collisionArea[k].shape;
                        ...
                        let maxScreen = this._camera3dComponent.worldToScreen(max); //--左下(cocos左下角為(0,0))
        ...
        prefareCollisionData(fd, bullet) {
          ...
          if (bullet.lockFishTarget != -1) {
            ...
            //--auto ban fish
            let f = this._aryBannedFishType.every(num => num !== fd.fishType);
        ...
        finalMissCheck(bulletRect, fishRect) {
          let finalCollisionResult = false;

          for (let i = 0; i < bulletRect.length; i++) {
...
//# sourceMappingURL=a62d6c68f44b8bd6143218545bb37a41aeca5e60.js.map