    ...
    execute: function () {
      ...
      _export("BgAniEffect", BgAniEffect = class BgAniEffect {
        ...
        constructor(...args) {
          ...
          let materialsInfo = args[0].other.spriteFrameIds;
          let len = materialsInfo.length;

          for (let i = 0; i < len; i++) {
            let material = new Material(); //--effectName=屬性檢查器裡面的Effect名字
            ...
            let planeNode = new Node(materialsInfo[i]);
            let modelComponent = planeNode.addComponent(MeshRenderer); //--內建建立平面(內建是躺在Y周上垂直X軸的樣式)
            ...
            let vertices = [-(_crd && CocosGameSetting === void 0 ? (_reportPossibleCrUseOfCocosGameSetting({
              error: Error()
            ...
            let colorData = [255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255]; //--半透明測試
            ...
            let indices = [0, 1, 2, // 三角形1的頂點index
            0, 2, 3 // 三角形2的頂點index
            ];
            let geometryData = {
              positions: vertices,
            ...
            let mesh = new Mesh();
            utils.MeshUtils.createMesh(geometryData, mesh, {
          ...
          let mesh = this._aryImgBg[0].getComponent(MeshRenderer).mesh;

          let max = (_mesh$struct = mesh.struct) == null ? void 0 : _mesh$struct.maxPosition;
          let min = (_mesh$struct2 = mesh.struct) == null ? void 0 : _mesh$struct2.minPosition; //--抽出來的點{x: 960, y: 0, z: 540}
          //--ps x軸有被旋轉過90
        ...
        bgToDarkBlack() {
          //return;
          let modelComponent = this._nowNodeBg.getComponent(MeshRenderer);
        ...
        */
        ...
        bgToWhite() {
          let modelComponent = this._aryImgBg[this._nowIndex].getComponent(MeshRenderer);
        ...
        changeMotion() {
          ...
          let twComponent = this._nowNodeBg.getComponent(_crd && TweenMaxCocosPlugin === void 0 ? (_reportPossibleCrUseOfTweenMaxCocosPlugin({
            error: Error()
          }), TweenMaxCocosPlugin) : TweenMaxCocosPlugin);
          ...
          TweenMax.to(twComponent, this._motionTime, {
            ...
            onUpdate: value => {
              ...
              let bgPos = v3(value.node.position.x - this._bgWH.w / 2, value.node.position.y, value.node.position.z); //---world to screen

              let wts = cameraComponent.worldToScreen(bgPos); //--canvas camera cameracomponent

              let canvasCameraComponent = this._canvasCameraNode.getComponent(CameraComponent); //--screen to world
...
//# sourceMappingURL=d38d490d5109e3d82f3d62212192876272d520ad.js.map