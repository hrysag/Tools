...
import {IfConnectStrategy} from '../../../framework/game/strategy/Strategy';
import {ServerResCode} from '../../../framework/logic/connect/ConnectBaseDefinitions';
import {ResultForConnect} from '../../../framework/game/strategy/Strategy';
import {log} from 'cc';



export class Fish1ConnectStrategy implements IfConnectStrategy
{
    /**
    ...
    public strategyConnectDataFromPomelo=(code: string, data: any):ResultForConnect=>
    {
        ...
        switch(code)
        {
            ...
            //--ps要在處理送過來的資料{error!=null}的情況    
             ...
             //--舊版是在initgame執行後就會關閉laodingbar(closeLoadingScreen)    
            ...
            */
            ...
            //-{ s: 0 }離開房間的座位號0-3
               ...
               returnObj={index:data.s};
            
            break;

            case ServerResCode.CashOut:
            break;
            ...
            case ServerResCode.ShootBullet:
                 ...
                 */
                let bulletlockTarget=(data.l)?data.l:-1;

                log('ServerResCode.shootBullet',data);
                ...
                returnObj=
                {
                    ...
                    credit:data.p,
                    sn:data.id,
                    lockTarget:bulletlockTarget,//--有鎖定才會有
                    weaponType:data.w,
                    //--使用者送出的資料
                    info:
                    {
            ...
            case ServerResCode.HitFish:
                ...
                */
                ...
                returnObj={
                    siteIndex:data.s,//-0-3
                    credit:data.p,//-最新的分數餘額
                    bsn:data.id,//-子彈id
                    btp:data.w,//--武器的type
                    fish:
                    {
                       sn:data.hf.fid,//--fish id
                       ftp:data.hf.ft,//--fish type
                       bonus:data.hf.bt,//--bonus type
                       odds:data.hf.r,//--ratio賠率
                       payoff:data.hf.ws,//--贏分(payoff?)
                       iskill:data.hf.d,//--是否死亡
                       props:data.hf.pt,//--道具(沒有0)
                       //-(打中-特殊魚死亡)---這是client端送出範圍內的魚..
                       hitSpFish:data.hf.cf,//--[{fid:379,ft:16}...]
            ...
            log('ServerResCode_UpdateProp_fish1Strategy',data);
            ...
            returnObj= 
            {
                error:data.error,//---使用道具失敗原因(成功=null)
                callFish:data.f,
                propType:data.p,
                coldDownTime:data.pt/1000,//--換成秒為單位
                tableIndex:data.s,//--哪個玩家使用了--這邊要注意是該玩家才啟動colddown
                startTime:data.st
            };   
            /**
             * --非召喚
