...
import {ParticleSystemComponent} from 'cc';
import { find } from 'cc';
import { math } from 'cc';
import { Node } from 'cc';
...
import { BuiltinShape2D } from 'cc';
import {LoadingResManager} from '../../../framework/logic/loading/LoadingResManager';
import {FishView} from '../../../framework/logic/views/fishView/FishView';
...
import { PathCenter } from "./pathCore/PathCenter";
import { ResetPathData } from "./pathCore/PathCenter";
import { PathFlockUnit } from "./pathCore/basePath/BasePath";
import {TweenMaxCocosPlugin} from '../../../framework/utils/TweenMaxPlugin';
import { GameUtils,cover2dTo3dInfo,frustumInfoData} from '../../../framework/utils/GameUtils';
import { AnimationSequencePlayer,AnimationStatus} from './AnimationSequencePlayer';
import { FishRotationState,FishInitData,fishMeshState,OddsInfo } from '../../../framework/game/model/ModelDefinitionsBase';
...
import { TimeUpdateForMove } from '../../../framework/utils/TimeUpdateForMove';
...
export class Fish1View extends FishView
{
    ...
    @viewBind _addPopFishs:addFish[];

    @viewBind _roomStatus;

    @viewBind _hitFishs;

    @viewBind _bossStatus;//--boss的離場狀態
    
    @viewBind _fishTypeSpeedMap;


    //@viewBind _freeze;//--使用冰凍道具

    private _pathCenter:PathCenter;

    private _frustumData:frustumInfoData;

    //private _fish3DZindex:number;

    private _exitAllFishForBoss:boolean;//--紀錄現在是否為金龍進場,魚群出場

    private _isFreeze:boolean;//--紀錄是否為冰凍狀態

    private _updateDeltaTime:number;//--紀錄每次更新的單位時間(冰凍要手動更新一次)
     ...
     */
    private _delayFdata:Fish1FishData[];

    private _spBossId:number;//--20240308

    private _spBossPathFlockUnit:PathFlockUnit;

    private _spBossAlreadyServerTime:number;

    private _spBossPathId:string;

    private _spBossGroupPathId:string;

    private _spChangeRoomTime:number;

    private _aryOutSideBanndedType:number[];

    private _aimLockNode:Node;//--aim texture-

    private _defaultZindex:number[];

    //private _aryHitAnimations:Node[];//--20220919-把爆炸效果移到魚身上(原本是魚網)
    //private _testTimeUpdate:TimeUpdateForMove;
   
    private static Axis:{x:Vec3,y:Vec3,z:Vec3};//--右手坐標系正軸旋轉vec3

    private static Axis_Negative:{x:Vec3,y:Vec3,z:Vec3};//--右手坐標系逆軸旋轉vec3
    ...
    constructor()
    {
        ...
        //this._pathCenter=new PathCenter({x:0,y:0,w:1920,h:1080});
        this._pathCenter=new PathCenter();

        this._lockFish=null;

        this._aryblockBoundaryTest=[];
        ...
        this._frustumData=GameUtils.getFrustumData();
    ...
    protected onLoad():void
    {
        ...
        let texture:SpriteFrame=LoadingResManager.getInstance().getSpriteFrames('target_mc')[0];

        this._aimLockNode=new Node('_aimLockNode');

        this._aimLockNode.addComponent(TweenMaxCocosPlugin);

        let spr=this._aimLockNode.addComponent(Sprite);
        ...
        let uiTransForm=this._aimLockNode.addComponent(UITransform);
    ...
    */
       ...
       switch(sub)
       {
           ...
           case '_roomStatus':
            ...
            if(value[0].status==0)
            {
               ...
               if(!SoundsManager.getInstance().isPlaying('sounds/BGM01'))
               {
            ...
            }else if(value[0].status==2)
            {
               ...
               //--其他的背景音就會停掉了 
               SoundsManager.getInstance().playBGMusic('sounds/dragonbattlebgm-1');
            ...
            }else if(value[0].status==1)
            {
                ...
                //this._freezeLastTime=value[0].startTime;
    ...
    private checkexitFishForBoss():void
    {
        if(!this._exitAllFishForBoss)
        {
            this._exitAllFishForBoss=true;

            this._spChangeRoomTime=Date.now();
            //--離場
    ...
    public getData(dataKey:string,value?:any):any
    {
        ...
        switch(dataKey)
        {
            case GameViewMediatorUserDataKey.Fish_getFishs:

            data=this._aryFishData;

            break;

            case GameViewMediatorUserDataKey.Fish_changeSingleFishAnimation:

            this.changeSingleFishAnimation(value);

            break;

            case GameViewMediatorUserDataKey.Fish_getFishById:
            
            data=this.getFishById(value);

            break;

            case GameViewMediatorUserDataKey.Fish_removeFishById:
            ...
            case GameViewMediatorUserDataKey.Fish_removeSinglePathUnitByFishId:
            ...
            case GameViewMediatorUserDataKey.Fish_hitFishAniComplete:
            ...
            case GameViewMediatorUserDataKey.Fish_getOutsideFish:
            ...
            case GameViewMediatorUserDataKey.Fish_addFishAimLock:
            
            log('Fish_addFishAimLock',value);
            //-fid:value,isPlayer:true,index:this._userTableIndex
            this.addFishAimLock(value.fid,value.isPlayer,value.index);
            //-
            break;

            case GameViewMediatorUserDataKey.Fish_removeFishAimLock:
            ...
            case GameViewMediatorUserDataKey.Fish_removeFishAimLockByLockId:
     ...
     * 20240315-
     */
    public setPlayerIdAfterCoordinateMode(playerId:number):void
    {
        /**
         * ps-這個變數只會寫一次,玩家進桌後
    ...
    //--override
    public createFish(fishInfo:any):void
    {
       
       let aryFishs:Fish1FishData[]=[]; 
       
       let fd:Fish1FishData;
       
       log('createFish_data_init',this._fishTypeKeyMap,fishInfo,fishInfo.length);
          
       let mesh:Node | FishCustomAnimation;
       ...
       for(let fish of fishInfo)
       {
             ...
             */
            fd=(this._poolFishData.length==0)?new Fish1FishData():this._poolFishData.pop() as Fish1FishData;
            ...
            fd.odds=this.getOddsByFishType(fish.type);

            //--boss要分開設定資料,因為藉由房間狀態改變,已經先產生了路徑--
            //-好像也沒差,因為開場是固定的時間

            let t:number=Date.now();//--ms(正式要打開20240401)
            ...
            if(fd.fishType==21)
            {
                ...
                this._spBossId=fish.id;
                 
                //fd.alreadyServerTime=(this._spChangeRoomTime==0)?0:t-this._spChangeRoomTime;//--ms(進場所需的時間)
                ...
                //fd.alreadyServerTime=unit[2];//--ms
            ...
            log('check_createPath_createrTime',fd,fd.fishFlockUnit,fd.alreadyServerTime,this._spChangeRoomTime);
           

            //let t:number=Date.now();//--ms
            //fd.pathID=unit[3]+'_'+t;
            ...
            fd.lv=this._fishTypeKeyMap[fish.type].lv;
            //log('check_time',Date.now(),new Date().getTime());
            ...
            if(fd.fishType==999)
            {
                //--特殊需要自己建立的魚種(FishCustomAnimation)
                ...
                let node:Prefab;
                if(fd.fishMeshState==fishMeshState.fish2D)
                {
                    //node=LoadingResManager.getInstance().getPrefab('prefab/fish/'+this._fishTypeKeyMap[unit[1]].meshId);
                    node=LoadingResManager.getInstance().getPrefab('prefab/fish/'+this._fishTypeKeyMap[fish.type].meshId);
                    log('check_fishprefab',node);
                    //let node=LoadManager.getInstance().getPrefab('prefab/fish/'+'fish_11');
                    //--幹,好爛要加上sprite才能用
                    /*
                    ...
                    */
                    ...
                    //let test=mesh.children[0].children[0].getComponent(Sprite);
                    //test.color=Color.BLACK;
                    //test.color=color(0,0,0,60);
                    //test.addComponent(BlockInputEvents);
                    
                    mesh.active=true;
                    //log('testFish',mesh.children[0].children[0]);
                    ...
                    mesh.setPosition(v3(-CocosGameSetting.Game_Width,-CocosGameSetting.Game_Height));
                    //--PS 要把它加入顯示清單才能操作相關的animationState物件,不然會找不到
                    log('check_fisd_type',fd.fishType,this._fishTypeKeyMap[fish.type].meshId);

                    let aniComponent:Animation=mesh.getChildByName('fish').getComponent(Animation);
                    //-有些物件沒有動作(例如海星fisd_type 1 ,fish_02)
                    let clips=aniComponent.clips; 
                    
                    let playerFrame:number;
                    
                    let timeOffsetInSeconds:number;
                    ...
                    if(aniComponent)
                    {
                        ...
                        for(let a:number=0;a<clips.length;a++)
                        {
                           let durationInSeconds:number=clips[a].duration;//-totaltime
                           
                           let frameRate:number=clips[a].sample;//--frameRate

                           let totalFrame:number=Math.floor(durationInSeconds*frameRate);
                           //--(int)(Math.random() * (Y-X+1)) + X;
                           //--要先執行play方法,讓animationstate物件setting好,才能接著後續操作
                           aniComponent.play(clips[a].name);
                           //playerFrame=Math.floor(Math.random()*(totalFrame-0+1));
                           ...
                           //-https://forum.cocos.org/t/topic/141549/3
                           //--讀取json檔案
                        }
                    }
                    ...
                    let clipsId:string=this._fishTypeKeyMap[fish.type].meshId.match(/\d+/)[0];
                    log('check_clipID-->','fish'+clipsId+'Swim');
                    
                    let aniShadowClip:AnimationClip=LoadingResManager.getInstance().getAnimationClip('fish'+clipsId+'Swim');
                    log('check_aniClip',aniShadowClip);
                    
                    let topFishNode:Node=new Node('fish');
                    
                    let otherFishNode:Node=new Node('fish');
                    ...
                    let shadowSpr=otherFishNode.addComponent(Sprite);
                    
                    shadowSpr.color=color(0,0,0,50);
                    
                    shadowSpr.addComponent(BlockInputEvents);
                    
                    let aniShadow:Animation=topFishNode.addComponent(Animation);
                    ...
                    if(this._playerTableCoordinate>0)
                    {
                        ...
                        if(this._coordinateMode==GameCoordinateMode.GameViewMode_Four_in_one)
                        {
                            ...
                            //------測試關閉-20231011
                            if(fd.rotationState==FishRotationState.noRotation)
                            {
                                //---預設的魚頭都是朝左邊(sin(0))
                                //----type 1海星他一開始就是做正的,所以不用轉
                                if(fd.fishType!=1)
                                {
                                    let rv:number=(this._playerTableCoordinate==1 || this._playerTableCoordinate==2)?-1:1;


                                    mesh.angle=math.toDegree(rv*Math.PI/2);
                        ...
                        mesh.setScale(v3(mesh.scale.x,mesh.scale.y*this._rotateValue));
                    ...
                    */
                    ...
                    log('check_fishMap',this._fishTypeKeyMap[fish.type].meshId);
                    //-Prefab                    
                    node=LoadingResManager.getInstance().getPrefab('prefab/fish/3d/'+this._fishTypeKeyMap[fish.type].meshId);
                    ...
                    let meshScaleRate:number=this._fishTypeKeyMap[fish.type].meshScale;
                    ...
                    if(fd.fishType!=21)
                    {
                         ...
                         * 
                         */
                        let changeposData:cover2dTo3dInfo=
                        {
                            //pos2d:new Vec2(0,700),
                            pos2d:new Vec2(-CocosGameSetting.Game_Width,-CocosGameSetting.Game_Height),
                            //pos2d:new Vec2(0,0),
                        ...
                        let pos3dchange:Vec3=GameUtils.conver2dposTo3dpos(changeposData);

                        let zindex:number=this.checkNowThreeDfishZindex(fd.fishType);
                         
                        //-有些模型的動作表較大,這是安全深度
                        if(zindex!=-1)
                        {
                            //--場上同時有同一個魚種
                            mesh.setPosition(v3(pos3dchange.x,pos3dchange.y,zindex));
                            //mesh.setPosition(v3(0,0,zindex));

                        }else{
                            
                            mesh.setPosition(v3(pos3dchange.x,pos3dchange.y, this._fishTypeKeyMap[fish.type].zindex));
                            //mesh.setPosition(v3(0,0, this._fishTypeKeyMap[fish.type].zindex));
                        }
                        ...
                        mesh.addComponent(TweenMaxCocosPlugin);

                        mesh.addComponent(TimeUpdateForMove);
                    ...
                    //--20 -2000
                    log('check_3Dfish_z_index',mesh.position.z,'fishType',fd.fishType,'path',fd.pathGroupID);
                    //---for test---
                    ...
                    fd.fishSkinnedMeshRenderer=this.getSkinnedMeshRenderer(mesh,fd.fishType);
                    
                    //=======抽取animation物件  
                    fd.animation=this.getAnimationFromFishMesh3D(mesh,fd.fishType);
                    
                    //====炸彈專屬===========
                    let defaultAniClipId:string='swin';
                    ...
                    {
                        //--播放預設
                        let aniClip:AnimationClip=this.getAnimationClip(fd.animation,defaultAniClipId);
                        ...
                        if(aniClip)
                        {
                            ...
                            fd.animation.play(aniClip.name);
                            //fd.animation.stop()
                        }
                        ...
                        fd.particle=this.getParticle(mesh);
                        
                        //---boss(animationSequencePlayer)children[0]
                        fd.animationSequencePlayer=fd.animation.node.addComponent(AnimationSequencePlayer);

                        //fd.animationSequencePlayer=new AnimationSequencePlayer(fd.animation);
                        ...
                        {
                            start:{index:-1,sequence:[{keyframe:'in',clip:this.getAnimationClip(fd.animation,'in'),loop:1,count:1}]},
                            ...
                            sequence:
                            {
                                index:-1,sequence:[
                                    ...
                                    {keyframe:'attack01',clip:this.getAnimationClip(fd.animation,'attack01'),loop:1,count:1,sendEvtFrame:8},
                                    ...
                                    {keyframe:'attack02',clip:this.getAnimationClip(fd.animation,'attack02'),loop:1,count:1,sendEvtFrame:18}
                            ...
                            standby:{index:-1,sequence:[{keyframe:'idle',clip:this.getAnimationClip(fd.animation,'idle'),loop:1,count:1}]},
                            
                            end:{index:-1,sequence:[{keyframe:'out',clip:this.getAnimationClip(fd.animation,'out'),loop:1,count:1}]}
                            
                           
                        }; 
                        //-animationSequencePlayerEvent
                        fd.animationSequencePlayer.eventTarget.on(BaseEvent.PLAY_ANI,this.animationSequencePlayerEvent);

                        //fd.animationSequencePlayer.play();
       ...
       log('createFish_Done',aryFishs,aryFishs.length);

       //--因為server 會在進入房間資訊前就會送產生魚的資訊,因此抓不到_playerTableCoordinate資料
       if(this._playerTableCoordinate==0)
       {
            //---延後產生
    ...
    private delayCreateFish(aryFishs:Fish1FishData[]):void
    {
        //---測試關閉
        ...
        for(let i:number=0;i<aryFishs.length;i++)
        {
    ...
    private resetFreezeDuringTime():void
    {
        //-_freezeLastTime
    ...
    private delayRotationCreateFish():void
    {
        log('delayRotationCreateFish',this._delayFdata);
        //---測試關閉
        let nowTime:number=0;
        ...
        for(let i:number=0;i<this._delayFdata.length;i++)
        {
            ...
            this._pathCenter.setUnitInFlock(
                this._delayFdata[i].pathID,
                [this._delayFdata[i].fishFlockUnit],
                
                (this._delayFdata[i].alreadyServerTime+(nowTime-this._delayFdata[i].creatTime))/1000,//--路徑系統以秒為單位
                //0,
                this._delayFdata[i].fishMeshState,
                1
            )

            
            if(this._delayFdata[i].fishMeshState==fishMeshState.fish2D)
            {
                ...
                if(this._coordinateMode==GameCoordinateMode.GameViewMode_Four_in_one)
                {
                    ...
                    if(this._delayFdata[i].rotationState==FishRotationState.noRotation)
                    {
                        //---預設的魚頭都是朝左邊(sin(0))
                        //----type 1海星他一開始就是做正的,所以不用轉
                        if(this._delayFdata[i].fishType!=1)
                        {
                            ...
                            this._delayFdata[i].fishMesh.angle=math.toDegree(rv*Math.PI/2);
     ...
     * @returns 
     */
    private checkNowThreeDfishZindex(flshType:number):number
    {
        let baseZinex:number[]=GameUtils.deepCloneForObject(this._defaultZindex);

        let ogLength:number=baseZinex.length;
     ...
     * 
     */
    public async removeAllFish():Promise<void>
    {
        //--後續要在處理
        return new Promise<void>((resolve,reject)=>
        {
            for(let i:number=0;i<this._aryFishData.length;i++)
            {
                ...
                let fd:Fish1FishData=this._aryFishData[i] as Fish1FishData;
                    
                if(fd.fishMeshState==fishMeshState.fish2D)
                {
                    
                    let ani:Animation=fd.animation;
                        
                    ani.stop();

                    this._fishContainer.removeChild(fd.fishMesh);
                    ...
                    if(fd.fishShadow)
                    {
                        ...
                        this._fishShadowContainer.removeChild(fd.fishShadow);
                    }

                }else{
        
                    let tweenmax:TweenMaxCocosPlugin=fd.fishMesh.getComponent(TweenMaxCocosPlugin);
                    ...
                    let timeUpdate=fd.fishMesh.getComponent(TimeUpdateForMove);
                ...
                if(fd.animationSequencePlayer)
                {
                    ...
                    fd.animationSequencePlayer.eventTarget.off(BaseEvent.PLAY_ANI,this.animationSequencePlayerEvent);
                }

                if(fd.fishAimLock && this._lockFish.id==fd.id)
                {
                    this._aimLockNode.active=false;
                }

                //--現在都是一條魚配一個路徑,所以不會有一條路徑多條魚的情況了
                this._pathCenter.deleteGroupPath(fd.pathID);

                this._aryFishData.splice(i,1);
                
                if(this._poolFishData.length<100)
                {
                    fd.init();

                    this._poolFishData.push(fd); 
    ...
    public async removeFishById(id:number):Promise<void>
    {
    ...
    //--拿可視範圍內的魚(自己不拿)
    public getOutsideFish(fid:number):number[]
    {
        let len:number=this._aryFishData.length;
        ...
        for(var i:number=0;i<len;i++)
        {
            ...
            {
                if((<Fish1FishData>this._aryFishData[i]).fishFlockUnit.time>0 && this._aryOutSideBanndedType.indexOf(this._aryFishData[i].fishType)==-1)
                {
                    if((<Fish1FishData>this._aryFishData[i]).fishFlockUnit.position.x<=CocosGameSetting.Game_Width-50 && (<Fish1FishData>this._aryFishData[i]).fishFlockUnit.position.x>75)
                    {
    
                        if((<Fish1FishData>this._aryFishData[i]).fishFlockUnit.position.y<=CocosGameSetting.Game_Height-100 && (<Fish1FishData>this._aryFishData[i]).fishFlockUnit.position.y>55)
                        {
    ...
    //--秀出禁止打擊的符號在魚身上
    protected  displayProhibitSign(fd:Fish1FishData):void
    {

    }

    protected  unDisplayProhibitSign(fd:Fish1FishData):void
    {

    }

    //--持續改變顯示魚的賠率 
    public updateOddsForSPFish(value:{fishSn:number,odds:number}):void
    {
    ...
    public reSetPathTime(fishInfo:any):void
    {
    ...
    public reSetFishBronTime():void
    {
        ...
        let resetDatas: ResetPathData[]=[];

        let nowTimestamp=new Date().getTime();
        
        for(let i:number=0;i<this._aryFishData.length;i++)
        {
           let realTime:number=nowTimestamp- (<Fish1FishData>this._aryFishData[i]).createServerTime;
           
           (<Fish1FishData>this._aryFishData[i]).fishFlockUnit.reset();
            
           let reSetBornTimeData={
                
                pathTokenID:(<Fish1FishData>this._aryFishData[i]).pathID,
                
                reNewBornTime:realTime/1000,//--路徑以秒為單位

                flockUnit:(<Fish1FishData>this._aryFishData[i]).fishFlockUnit
    ...
    protected getFishByShowGroupId(id:string):Fish1FishData[]
    {
        return null;
    }
    ...
    public exitSingleGloupPath(groupId:string):void
    {
        ...
        for(let i:number=0;i<len;i++)
        {
            if(this._aryFishData[i].pathGroupID==groupId)
            {
                ...
                aryExit.push({f:(<Fish1FishData>this._aryFishData[i]).fishFlockUnit,pathId:this._aryFishData[i].pathID});
            }
        }
    ...
    public removeSinglePathUnitByFishId(fishId:number):void
    {
        ...
        for(let i:number=0;i<len;i++)
        {
            if(this._aryFishData[i].id==fishId)
            {
                ...
                fishPathLen=this._pathCenter.removeSingleUnit(this._aryFishData[i].pathID,(<Fish1FishData>this._aryFishData[i]).fishFlockUnit);
     ...
     * @param id 表演路徑單一識別碼
     */
    protected  exitSinglePathByPathId(id:string):void
    {
        ...
        for(let i:number=0;i<len;i++)
        {
            ...
            {
                aryFlockUnit.push((<Fish1FishData>this._aryFishData[i]).fishFlockUnit);
            }
        }
    ...
    //===================animation===================================================================================================
    private pauseAllFishAnimation():void
    {
        for(let i:number=0;i<this._aryFishData.length;i++)
        {
            let fd:Fish1FishData=this._aryFishData[i] as Fish1FishData;
    ...
    private resumeAllFishAnimation():void
    {
        for(let i:number=0;i<this._aryFishData.length;i++)
        {
            let fd:Fish1FishData=this._aryFishData[i] as Fish1FishData;
    ...
    private animationSequencePlayerEvent=(e)=>
    {
       log('animationSequencePlayerEvent',e);
       this._gameMediator.getViewUserData(GameViewMediatorUser.AniEffectSystemView,GameViewMediatorUserDataKey.Effect_bossShake);
    }
  

    //---擊中動畫效果
    //---擊中動畫效果
    protected changeSingleFishAnimation(id:number):void
    {
        let fd:Fish1FishData=this.getFishById(id) as Fish1FishData;
        ...
        if(fd!=null)
        {
            ...
            if(fd.fishMesh instanceof Animation)
            {
                //--spine?

            }else if(fd.fishMesh instanceof FishCustomAnimation)
            {
               //--自訂義的圓盤類型的魚
            ...
            }else if(fd.fishMesh instanceof Node)
            {
               ...
               if(fd.fishMeshState==fishMeshState.fish2D)
               {
                    let spr=fd.fishMesh.getChildByName('fish').getChildByName('fish').getComponent(Sprite);
                    //log('check_fishMeshNode',fd.fishMesh,spr);
                    ...
                    }else if(fd.fishType==23)
                    {
                        ...
                        {
                            //---bomb-donothing
                            if(!TweenMax.isTweening(fd.fishMesh.getComponent(TweenMaxCocosPlugin)))
                            {
                                TweenMax.to(fd.fishMesh.getComponent(TweenMaxCocosPlugin),.08,
                                {
                                    scale:1.5,
                                    repeat:3,
                                    yoyo:true
                       ...
                       if(this._viewModel['_roomStatus'].status!=1)
                       {
                            let clip:AnimationClip=this.getAnimationClip(fd.animation,'hurt');
                            ...
                            if(clip)
                            {
                                ...
                                //log('check_hurtClip',clip,fd.animation.getState(clip.name));     
     ...
     * @param fd FishData
     */
    protected hitFishAniComplete(fd:FishData):void
    {
        if(!fd)
        {
            return;
        ...
        (<Fish1FishData>fd).isHit=false;

        fd.countHitAni=0;
        
        if((<Fish1FishData>fd).fishMesh instanceof Animation)
        {
            //--spine?

        }else if((<Fish1FishData>fd).fishMesh instanceof FishCustomAnimation)
        {
           //--自訂義的圓盤類型的魚

        }else if((<Fish1FishData>fd).fishMesh instanceof Node)
        {
           //---一般魚
           if((<Fish1FishData>fd).fishMeshState==fishMeshState.fish2D)
           {

              let spr=(<Fish1FishData>fd).fishMesh.getChildByName('fish').getChildByName('fish').getComponent(Sprite);
              //log('finishHitComplete',fd.fishMesh,spr);
              ...
              }else if(fd.fishType==23)
              {
                ...
                {
                    //---bomb-donothing
                    if(TweenMax.isTweening((<Fish1FishData>fd).fishMesh.getComponent(TweenMaxCocosPlugin)))
                    {
                        TweenMax.killTweensOf((<Fish1FishData>fd).fishMesh.getComponent(TweenMaxCocosPlugin));
                    }

                    (<Fish1FishData>fd).fishMesh.setScale(v3(1,1,1));
                }
                ...
                let defaultClipId:string=(fd.fishType==14 || fd.fishType==15 || fd.fishType==18 || fd.fishType==19)?'swim':'swin';

                if(this._viewModel['_roomStatus'].status!=1)
                {
                    let clip:AnimationClip=this.getAnimationClip((<Fish1FishData>fd).animation,defaultClipId);
                       
                    if(clip)
                    {
                      
                      (<Fish1FishData>fd).animation.stop();
                                 
                      //let anistates:AnimationState=fd.animation.getState(clip.name);
      
                      //anistates.repeatCount=Infinity;--無限播放
                           
                      (<Fish1FishData>fd).animation.play(clip.name);
                      
                      log('check_SwinClip',clip,(<Fish1FishData>fd).animation.getState(clip.name));
              ...
              (<Fish1FishData>fd).fishSkinnedMeshRenderer.materials[0].setProperty('mainColor',color(255,255,255,255));
    ...
    public addFishAimLock(id:number,isPlayer:boolean,table:number):void
    {
        let fd:Fish1FishData=this.getFishById(id) as Fish1FishData;
        ...
        if(fd)
        {
            if(this._lockFish)
            {
                (<Fish1FishData>this._lockFish).fishAimLock=false;
            ...
            if(fd.fishMeshState==fishMeshState.fish2D)
            {
                ...
                if(fd.fishType!=21)
                {
                    localPos=GameUtils.conver3dposTo2dpos(this._fishAimContainer,'Canvas/Camera','Main Camera',fd.fishMesh.position);
                    ...
                    let shape:BuiltinBoxShape=(<BoxCollider>fd.collisionArea[6]).shape;

                    let worldobb=shape.worldObb;

                    localPos=GameUtils.conver3dposTo2dpos(this._fishAimContainer,'Canvas/Camera','Main Camera',worldobb.center);
            ...
            let tweenComponent=this._aimLockNode.getComponent(TweenMaxCocosPlugin);
    ...
    public removeFishAimLockByLockId(id:number):void
    {
        if(this._lockFish)
        {
    ...
    private getParticle(target:Node):{activeNode:Node,particle:{[key:string]:ParticleSystem}}
    {
        let particle:{activeNode:Node,particle:{[key:string]:ParticleSystem}}={activeNode:null,particle:{}};
       
        let node=target.children[0].getChildByName('GoldDragon_LP_all Socket').children[0].children[0].children[0].children[0].children[0].children[0].children[0].getChildByName('CoinBorn').children[0];
    ...
    private getAnimationFromFishMesh3D(aniTargetNode:Node,type:number):Animation
    {
        //--PS 要把它加入顯示清單才能操作相關的animationState物件,不然會找不到
        ...
        {
           //--金海龜
           ani=aniTargetNode.children[0].getChildByName('stingray_LP').getComponent(SkeletalAnimation);
           //log('checkType17',ani);
     ...
     */ 
    private getSkinnedMeshRenderer(targteNode:Node,type:number):MeshRenderer
    {
        let skinMeshRender:MeshRenderer;
     ...
     */
    private getSPFishColliders(fd:Fish1FishData):Collider[]|Collider2D[]
    {
        let r;
     ...
     * 3D要在處理'這邊只是2D的排序
     */
    private sortAllFishLayer():void
    {
       
        //this._aryFishData=this.bubbleSort(this._aryFishData);
        this._aryFishData.sort((a,b)=>
           a.lv-b.lv
        )
        ...
        for(let i:number=0;i<this._aryFishData.length;i++)
        {
            ...
            if(this._aryFishData[i].fishMeshState==fishMeshState.fish2D)
            {
                this._fishContainer.insertChild((<Fish1FishData>this._aryFishData[i]).fishMesh,i);
    ...
    public updateFish=(t:number)=>
    {
        ...
        if(this._aryFishData.length>0 && this._canUpdate)
        {
            ...
            //log('updateFishData@@');
            let second:number=t/1000;

            //this.sortAllFishLayer();
     ...
     * @param t sec
     */
    public updateFrezzeHitAniUpdate(t:number):void
    {
        
        for(let i:number=0;i<this._aryFishData.length;i++)
        {
            if((<Fish1FishData>this._aryFishData[i]).isHit)
            {
    ...
    protected async renderFish(t:number):Promise<void>
    {
        ...
        if(this._aryFishData.length>0)
       {
          ...
          for(let i:number=0;i<this._aryFishData.length;i++)
          {
            if(!this._aryFishData[i].fishIsFlash && !this._aryFishData[i].isDead)
            {
                flockUnit=(<Fish1FishData>this._aryFishData[i]).fishFlockUnit;
                ...
                {
                    this._aryFishData[i].countHitAni-=t;
                    //log('check_isHitFish',this._aryFishData[i].countHitAni);
                ...
                let theta:number = Math.atan2(flockUnit.velocity.y,flockUnit.velocity.x);

                if((<Fish1FishData>this._aryFishData[i]).fishMeshState==fishMeshState.fish2D)
                {
                    //--一般的魚
                   ...
                   // log('check_fishFlockUnit_pos',flockUnit.nowIndex,flockUnit.position);
                    ...
                    (<any>(<Fish1FishData>this._aryFishData[i]).fishMesh).setPosition(new Vec3(flockUnit.position.x,flockUnit.position.y,flockUnit.position.z));
                   
                    
                    //--一般正常狀態的旋轉(一般魚,非自訂義的魚)
                    if((<Fish1FishData>this._aryFishData[i]).fishMesh instanceof Node && (<Fish1FishData>this._aryFishData[i]).rotationState==FishRotationState.normalRotation)
                    {
                        (<any>(<Fish1FishData>this._aryFishData[i]).fishMesh).angle=math.toDegree(theta);
                        
                        if((<Fish1FishData>this._aryFishData[i]).fishShadow)
                        {
                            (<any>(<Fish1FishData>this._aryFishData[i]).fishShadow).setPosition(v3(flockUnit.position.x+30,flockUnit.position.y-30,flockUnit.position.z));
                            
                            (<any>(<Fish1FishData>this._aryFishData[i]).fishShadow).angle=<any>(<Fish1FishData>this._aryFishData[i]).fishMesh.angle;
                    ...
                    }else if((<Fish1FishData>this._aryFishData[i]).rotationState==FishRotationState.horizontalRotation)
                    {
                        //--不旋轉,但是左右水平翻轉
                        /*
                        if(theta < Math.PI/2 && theta > -Math.PI/2)
                    ...
                    }else if((<Fish1FishData>this._aryFishData[i]).rotationState==FishRotationState.noRotation)
                    {
                    ...
                    if((<Fish1FishData>this._aryFishData[i]).fishAimLock)
                    {
                        worldPos=this._fishContainer.getComponent(UITransform).convertToWorldSpaceAR(this._aryFishData[i].fishMesh.position);
                    ...
                    */
                    ...
                    //log('frustumData',this._frustumData);
                    if(this._aryFishData[i].fishType!=21)
                    {
                       //--一般3d的魚
                       (<Node>(<Fish1FishData>this._aryFishData[i]).fishMesh).setPosition(pos3dchange.x,pos3dchange.y,(<Fish1FishData>this._aryFishData[i]).fishMesh.position.z);
                       ...
                       if((<Fish1FishData>this._aryFishData[i]).fishAimLock)
                       {
                        ...
                        //let tweenPlugin:TweenMaxCocosPlugin;
                        let tweenPlugin:TimeUpdateForMove=(<Fish1FishData>this._aryFishData[i]).fishMesh.getComponent(TimeUpdateForMove);

                        let doNextActionFlag:boolean=false;
                                
                        let finaldDstination:number=0;
                        ...
                        if(flockUnit.time>=5 && flockUnit.time<=19)
                        {
                           ...
                           //if(flockUnit.time>=5 && !((<Fish1FishData>this._aryFishData[i]).spBossInStates.bottom))
                            if(flockUnit.time>=5)
                            {
                                ...
                                if(this._playerTableCoordinate==1 || this._playerTableCoordinate==2)
                                {
                                    if(!((<Fish1FishData>this._aryFishData[i]).spBossInStates.top))
                                    {
                                        doNextActionFlag=true;

                                        (<Fish1FishData>this._aryFishData[i]).spBossInStates.top=true;

                                        (<Fish1FishData>this._aryFishData[i]).fishMesh.setPosition(v3(0,this._frustumData.topPoint+1000,-500));

                                        finaldDstination=this._frustumData.topPoint*1.4;
                                    }

                                }else{

                                    if(!((<Fish1FishData>this._aryFishData[i]).spBossInStates.bottom))
                                    {
                                        doNextActionFlag=true;

                                        (<Fish1FishData>this._aryFishData[i]).spBossInStates.bottom=true;

                                        (<Fish1FishData>this._aryFishData[i]).fishMesh.setPosition(v3(0,this._frustumData.bottomPoint-1000,-500));

                                        finaldDstination=this._frustumData.bottomPoint*1.4;
                                    }
                                }
                                ...
                                {
                                    //mesh.setPosition(v3(this._frustumData.leftPoint-1000,0,-500));
                                    (<Fish1FishData>this._aryFishData[i]).animationSequencePlayer.play(AnimationStatus.start);

                                    if(!tweenPlugin.isRunning)
                                    {
                                        let startPos={x:(<Fish1FishData>this._aryFishData[i]).fishMesh.position.x,y:(<Fish1FishData>this._aryFishData[i]).fishMesh.position.y};
                                        
                                        let endPos={x:(<Fish1FishData>this._aryFishData[i]).fishMesh.position.x,y:finaldDstination};
                            ...
                            if(flockUnit.time>=18)
                            {
                                ...
                                if(this._playerTableCoordinate==1 || this._playerTableCoordinate==2)
                                {
                                    ...
                                    if(!(<Fish1FishData>this._aryFishData[i]).spBossInStates.to)
                                    {
                                        doNextActionFlag=true;
                                        
                                        (<Fish1FishData>this._aryFishData[i]).spBossInStates.to=true;

                                        p=(<Fish1FishData>this._aryFishData[i]).fishMesh.position;
                                    ...
                                    if(!(<Fish1FishData>this._aryFishData[i]).spBossInStates.bo)
                                    {
                                        doNextActionFlag=true;
                                        
                                        (<Fish1FishData>this._aryFishData[i]).spBossInStates.bo=true;
                                ...
                                {
                                    //--out 沒反應
                                    (<Fish1FishData>this._aryFishData[i]).animationSequencePlayer.play(AnimationStatus.end);
                        ...
                        }else if(flockUnit.time>=20 && flockUnit.time<=34)
                        {
                           ...
                           if(flockUnit.time>=20)
                           {
                                ...
                                if(this._playerTableCoordinate==1 || this._playerTableCoordinate==2)
                                {
                                    if(!((<Fish1FishData>this._aryFishData[i]).spBossInStates.right))
                                    {
                                        doNextActionFlag=true;

                                        (<Fish1FishData>this._aryFishData[i]).spBossInStates.right=true;
                                 
                                        (<Fish1FishData>this._aryFishData[i]).fishMesh.setPosition(v3(this._frustumData.rightPoint+1000,0,-500));
                                        
                                        finaldDstination=this._frustumData.rightPoint
        
                                    }

                                }else{

                                    if(!((<Fish1FishData>this._aryFishData[i]).spBossInStates.left))
                                    {
                                        doNextActionFlag=true;

                                        (<Fish1FishData>this._aryFishData[i]).spBossInStates.left=true;

                                        (<Fish1FishData>this._aryFishData[i]).fishMesh.setPosition(v3(this._frustumData.leftPoint-1000,0,-500));
                                       
                                        finaldDstination=this._frustumData.leftPoint
                                ...
                                if(doNextActionFlag)
                                {
                                    ...
                                    if(!tweenPlugin.isRunning)
                                    {
                                        ...
                                        let endPos={x:finaldDstination,y:(<Fish1FishData>this._aryFishData[i]).fishMesh.position.y};
                            ...
                            if(flockUnit.time>=33)
                            {
                                ...
                                if(this._playerTableCoordinate==1 || this._playerTableCoordinate==2)
                                {
                                    if(!(<Fish1FishData>this._aryFishData[i]).spBossInStates.ro)
                                    {
                                        doNextActionFlag=true;
                                        
                                        (<Fish1FishData>this._aryFishData[i]).spBossInStates.ro=true;
                                    ...
                                    if(!(<Fish1FishData>this._aryFishData[i]).spBossInStates.lo)
                                    {
                                        doNextActionFlag=true;
                                        
                                        (<Fish1FishData>this._aryFishData[i]).spBossInStates.lo=true;
                    ...
                    */
                    ...
                    if(this._aryFishData[i].fishType!=21)
                    {
                        ...
                        if((<Fish1FishData>this._aryFishData[i]).rotationState==FishRotationState.normalRotation)
                        {
                          //--旋轉(隨著路徑旋轉)
                          rotationQuaternion=Quat.identity((<Fish1FishData>this._aryFishData[i]).rotationQuaternion);
                          ...
                          (<Node>(<Fish1FishData>this._aryFishData[i]).fishMesh).setRotation(rotationQuaternion);
                       ...
                       }else if((<Fish1FishData>this._aryFishData[i]).rotationState==FishRotationState.noRotation)
                       {
                           ...
                           */
                          //--這是弧度
                          (<Fish1FishData>this._aryFishData[i]).other.rotation+=0.05;

                          //--outX
                          let quatX:Quat=Quat.identity((<Fish1FishData>this._aryFishData[i]).other.AxisXQuat); 
                          
                          let rotationAxisX:Vec3=Fish1View.Axis_Negative.x.normalize();
                          
                          Quat.fromAxisAngle(quatX,rotationAxisX,(<Fish1FishData>this._aryFishData[i]).other.rotation);

                          //--outZ
                          let quatZ:Quat=Quat.identity((<Fish1FishData>this._aryFishData[i]).other.AxisZQuat); 
                          
                          let rotationAxisZ:Vec3=Fish1View.Axis_Negative.z.normalize();
                          
                          Quat.fromAxisAngle(quatZ,rotationAxisZ,(<Fish1FishData>this._aryFishData[i]).other.rotation);
                       ...
                       }else if((<Fish1FishData>this._aryFishData[i]).rotationState==FishRotationState.horizontalRotation)
                       {
                        ...
                        if(flockUnit.time>=5 && flockUnit.time<=19)
                        {
                            ...
                            //--旋轉(隨著路徑旋轉)
                            //theta=Math.PI / 2;
                            theta=(this._playerTableCoordinate==1 || this._playerTableCoordinate==2)?-Math.PI / 2:Math.PI / 2;
                        ...
                        }else if(flockUnit.time>=20 && flockUnit.time<=34)
                        {
                           ...
                           //--34退場 
                           //theta=0;
                           theta=(this._playerTableCoordinate==1 || this._playerTableCoordinate==2)?Math.PI:0;
                        ...
                        }else if(flockUnit.time>=50 && flockUnit.time<=66)
                        {
                            ...
                            //--64退場
                            //theta=Math.PI;
                            theta=(this._playerTableCoordinate==1 || this._playerTableCoordinate==2)?0:Math.PI;
                        ...
                        {
                            //localPos=GameUtils.conver3dposTo2dpos(this._fishAimContainer,'Canvas/Camera','Main Camera',this._aryFishData[i].fishMesh.position);
                            let shape:BuiltinBoxShape=(<BoxCollider>(<Fish1FishData>this._aryFishData[i]).collisionArea[6]).shape;
                ...
                if(this._aryFishData[i].fishType!=21)
                {
                    for(let c:number=0;c<(<Fish1FishData>this._aryFishData[i]).collisionArea.length;c++)
                    {
                       ...
                       (<BuiltinShape2D>(<Collider2D>(<Fish1FishData>this._aryFishData[i]).collisionArea[c])).impl.update();
            ...
            if(!flockUnit)
            {
                log('check_FishAwway',flockUnit);
                //--20240126已經死掉但是還沒刪除的(death=true) ,這樣會無法有效刪除
                flockUnit=(<Fish1FishData>this._aryFishData[i]).fishFlockUnit;
            }
            ...
            if(flockUnit.isAway)
            {
                ...
                //{
                    let fd:Fish1FishData=<Fish1FishData>this._aryFishData[i];
                    
                    fd.isDead=true;

                    fd.prohibit=false;

                    flockUnit.isDeath=true;

                    //log('delete Befor_aryLen',this._aryFishData.length+'\n','check_fishData',this._aryFishData,'\n'+'id',fd.id,'\n'+'index',i);
                    ...
                    {
                       //--finish-
                       this.hitFishAniComplete((<Fish1FishData>this._aryFishData[i]));
                    }
                    ...
                    if(fd.fishAimLock)
                    {
                        ...
                        this._aimLockNode.setPosition(v3(0,0));
                    }
