...
export class BasePointData {
    ...
    * 用於判斷是否是頭尾 與 重複區塊的交界
     */
    public type: LineSpritePointType = LineSpritePointType.body;
}
...
export class SimpleLineSprite extends Sprite {
    ...
    //@property({ type: CCFloat, serializable: true, visible: false })
    //private _segmentLengthList: number[] = [];
    ...
    protected updateFullLineGeometry() {
        ...
        for (let i = 0; i < this.posList.length; i++) {
            ...
            if (i < this.posList.length - 1) {
                const segmentLength = Vec2.distance(this.posList[i], this.posList[i + 1]);
                //this._segmentLengthList.push(segmentLength);
    ...
    */
    protected updateVisibleSegment() {
        ...
        //根據X查找 端點在線上的具體位置
        //要顯示的線最左與最右的點
        const fillFirstPos: ExtendedPointData = this.findPointByX(fillFirst, this._posList);
        const fillFinalPos: ExtendedPointData = this.findPointByX(fillFinal, this._posList);
    ...
    */
    protected findPointByX(targetX: number, posData: Vec2[]): ExtendedPointData {
        let left = 0;
        let right = posData.length - 1;

        // 二分查找确定范围
        while (left < right - 1) {
            const mid = Math.floor((left + right) / 2);
            if (posData[mid].x < targetX) {
                left = mid;
            } else {
                right = mid;
            }
        ...
        const point1 = posData[left];
        const point2 = posData[right];
    ...
    */
    ...
    protected handleSlicedLine() {
        if (this.type !== Sprite.Type.SLICED || this._spriteFrame === null) { return; }

        //檢查長度是否足夠 容許完整頭尾呈現
        const currentLineLength = this._lineTotalLength * (this.fillFinal - this.fillFirst);
        if (currentLineLength > this.spriteFrame.insetLeft + this.spriteFrame.insetRight) {
            //長度足夠就加點
            //先對尾部做處理 和 push 防止順序亂掉

            let tailPoint: ExtendedPointData = this.getPointOnPolyline(this.spriteFrame.insetRight, true);
            tailPoint.topPoint = this.extendedCheck(this._sampledFinalData[tailPoint.indexLeft].topPoint, tailPoint.topPoint, this._sampledFinalData[tailPoint.indexRight].topPoint);
            tailPoint.bottomPoint = this.extendedCheck(this._sampledFinalData[tailPoint.indexLeft].bottomPoint, tailPoint.bottomPoint, this._sampledFinalData[tailPoint.indexRight].bottomPoint);
            this._sampledFinalData.splice(tailPoint.indexRight, 0, tailPoint);

            let headPoint: ExtendedPointData = this.getPointOnPolyline(this.spriteFrame.insetLeft, false);
            headPoint.topPoint = this.extendedCheck(this._sampledFinalData[headPoint.indexLeft].topPoint, headPoint.topPoint, this._sampledFinalData[headPoint.indexRight].topPoint);
            headPoint.bottomPoint = this.extendedCheck(this._sampledFinalData[headPoint.indexLeft].bottomPoint, headPoint.bottomPoint, this._sampledFinalData[headPoint.indexRight].bottomPoint);
            this._sampledFinalData.splice(headPoint.indexRight, 0, headPoint);
        }
        else if (currentLineLength > 0) {
            //不夠的話從中間分半
            this.centerPosHandle();
        }
        //console.log(this._sampledFinalData);
    }

    protected centerPosHandle() {
        if (this._sampledFinalData.length > 3) {
            console.error("線段過於複雜 有至少兩點位置過近");
            console.log(this._sampledFinalData);
        }
        // 取頭尾的點
        const first: ExtendedPointData = this._sampledFinalData[0];
        const last: ExtendedPointData = this._sampledFinalData[this._sampledFinalData.length - 1];
        const originPoints: Vec2[] = this._sampledFinalData.map(p => p.originPoint);

        let centerPos: ExtendedPointData = this.findPointByX((first.originPoint.x + last.originPoint.x) / 2, originPoints);
        let isNew: boolean = centerPos.index === null;

        if (isNew) {
            //插值點
            let headPoint = this.calculateExtendedPoints(this.posList[centerPos.indexLeft], centerPos.originPoint, this.posList[centerPos.indexLeft]);
            let tailPoint = this.calculateExtendedPoints(this.posList[centerPos.indexLeft], centerPos.originPoint, this.posList[centerPos.indexLeft]);

            headPoint.originPoint = centerPos.originPoint;
            headPoint.indexRight = centerPos.indexRight;
            headPoint.indexLeft = centerPos.indexLeft;
            headPoint.type = LineSpritePointType.Head;

            tailPoint.originPoint = centerPos.originPoint;
            tailPoint.indexRight = centerPos.indexRight;
            tailPoint.indexLeft = centerPos.indexLeft;
            tailPoint.type = LineSpritePointType.Tail;
            ...
            let centerPoint: ExtendedPointData = this.calculateTwoEndPointsExtendedPoints(centerPos);
            let headPoint: ExtendedPointData = new ExtendedPointData();
            let tailPoint: ExtendedPointData = new ExtendedPointData();

            headPoint.bottomPoint = centerPoint.bottomPoint;
            headPoint.originPoint = centerPos.originPoint;
            headPoint.indexRight = centerPos.indexRight;
            headPoint.topPoint = centerPoint.topPoint;
            headPoint.indexLeft = centerPos.indexLeft;
            headPoint.type = LineSpritePointType.Head;

            tailPoint.bottomPoint = centerPoint.bottomPoint;
            tailPoint.originPoint = centerPos.originPoint;
            tailPoint.indexRight = centerPos.indexRight;
            tailPoint.topPoint = centerPoint.topPoint;
            tailPoint.indexLeft = centerPos.indexLeft;
            tailPoint.type = LineSpritePointType.Tail;

            this._sampledFinalData = [first, headPoint, tailPoint, last];
        }
    }

    protected getPointOnPolyline(distance: number, fromEnd: boolean): ExtendedPointData {
        const dataList = fromEnd ? [...this._sampledFinalData].reverse() : this._sampledFinalData;
        const count = dataList.length;
        if (count < 2) return null;

        let accumulated = 0;
        let type: LineSpritePointType = fromEnd ? LineSpritePointType.Tail : LineSpritePointType.Head;
        ...
        for (let i = 0; i < count - 1; i++) {
            ...
            const p1 = dataList[i].originPoint;
            const p2 = dataList[i + 1].originPoint;
            const segLen = Vec2.distance(p1, p2);
            ...
            if (accumulated + segLen > distance) {
                ...
                let previousPoint: Vec2 = fromEnd ? dataList[i + 1].originPoint : dataList[i].originPoint;
                let nextPoint: Vec2 = fromEnd ? dataList[i].originPoint : dataList[i + 1].originPoint;
                let currentPoint: Vec2 = new Vec2(p1.x + (p2.x - p1.x) * ratio, p1.y + (p2.y - p1.y) * ratio);
                let result: ExtendedPointData = this.calculateExtendedPoints(previousPoint, currentPoint, nextPoint);

                result.type = type;
                result.originPoint = currentPoint;
                result.indexLeft = fromEnd ? (count - 2 - i) : i;
                result.indexRight = fromEnd ? (count - 1 - i) : i + 1;
                return result;
            }
        ...
        // 最後一點特例：剛好在終點也視為 null
        if (distance === accumulated) return null;
    ...
    protected fillUV() {
        ...
        switch (this.type) {
            case Sprite.Type.SLICED:
                /* 九宮格 */
                this.slicedUV(this._vertexData.length / 2);
                break;

            default:
                /* 默認平鋪 */
                this.tiledUV(this._vertexData.length / 2);
                break;
        }
    }
    ...
    * - 九宮格的UV
    */
    protected slicedUV(length: number) {
        const result: number[] = [0];
        const currentLineLength = this._lineTotalLength * (this.fillFinal - this.fillFirst);

        //if (currentLineLength > this.spriteFrame.insetLeft + this.spriteFrame.insetRight) {
        let endpointLength: number = this.spriteFrame.insetLeft + this.spriteFrame.insetRight;
        let bodyProportion: number = (this.spriteFrame.width - endpointLength) / this.spriteFrame.width;
        let tailU: number = 1 - (this.spriteFrame.insetRight / this.spriteFrame.width);
        let headU: number = this.spriteFrame.insetLeft / this.spriteFrame.width;
        let bodyU: number = currentLineLength - endpointLength;
        let distanceAccumulation: number = 0;
        let handleHead: boolean = true;
        let handleTail: boolean = true;

        for (let i = 1; i < length - 1; i++) {
            if (this._sampledFinalData[i].type === LineSpritePointType.Head) {
                distanceAccumulation = 0;
                handleHead = false;
                result.push(headU);
            }
            else if (this._sampledFinalData[i].type === LineSpritePointType.Tail) {
                distanceAccumulation = 0;
                handleTail = false;
                result.push(tailU);
            }
            else {
                let distance: number = Vec2.distance(this._sampledFinalData[i - 1].originPoint, this._sampledFinalData[i].originPoint);
                distanceAccumulation += distance;

                if (handleHead) {
                    //頭前
                    result.push(distanceAccumulation / this.spriteFrame.insetLeft * headU);
                }
                else if (handleTail) {
                    //身體
                    result.push(distanceAccumulation / bodyU * bodyProportion + headU);
                }
                else {
                    //尾部
                    result.push(distanceAccumulation / this.spriteFrame.insetRight + tailU);
                }
            }
        }
        result.push(1);

        this._uvData = [...result, ...result];
    }

    /**
    * - 平鋪的UV
    */
    protected tiledUV(length: number) {
        const result: number[] = [];
