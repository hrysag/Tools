...
import { BetData, NetworkEvent, NetworkHandler, PlayerInfo } from '../Networks/v2';
...
export class GameController extends Component {
    ...
    public init(gameNumber: number, isOnline: boolean): void {
    ...
    public onUpdateBetValue(betValue: number): void {
        // BetSelectUI 按完按鈕會呼叫此方法
        // BuyFeatureInfoUI 切換左右按鈕也會呼叫此方法
        // 如果遊戲有切換下注紀錄後要提示玩家會喪失其他權益的功能 要在繼承後的子類別覆寫此方法實作
    ...
    public sendBet(bet: number, additionalPurchaseType: AdditionalPurchaseType = AdditionalPurchaseType.None, otherParameter: number = 0): void {
        NetworkHandler.instance.send(
            NetworkEvent.Bet,       // args[0]
            this.gameNumber,        // args[1]
            bet, PlayerInfo.balance,// args[2]
            additionalPurchaseType, // args[3]
            this.playerToken,       // args[4]
            otherParameter          // args[5]
        );
    }
    ...
    public onReceiveBet(betData: BetData): void {
        ...
        if (this.isOnline) {
            ...
            if (NetworkHandler.instance.getIsDemo() !== true) {
    ...
    public onSpinFail(spinId: string): void {
        // 下注失敗時紀錄單號
    ...
    protected showBankruptcyError(): void {
        ErrorHandler.Instance.TriggerError(ErrorCode.Client_BetBankruptcy, true);
    }

    protected showBalanceLessThanThresholdMessage(): void {
        // "餘額小於設定值"
    ...
    protected showThresholdMustGreaterThanBalanceMessage(): void {
        // "設定值需大於餘額"
        const title = Localization.instance.t('ErrorTitle.Default_Title');
    ...
    public setupBeforeGame(): Promise<void> {
    ...
    protected onReceiveLastPlantData(base64Data: string): void {
        // 繼承後可以覆寫此方法，實作取得初始盤面後的邏輯
        // 例如把盤面資料傳給GameLogic

    }

    protected onTimeScaleChange(): void {
    ...
    protected onSetMainUIToSpinMode(): void {
    ...
    protected onSetMainUIToNormalMode(): void {
    ...
    public onNewFlashBtnSwitch(mode: NewFlashModeEnum): void {
