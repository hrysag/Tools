...
通用計時器 v2 - 支持複合任務（sequence/parallel）的進階版本。

## 新增功能 (v2)

### 複合任務系統

支持嵌套的 sequence（序列）和 parallel（並行）任務組合：
...
uniTimer.addTask(
    parallel(
        ...
        sequence(
            parallel(
                UniTimeEffectFactory.Node.scaleTo(bg, midScale, 0.5),
                UniTimeEffectFactory.Node.moveTo(bg, midPos, 0.5)
            ),
            parallel(
                UniTimeEffectFactory.Node.scaleTo(bg, finalScale, 0.75),
                UniTimeEffectFactory.Node.moveTo(bg, finalPos, 0.58)
            )
        ),
        // Opacity 時間線：獨立從 0 秒開始
        sequence(
            delay(1.167),
            UniTimeEffectFactory.UIOpacity.opacityTo(uiOpacity, 255, 0.33)
        )
    )
);
```
...
新增 `UIOpacity.opacityTo()` 方法：

```typescript
UniTimeEffectFactory.UIOpacity.opacityTo(uiOpacity, 255, 0.5, { ease: EaseType.SineOut });
```

### Skip 行為保證

複合任務的 skip 機制確保所有嵌套子任務都執行完整生命週期：
- `onStart()` → `onUpdate(1.0)` → `onComplete()`
