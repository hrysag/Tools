...
export class UniTask {
    ...
    /**子任務陣列（用於 sequence/parallel 組合） */
    public children?: UniTask[] = null;

    /**組合類型（sequence: 序列執行, parallel: 並行執行） */
    public compositeType?: 'sequence' | 'parallel' = null;
}
 ...
 * 
 * v2 新增功能：
 * - 支持複合任務（sequence/parallel）的嵌套組合
 * - 統一的複合任務 skip 機制，確保所有子任務完整執行生命週期
 * - 遞歸處理嵌套複合任務的執行邏輯
 * 
 * @example
 * ```typescript
 * import { UniTimer, UniTimeEffectFactory } from 'db://assets/Scripts/Utils/UniTimer/v2';
 * const { sequence, parallel, delay } = UniTimeEffectFactory.Utility;
 * 
 * const timer = new UniTimer();
 * 
 * // 基礎用法
 * timer.addTask(UniTimeEffectFactory.Node.moveTo(node, targetPos, 1.0));
 * 
 * // 複合任務
 * timer.addTask(
 *     parallel(
 *         sequence(
 *             UniTimeEffectFactory.Node.scaleTo(node, scale1, 0.5),
 *             UniTimeEffectFactory.Node.moveTo(node, pos1, 0.5)
 *         ),
 *         sequence(
 *             delay(0.5),
 *             UniTimeEffectFactory.UIOpacity.opacityTo(uiOpacity, 255, 0.5)
 *         )
 *     )
 * );
 * ```
 ...
 */
     ...
     * 
     * @deprecated 建議使用 UniTimeEffectFactory.Utility.parallel() 工廠方法代替
     * 
     * @example
     * ```typescript
     * // 舊方式（仍支持）
     ...
     * 
     * // 新方式（推薦）
     * const { parallel } = UniTimeEffectFactory.Utility;
     * timer.addTask(parallel(moveTask, scaleTask));
     * ```
     */
    public addParallel(tasks: UniTask[]): void {
        ...
        // v2: 直接使用複合任務邏輯
        const parallelTask = new UniTask();
        parallelTask.duration = Math.max(...tasks.map(t => t.duration));
        parallelTask.children = tasks;
        parallelTask.compositeType = 'parallel';

        this.addTask(parallelTask);
    }
    ...
    protected _skipCurrentTask(): void {
        ...
        // 檢查是否為複合任務
        if (this._currentTask.config.children && this._currentTask.config.compositeType) {
            this._skipCompositeTask(this._currentTask.config);
        } else {
            // 單一任務：設置到完成狀態
    ...
    /**
     * 跳過複合任務（sequence/parallel）
     * 確保所有子任務都經歷完整生命週期
     */
    protected _skipCompositeTask(task: UniTask): void {
        if (!task.children || task.children.length === 0) {
            return;
        }

        if (task.compositeType === 'sequence') {
            // 序列：依次跳過每個子任務
            task.children.forEach(child => {
                this._skipSingleTask(child);
            });
        } else if (task.compositeType === 'parallel') {
            // 並行：同時跳過所有子任務
            // 先觸發所有 onStart
            task.children.forEach(child => {
                child.onStart?.();
     ...
     * 跳過單個任務（可能是嵌套的複合任務）
     */
    protected _skipSingleTask(task: UniTask): void {
        // 遞歸處理嵌套的複合任務
        if (task.children && task.compositeType) {
            task.onStart?.();
            this._skipCompositeTask(task);
            task.onComplete?.();
        } else {
            // 普通任務
            task.onStart?.();
    ...
    protected _processTasks(): void {
        ...
        // 檢查是否為複合任務
        if (this._currentTask.config.children && this._currentTask.config.compositeType) {
            this._processCompositeTask();
        } else {
            this._processSingleTask();
        }
     ...
     * 處理單一任務
     */
    protected _processSingleTask(): void {
        if (this._currentTask === null) return;
    ...
    /**
     * 處理複合任務（sequence/parallel）
     */
    protected _processCompositeTask(): void {
        if (this._currentTask === null) return;

        const task = this._currentTask.config;
        const elapsed = this._currentTask.currentTime;
        const deltaTime = Math.min(this._leftDeltaTime, this._currentTask.remainingTime);

        // 更新時間
        this._currentTask.currentTime += deltaTime;
        this._leftDeltaTime -= deltaTime;

        const newElapsed = this._currentTask.currentTime;
        const totalDuration = task.duration;

        if (task.compositeType === 'sequence') {
            this._processSequence(task, elapsed, newElapsed, totalDuration);
        } else if (task.compositeType === 'parallel') {
            this._processParallel(task, elapsed, newElapsed, totalDuration);
        }

        // 調用父任務的更新回調
        if (task.onUpdate) {
            task.onUpdate(this._currentTask.progress);
        }
     ...
     * 處理序列任務
     */
    protected _processSequence(task: UniTask, prevElapsed: number, newElapsed: number, totalDuration: number): void {
        let accumulated = 0;

        task.children.forEach((child, index) => {
            const childStart = accumulated;
            const childEnd = accumulated + child.duration;

            // 檢查是否應該觸發 onStart
            if (prevElapsed <= childStart && newElapsed > childStart) {
                child.onStart?.();
            }

            // 檢查是否在執行範圍內
            if (newElapsed > childStart && newElapsed <= childEnd) {
                const childProgress = child.duration > 0
                    ? (newElapsed - childStart) / child.duration
                    : 1;
                const prevChildProgress = child.duration > 0
                    ? Math.max(0, (prevElapsed - childStart) / child.duration)
                    : 0;

                // 如果是嵌套的複合任務，遞歸處理
                if (child.children && child.compositeType) {
                    const childPrevElapsed = prevChildProgress * child.duration;
                    const childNewElapsed = childProgress * child.duration;

                    if (child.compositeType === 'sequence') {
                        this._processSequence(child, childPrevElapsed, childNewElapsed, child.duration);
                    } else if (child.compositeType === 'parallel') {
                        this._processParallel(child, childPrevElapsed, childNewElapsed, child.duration);
                    }
                } else {
                    // 普通任務直接調用 onUpdate
                    child.onUpdate?.(Math.min(childProgress, 1));
                }
            } else if (newElapsed > childEnd) {
                // 如果跨過了這個任務，確保觸發完成
                if (prevElapsed < childEnd) {
                    if (child.children && child.compositeType) {
                        // 嵌套複合任務：從當前進度跳到結束
                        const childPrevElapsed = Math.max(0, (prevElapsed - childStart));
                        if (child.compositeType === 'sequence') {
                            this._processSequence(child, childPrevElapsed, child.duration, child.duration);
                        } else if (child.compositeType === 'parallel') {
                            this._processParallel(child, childPrevElapsed, child.duration, child.duration);
                        }
                    } else {
                        child.onUpdate?.(1);
                    }
            ...
            // 檢查是否剛完成
            if (prevElapsed < childEnd && newElapsed >= childEnd) {
                child.onComplete?.();
            }

            accumulated += child.duration;
        });
    }
     ...
     * 處理並行任務
     */
    protected _processParallel(task: UniTask, prevElapsed: number, newElapsed: number, totalDuration: number): void {
        task.children.forEach((child, index) => {
            const childDuration = child.duration;

            // 檢查是否應該觸發 onStart（只在第一次更新時觸發）
            if (prevElapsed === 0 && newElapsed > 0) {
                child.onStart?.();
            }

            // 計算子任務進度
            if (childDuration > 0) {
                const childProgress = Math.min(newElapsed / childDuration, 1);
                const prevChildProgress = Math.min(prevElapsed / childDuration, 1);

                if (childProgress > prevChildProgress) {
                    // 如果是嵌套的複合任務，遞歸處理
                    if (child.children && child.compositeType) {
                        const childPrevElapsed = prevChildProgress * childDuration;
                        const childNewElapsed = childProgress * childDuration;

                        if (child.compositeType === 'sequence') {
                            this._processSequence(child, childPrevElapsed, childNewElapsed, childDuration);
                        } else if (child.compositeType === 'parallel') {
                            this._processParallel(child, childPrevElapsed, childNewElapsed, childDuration);
                        }
                    } else {
                        // 普通任務直接調用 onUpdate
                        child.onUpdate?.(childProgress);
                    }
                }

                // 檢查是否剛完成
                if (prevChildProgress < 1 && childProgress >= 1) {
                    child.onComplete?.();
                }
            } else {
                // duration為0的立即任務
