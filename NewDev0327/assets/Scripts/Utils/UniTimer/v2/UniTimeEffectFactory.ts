import { Node, Vec3, Sprite, Color, UITransform, Size, Widget, UIOpacity } from 'cc';
...
export namespace UniTimeEffectFactory {
     ...
     * UIOpacity 相關的動畫效果
     */
    export namespace UIOpacity {

        /**
         * 透明度游移到指定值
         * @param uiOpacity 目標 UIOpacity 組件
         * @param opacity 目標透明度 (0-255)
         * @param duration 持續時間
         * @param options 動畫選項
         * @returns UniTask 對象
         */
        export function opacityTo(
            uiOpacity: UIOpacity,
            opacity: number,
            duration: number,
            ...
            param.onStart = () => {
                startOpacity = uiOpacity.opacity;
            };
            ...
            param.onUpdate = (progress: number) => {
                ...
                // 插值計算當前透明度
                const currentOpacity = startOpacity + (opacity - startOpacity) * easedProgress;

                // 設置透明度（確保在 0-255 範圍內）
                uiOpacity.opacity = Math.max(0, Math.min(255, currentOpacity));
            };
    ...
    export namespace Utility {
        ...
        /**
         * 序列執行多個任務
         * @param tasks 任務數組，將依次執行
         * @returns 包裝後的 UniTask
         * 
         * @example
         * ```typescript
         * const seq = sequence(
         *     moveTo(node, pos1, 0.5),
         *     scaleTo(node, scale1, 0.5),
         *     moveTo(node, pos2, 1.0)
         * );
         * uniTimer.addTask(seq);
         * ```
         */
        export function sequence(...tasks: UniTask[]): UniTask {
            if (!tasks || tasks.length === 0) {
                console.warn('UniTimeEffectFactory: Empty sequence');
                return delay(0);
            }

            const wrapper = new UniTask();
            wrapper.duration = tasks.reduce((sum, t) => sum + t.duration, 0);
            wrapper.children = tasks;
            wrapper.compositeType = 'sequence';
        ...
        /**
         * 並行執行多個任務
         * @param tasks 任務數組，將同時執行
         * @returns 包裝後的 UniTask
         * 
         * @example
         * ```typescript
         * const par = parallel(
         *     moveTo(node, pos1, 1.0),
         *     scaleTo(node, scale1, 1.5)
         * );
         * uniTimer.addTask(par);
         * ```
         */
        export function parallel(...tasks: UniTask[]): UniTask {
            if (!tasks || tasks.length === 0) {
                console.warn('UniTimeEffectFactory: Empty parallel group');
                return delay(0);
            }

            const wrapper = new UniTask();
            wrapper.duration = Math.max(...tasks.map(t => t.duration));
            wrapper.children = tasks;
            wrapper.compositeType = 'parallel';
