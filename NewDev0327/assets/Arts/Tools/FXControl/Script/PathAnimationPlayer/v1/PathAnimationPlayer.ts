...
export class PathAnimationPlayer extends Component {
    ...
    public async play(AnimationName: string, rootStartWorldPos: Vec3, rootEndWorldPos: Vec3): Promise<void> {
        this.playMotionStreak(AnimationName);
    ...
    protected playMotionStreak(AnimationName: string): void {
        for (let item of this.pathAnimationList) {
            if (item.hasFollowNode) {
                this.follow(item.followNode, item.targetNode, AnimationName);
            }
        }
    }

    protected follow(follower: Node, target: Node, AnimationName: string): void {
        const duration = this.moveAnimation.getState(AnimationName).duration;
