...
import { FakeBoardProbability } from '../../Scripts/Utils/FakeProbability/FakeBoardProbability';
import { ProbabilitySettingData } from '../../Scripts/Utils/FakeProbability/ProbabilitySettingData';
...
export class ReelExample extends UniReel<SymbolNumber, IconExample> {
    ...
    private probabilitySettingData: ProbabilitySettingData = null;
    private fakeBoardProbability: FakeBoardProbability = new FakeBoardProbability();

    public init(reelID: number): void {
        this.reelSettingData = RunTimeData.instance.reelData;
        this.probabilitySettingData = this.getComponent(ProbabilitySettingData);
        this.fakeBoardProbability.init(this.probabilitySettingData);
    ...
    protected generateRandomIconData(): SymbolNumber[] {
        ...
        let symbolDataList = this.fakeBoardProbability.generateRandomSymbolData(this.iconAmount, 2);

        for (let index = 0; index < symbolDataList.length; index++) {
            let symbol = SymbolNumber.pool.instance();
            symbol.symbolID = symbolDataList[index];
